...
void FileMemMap::Close()
{
    ...
    if (m_hMap not_eq INVALID_HANDLE_VALUE)
        CloseHandle(m_hMap);

    if (m_hFile not_eq INVALID_HANDLE_VALUE)
        CloseHandle(m_hFile);
...
{
    m_hFile = CreateFile(
        filename, rw == TRUE ? GENERIC_WRITE : GENERIC_READ,
        rw == TRUE ? FILE_SHARE_WRITE : FILE_SHARE_READ, NULL, OPEN_EXISTING,
    ...
    m_Data = (BYTE *)MapViewOfFileEx(
        m_hMap, rw == TRUE ? FILE_MAP_WRITE : FILE_MAP_READ, 0, 0, 0, NULL);
