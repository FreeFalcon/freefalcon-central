...
falcon4LeakCheck::~falcon4LeakCheck()
{
    ...
    if (lastErrorFn)
    {
        dbgMemReportLeakage(NULL, MAIN_INIT_CHECK_POINT,
                            MAIN_DEINIT_CHECK_POINT);
    ...
    case MEM_LEAKAGE:
        if (errorInfo->objectCreationInfo)
            MonoPrint("FILE %s LINE %d SIZE %d LOCATION 0x%p\n",
                      errorInfo->objectCreationInfo->file,
                      errorInfo->objectCreationInfo->line,
                      errorInfo->objectCreationInfo->argSize,
                      errorInfo->objectCreationInfo->argPtr);
        else
            MonoPrint("FILE %s LINE %d SIZE %d LOCATION 0x%p\n",
                      errorInfo->file, errorInfo->line, errorInfo->argSize,
                      errorInfo->argPtr);
