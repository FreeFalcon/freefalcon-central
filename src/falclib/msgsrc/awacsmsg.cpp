...
VU_ID FindAircraftTarget(
    AircraftClass
        *theVehicle);  // 2001-10-26 ADDED BY S.G. Need it in request for help

FalconAWACSMessage::FalconAWACSMessage(VU_ID entityId, VuTargetEntity *target,
                                       VU_BOOL loopback)
    : FalconEvent(AWACSMsg, FalconEvent::SimThread, entityId, target, loopback)
{
    // Your Code Goes Here
...
FalconAWACSMessage::FalconAWACSMessage(VU_MSG_TYPE type, VU_ID senderid,
                                       VU_ID target)
    : FalconEvent(AWACSMsg, FalconEvent::SimThread, senderid, target)
{
    // Your Code Goes Here
    type;
}
...
static void CollectAssistCandidates(AircraftClass *plane,
                                    stdRange2FlightMap &arrCandidates)
{
        ...
        while (p)
        {
                ...
                (plane->GetTeam() == p->GetTeam()))
            {
                ...
                if (fRange <= fRangeMax)
                {
                    ...
                    {
                        arrCandidates.insert(
                            std::map<float, FlightClass *>::value_type(
                                fRange, pFlight));
    ...
    for (it = arrCandidates.begin(); it not_eq arrCandidates.end(); it++)
    {
        ...
        GetCallsign(it->second->callsign_id, it->second->callsign_num,
                    strFlightName);
...
int FalconAWACSMessage::Process(uchar autodisp)
{
    ...
    FalconEntity *otherThing =
        NULL; // 2002-02-28 REINSTATED BY S.G. Could be a sim or a campaign object
    ...
    ulong delay1 =
        g_nChatterInterval * CampaignSeconds; // FRB - user-controlled delay
    ...
    if (plane and plane->IsSim())
    {
        ...
        if (g_bAWACSRequired) // JPO - an awacs for this flight is required
        {
            ...
            if (awacs ==
                NULL) // 2001-09-23 ADDED BY M.N. no AWACS assigned to the package ?
            {
        ...
        case RequestPicture:
        {
            ...
            if (plane->IsLocal())
            {
                ...
                FalconAWACSMessage *awacsMsg =
                    new FalconAWACSMessage(plane->Id(), FalconLocalGame);
        ...
        case GivePicture:
        {
            ...
            theThreat =
                SimDriver.FindNearestEnemyPlane(plane, &X, &Y, &altitude);
            ...
            if (theThreat)
            {
                ...
                radioMessage->dataBlock.edata[8] =
                    (short)FloatToInt32(altitude);
                MonoPrint("Making threat call\n");
            }
            else
            {
                campThreat =
                    SimDriver.FindNearestCampEnemy(plane, &X, &Y, &altitude);

                if (campThreat)
                {
                    radioMessage =
                        CreateCallFromAwacsPlane(plane, rcPICTUREBRA);
                    //for now 4 sounds better (also almost always true :)
                    radioMessage->dataBlock.edata[4] = 4; //picture
                    //for now let's not play this part
                    radioMessage->dataBlock.edata[5] = -1; //split type
                ...
                {
                    radioMessage = CreateCallFromAwacsPlane(
                        plane, rcPICTURECLEAR, FalconLocalSession);
                    MonoPrint("No threats detected :->\n");
                }
            }

            radioMessage->dataBlock.time_to_play =
                delay; //wait a little bit so the request will play first
            FalconSendMessage(radioMessage, FALSE);
        }
        break;
        ...
        case RequestHelp:
            if (flight)
            {
                ...
                CampBaseClass *campThreat = NULL;
                SimBaseClass *simThreat =
                    NULL; //SimDriver.FindNearestEnemyPlane(plane, &X, &Y, &altitude);
                stdRange2FlightMap arrCandidates;
                ...
                if (tgtId)
                {
                    FalconEntity *newTarg =
                        (FalconEntity *)vuDatabase->Find(tgtId);

                    if (newTarg and
                        (newTarg->IsFlight() or newTarg->IsAirplane()))
                    {
                        if (newTarg->IsFlight())
                            ...
                            pEnemyFlight =
                                (FlightClass *)((SimBaseClass *)newTarg)
                                    ->GetCampaignObject();
                    }
                }
                ...
                {
                    simThreat =
                        SimDriver.FindNearestThreat(plane, &X, &Y, &altitude);

                    if (simThreat and
                        (simThreat->IsFlight() or simThreat->IsAirplane()))
                    {
                        if (simThreat->IsFlight())
                            ...
                            pEnemyFlight =
                                (FlightClass *)simThreat->GetCampaignObject();
                    }

                    else
                    {
                        campThreat = SimDriver.FindNearestCampThreat(
                            plane, &X, &Y, &altitude);
                ...
                if (pEnemyFlight and not pEnemyFlight->Aborted())
                {
                    ...
                    radioMessage->dataBlock.edata[4] =
                        -1; // No ETA time calculatable at this time
                    radioMessage->dataBlock.time_to_play =
        ...
        case RequestRelief:
            if (flight)
            {
                ...
                flight_ptr =
                    TheCampaign.MissionEvaluator->FindFlightData(flight);
                ...
                if (not hasFuel or not hasWeaps or
                    ((flight->GetEvalFlags() bitand FEVAL_MISSION_STARTED) and
                     (flight->GetEvalFlags() bitand FEVAL_GOT_TO_TARGET) and
                     (meflags bitand MISEVAL_FLIGHT_STATION_OVER)))
                {
                    if (rand() % 2)
        ...
        case OnStation:
            ...
            //TJL 12/14/03 Begin the new Check-in/PseudoFAC code
            {
                ...
                if (MissionData[flight->GetUnitMission()].flags bitand
                    AMIS_EXPECT_DIVERT)
                {
                    flight->SetUnitPriority(0);
                ...
                target = (CampBaseClass *)vuDatabase->Find(
                    flight->GetAssignedTarget());

                if (not target)
                    target = (CampBaseClass *)vuDatabase->Find(
                        flight->GetUnitMissionTargetID());
                ...
                if (not target)
                {
                    ...
                    radioMessage->dataBlock.edata[0] = flight->callsign_id;
                    radioMessage->dataBlock.edata[1] =
                        (short)ConvertFlightNumberToCallNumber(
                            flight->callsign_num);
                ...
                {
                    radioMessage =
                        CreateCallFromAwacs(flight, rcATTACKMYTARGET);
                    radioMessage->dataBlock.edata[5] =
                        SimToGrid(target->YPos());
                    radioMessage->dataBlock.edata[6] =
                        SimToGrid(target->XPos());
                }
        ...
        case OffStation:
            if (flight)
            {
                ...
                radioMessage = CreateCallFromAwacs(
                    flight, rcCOPY); // rcROGER is broken somehow
                radioMessage->dataBlock.time_to_play = delay;
                FalconSendMessage(radioMessage, FALSE);
            }
        ...
        case VectorToAltAirfield:
            if (flight)
            {
                ...
                ObjectiveClass *divertBase =
                    FindNearestFriendlyAirbase(plane->GetTeam(), X, Y);

                if (divertBase)
                {
                    pos.x = divertBase->XPos();
                    pos.y = divertBase->YPos();
                    ConvertSimToGrid(&pos, &radioMessage->dataBlock.edata[4],
                                     &radioMessage->dataBlock.edata[5]);
                }
                ...
                {
                    radioMessage->dataBlock.from =
                        flight->GetAWACSFlight()->Id();
                    radioMessage->dataBlock.voice_id =
                        (uchar)(flight->GetAWACSFlight())
                            ->GetFlightLeadVoiceID();
                    radioMessage->dataBlock.edata[2] =
                        (flight->GetAWACSFlight())->callsign_id;
                }
                else
                {
                ...
                if (w)
                    w->GetWPLocation(&radioMessage->dataBlock.edata[4],
                                     &radioMessage->dataBlock.edata[5]);
                else
                {
                    // KCK: No alternative airstrip.. should we try finding one? ATC List?
        ...
        case VectorToPackage:
            if (flight)
            {
                ...
                {
                    radioMessage =
                        CreateCallFromAwacs(flight, rcVECTORTOFLIGHT);
                    radioMessage->dataBlock.time_to_play =
                        delay; //wait a little bit so the request will play first
                    radioMessage->dataBlock.edata[4] =
                        SimToGrid(leadElement->YPos());
                    radioMessage->dataBlock.edata[5] =
                        SimToGrid(leadElement->XPos());
                }
                else if (flight->GetACCount() > 1)
                {
                    ...
                    radioMessage->dataBlock.time_to_play =
                        delay; //wait a little bit so the request will play first
                    radioMessage->dataBlock.edata[4] =
                        SimToGrid(flight->YPos());
                    radioMessage->dataBlock.edata[5] =
                        SimToGrid(flight->XPos());
                }
                else
                {
        ...
        case VectorToTanker:
            ...
            if (tankerFlight)
            {
                ...
                radioMessage->dataBlock.edata[5] =
                    (short)ConvertFlightNumberToCallNumber(
                        tankerFlight->callsign_num);

                if (tanker)
                {
                    radioMessage->dataBlock.edata[6] =
                        SimToGrid(tanker->YPos());
                    radioMessage->dataBlock.edata[7] =
                        SimToGrid(tanker->XPos());
                }
                else
                {
                    radioMessage->dataBlock.edata[6] =
                        SimToGrid(tankerFlight->YPos());
                    radioMessage->dataBlock.edata[7] =
                        SimToGrid(tankerFlight->XPos());
                }

                radioMessage->dataBlock.edata[8] =
                    (short)(tankerFlight->tacan_channel - g_nMinTacanChannel);
            ...
            if (flight)
            {
                ...
                float dist, bestdist = 99999.9F, dx, dy;
                {
                    ...
                    while (nu)
                    {
                        ...
                        if (cf->GetTeam() not_eq plane->GetTeam() or
                            cf->IsDead())
                            continue;

                        // RV - Biker - All naval units return TRUE here so better check for subtype
                        if (not cf->IsTaskForce() or
                            cf->GetSType() not_eq STYPE_UNIT_CARRIER)
                            continue;
                ...
                if (carrier)
                {
                    //awacs response
                    radioMessage =
                        CreateCallFromAwacs(flight, rcVECTORHOMECARRIER);

                    radioMessage->dataBlock.time_to_play =
                        delay; //wait a little bit so the request will play first

                    radioMessage->dataBlock.edata[4] =
                        SimToGrid(carrier->YPos());
                    radioMessage->dataBlock.edata[5] =
                        SimToGrid(carrier->XPos());
                    radioMessage->dataBlock.edata[6] =
                        (short)(carrier->tacan_channel - g_nMinTacanChannel);
        ...
        case VectorToThreat:
            if (flight)
            {
                ...
                {
                    numAircraft =
                        simThreat->GetCampaignObject()->NumberOfComponents();
                    radioMessage =
                        CreateCallFromAwacs(flight, rcNEARESTTHREATRSP);

                    //for this request we just want the BRA part
                    if (numAircraft > 1)
                        radioMessage->dataBlock.edata[4] =
                            (short)((simThreat->Type() - VU_LAST_ENTITY_TYPE) *
                                        2 +
                                    1); //type
                    else
                        radioMessage->dataBlock.edata[4] =
                ...
                {
                    campThreat = SimDriver.FindNearestCampThreat(plane, &X, &Y,
                                                                 &altitude);
                    ...
                    if (campThreat)
                    {
                        ...
                        if (numAircraft > 1)
                            radioMessage->dataBlock.edata[4] =
                                (short)(((Unit)campThreat)->GetVehicleID(0) *
                                            2 +
                                        1); //type
                        else
                            radioMessage->dataBlock.edata[4] =
        ...
        case VectorToTarget:
            if (flight)
            {
                ...
                else if (target->IsUnit() and target->GetDomain() == DOMAIN_AIR)
                {
                    ...
                    target->GetLocation(&radioMessage->dataBlock.edata[4],
                                        &radioMessage->dataBlock.edata[5]);
                    radioMessage->dataBlock.edata[6] =
        ...
        case DeclareAircraft:
            ...
            {
                RadarClass *theRadar =
                    (RadarClass *)FindSensor(plane, SensorClass::Radar);
                VU_ID targetId;
                ...
                if (theRadar)
                {
                    ...
                    if (targetId == FalconNullId and
                        theRadar->CurrentTarget() and
                        theRadar->CurrentTarget()->BaseData())
                    {
            ...
            if (otherThing)
            {
                ...
                if (otherThing->IsSim())
                {
                    ...
                    campThing =
                        ((SimBaseClass *)otherThing)->GetCampaignObject();
                }
                else
                {
                    campThing = (CampBaseClass *)otherThing;
                    simThing =
                        0; // MLR 2003-11-20 - CTD because this may not get initialized below.
                }
                ...
                if (campThing and campThing->GetIdentified(plane->GetTeam()))
                {
                    ...
                    radioMessage = CreateCallFromAwacsPlane(
                        plane, rcDECLAREIDENTIFIEDTARGET);

                    if (GetTTRelations(otherThing->GetTeam(),
                                       plane->GetTeam()) < Hostile)
                    {
                    ...
                    else
                    {
                        ...
                        else if (randNum < 20 and
                                 GetTTRelations(otherThing->GetTeam(),
                                                plane->GetTeam()) < War)
                        {
                    ...
                    if (simThing)
                    {
                        ...
                        radioMessage->dataBlock.edata[5] =
                            (short)((simThing->Type() - VU_LAST_ENTITY_TYPE) *
                                    2); //type
                    }
                    else
                    {
                        ...
                        if (numAircraft > 1)
                        {
                            //type
                            radioMessage->dataBlock.edata[5] =
                                (short)(((Unit)campThing)->GetVehicleID(0) * 2 +
                                        1);
                        }
                        else
                        {
