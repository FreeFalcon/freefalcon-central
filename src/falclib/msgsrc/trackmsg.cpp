...
FalconTrackMessage::FalconTrackMessage(int reliable, VU_ID entityId,
                                       VuTargetEntity* target, VU_BOOL loopback)
    : FalconEvent(TrackMsg, FalconEvent::SimThread, entityId, target, loopback)
{
    if (reliable == 1)
    {
...
FalconTrackMessage::FalconTrackMessage(VU_MSG_TYPE type, VU_ID senderid,
                                       VU_ID target)
    : FalconEvent(TrackMsg, FalconEvent::SimThread, senderid, target)
{
}
...
int FalconTrackMessage::Process(uchar autodisp)
{
    ...
    FalconEntity* taggedEntity =
        static_cast<FalconEntity*>(vuDatabase->Find(dataBlock.id));
    ...
    if (theEntity->IsMover())
    {
        ...
        if (not mEntity->IsLocal() and mEntity->numSensors > 1 and
            // MN 2002-03-03 CTD
    ...
    if (taggedEntity)
    {
        ...
        else if (dataBlock.trackType == Track_JettisonWeapon)
        {
            if ((not IsLocal()) and (taggedEntity->IsAirplane()))
            {
                ...
                ((AircraftClass*)taggedEntity)
                    ->Sms->JettisonWeapon(dataBlock.hardpoint);
            }
        }
        else if (dataBlock.trackType == Track_RemoveWeapon)
        {
            if ((not IsLocal()) and (taggedEntity->IsAirplane()))
            {
                ...
                ((AircraftClass*)taggedEntity)
                    ->Sms->RemoveWeapon(dataBlock.hardpoint);
            }
        }
        else if (taggedEntity->IsLocal())
        {
            ...
            if (taggedEntity->IsAirplane())
            {
                // Find RWR and/or HTS
                for (int j = 0;
                     j < ((SimVehicleClass*)taggedEntity)->numSensors; j++)
                {
                    if (((SimVehicleClass*)taggedEntity)
                            ->sensorArray[j]
                            ->Type() == SensorClass::RWR)
                    {
                        ((RwrClass*)((SimVehicleClass*)taggedEntity)
                             ->sensorArray[j])
                            ->ObjectDetected(
                                theEntity, dataBlock.trackType,
                                dataBlock
                                    .hardpoint); // 2002-02-09 MODIFIED BY S.G. Added hardpoint to ObjectDetected
                    }

                    if (((SimVehicleClass*)taggedEntity)
                            ->sensorArray[j]
                            ->Type() == SensorClass::HTS)
                    {
                        ((RwrClass*)((SimVehicleClass*)taggedEntity)
                             ->sensorArray[j])
                            ->ObjectDetected(
            ...
            {
                if ((dataBlock.trackType == Track_Lock) or
                    (dataBlock.trackType == Track_Launch))
                {
