...
int SendSimMoverPosition::Decode(VU_BYTE **buf, long *rem)
{
...
int SendSimMoverPosition::Encode(VU_BYTE **buf)
{
    FalconEvent::Encode(buf);

    memcpy(*buf, &yaw_, sizeof(yaw_));
    *buf += sizeof(yaw_);
    memcpy(*buf, &pitch_, sizeof(pitch_));
    *buf += sizeof(pitch_);
    memcpy(*buf, &roll_, sizeof(roll_));
    *buf += sizeof(roll_);
    memcpy(*buf, &dyaw_, sizeof(yaw_));
    *buf += sizeof(dyaw_);
    memcpy(*buf, &dpitch_, sizeof(pitch_));
    *buf += sizeof(dpitch_);
    memcpy(*buf, &droll_, sizeof(roll_));
    *buf += sizeof(droll_);
    ...
    *buf += sizeof(z_);
    memcpy(*buf, &dx_, sizeof(dx_));
    *buf += sizeof(dx_);
    memcpy(*buf, &dy_, sizeof(dy_));
    *buf += sizeof(dy_);
    memcpy(*buf, &dz_, sizeof(dz_));
    *buf += sizeof(dz_);

    return Size();
}

int SendSimMoverPosition::Process(uchar autodisp)
{
    if (autodisp)
    {
        return 0;
