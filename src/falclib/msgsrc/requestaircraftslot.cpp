...
UI_RequestAircraftSlot::UI_RequestAircraftSlot(VU_ID entityId,
                                               VuTargetEntity *target,
                                               VU_BOOL loopback)
    : FalconEvent(RequestAircraftSlot, FalconEvent::SimThread, entityId, target,
                  loopback)
{
...
UI_RequestAircraftSlot::UI_RequestAircraftSlot(VU_MSG_TYPE type, VU_ID senderid,
                                               VU_ID target)
...
int UI_RequestAircraftSlot::ChangeFlightType(Flight flight)
{
    MonoPrint("Request Type Change %08x To %d\n", flight,
              dataBlock.requested_type);
    flight->SetEntityType(dataBlock.requested_type);
    flight->class_data =
        (UnitClassDataType *)
            Falcon4ClassTable[dataBlock.requested_type - VU_LAST_ENTITY_TYPE]
                .dataPtr;
...
int UI_RequestAircraftSlot::EmptyFlightSlot(Flight flight)
{
    MonoPrint("Empty Flight Slot %08x From %d To %d\n", flight,
              dataBlock.current_pilot_slot, dataBlock.requested_slot);
    ...
    if (FalconLocalGame->GetGameType() == game_Dogfight)
    {
        ...
        if (flight->GetTotalVehicles() < 1)
        {
            RegroupFlight(flight);
            vuDatabase->Remove(
                flight); // Also, remove it from database immediately
        }
    }
...
int UI_RequestAircraftSlot::AddFlightSlot(Flight flight)
{
    ...
    if (not flight)
    {
        if (FalconLocalGame->GetGameType() == game_Dogfight)
        {
            MonoPrint("NT %d T %d : ", dataBlock.requested_type,
                      dataBlock.team);
    ...
    {
        MonoPrint("R %08x %d %d : ", requester->GetPlayerFlight(),
                  requester->GetAircraftNum(), requester->GetPilotSlot());
    ...
    if (flight)
    {
            ...
            for (i = 0; i < PILOTS_PER_FLIGHT and not retval; i++)
            {
                if (flight->plane_stats[i] == AIRCRAFT_NOT_ASSIGNED)
                {
                    ...
                    if (dataBlock.request_type == REQUEST_SLOT_JOIN_PLAYER)
                    {
                        ...
                        msg->dataBlock.game_type =
                            (uchar)FalconLocalGame->GetGameType();
                        ...
                        msg->dataBlock.got_pilot_slot =
                            dataBlock.current_pilot_slot;
                        flight->player_slots[i] = dataBlock.current_pilot_slot;
                        flight->pilots[i] = NO_PILOT;
                        msg->dataBlock.got_slot = (uchar)i;
                        requester->SetAssignedPlayerFlight(flight);
                        requester->SetAssignedAircraftNum(
                            msg->dataBlock.got_slot);
                        requester->SetAssignedPilotSlot(
                            msg->dataBlock.got_pilot_slot);
                        flight->MakeFlightDirty(DIRTY_PILOTS,
                                                SEND_RELIABLEANDOOB);
                        flight->MakeFlightDirty(DIRTY_PLANE_STATS,
                                                SEND_RELIABLEANDOOB);
                    }
                    else
                    {
        ...
        else if (dataBlock.request_type not_eq REQUEST_SLOT_JOIN_AI)
        {
            ...
            for (i = 0; i < PILOTS_PER_FLIGHT; i++)
            {
                MonoPrint("  Slot %d Stats %d\n", flight->player_slots[i],
                          flight->plane_stats[i]);
            }

            if (flight->player_slots[dataBlock.requested_slot] == 255 and
                flight->plane_stats[dataBlock.requested_slot] ==
                    AIRCRAFT_AVAILABLE)
            {
                ...
                flight->player_slots[dataBlock.requested_slot] =
                    dataBlock.current_pilot_slot;
                msg->dataBlock.result = REQUEST_RESULT_SUCCESS;
            ...
            else
            {
                ...
                for (i = 0; i < PILOTS_PER_FLIGHT and not retval; i++)
                {
                    if (flight->player_slots[i] == 255 and
                        flight->plane_stats[i] == AIRCRAFT_AVAILABLE)
                    {
                        if (dataBlock.current_pilot_slot == 255)
                        {
