...
FalconAirTaskingMessage::FalconAirTaskingMessage(VU_ID entityId,
                                                 VuTargetEntity *target,
                                                 VU_BOOL loopback)
    : FalconEvent(AirTaskingMsg, FalconEvent::CampaignThread, entityId, target,
                  loopback)
{
...
FalconAirTaskingMessage::FalconAirTaskingMessage(VU_MSG_TYPE type,
...
int FalconAirTaskingMessage::Process(uchar autodisp)
{
    ...
    if (not TeamInfo[dataBlock.team]->atm or
        not TeamInfo[dataBlock.team]->atm->IsLocal())
    {
        return -1;
    }
