...
FalconUnitMessage::FalconUnitMessage(VU_ID entityId, VuTargetEntity *target,
                                     VU_BOOL loopback)
    : FalconEvent(UnitMsg, FalconEvent::CampaignThread, entityId, target,
                  loopback)
{
...
FalconUnitMessage::FalconUnitMessage(VU_MSG_TYPE type, VU_ID senderid,
                                     VU_ID target)
    : FalconEvent(UnitMsg, FalconEvent::CampaignThread, senderid, target)
{
    // Your Code Goes Here
    type;
}
...
int FalconUnitMessage::Process(uchar autodisp)
{
    ...
    case unitRequestMet:
        u->HandleRequestReceipt(dataBlock.data1, dataBlock.data2,
                                dataBlock.from);
        break;
    ...
    case unitSchedulePilots:
        ...
        {
            ((Squadron)u)
                ->SetPilotStatus(dataBlock.data1, (uchar)dataBlock.data2);

            if (dataBlock.data3 >= 0)
                ((Squadron)u)->GetPilotData(dataBlock.data1)->pilot_id =
                    dataBlock.data3;
        }
    ...
    case unitScheduleAC:
        ...
        if (u->GetType() == TYPE_SQUADRON and u->GetDomain() == DOMAIN_AIR)
            ((Squadron)u)
                ->SetSchedule(dataBlock.data1,
                              dataBlock.data2 bitor dataBlock.data3);
    ...
    case unitSetVehicles:
        ...
        else if (dataBlock.data1 == UMSG_FROM_RESERVE)
        {
            ...
            if (u->GetType() == TYPE_SQUADRON and u->GetDomain() == DOMAIN_AIR)
            {
                ...
                if (losses)
                    ((Squadron)u)
                        ->SetTotalLosses(
                            (uchar)(((Squadron)u)->GetTotalLosses() + losses));
            ...
            for (i = VEHICLE_GROUPS_PER_UNIT - 1; i >= 0 and dataBlock.data2;
                 i--)
            {
        ...
        if (u->GetType() == TYPE_FLIGHT and u->GetDomain() == DOMAIN_AIR)
        {
            ...
            {
                if (((Flight)u)->slots[i] > VEHICLE_GROUPS_PER_UNIT or
                    ((Flight)u)->slots[i] == dataBlock.data1)
                {
