...
FalconATCMessage::FalconATCMessage(VU_ID entityId, VuTargetEntity *target,
                                   VU_BOOL loopback)
    : FalconEvent(ATCMsg, FalconEvent::SimThread, entityId, target, loopback)
{
    //me123 RequestOutOfBandTransmit ();
    RequestReliableTransmit(); //me123
}

FalconATCMessage::FalconATCMessage(VU_MSG_TYPE type, VU_ID senderid,
                                   VU_ID target)
    : FalconEvent(ATCMsg, FalconEvent::SimThread, senderid, target)
{
    // Your Code Goes Here
}
...
void HandleInboundFlight(ObjectiveClass *atc, Flight flight)
{
    ...
    if (index)
    {
        ...
        {
        case 0:
            radioMessage = CreateCallFromATC(atc, aircraft, rcCONTINUEINBOUND1,
                                             FalconLocalGame);
            //M.N. changed to 32767 -> flexibly use randomized value of max available eval indexes
            radioMessage->dataBlock.edata[4] = 32767;
            radioMessage->dataBlock.edata[5] = (short)atcBrain->GetRunwayName(
                atcBrain->GetOppositeRunway(index));
        ...
        case 1:
            radioMessage = CreateCallFromATC(atc, aircraft, rcCONTINUEINBOUND2,
                                             FalconLocalGame);
        ...
        case 2:
            radioMessage = CreateCallFromATC(atc, aircraft, rcCONTINUEINBOUND3,
                                             FalconLocalGame);
            radioMessage->dataBlock.edata[4] = -1;
            radioMessage->dataBlock.edata[5] = -1;
            radioMessage->dataBlock.edata[6] = 32767;
            radioMessage->dataBlock.edata[7] = (short)atcBrain->GetRunwayName(
                atcBrain->GetOppositeRunway(index));
    ...
    else
    {
        ...
        while (aircraft)
        {
            ...
            FalconATCCmdMessage *ATCCmdMessage =
                new FalconATCCmdMessage(aircraft->Id(), FalconLocalGame);
...
int FalconATCMessage::Process(uchar autodisp)
{
    ...
    if (aircraft and aircraft->IsAirplane())
    {
        ...
        case RequestClearance:
            if (not aircraft->IsPlayer() or not aircraft->IsLocal())
            {
                if (aircraft->pctStrength < STRENGTH_PCT_FOR_EMERG_LDG)
                    SendCallToATC(aircraft, EntityId(), rcLANDCLEAREMERGENCY,
                                  FalconLocalSession);
                else
                    SendCallToATC(aircraft, EntityId(), rcLANDCLEARANCE,
                                  FalconLocalSession);
            }
            ...
            if (atcBrain and atc->IsLocal())
            {
                ...
                if (info)
                {
                    cosAngle = atcBrain->DetermineAngle(
                        aircraft, acBrain->Runway(), lOnFinal);
                    ...
                    if (info->status >=
                        tReqTaxi) // JB It appears to work but this was called a bit much for my comfort level. We'll try another approach.
                    {
                        if (not aircraft->OnGround())
                        {
                            if (dist <
                                (TOWER_RANGE + 100) * NM_TO_FT * NM_TO_FT)
                            {
                                if (not aircraft->IsPlayer() and
                                    aircraft->pctStrength <
                                        STRENGTH_PCT_FOR_EMERG_LDG)
                                    atcBrain->RequestEmerClearance(aircraft);
                                else
                                    atcBrain->RequestClearance(aircraft);
                            }
                            else if (dist < APPROACH_RANGE * NM_TO_FT *
                                                NM_TO_FT and
                                     dist >= (TOWER_RANGE + 100) * NM_TO_FT *
                                                 NM_TO_FT)
                            {
                                if (not aircraft->IsPlayer() and
                                    aircraft->GetCampaignObject()
                                            ->GetComponentLead() == aircraft)
                                    HandleInboundFlight(
                                        atc,
                            ...
                            else
                            {
                                ...
                                radioMessage = CreateCallFromATC(
                                    atc, aircraft, rcOUTSIDEAIRSPEED,
                                    FalconLocalGame);
                    ...
                    else if (info->rwindex)
                    {
                        ...
                        case lTakingPosition:
                            ...
                            radioMessage = CreateCallFromATC(atc, aircraft,
                                                             rcCONTINUEINBOUND1,
                                                             FalconLocalGame);
                            //M.N. changed to 32767 -> flexibly use randomized value of max available eval indexes
                            radioMessage->dataBlock.edata[4] = 32767;
                            radioMessage->dataBlock.edata[5] =
                                (short)atcBrain->GetRunwayName(
                                    atcBrain->GetOppositeRunway(info->rwindex));
                        ...
                        case lAborted:
                            ...
                            radioMessage =
                                CreateCallFromATC(atc, aircraft, rcATCGOAROUND,
                                                  FalconLocalSession);
                        ...
                        case lHolding:
                            radioMessage = CreateCallFromATC(
                                atc, aircraft, rcATCORBIT2, FalconLocalGame);
                            radioMessage->dataBlock.edata[2] =
                                -1; //altitude in thousands
                            radioMessage->dataBlock.edata[3] =
                                -1; //altitude in thousands
                            break;
                        ...
                        case lToFinal:
                            radioMessage = CreateCallFromATC(atc, aircraft,
                                                             ...
                                                             FalconLocalGame);
                            radioMessage->dataBlock.edata[4] =
                                (short)atcBrain->GetLandingNumber(info);
                            //atcBrain->SendCmdMessage(aircraft, info);
                            break;

                        case lOnFinal:
                        case lClearToLand:
                            if (aircraft->DBrain()->IsSetATC(
                                    DigitalBrain::ClearToLand))
                            {
                                radioMessage = CreateCallFromATC(
                                    atc, aircraft, rcCLEAREDLAND,
                                    FalconLocalGame);
                                radioMessage->dataBlock.edata[4] =
                                    (short)atcBrain->GetRunwayName(
                                        atcBrain->GetOppositeRunway(
                                            acBrain->Runway()));
                            }
                            else
                            {
                                radioMessage = CreateCallFromATC(
                                    atc, aircraft, rcATCLANDSEQUENCE,
                                    FalconLocalGame);
                                radioMessage->dataBlock.edata[4] =
                                    (short)atcBrain->GetLandingNumber(info);
                            }
                        ...
                        case lLanded:
                            radioMessage = CreateCallFromATC(
                                atc, aircraft, rcTAXICLEAR, FalconLocalGame);
                            break;
                    ...
                    {
                        radioMessage = CreateCallFromATC(
                            atc, aircraft, rcCONTINUEINBOUND2, FalconLocalGame);
                        ...
                        {
                            time_in_minutes =
                                TheCampaign.GetMinutesSinceMidnight();
                ...
                else
                {
                    ...
                    {
                        if (aircraft->GetCampaignObject()->GetComponentLead() ==
                            aircraft)
                        {
                            ...
                            while (element)
                            {
                                runwayQueueStruct *tempinfo =
                                    atcBrain->InList(element->Id());

                                if (not tempinfo)
                                {
                                    if (not element->IsPlayer() and
                                        element->pctStrength <
                                            STRENGTH_PCT_FOR_EMERG_LDG)
                                    {
                                        SendCallToATC(element, EntityId(),
                                                      rcLANDCLEAREMERGENCY,
                                                      FalconLocalGame);
                        ...
                        else
                        {
                            if (not aircraft->IsPlayer() and
                                aircraft->pctStrength <
                                    STRENGTH_PCT_FOR_EMERG_LDG)
                            {
                                SendCallToATC(aircraft, EntityId(),
                                              rcLANDCLEAREMERGENCY,
                                              FalconLocalGame);
                                atcBrain->RequestEmerClearance(aircraft);
                            }
                    ...
                    else if (dist < APPROACH_RANGE * NM_TO_FT * NM_TO_FT and
                             dist > (TOWER_RANGE + 100) * NM_TO_FT * NM_TO_FT)
                    {
                        if (aircraft->GetCampaignObject()->GetComponentLead() ==
                            aircraft)
                            HandleInboundFlight(
                                atc, (Flight)aircraft->GetCampaignObject());
                        else
                            HandleInbound(atc, aircraft);
                    }
                    else
                    {
                        //note this comm should be rcOUTSIDEAIRSPACE, but Joe misnamed it
                        radioMessage = CreateCallFromATC(
                            atc, aircraft, rcOUTSIDEAIRSPEED, FalconLocalGame);
        ...
        case RequestTakeoff:
            if (not aircraft->IsPlayer() or not aircraft->IsLocal())
                SendCallToATC(aircraft, EntityId(), rcREADYFORDERARTURE,
                              FalconLocalSession);
...
#define TEST_RADIO 0
            ...
            if (atcBrain and atc->IsLocal())
            {
                ...
                SendCallFromATC(atc, aircraft, rcOUTSIDEAIRSPEED,
                                static_cast<VuTargetEntity *>(
                                    vuDatabase->Find(aircraft->OwnerId())));
        ...
        case AbortApproach:
            if (not aircraft->IsPlayer() or not aircraft->IsLocal())
                SendCallToATC(aircraft, EntityId(), rcABORTAPPROACH,
                              FalconLocalSession);
        ...
        case RequestHotpitRefuel:
            ...
            aircraft->requestHotpitRefuel =
                TRUE; //Cobra 11/13/04 TJL will this make online work?
            ...
            if (atcBrain)
            {
                if (atc->IsLocal())
                {
                    ...
                    if (info)
                    {
                        ...
                        case lCrashed:
                            ...
                            info->status = lCrashed;
                            {
                                ...
                                if (GetQueue(Runway) not_eq
                                    GetQueue(info->rwindex))
                                {
                                    atcBrain->RemoveTraffic(
                                        aircraft->Id(),
                                        GetQueue(info->rwindex));
                                    atcBrain->AddTraffic(aircraft->Id(),
                                                         lCrashed, Runway,
                                                         SimLibElapsedTime);
                                }
                    ...
                    else
                    {
                        ...
                        case lCrashed:
                        {
                            atcBrain->RemoveFromAllOtherATCs(aircraft);
                            int Runway = atcBrain->IsOverRunway(aircraft);
                            atcBrain->AddTraffic(aircraft->Id(), lCrashed,
                                                 Runway, SimLibElapsedTime);
                            atcBrain->FindNextEmergency(GetQueue(Runway));
                            atcBrain->SetEmergency(GetQueue(Runway));
                        }
                        break;
                ...
                case lTakingPosition:
                    ...
                    acBrain->SetTrackPoint(
                        x, y, atcBrain->GetAltitude(aircraft, lTakingPosition));
                    break;

                case lEmerHold:
                case lHolding:
                    acBrain->SetTrackPoint(
                        aircraft->XPos(), aircraft->YPos(),
                        atcBrain->GetAltitude(aircraft, lHolding));
                    break;

                case lFirstLeg:
                    if (acBrain->ATCStatus() not_eq lFirstLeg and
                        acBrain->ATCStatus() <= lOnFinal)
                    {
                        atcBrain->FindFinalPt(aircraft, acBrain->Runway(),
                                              &finalX, &finalY);
                        cosAngle = atcBrain->DetermineAngle(
                            aircraft, acBrain->Runway(), lFirstLeg);

                        if (cosAngle < 0.0F)
                        {
                            atcBrain->FindBasePt(aircraft, acBrain->Runway(),
                                                 finalX, finalY, &baseX,
                                                 &baseY);
                            atcBrain->FindFirstLegPt(
                                aircraft, acBrain->Runway(), acBrain->RwTime(),
                ...
                case lToBase:
                    if (acBrain->ATCStatus() not_eq lToBase and
                        acBrain->ATCStatus() <= lOnFinal)
                    {
                        atcBrain->FindFinalPt(aircraft, acBrain->Runway(),
                                              &finalX, &finalY);
                        ...
                        acBrain->SetTrackPoint(
                            baseX, baseY,
                            atcBrain->GetAltitude(aircraft, lToBase));
                        acBrain->CalculateNextTurnDistance();
                    }
                ...
                case lToFinal:
                    if (acBrain->ATCStatus() not_eq lToFinal and
                        acBrain->ATCStatus() <= lOnFinal)
                    {
                        atcBrain->FindFinalPt(aircraft, acBrain->Runway(),
                                              &finalX, &finalY);
                ...
                case lOnFinal:
                    TranslatePointData(atc, GetFirstPt(acBrain->Runway()), &x,
                                       &y);

                    //if we sent the message we already know this
                    if (acBrain->ATCStatus() not_eq lOnFinal and
                        acBrain->ATCStatus() <= lOnFinal)
                    {
                    ...
                    if (not aircraft->IsPlayer())
                    {
                        ...
                        if (atcBrain->CalculateStandRateTurnToPt(aircraft, x, y,
                                                                 &finalHdg) > 0)
                            radioMessage->dataBlock.edata[2] = 0;
                        else
                            radioMessage->dataBlock.edata[2] = 1;
...
#endif
                        finalHdg =
                            PtHeaderDataTable[acBrain->Runway()].data + 180.0F;
                ...
                case lAborted:
                    ...
                    if (atc->IsLocal())
                    {
                        atcBrain->RemoveTraffic(
                            aircraft->Id(),
                            PtHeaderDataTable[acBrain->Runway()].runwayNum);
                    }
                ...
                case lEmergencyOnFinal:
                    ...
                    {
                        TranslatePointData(atc, GetFirstPt(acBrain->Runway()),
                                           &x, &y);
                ...
                case tTaxi:
                    if (acBrain->ATCStatus() not_eq tTaxi)
                    {
                        ...
                        TranslatePointData(atc, acBrain->GetTaxiPoint(), &x,
                                           &y);
                        acBrain->SetTrackPoint(
                            x, y, atcBrain->GetAltitude(aircraft, tTaxi));
                    }
                ...
                case tHoldShort:
                    ...
                    if (acBrain->ATCStatus() not_eq tHoldShort)
                    {
                        ...
                        acBrain->SetTrackPoint(
                            x, y, atcBrain->GetAltitude(aircraft, tHoldShort));
                    }
                ...
                case tPrepToTakeRunway:
                    ...
                    if (acBrain->ATCStatus() not_eq tTaxi)
                    {
                        ...
                        if (PtDataTable[acBrain->GetTaxiPoint()].type ==
                            TakeoffPt)
                            atcBrain->FindTakeoffPt(
                                (Flight)aircraft->GetCampaignObject(),
                                aircraft->vehicleInUnit, acBrain->Runway(), &x,
                                &y);
                        else
                            TranslatePointData(atc, acBrain->GetTaxiPoint(), &x,
                                               &y);
                ...
                case tTakeRunway:
                    if (acBrain->ATCStatus() not_eq tTakeRunway)
                    {
                        ...
                        atcBrain->FindTakeoffPt(
                            (Flight)aircraft->GetCampaignObject(),
                            aircraft->vehicleInUnit, acBrain->Runway(), &x, &y);
                        acBrain->SetTrackPoint(
                            x, y, atcBrain->GetAltitude(aircraft, tTakeRunway));
                    }
