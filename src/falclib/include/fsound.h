...
#define VF_FLIGHTNUMBER_OFFSET 36
    ...
    void F4PitchBend(
        int soundIdx,
        float PitchMultiplier); // new frequency = orignal freq * PitchMult
    void F4PanSound(int soundIdx,
                    int Direction); // dBs -10000 to 10000 (0=Center)
    ...
    // MLR 2003-11-05
    void F4ReloadSFX(
        void); // dump and reload sfx table - useful for developing the sound table.

    void F4SoundEntering3d(
        void); // MLR 12/13/2003 - called when OTW is going to the 3d world
    void F4SoundLeaving3d(
        void); // MLR 12/13/2003 - called when OTW is leaving the 3d world
...
#include "soundgroups.h"
    ...
    void F4SoundFXSetPos(int sfxId, int override, float x, float y, float z,
                         float pscale, float volume = 0.0F, int uid = 0);
    ...
    void F4SoundFXSetCamPosAndOrient(
        Tpoint *campos, Trotation *camrot,
        Tpoint
            *camvel); // MLR 12/2/2003 - Added velocity (back) to this function
...
public:
    ...
    void Sfx(int SfxID, int subid, float pscale, float vol, float x, float y,
             float z);
    void SfxRel(int SfxID, int subid, float pscale, float vol, float x, float y,
                float z);
