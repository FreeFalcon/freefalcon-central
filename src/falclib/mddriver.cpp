...
static char *commonFileList[] = {
    ...
    "$A/campaign/save/taceng.db = b934573361ea317fe63bf1bb06595471",
...
static char **localizedFiles[] = {englishFiles, englishFiles,  englishFiles,
                                  germanFiles,  frenchFiles,   spanishFiles,
                                  italianFiles, brazilianFiles};
                   ...
                   unsigned int len)
{
    ...
    for (i = 0, j = 0; j < len; i++, j += 4)
        output[i] = ((u_int32_t)input[j]) bitor
                    (((u_int32_t)input[j + 1]) << 8) |
                    (((u_int32_t)input[j + 2]) << 16) bitor
...
char *MD5File(const char *filename, char *buf)
{
    ...
    f = _open(
        filename,
        O_RDONLY); // #104: _open so the Linux shim normalises '\'->'/' + case-folds the path
