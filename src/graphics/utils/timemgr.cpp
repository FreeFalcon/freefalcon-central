...
void TimeManager::ReleaseTimeUpdateCB(void (*fn)(void *), void *self)
{
    ...
    // The callback array can already be gone during shutdown teardown (Cleanup sets CBlist = NULL), and a late
    // release (e.g. CTimeOfDay::Cleanup on exit) would then dereference NULL -> SIGSEGV on exit. RegisterTimeUpdateCB
