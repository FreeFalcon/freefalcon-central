...
bool          g_bUseD3D12     = true;   // Artscout - 2026: DX12 is the default backend now (override via cfg "set g_bUseD3D12 0")

// Artscout - 2026: #DX12 -- "GPU render mode" = D3D11 OR D3D12 (as opposed to the dead legacy DDraw7 path).
// Set true in DXContext::Init for BOTH GPU backends. The pervasive `if(g_bUseD3D11){GPU}else{DDraw7}` gates
// conflate two meanings: (a) "GPU vs DDraw" and (b) "the D3D11 renderer specifically". Meaning-(a) gates use
// g_bUseGpu (so D3D12 never falls into the null-deref DDraw7 else-branch); meaning-(b) gates stay on
// g_bUseD3D11 (D3D12 skips those D3D11-only render calls). DDraw7 is never initialized in either GPU mode.
...
#define D12_RELEASE(p) do { if (p) { (p)->Release(); (p) = 0; } } while (0)
...
bool D3D12Backend::Init(HWND hWnd, int nWidth, int nHeight, int nDepth, bool bFullscreen)
{
	D12Log("D3D12Backend::Init(0x%p, %d, %d, %d, %d)\n", (void*)hWnd, nWidth, nHeight, nDepth, (int)bFullscreen);
	...
	{
		m_hWnd = hWnd;
		if (nWidth != m_nWidth || nHeight != m_nHeight) Resize(nWidth, nHeight);
		D12Log("D3D12Backend::Init - already initialized, reusing the device\n");
		return true;
	}
...
#ifdef _DEBUG
	{
		...
		if (!hD12) hD12 = LoadLibraryA("d3d12.dll");
		if (hD12)
		{
	...
	HRESULT hr = D3D12CreateDevice(pChosenAdapter, D3D_FEATURE_LEVEL_11_0, IID_PPV_ARGS(&m_pDevice));
	if ((FAILED(hr) || !m_pDevice) && pChosenAdapter)   // chosen adapter failed -> retry on default
	{
		D12Log("[D3D12] D3D12CreateDevice(chosen adapter) failed 0x%08X, retrying default\n", (unsigned)hr);
	...
	if (FAILED(hr)) { D12Log("[D3D12] CreateCommandQueue failed 0x%08X\n", (unsigned)hr); return false; }

	// --- flip swap chain via DXGI factory ---
	IDXGIFactory4* factory = 0;
	hr = CreateDXGIFactory2(0, IID_PPV_ARGS(&factory));
	if (FAILED(hr) || !factory) { D12Log("[D3D12] CreateDXGIFactory2 failed 0x%08X\n", (unsigned)hr); return false; }
	...
	hr = factory->CreateSwapChainForHwnd(m_pQueue, hWnd, &scd, NULL, NULL, &sc1);
	if (FAILED(hr) || !sc1) { D12Log("[D3D12] CreateSwapChainForHwnd failed 0x%08X\n", (unsigned)hr); factory->Release(); return false; }
	factory->MakeWindowAssociation(hWnd, DXGI_MWA_NO_ALT_ENTER);   // we manage fullscreen
	hr = sc1->QueryInterface(IID_PPV_ARGS(&m_pSwapChain));
	sc1->Release();
	factory->Release();
	if (FAILED(hr) || !m_pSwapChain) { D12Log("[D3D12] QI IDXGISwapChain3 failed 0x%08X\n", (unsigned)hr); return false; }
	...
	if (FAILED(hr)) { D12Log("[D3D12] CreateDescriptorHeap(RTV) failed 0x%08X\n", (unsigned)hr); return false; }
	...
	if (!CreateDepthBuffer())     return false;   // Artscout - 2026: #DX12 Phase 3 -- scene depth

	// --- per-frame command allocators + one command list (created closed) ---
	for (int i = 0; i < kFrameCount; ++i)
	{
		hr = m_pDevice->CreateCommandAllocator(D3D12_COMMAND_LIST_TYPE_DIRECT, IID_PPV_ARGS(&m_pAlloc[i]));
		if (FAILED(hr)) { D12Log("[D3D12] CreateCommandAllocator[%d] failed 0x%08X\n", i, (unsigned)hr); return false; }
	}
	hr = m_pDevice->CreateCommandList(0, D3D12_COMMAND_LIST_TYPE_DIRECT, m_pAlloc[m_frameIndex], NULL, IID_PPV_ARGS(&m_pList));
	if (FAILED(hr)) { D12Log("[D3D12] CreateCommandList failed 0x%08X\n", (unsigned)hr); return false; }
...
{
	D3D12_CPU_DESCRIPTOR_HANDLE h = m_pRtvHeap->GetCPUDescriptorHandleForHeapStart();
	for (int i = 0; i < kFrameCount; ++i)
	{
		HRESULT hr = m_pSwapChain->GetBuffer(i, IID_PPV_ARGS(&m_pBackBuffer[i]));
		if (FAILED(hr) || !m_pBackBuffer[i]) { D12Log("[D3D12] GetBuffer[%d] failed 0x%08X\n", i, (unsigned)hr); return false; }
...
{
	for (int i = 0; i < kFrameCount; ++i) D12_RELEASE(m_pBackBuffer[i]);
}
...
bool D3D12Backend::CreateDepthBuffer()
{
	...
	if (!m_pDsvHeap)
	{
		...
		hd.Flags = D3D12_DESCRIPTOR_HEAP_FLAG_NONE;
		if (FAILED(m_pDevice->CreateDescriptorHeap(&hd, IID_PPV_ARGS(&m_pDsvHeap)))) { D12Log("[D3D12] DSV heap failed\n"); return false; }
	}
	...
	rd.Format = DXGI_FORMAT_D32_FLOAT_S8X24_UINT; rd.SampleDesc.Count = 1;
	rd.Layout = D3D12_TEXTURE_LAYOUT_UNKNOWN;
	rd.Flags = D3D12_RESOURCE_FLAG_ALLOW_DEPTH_STENCIL;

	D3D12_CLEAR_VALUE cv; ZeroMemory(&cv, sizeof(cv));
	cv.Format = DXGI_FORMAT_D32_FLOAT_S8X24_UINT; cv.DepthStencil.Depth = 0.0f; cv.DepthStencil.Stencil = 0;

	if (FAILED(m_pDevice->CreateCommittedResource(&hp, D3D12_HEAP_FLAG_NONE, &rd,
		D3D12_RESOURCE_STATE_DEPTH_WRITE, &cv, IID_PPV_ARGS(&m_pDepthTex))))
	...
	m_pDevice->CreateDepthStencilView(m_pDepthTex, &dv, m_pDsvHeap->GetCPUDescriptorHandleForHeapStart());
	return true;
}

void D3D12Backend::ReleaseDepthBuffer() { D12_RELEASE(m_pDepthTex); }

unsigned __int64 D3D12Backend::DepthDsvPtr() const
{
	return m_pDsvHeap ? (unsigned __int64)m_pDsvHeap->GetCPUDescriptorHandleForHeapStart().ptr : 0;
}
int D3D12Backend::BackBufferFormat() { return (int)DXGI_FORMAT_R8G8B8A8_UNORM; }
int D3D12Backend::DepthFormat()      { return (int)DXGI_FORMAT_D32_FLOAT_S8X24_UINT; }
...
void D3D12Backend::BeginFrame(unsigned long argb)
{
	...
	m_curRtvPtr = (unsigned __int64)rtv.ptr;   // #DX12: back buffer is the current RTV
	// Artscout - 2026: #DX12 Phase 3 -- bind + clear the scene depth buffer alongside the RTV.
	D3D12_CPU_DESCRIPTOR_HANDLE dsv = m_pDsvHeap ? m_pDsvHeap->GetCPUDescriptorHandleForHeapStart() : D3D12_CPU_DESCRIPTOR_HANDLE();
	...
	if (m_pDsvHeap)
		m_pList->ClearDepthStencilView(dsv, D3D12_CLEAR_FLAG_DEPTH | D3D12_CLEAR_FLAG_STENCIL, 0.0f, 0, 0, NULL);

	D3D12_VIEWPORT vp; vp.TopLeftX = 0; vp.TopLeftY = 0; vp.Width = (FLOAT)m_nWidth; vp.Height = (FLOAT)m_nHeight; vp.MinDepth = 0.0f; vp.MaxDepth = 1.0f;
	D3D12_RECT     sc; sc.left = 0; sc.top = 0; sc.right = m_nWidth; sc.bottom = m_nHeight;
...
bool D3D12Backend::Resize(int nWidth, int nHeight)
{
	...
	if (nWidth == m_nWidth && nHeight == m_nHeight) return true;
	if (nWidth < 1) nWidth = 1; if (nHeight < 1) nHeight = 1;
	WaitForGpu();
...
void D3D12Backend::BindBackBuffer(bool bClearDepth)
{
	...
	(void)bClearDepth;
	if (!m_bRecording) BeginFrame(0xFF000000);
}
void D3D12Backend::ClearDepth()
{
	...
	if (!dsvPtr) dsvPtr = m_pDsvHeap ? (unsigned __int64)m_pDsvHeap->GetCPUDescriptorHandleForHeapStart().ptr : 0;
	if (!dsvPtr) return;
	D3D12_CPU_DESCRIPTOR_HANDLE dsv; dsv.ptr = (SIZE_T)dsvPtr;
	m_pList->ClearDepthStencilView(dsv, D3D12_CLEAR_FLAG_DEPTH | D3D12_CLEAR_FLAG_STENCIL, 0.0f, 0, 0, NULL);
}
void D3D12Backend::SetViewportRect(int x, int y, int w, int h)
{
	if (!m_pList || !m_bRecording) return;
	D3D12_VIEWPORT vp; vp.TopLeftX = (FLOAT)x; vp.TopLeftY = (FLOAT)y; vp.Width = (FLOAT)w; vp.Height = (FLOAT)h; vp.MinDepth = 0.0f; vp.MaxDepth = 1.0f;
	D3D12_RECT     sc; sc.left = x; sc.top = y; sc.right = x + w; sc.bottom = y + h;
	m_pList->RSSetViewports(1, &vp);
	m_pList->RSSetScissorRects(1, &sc);
}
...
int D3D12Backend::ResolveMsaaSamples()
{
	...
	if (!m_pDevice || !g_bMsaaEnable) return 1;
	int want = g_nMsaaSamples; if (want < 2) return 1; if (want > 8) want = 8;
	for (int s = want; s >= 2; --s)
	{
		D3D12_FEATURE_DATA_MULTISAMPLE_QUALITY_LEVELS ql; ZeroMemory(&ql, sizeof(ql));
		ql.Format = (DXGI_FORMAT)BackBufferFormat(); ql.SampleCount = (UINT)s;
		if (SUCCEEDED(m_pDevice->CheckFeatureSupport(D3D12_FEATURE_MULTISAMPLE_QUALITY_LEVELS, &ql, sizeof(ql)))
...
{
	D12_RELEASE(m_pMsaaColorTex); D12_RELEASE(m_pMsaaRtvHeap);
	D12_RELEASE(m_pMsaaDepthTex); D12_RELEASE(m_pMsaaDsvHeap);
	m_msaaW = m_msaaH = 0;
}
...
bool D3D12Backend::CreateMsaaTargets(int w, int h)
{
	...
	if (m_msaaSamples <= 0) m_msaaSamples = ResolveMsaaSamples();   // resolve once
	if (m_msaaSamples < 2) return false;
	if (m_pMsaaColorTex && m_msaaW == w && m_msaaH == h) return true;   // already up at this size
	ReleaseMsaaTargets();
	...
	rd.Format = (DXGI_FORMAT)BackBufferFormat(); rd.SampleDesc.Count = (UINT)m_msaaSamples;
	rd.Layout = D3D12_TEXTURE_LAYOUT_UNKNOWN; rd.Flags = D3D12_RESOURCE_FLAG_ALLOW_RENDER_TARGET;
	D3D12_CLEAR_VALUE ccv; ZeroMemory(&ccv, sizeof(ccv)); ccv.Format = (DXGI_FORMAT)BackBufferFormat();
	if (FAILED(m_pDevice->CreateCommittedResource(&hp, D3D12_HEAP_FLAG_NONE, &rd,
		D3D12_RESOURCE_STATE_RENDER_TARGET, &ccv, IID_PPV_ARGS(&m_pMsaaColorTex)))) { m_msaaSamples = 1; return false; }
	{
		D3D12_DESCRIPTOR_HEAP_DESC hd; ZeroMemory(&hd, sizeof(hd)); hd.NumDescriptors = 1; hd.Type = D3D12_DESCRIPTOR_HEAP_TYPE_RTV;
		if (FAILED(m_pDevice->CreateDescriptorHeap(&hd, IID_PPV_ARGS(&m_pMsaaRtvHeap)))) { ReleaseMsaaTargets(); m_msaaSamples = 1; return false; }
		D3D12_RENDER_TARGET_VIEW_DESC rv; ZeroMemory(&rv, sizeof(rv));
		rv.Format = (DXGI_FORMAT)BackBufferFormat(); rv.ViewDimension = D3D12_RTV_DIMENSION_TEXTURE2DMS;
		m_pDevice->CreateRenderTargetView(m_pMsaaColorTex, &rv, m_pMsaaRtvHeap->GetCPUDescriptorHandleForHeapStart());
	}

	// MSAA depth (D24S8) + DSV
	D3D12_RESOURCE_DESC dd; ZeroMemory(&dd, sizeof(dd));
	dd.Dimension = D3D12_RESOURCE_DIMENSION_TEXTURE2D;
	dd.Width = (UINT64)w; dd.Height = (UINT)h; dd.DepthOrArraySize = 1; dd.MipLevels = 1;
	dd.Format = DXGI_FORMAT_D32_FLOAT_S8X24_UINT; dd.SampleDesc.Count = (UINT)m_msaaSamples;
	dd.Layout = D3D12_TEXTURE_LAYOUT_UNKNOWN; dd.Flags = D3D12_RESOURCE_FLAG_ALLOW_DEPTH_STENCIL;
	D3D12_CLEAR_VALUE dcv; ZeroMemory(&dcv, sizeof(dcv)); dcv.Format = DXGI_FORMAT_D32_FLOAT_S8X24_UINT; dcv.DepthStencil.Depth = 0.0f;
	if (FAILED(m_pDevice->CreateCommittedResource(&hp, D3D12_HEAP_FLAG_NONE, &dd,
		D3D12_RESOURCE_STATE_DEPTH_WRITE, &dcv, IID_PPV_ARGS(&m_pMsaaDepthTex)))) { ReleaseMsaaTargets(); m_msaaSamples = 1; return false; }
	{
		D3D12_DESCRIPTOR_HEAP_DESC hd; ZeroMemory(&hd, sizeof(hd)); hd.NumDescriptors = 1; hd.Type = D3D12_DESCRIPTOR_HEAP_TYPE_DSV;
		if (FAILED(m_pDevice->CreateDescriptorHeap(&hd, IID_PPV_ARGS(&m_pMsaaDsvHeap)))) { ReleaseMsaaTargets(); m_msaaSamples = 1; return false; }
		D3D12_DEPTH_STENCIL_VIEW_DESC dv; ZeroMemory(&dv, sizeof(dv));
		dv.Format = DXGI_FORMAT_D32_FLOAT_S8X24_UINT; dv.ViewDimension = D3D12_DSV_DIMENSION_TEXTURE2DMS;
		m_pDevice->CreateDepthStencilView(m_pMsaaDepthTex, &dv, m_pMsaaDsvHeap->GetCPUDescriptorHandleForHeapStart());
	}
	m_msaaW = w; m_msaaH = h;
...
{
	D3D12_CPU_DESCRIPTOR_HANDLE rtv = m_pMsaaRtvHeap->GetCPUDescriptorHandleForHeapStart();
	D3D12_CPU_DESCRIPTOR_HANDLE dsv = m_pMsaaDsvHeap->GetCPUDescriptorHandleForHeapStart();
...
{
	if (!m_pList || !m_bRecording || !MsaaActive()) return;
	ID3D12Resource* bb = m_pBackBuffer[m_frameIndex];
	D3D12_RESOURCE_BARRIER b[2]; ZeroMemory(b, sizeof(b));
	b[0].Type = D3D12_RESOURCE_BARRIER_TYPE_TRANSITION; b[0].Transition.pResource = bb;
	b[0].Transition.Subresource = D3D12_RESOURCE_BARRIER_ALL_SUBRESOURCES;
	b[0].Transition.StateBefore = D3D12_RESOURCE_STATE_RENDER_TARGET; b[0].Transition.StateAfter = D3D12_RESOURCE_STATE_RESOLVE_DEST;
	b[1].Type = D3D12_RESOURCE_BARRIER_TYPE_TRANSITION; b[1].Transition.pResource = m_pMsaaColorTex;
	b[1].Transition.Subresource = D3D12_RESOURCE_BARRIER_ALL_SUBRESOURCES;
	b[1].Transition.StateBefore = D3D12_RESOURCE_STATE_RENDER_TARGET; b[1].Transition.StateAfter = D3D12_RESOURCE_STATE_RESOLVE_SOURCE;
	m_pList->ResourceBarrier(2, b);
	m_pList->ResolveSubresource(bb, 0, m_pMsaaColorTex, 0, (DXGI_FORMAT)BackBufferFormat());
	b[0].Transition.StateBefore = D3D12_RESOURCE_STATE_RESOLVE_DEST;   b[0].Transition.StateAfter = D3D12_RESOURCE_STATE_RENDER_TARGET;
	b[1].Transition.StateBefore = D3D12_RESOURCE_STATE_RESOLVE_SOURCE; b[1].Transition.StateAfter = D3D12_RESOURCE_STATE_RENDER_TARGET;
	m_pList->ResourceBarrier(2, b);
	...
	m_curRtvPtr = (unsigned __int64)rtv.ptr; m_curSampleCount = 1;
	D3D12_VIEWPORT vp; vp.TopLeftX = 0; vp.TopLeftY = 0; vp.Width = (FLOAT)m_nWidth; vp.Height = (FLOAT)m_nHeight; vp.MinDepth = 0; vp.MaxDepth = 1;
	D3D12_RECT sc; sc.left = 0; sc.top = 0; sc.right = m_nWidth; sc.bottom = m_nHeight;
	m_pList->RSSetViewports(1, &vp); m_pList->RSSetScissorRects(1, &sc);
}
...
bool D3D12Backend::EnsureQuadPipeline()
{
	...
	if (!m_pSrvHeap)
	{
		...
		if (FAILED(m_pDevice->CreateDescriptorHeap(&hd, IID_PPV_ARGS(&m_pSrvHeap)))) { D12Log("[D3D12] SRV heap failed\n"); return false; }
	}

	// Root signature: 1 SRV table (t0, PS) + 1 static linear-clamp sampler (s0, PS).
	D3D12_DESCRIPTOR_RANGE range; ZeroMemory(&range, sizeof(range));
	range.RangeType = D3D12_DESCRIPTOR_RANGE_TYPE_SRV;
	range.NumDescriptors = 1;
	range.OffsetInDescriptorsFromTableStart = D3D12_DESCRIPTOR_RANGE_OFFSET_APPEND;

	D3D12_ROOT_PARAMETER param; ZeroMemory(&param, sizeof(param));
	param.ParameterType = D3D12_ROOT_PARAMETER_TYPE_DESCRIPTOR_TABLE;
	param.DescriptorTable.NumDescriptorRanges = 1;
	param.DescriptorTable.pDescriptorRanges = &range;
	param.ShaderVisibility = D3D12_SHADER_VISIBILITY_PIXEL;

	D3D12_STATIC_SAMPLER_DESC samp; ZeroMemory(&samp, sizeof(samp));
	...
	ID3DBlob* rsBlob = 0; ID3DBlob* rsErr = 0;
	if (FAILED(D3D12SerializeRootSignature(&rsd, D3D_ROOT_SIGNATURE_VERSION_1, &rsBlob, &rsErr)))
	{ D12Log("[D3D12] SerializeRootSignature failed\n"); if (rsErr) rsErr->Release(); return false; }
	HRESULT hr = m_pDevice->CreateRootSignature(0, rsBlob->GetBufferPointer(), rsBlob->GetBufferSize(), IID_PPV_ARGS(&m_pQuadRS));
	rsBlob->Release(); if (rsErr) rsErr->Release();
	if (FAILED(hr)) { D12Log("[D3D12] CreateRootSignature failed 0x%08X\n", (unsigned)hr); return false; }
	...
	ID3DBlob* vs = 0; ID3DBlob* ps = 0; ID3DBlob* err = 0;
	if (FAILED(D3DCompile(kSrc, strlen(kSrc), "quad", 0, 0, "VSMain", "vs_5_0", 0, 0, &vs, &err)))
	...
	if (FAILED(D3DCompile(kSrc, strlen(kSrc), "quad", 0, 0, "PSMain", "ps_5_0", 0, 0, &ps, &err)))
	{ D12Log("[D3D12] PS compile failed: %s\n", err ? (const char*)err->GetBufferPointer() : "?"); if (err) err->Release(); if (vs) vs->Release(); return false; }
	if (err) { err->Release(); err = 0; }

	D3D12_GRAPHICS_PIPELINE_STATE_DESC pso; ZeroMemory(&pso, sizeof(pso));
	pso.pRootSignature = m_pQuadRS;
	pso.VS.pShaderBytecode = vs->GetBufferPointer(); pso.VS.BytecodeLength = vs->GetBufferSize();
	pso.PS.pShaderBytecode = ps->GetBufferPointer(); pso.PS.BytecodeLength = ps->GetBufferSize();
	...
	hr = m_pDevice->CreateGraphicsPipelineState(&pso, IID_PPV_ARGS(&m_pQuadPSO));
	if (FAILED(hr)) { D12Log("[D3D12] CreateGraphicsPipelineState failed 0x%08X\n", (unsigned)hr); vs->Release(); ps->Release(); return false; }
	...
	hr = m_pDevice->CreateGraphicsPipelineState(&pso, IID_PPV_ARGS(&m_pQuadPSOBlend));
	vs->Release(); ps->Release();
	if (FAILED(hr)) { D12Log("[D3D12] CreateGraphicsPipelineState(blend) failed 0x%08X\n", (unsigned)hr); return false; }
...
{
	if (!m_bRecording || !pSrc565 || srcW < 1 || srcH < 1) return;
	if (!EnsureQuadPipeline()) return;
	if (!EnsureQuadTexture(srcW, srcH)) return;

	void* mapped = 0;
	D3D12_RANGE noRead; noRead.Begin = 0; noRead.End = 0;
	if (FAILED(m_pQuadUpload->Map(0, &noRead, &mapped)) || !mapped) return;
	const unsigned short* s = (const unsigned short*)pSrc565;
	for (int y = 0; y < srcH; ++y)
	{
		unsigned* dst = (unsigned*)((unsigned char*)mapped + (size_t)y * m_quadRowPitch);
		const unsigned short* row = s + (size_t)y * srcW;
		for (int x = 0; x < srcW; ++x)
		{
			unsigned short v = row[x];
			unsigned a = v ? 0xFF000000u : 0u;   // black 565 -> transparent (3D behind shows)
			unsigned r = (v >> 11) & 0x1F; r = (r << 3) | (r >> 2);
			unsigned g = (v >>  5) & 0x3F; g = (g << 2) | (g >> 4);
			unsigned b =  v        & 0x1F; b = (b << 3) | (b >> 2);
			dst[x] = a | (b << 16) | (g << 8) | r;
		}
	}
	...
	if (m_quadTexState != (unsigned)D3D12_RESOURCE_STATE_COPY_DEST)
	{
		D3D12_RESOURCE_BARRIER b; ZeroMemory(&b, sizeof(b));
		b.Type = D3D12_RESOURCE_BARRIER_TYPE_TRANSITION;
		b.Transition.pResource = m_pQuadTex; b.Transition.Subresource = D3D12_RESOURCE_BARRIER_ALL_SUBRESOURCES;
		b.Transition.StateBefore = (D3D12_RESOURCE_STATES)m_quadTexState;
		b.Transition.StateAfter  = D3D12_RESOURCE_STATE_COPY_DEST;
		m_pList->ResourceBarrier(1, &b); m_quadTexState = (unsigned)D3D12_RESOURCE_STATE_COPY_DEST;
	}
	D3D12_TEXTURE_COPY_LOCATION dstL; ZeroMemory(&dstL, sizeof(dstL));
	dstL.pResource = m_pQuadTex; dstL.Type = D3D12_TEXTURE_COPY_TYPE_SUBRESOURCE_INDEX; dstL.SubresourceIndex = 0;
	D3D12_TEXTURE_COPY_LOCATION srcL; ZeroMemory(&srcL, sizeof(srcL));
	srcL.pResource = m_pQuadUpload; srcL.Type = D3D12_TEXTURE_COPY_TYPE_PLACED_FOOTPRINT;
	...
	D3D12_RESOURCE_BARRIER b2; ZeroMemory(&b2, sizeof(b2));
	b2.Type = D3D12_RESOURCE_BARRIER_TYPE_TRANSITION;
	b2.Transition.pResource = m_pQuadTex; b2.Transition.Subresource = D3D12_RESOURCE_BARRIER_ALL_SUBRESOURCES;
	...
	m_pList->ResourceBarrier(1, &b2); m_quadTexState = (unsigned)D3D12_RESOURCE_STATE_PIXEL_SHADER_RESOURCE;
	...
	m_pList->SetGraphicsRootDescriptorTable(0, m_pSrvHeap->GetGPUDescriptorHandleForHeapStart());
...
bool D3D12Backend::EnsureQuadTexture(int w, int h)
{
	...
	if (m_pQuadTex && w == m_quadTexW && h == m_quadTexH) return true;
	...
	m_quadTexW = w; m_quadTexH = h;

	D3D12_HEAP_PROPERTIES hpDefault; ZeroMemory(&hpDefault, sizeof(hpDefault)); hpDefault.Type = D3D12_HEAP_TYPE_DEFAULT;
	...
	if (FAILED(m_pDevice->CreateCommittedResource(&hpDefault, D3D12_HEAP_FLAG_NONE, &td, D3D12_RESOURCE_STATE_COPY_DEST, NULL, IID_PPV_ARGS(&m_pQuadTex))))
	...
	m_pDevice->CreateShaderResourceView(m_pQuadTex, &srv, m_pSrvHeap->GetCPUDescriptorHandleForHeapStart());

	m_quadRowPitch = (unsigned)(((w * 4) + 255) & ~255);
	UINT64 uploadSize = (UINT64)m_quadRowPitch * (UINT64)h;
	D3D12_HEAP_PROPERTIES hpUpload; ZeroMemory(&hpUpload, sizeof(hpUpload)); hpUpload.Type = D3D12_HEAP_TYPE_UPLOAD;
	...
	if (FAILED(m_pDevice->CreateCommittedResource(&hpUpload, D3D12_HEAP_FLAG_NONE, &bd, D3D12_RESOURCE_STATE_GENERIC_READ, NULL, IID_PPV_ARGS(&m_pQuadUpload))))
...
{
	if (!m_bRecording || !pSrc565 || srcW < 1 || srcH < 1) return;   // must be between BeginFrame/Present
	if (!EnsureQuadPipeline()) return;
	if (!EnsureQuadTexture(srcW, srcH)) return;
	...
	for (int y = 0; y < srcH; ++y)
	{
		...
		for (int x = 0; x < srcW; ++x)
		{
			...
			unsigned g = (v >>  5) & 0x3F; g = (g << 2) | (g >> 4);
			unsigned b =  v        & 0x1F; b = (b << 3) | (b >> 2);
			dst[x] = 0xFF000000u | (b << 16) | (g << 8) | r;   // R8G8B8A8 in memory (little-endian 0xAABBGGRR)
		}
	}
	m_pQuadUpload->Unmap(0, NULL);
...
void D3D12Backend::SetGScreenSize(int w, int h)
{
	...
	if (g_pD3D12Renderer) g_pD3D12Renderer->SetViewportSize(w, h);
}
void D3D12Backend::ClearCurrentRTV(float r, float g, float b, float a)
{
	if (!m_pList || !m_bRecording || !m_curRtvPtr) return;
	...
	D3D12_CPU_DESCRIPTOR_HANDLE rtv; rtv.ptr = (SIZE_T)m_curRtvPtr;
	const float c[4] = { r, g, b, a };
	m_pList->ClearRenderTargetView(rtv, c, 0, NULL);
}
void D3D12Backend::FlushContext()                                { /* D3D12 submits explicitly in Present */ }

// #DX12 п.5 (VR): (re)create the per-eye depth buffer (D32) sized to the eye image.
bool D3D12Backend::EnsureEyeDepth(int w, int h)
{
	if (w < 1 || h < 1) return false;
	if (m_pEyeDepthTex && w == m_eyeDepthW && h == m_eyeDepthH) return true;
	if (m_pEyeDepthTex) { m_pEyeDepthTex->Release(); m_pEyeDepthTex = 0; }
	if (!m_pEyeDsvHeap)
	{
		D3D12_DESCRIPTOR_HEAP_DESC hd; ZeroMemory(&hd, sizeof(hd));
		hd.NumDescriptors = 1; hd.Type = D3D12_DESCRIPTOR_HEAP_TYPE_DSV; hd.Flags = D3D12_DESCRIPTOR_HEAP_FLAG_NONE;
		if (FAILED(m_pDevice->CreateDescriptorHeap(&hd, IID_PPV_ARGS(&m_pEyeDsvHeap)))) return false;
	}
	D3D12_HEAP_PROPERTIES hp; ZeroMemory(&hp, sizeof(hp)); hp.Type = D3D12_HEAP_TYPE_DEFAULT;
	D3D12_RESOURCE_DESC rd; ZeroMemory(&rd, sizeof(rd));
	rd.Dimension = D3D12_RESOURCE_DIMENSION_TEXTURE2D; rd.Width = (UINT64)w; rd.Height = (UINT)h;
	rd.DepthOrArraySize = 1; rd.MipLevels = 1; rd.Format = DXGI_FORMAT_D32_FLOAT_S8X24_UINT; rd.SampleDesc.Count = 1;
	rd.Flags = D3D12_RESOURCE_FLAG_ALLOW_DEPTH_STENCIL;
	D3D12_CLEAR_VALUE cv; ZeroMemory(&cv, sizeof(cv)); cv.Format = DXGI_FORMAT_D32_FLOAT_S8X24_UINT; cv.DepthStencil.Depth = 0.0f;
	if (FAILED(m_pDevice->CreateCommittedResource(&hp, D3D12_HEAP_FLAG_NONE, &rd, D3D12_RESOURCE_STATE_DEPTH_WRITE, &cv, IID_PPV_ARGS(&m_pEyeDepthTex)))) return false;
	D3D12_DEPTH_STENCIL_VIEW_DESC dv; ZeroMemory(&dv, sizeof(dv)); dv.Format = DXGI_FORMAT_D32_FLOAT_S8X24_UINT; dv.ViewDimension = D3D12_DSV_DIMENSION_TEXTURE2D;
	m_pDevice->CreateDepthStencilView(m_pEyeDepthTex, &dv, m_pEyeDsvHeap->GetCPUDescriptorHandleForHeapStart());
	m_eyeDepthW = w; m_eyeDepthH = h;
	return true;
}
...
void D3D12Backend::EnsureMenuRtt(int w, int h)
{
	...
	if (m_pMenuRtt && m_pMenuRtt->tex && m_menuRttW == w && m_menuRttH == h) return;   // already at size

	if (m_pMenuRtt)
	{
		if (g_pD3D12TextureManager) g_pD3D12TextureManager->Destroy(*m_pMenuRtt);
		delete m_pMenuRtt; m_pMenuRtt = 0;
	}
	...
	{ delete m_pMenuRtt; m_pMenuRtt = 0; return; }
	...
	if (!m_pMenuDsvHeap)
	{
		D3D12_DESCRIPTOR_HEAP_DESC hd; ZeroMemory(&hd, sizeof(hd));
		hd.NumDescriptors = 1; hd.Type = D3D12_DESCRIPTOR_HEAP_TYPE_DSV; hd.Flags = D3D12_DESCRIPTOR_HEAP_FLAG_NONE;
		if (FAILED(m_pDevice->CreateDescriptorHeap(&hd, IID_PPV_ARGS(&m_pMenuDsvHeap)))) m_pMenuDsvHeap = 0;
	}
	if (m_pMenuDsvHeap)
	{
		D3D12_HEAP_PROPERTIES hp; ZeroMemory(&hp, sizeof(hp)); hp.Type = D3D12_HEAP_TYPE_DEFAULT;
		...
		rd.Flags = D3D12_RESOURCE_FLAG_ALLOW_DEPTH_STENCIL;
		D3D12_CLEAR_VALUE cv; ZeroMemory(&cv, sizeof(cv)); cv.Format = DXGI_FORMAT_D32_FLOAT_S8X24_UINT; cv.DepthStencil.Depth = 0.0f;
		if (SUCCEEDED(m_pDevice->CreateCommittedResource(&hp, D3D12_HEAP_FLAG_NONE, &rd, D3D12_RESOURCE_STATE_DEPTH_WRITE, &cv, IID_PPV_ARGS(&m_pMenuDepthTex))))
		{
			D3D12_DEPTH_STENCIL_VIEW_DESC dv; ZeroMemory(&dv, sizeof(dv)); dv.Format = DXGI_FORMAT_D32_FLOAT_S8X24_UINT; dv.ViewDimension = D3D12_DSV_DIMENSION_TEXTURE2D;
			m_pDevice->CreateDepthStencilView(m_pMenuDepthTex, &dv, m_pMenuDsvHeap->GetCPUDescriptorHandleForHeapStart());
		}
	}
...
{
	if (!m_pList || !m_bRecording || !m_pMenuRtt || !m_pMenuRtt->tex) return;

	if (m_pMenuRtt->rtState != (unsigned)D3D12_RESOURCE_STATE_RENDER_TARGET)
	{
		D3D12_RESOURCE_BARRIER b; ZeroMemory(&b, sizeof(b));
		b.Type = D3D12_RESOURCE_BARRIER_TYPE_TRANSITION;
		b.Transition.pResource = m_pMenuRtt->tex; b.Transition.Subresource = D3D12_RESOURCE_BARRIER_ALL_SUBRESOURCES;
		b.Transition.StateBefore = (D3D12_RESOURCE_STATES)m_pMenuRtt->rtState;
		b.Transition.StateAfter  = D3D12_RESOURCE_STATE_RENDER_TARGET;
		m_pList->ResourceBarrier(1, &b); m_pMenuRtt->rtState = (unsigned)D3D12_RESOURCE_STATE_RENDER_TARGET;
	}
	D3D12_CPU_DESCRIPTOR_HANDLE rtv; rtv.ptr = (SIZE_T)m_pMenuRtt->rtvCpuPtr;
	D3D12_CPU_DESCRIPTOR_HANDLE dsv = m_pMenuDsvHeap ? m_pMenuDsvHeap->GetCPUDescriptorHandleForHeapStart() : D3D12_CPU_DESCRIPTOR_HANDLE();
	m_curRtvPtr = (unsigned __int64)rtv.ptr;   // ClearCurrentRTV targets the menu RTT while it is bound
	m_curSampleCount = 1;   // menu RTT is single-sample
	m_pList->OMSetRenderTargets(1, &rtv, FALSE, m_pMenuDepthTex ? &dsv : NULL);
	if (g_pD3D12Renderer) g_pD3D12Renderer->SetDepthTargetBound(m_pMenuDepthTex != 0);
	D3D12_VIEWPORT vp; vp.TopLeftX = 0; vp.TopLeftY = 0; vp.Width = (FLOAT)m_menuRttW; vp.Height = (FLOAT)m_menuRttH; vp.MinDepth = 0; vp.MaxDepth = 1;
	D3D12_RECT sc; sc.left = 0; sc.top = 0; sc.right = m_menuRttW; sc.bottom = m_menuRttH;
	m_pList->RSSetViewports(1, &vp); m_pList->RSSetScissorRects(1, &sc);
	if (clear)
	{
		const float z[4] = { 0, 0, 0, 0 };   // transparent canvas: the menu alpha-keys the empty area out on the quad
		m_pList->ClearRenderTargetView(rtv, z, 0, NULL);
		if (m_pMenuDepthTex) m_pList->ClearDepthStencilView(dsv, D3D12_CLEAR_FLAG_DEPTH | D3D12_CLEAR_FLAG_STENCIL, 0.0f, 0, 0, NULL);
	}
}
...
void D3D12Backend::BeginEyeFrame(void* eyeImg, unsigned __int64 eyeRtvPtr, int w, int h)
{
	...
	(void)eyeImg;
	D3D12_CPU_DESCRIPTOR_HANDLE rtv; rtv.ptr = (SIZE_T)eyeRtvPtr;
	D3D12_CPU_DESCRIPTOR_HANDLE dsv = m_pEyeDsvHeap ? m_pEyeDsvHeap->GetCPUDescriptorHandleForHeapStart() : D3D12_CPU_DESCRIPTOR_HANDLE();
	m_pList->OMSetRenderTargets(1, &rtv, FALSE, m_pEyeDsvHeap ? &dsv : NULL);
	if (g_pD3D12Renderer) g_pD3D12Renderer->SetDepthTargetBound(m_pEyeDsvHeap != 0);
	const float black[4] = { 0, 0, 0, 1 };
	m_pList->ClearRenderTargetView(rtv, black, 0, NULL);
	if (m_pEyeDsvHeap) m_pList->ClearDepthStencilView(dsv, D3D12_CLEAR_FLAG_DEPTH | D3D12_CLEAR_FLAG_STENCIL, 0.0f, 0, 0, NULL);
	D3D12_VIEWPORT vp; vp.TopLeftX = 0; vp.TopLeftY = 0; vp.Width = (FLOAT)w; vp.Height = (FLOAT)h; vp.MinDepth = 0; vp.MaxDepth = 1;
	D3D12_RECT sc; sc.left = 0; sc.top = 0; sc.right = w; sc.bottom = h;
	m_pList->RSSetViewports(1, &vp); m_pList->RSSetScissorRects(1, &sc);
...
{
	if (!m_pList || !m_bRecording || !eyeImg) return;
...
{
	EnsureFrameStarted();
	if (!m_pList || !m_bRecording || !handle) return;
	D3D12Texture* t = (D3D12Texture*)handle;
	if (!t->tex || !t->rtvCpuPtr) return;

	if (t->rtState != (unsigned)D3D12_RESOURCE_STATE_RENDER_TARGET)
	{
		D3D12_RESOURCE_BARRIER b; ZeroMemory(&b, sizeof(b));
		b.Type = D3D12_RESOURCE_BARRIER_TYPE_TRANSITION;
		b.Transition.pResource = t->tex; b.Transition.Subresource = D3D12_RESOURCE_BARRIER_ALL_SUBRESOURCES;
		b.Transition.StateBefore = (D3D12_RESOURCE_STATES)t->rtState;
		b.Transition.StateAfter  = D3D12_RESOURCE_STATE_RENDER_TARGET;
		m_pList->ResourceBarrier(1, &b); t->rtState = (unsigned)D3D12_RESOURCE_STATE_RENDER_TARGET;
	}
	D3D12_CPU_DESCRIPTOR_HANDLE rtv; rtv.ptr = (SIZE_T)t->rtvCpuPtr;
	m_curRtvPtr = (unsigned __int64)rtv.ptr;   // #DX12: the RTT atlas is now the current RTV (ClearCurrentRTV clears IT)
	m_curSampleCount = 1;   // the RTT atlas is single-sample -> single-sample PSOs
	m_pList->OMSetRenderTargets(1, &rtv, FALSE, NULL);   // 2D displays: no depth
	if (g_pD3D12Renderer) g_pD3D12Renderer->SetDepthTargetBound(false);   // no DSV -> force depth-off PSOs (#615)
	if (w < 1) w = t->width; if (h < 1) h = t->height;
	D3D12_VIEWPORT vp; vp.TopLeftX = 0; vp.TopLeftY = 0; vp.Width = (FLOAT)w; vp.Height = (FLOAT)h; vp.MinDepth = 0; vp.MaxDepth = 1;
	D3D12_RECT sc; sc.left = 0; sc.top = 0; sc.right = w; sc.bottom = h;
	m_pList->RSSetViewports(1, &vp); m_pList->RSSetScissorRects(1, &sc);
	if (clear) { const float black[4] = { 0, 0, 0, 0 }; m_pList->ClearRenderTargetView(rtv, black, 0, NULL); }
}
...
void D3D12Backend::UnbindSceneRtt(void* handle)
{
	if (!m_pList || !m_bRecording || !handle) return;
	D3D12Texture* t = (D3D12Texture*)handle;
	if (t->tex && t->rtState != (unsigned)D3D12_RESOURCE_STATE_PIXEL_SHADER_RESOURCE)
	{
		D3D12_RESOURCE_BARRIER b; ZeroMemory(&b, sizeof(b));
		b.Type = D3D12_RESOURCE_BARRIER_TYPE_TRANSITION;
		b.Transition.pResource = t->tex; b.Transition.Subresource = D3D12_RESOURCE_BARRIER_ALL_SUBRESOURCES;
		...
		m_pList->ResourceBarrier(1, &b); t->rtState = (unsigned)D3D12_RESOURCE_STATE_PIXEL_SHADER_RESOURCE;
	}
	BindBackBufferRTV();
}
...
void D3D12Backend::BindBackBufferRTV()
{
	...
	{
		D3D12_CPU_DESCRIPTOR_HANDLE r = m_pRtvHeap->GetCPUDescriptorHandleForHeapStart();
		r.ptr += (SIZE_T)m_frameIndex * m_rtvDescSize;
		rtvPtr = (unsigned __int64)r.ptr;
		dsvPtr = m_pDsvHeap ? (unsigned __int64)m_pDsvHeap->GetCPUDescriptorHandleForHeapStart().ptr : 0;
	...
	if (g_pD3D12Renderer) g_pD3D12Renderer->SetDepthTargetBound(dsvPtr != 0);   // scene DSV back -> depth allowed
	D3D12_VIEWPORT vp; vp.TopLeftX = 0; vp.TopLeftY = 0; vp.Width = (FLOAT)w; vp.Height = (FLOAT)h; vp.MinDepth = 0; vp.MaxDepth = 1;
	D3D12_RECT sc; sc.left = 0; sc.top = 0; sc.right = w; sc.bottom = h;
	m_pList->RSSetViewports(1, &vp); m_pList->RSSetScissorRects(1, &sc);
}
...
namespace {
	struct RbSlot
	{
		...
		unsigned __int64 copyFence;  // fence value the recording frame's Present will signal (completion target)
	...
	static inline unsigned Align256(unsigned v) { return (v + 255u) & ~255u; }
}

void D3D12Backend::ReadbackRttTo565(void* rttHandle, unsigned short* dst, int dstStridePix, int dstHeightPix,
                                    int x, int y, int w, int h)
{
	...
	if (!m_pDevice || !rttHandle || !dst) return;
	...
	{
		if (s.buf) { WaitForGpu(); s.buf->Release(); s.buf = 0; s.pending = false; }
		D3D12_HEAP_PROPERTIES hp; ZeroMemory(&hp, sizeof(hp)); hp.Type = D3D12_HEAP_TYPE_READBACK;
		D3D12_RESOURCE_DESC rd; ZeroMemory(&rd, sizeof(rd));
		...
		if (FAILED(m_pDevice->CreateCommittedResource(&hp, D3D12_HEAP_FLAG_NONE, &rd,
			D3D12_RESOURCE_STATE_COMMON, NULL, __uuidof(ID3D12Resource), (void**)&s.buf)))
		{ s.buf = 0; return; }
		s.rowPitch = rowPitch; s.w = t->width; s.h = t->height; s.pending = false;
	}
	...
	{
		void* mapped = 0;
		if (SUCCEEDED(s.buf->Map(0, NULL, &mapped)) && mapped)   // NULL read range = whole resource (no MAP_INVALIDRANGE)
		{
			int x0 = x < 0 ? 0 : x, y0 = y < 0 ? 0 : y;
			int x1 = x + (w > 0 ? w : t->width), y1 = y + (h > 0 ? h : t->height);
			...
			{
				const BYTE* src = base + (size_t)row * s.rowPitch + (size_t)x0 * 4;
				unsigned short* d = dst + (size_t)row * dstStridePix + x0;
				for (int col = x0; col < x1; ++col)
				{
					BYTE r = src[0], g = src[1], b = src[2];   // RTT is R8G8B8A8_UNORM
					*d++ = (unsigned short)(((r >> 3) << 11) | ((g >> 2) << 5) | (b >> 3));
					src += 4;
				}
			}
			D3D12_RANGE wr; wr.Begin = 0; wr.End = 0; s.buf->Unmap(0, &wr);   // CPU wrote nothing back
		}
		s.pending = false;
	}
	...
	if (!m_bRecording || !m_pList) return;
	unsigned before = t->rtState ? t->rtState : (unsigned)D3D12_RESOURCE_STATE_PIXEL_SHADER_RESOURCE;
	...
	D3D12_TEXTURE_COPY_LOCATION dstL; ZeroMemory(&dstL, sizeof(dstL));
	dstL.pResource = s.buf; dstL.Type = D3D12_TEXTURE_COPY_TYPE_PLACED_FOOTPRINT;
	...
	D3D12_TEXTURE_COPY_LOCATION srcL; ZeroMemory(&srcL, sizeof(srcL));
	srcL.pResource = t->tex; srcL.Type = D3D12_TEXTURE_COPY_TYPE_SUBRESOURCE_INDEX; srcL.SubresourceIndex = 0;
	m_pList->CopyTextureRegion(&dstL, 0, 0, 0, &srcL, NULL);

	D3D12_RESOURCE_BARRIER b2; ZeroMemory(&b2, sizeof(b2));
	b2.Type = D3D12_RESOURCE_BARRIER_TYPE_TRANSITION;
	b2.Transition.pResource = t->tex; b2.Transition.Subresource = D3D12_RESOURCE_BARRIER_ALL_SUBRESOURCES;
...
void D3D12Backend::CopyRtt(void* srcHandle, void* dstHandle)
{
	...
	unsigned sBefore = s->rtState ? s->rtState : (unsigned)D3D12_RESOURCE_STATE_PIXEL_SHADER_RESOURCE;
	unsigned dBefore = d->rtState ? d->rtState : (unsigned)D3D12_RESOURCE_STATE_PIXEL_SHADER_RESOURCE;

	D3D12_RESOURCE_BARRIER pre[2]; int n = 0;
	if (sBefore != (unsigned)D3D12_RESOURCE_STATE_COPY_SOURCE)
	{
		ZeroMemory(&pre[n], sizeof(pre[0])); pre[n].Type = D3D12_RESOURCE_BARRIER_TYPE_TRANSITION;
		pre[n].Transition.pResource = s->tex; pre[n].Transition.Subresource = D3D12_RESOURCE_BARRIER_ALL_SUBRESOURCES;
		pre[n].Transition.StateBefore = (D3D12_RESOURCE_STATES)sBefore;
		pre[n].Transition.StateAfter  = D3D12_RESOURCE_STATE_COPY_SOURCE; ++n;
	}
	if (dBefore != (unsigned)D3D12_RESOURCE_STATE_COPY_DEST)
	{
		ZeroMemory(&pre[n], sizeof(pre[0])); pre[n].Type = D3D12_RESOURCE_BARRIER_TYPE_TRANSITION;
		pre[n].Transition.pResource = d->tex; pre[n].Transition.Subresource = D3D12_RESOURCE_BARRIER_ALL_SUBRESOURCES;
		pre[n].Transition.StateBefore = (D3D12_RESOURCE_STATES)dBefore;
		pre[n].Transition.StateAfter  = D3D12_RESOURCE_STATE_COPY_DEST; ++n;
	}
	if (n) m_pList->ResourceBarrier(n, pre);

	m_pList->CopyResource(d->tex, s->tex);   // requires identical dims/format (GM panel == sweep RTT: RGBA8, GM_TEXTURE_SIZE)

	D3D12_RESOURCE_BARRIER post[2];
	ZeroMemory(&post[0], sizeof(post[0])); post[0].Type = D3D12_RESOURCE_BARRIER_TYPE_TRANSITION;
	post[0].Transition.pResource = s->tex; post[0].Transition.Subresource = D3D12_RESOURCE_BARRIER_ALL_SUBRESOURCES;
	...
	ZeroMemory(&post[1], sizeof(post[1])); post[1].Type = D3D12_RESOURCE_BARRIER_TYPE_TRANSITION;
	post[1].Transition.pResource = d->tex; post[1].Transition.Subresource = D3D12_RESOURCE_BARRIER_ALL_SUBRESOURCES;
...
void D3D12Backend::ReleaseReadbackFor(void* rttHandle)
{
	...
	if (it == s_readback.end()) return;
	if (it->second.buf) { if (m_pQueue && m_pFence) WaitForGpu(); it->second.buf->Release(); }
	s_readback.erase(it);
}

void D3D12Backend::Release()
{
	if (m_pQueue && m_pFence) WaitForGpu();   // don't destroy in-flight resources
	if (m_fenceEvent) { CloseHandle(m_fenceEvent); m_fenceEvent = 0; }
	...
	D12_RELEASE(m_pQuadUpload);
	D12_RELEASE(m_pList);
	for (int i = 0; i < kFrameCount; ++i) D12_RELEASE(m_pAlloc[i]);
	ReleaseBackBufferViews();
	ReleaseDepthBuffer();
