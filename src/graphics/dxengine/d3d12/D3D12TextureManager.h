...
{
	ID3D12Resource*  tex;
	unsigned __int64 srvCpuPtr;    // D3D12_CPU_DESCRIPTOR_HANDLE.ptr in the staging heap (copy source)
	...
	D3D12Texture() : tex(0), srvCpuPtr(0), width(0), height(0), mips(0), dxgiFormat(0),
	                 isRT(false), rtvCpuPtr(0), rtState(0),
	                 poolHeapIdx(-1), poolOffset(0), poolSize(0) {}
};
...
public:
	...
	bool Init();                 // grabs device from g_pD3D12Backend, builds the upload queue + staging heap
	void Release();
	...
	struct ID3D12Resource* CreateVertexBufferGPU(const void* data, unsigned bytes);
	...
	unsigned SrvIncrement() const { return m_srvInc; }

private:
	bool UploadAndBarrier(ID3D12Resource* dst, bool placed, const TexMipData* mips, int mipCount,
	                      const void* footprints, const unsigned* numRows, const unsigned __int64* rowSize,
	...
	struct ID3D12Resource* EnsureUploadRing(int idx, unsigned __int64 size); // slot idx's upload buffer, grown to >= size
	...
	int  AllocRtvSlot();
	void ReclaimSlots(const D3D12Texture& t);   // push t's SRV (+ RTV if isRT) slot indices back onto the free-lists
	...
	ID3D12Resource* AllocPlacedTexture(const void* resourceDesc, D3D12Texture& out);
	...
	void            DeferTextureFree(ID3D12Resource* res, int heapIdx, unsigned __int64 offset, unsigned __int64 size);
public:
	void TickFrame(unsigned renderEpoch);   // called from backend BeginFrame: advance epoch + reclaim regions
private:

	ID3D12Device*              m_pDevice;      // borrowed from g_pD3D12Backend
	ID3D12CommandQueue*        m_pQueue;       // dedicated upload queue
	ID3D12GraphicsCommandList* m_pList;        // single list; reset per submit onto a ring allocator
	...
	enum { UPLOAD_ALLOC_RING = 16 };
	ID3D12CommandAllocator*    m_pAllocRing[UPLOAD_ALLOC_RING];
	unsigned __int64           m_ringFenceVal[UPLOAD_ALLOC_RING]; // fence value signaled at each allocator's last use
	unsigned                   m_ringHead;                        // next allocator index (taken mod ring size)
	ID3D12Resource*            m_pUploadRing[UPLOAD_ALLOC_RING];  // reusable staging buffer per slot (grown on demand)
	unsigned __int64           m_uploadRingCap[UPLOAD_ALLOC_RING];// current byte capacity of each slot's buffer

	ID3D12DescriptorHeap*      m_pSrvStaging;  // CPU (non-shader-visible) SRV heap; copy source for the renderer
	...
	ID3D12DescriptorHeap*      m_pRtvHeap;     // #DX12 п.3 RTT: RTV descriptors for render-target textures
	unsigned                   m_rtvHead;
	unsigned                   m_rtvCount;
	unsigned                   m_rtvInc;

	std::vector<unsigned>      m_srvFree;      // #DX12: reclaimed staging-SRV slots (reused before bumping m_srvHead)
	...
	enum { TEX_HEAP_BYTES = 64u << 20 };       // 64 MB per pool heap
	enum { TEX_POOL_SAFE_FRAMES = 8 };         // frames a freed region waits before reuse (>> swapchain depth)
	struct TexHeap { ID3D12Heap* heap; unsigned __int64 used; };            // bump watermark within the heap
	std::vector<TexHeap>                         m_texHeaps;
	struct FreeRegion { int heapIdx; unsigned __int64 offset; };
	std::map<unsigned __int64, std::vector<FreeRegion> > m_texFreeBySize;   // size -> GPU-safe reusable regions
	struct PendingFree { ID3D12Resource* res; unsigned __int64 size; unsigned __int64 offset; int heapIdx; unsigned freeEpoch; };
	std::vector<PendingFree>                     m_texPendingFree;          // resources/regions waiting out SAFE_FRAMES
