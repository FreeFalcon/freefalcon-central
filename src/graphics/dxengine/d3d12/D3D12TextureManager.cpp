...
#define T12_RELEASE(p) do { if (p) { (p)->Release(); (p) = 0; } } while (0)
...
enum { STAGING_SRV_COUNT = 262144 };

D3D12TextureManager::D3D12TextureManager()
	: m_pDevice(0), m_pQueue(0), m_pList(0), m_pFence(0), m_fenceVal(0), m_fenceEvent(0),
	  m_ringHead(0),
	  m_pSrvStaging(0), m_srvHead(0), m_srvCount(0), m_srvInc(0),
	  m_pRtvHeap(0), m_rtvHead(0), m_rtvCount(0), m_rtvInc(0), m_frameEpoch(0), m_csInit(false)
{
	for (int i = 0; i < UPLOAD_ALLOC_RING; ++i)
	{ m_pAllocRing[i] = 0; m_ringFenceVal[i] = 0; m_pUploadRing[i] = 0; m_uploadRingCap[i] = 0; }
}
D3D12TextureManager::~D3D12TextureManager() { Release(); }

bool D3D12TextureManager::Init()
{
	if (!g_pD3D12Backend || !g_pD3D12Backend->IsValid()) { T12Log("[D3D12Tex] no backend\n"); return false; }
	m_pDevice = g_pD3D12Backend->GetDevice();
	if (!m_pDevice) return false;

	D3D12_COMMAND_QUEUE_DESC qd; ZeroMemory(&qd, sizeof(qd));
	qd.Type = D3D12_COMMAND_LIST_TYPE_DIRECT;
	if (FAILED(m_pDevice->CreateCommandQueue(&qd, IID_PPV_ARGS(&m_pQueue)))) { T12Log("[D3D12Tex] queue failed\n"); return false; }
	for (int i = 0; i < UPLOAD_ALLOC_RING; ++i)
		if (FAILED(m_pDevice->CreateCommandAllocator(D3D12_COMMAND_LIST_TYPE_DIRECT, IID_PPV_ARGS(&m_pAllocRing[i])))) return false;
	if (FAILED(m_pDevice->CreateCommandList(0, D3D12_COMMAND_LIST_TYPE_DIRECT, m_pAllocRing[0], NULL, IID_PPV_ARGS(&m_pList)))) return false;
	m_pList->Close();
	m_ringHead = 0;
	for (int i = 0; i < UPLOAD_ALLOC_RING; ++i) { m_ringFenceVal[i] = 0; m_pUploadRing[i] = 0; m_uploadRingCap[i] = 0; }
	...
	hd.Flags = D3D12_DESCRIPTOR_HEAP_FLAG_NONE;   // CPU staging (copy source for the renderer's shader-visible ring)
	if (FAILED(m_pDevice->CreateDescriptorHeap(&hd, IID_PPV_ARGS(&m_pSrvStaging)))) { T12Log("[D3D12Tex] staging heap failed\n"); return false; }
	...
	D3D12_DESCRIPTOR_HEAP_DESC rd; ZeroMemory(&rd, sizeof(rd));
	rd.NumDescriptors = 256; rd.Type = D3D12_DESCRIPTOR_HEAP_TYPE_RTV; rd.Flags = D3D12_DESCRIPTOR_HEAP_FLAG_NONE;
	if (FAILED(m_pDevice->CreateDescriptorHeap(&rd, IID_PPV_ARGS(&m_pRtvHeap)))) { T12Log("[D3D12Tex] RTV heap failed\n"); return false; }
	m_rtvCount = 256; m_rtvHead = 0;
	m_rtvInc   = m_pDevice->GetDescriptorHandleIncrementSize(D3D12_DESCRIPTOR_HEAP_TYPE_RTV);

	InitializeCriticalSection(&m_cs); m_csInit = true;
...
void D3D12TextureManager::Release()
{
	if (m_pQueue && m_pFence)   // drain any in-flight upload
	{
		...
		if (m_pFence->GetCompletedValue() < m_fenceVal) { m_pFence->SetEventOnCompletion(m_fenceVal, m_fenceEvent); WaitForSingleObject(m_fenceEvent, INFINITE); }
	}
	...
	for (int i = 0; i < UPLOAD_ALLOC_RING; ++i) { T12_RELEASE(m_pUploadRing[i]); m_uploadRingCap[i] = 0; }
	for (int i = 0; i < UPLOAD_ALLOC_RING; ++i) T12_RELEASE(m_pAllocRing[i]);
	...
	for (size_t i = 0; i < m_texPendingFree.size(); ++i) if (m_texPendingFree[i].res) m_texPendingFree[i].res->Release();
	m_texPendingFree.clear(); m_texFreeBySize.clear();
	for (size_t i = 0; i < m_texHeaps.size(); ++i) T12_RELEASE(m_texHeaps[i].heap);
	m_texHeaps.clear();
...
{
	if (f & MPR_TI_DXT1)   return DXGI_FORMAT_BC1_UNORM;
	if (f & MPR_TI_DXT3)   return DXGI_FORMAT_BC2_UNORM;
	if (f & MPR_TI_DXT5)   return DXGI_FORMAT_BC3_UNORM;
	if (f & MPR_TI_ARGB32) return DXGI_FORMAT_B8G8R8A8_UNORM;
	if (f & MPR_TI_RGB24)  return DXGI_FORMAT_B8G8R8A8_UNORM;
	if (f & MPR_TI_RGB16)  return DXGI_FORMAT_B5G6R5_UNORM;
	return DXGI_FORMAT_B8G8R8A8_UNORM;
}
bool D3D12TextureManager::IsBlockCompressed(int fmt)
{ return fmt == DXGI_FORMAT_BC1_UNORM || fmt == DXGI_FORMAT_BC2_UNORM || fmt == DXGI_FORMAT_BC3_UNORM; }
...
int D3D12TextureManager::BeginUploadList()
{
	...
	{ m_pFence->SetEventOnCompletion(m_ringFenceVal[idx], m_fenceEvent); WaitForSingleObject(m_fenceEvent, INFINITE); }
...
ID3D12Resource* D3D12TextureManager::EnsureUploadRing(int idx, unsigned __int64 size)
{
	if (size < 1) size = 1;
	if (m_pUploadRing[idx] && m_uploadRingCap[idx] >= size) return m_pUploadRing[idx];
	T12_RELEASE(m_pUploadRing[idx]); m_uploadRingCap[idx] = 0;
	...
	bd.Dimension = D3D12_RESOURCE_DIMENSION_BUFFER; bd.Width = size; bd.Height = 1; bd.DepthOrArraySize = 1;
	...
	{ if (FAILED(m_pDevice->CreateCommittedResource(&hpUp, D3D12_HEAP_FLAG_NONE, &bd,
		D3D12_RESOURCE_STATE_GENERIC_READ, NULL, IID_PPV_ARGS(&buf)))) return 0; }
	m_pUploadRing[idx] = buf; m_uploadRingCap[idx] = size;
	return buf;
}
...
{
	if (!renderQueue || !m_pFence) return;
	...
	if (v > 0) renderQueue->Wait(m_pFence, v);
}

bool D3D12TextureManager::UploadAndBarrier(ID3D12Resource* dst, bool placed, const TexMipData* mips, int mipCount,
                                           const void* footprints, const unsigned* numRows,
                                           const unsigned __int64* rowSize, unsigned __int64 total)
{
	const D3D12_PLACED_SUBRESOURCE_FOOTPRINT* fp = (const D3D12_PLACED_SUBRESOURCE_FOOTPRINT*)footprints;
	...
	if (!upload) { m_pList->Close(); LeaveCriticalSection(&m_cs); return false; }
	...
	if (placed)
	{
		D3D12_RESOURCE_BARRIER ab; ZeroMemory(&ab, sizeof(ab));
		ab.Type = D3D12_RESOURCE_BARRIER_TYPE_ALIASING;
		ab.Aliasing.pResourceBefore = NULL; ab.Aliasing.pResourceAfter = dst;
	...
	if (FAILED(upload->Map(0, &noRead, (void**)&mapped)) || !mapped) { m_pList->Close(); LeaveCriticalSection(&m_cs); return false; }
	for (int i = 0; i < mipCount; ++i)
	{
		...
		UINT copyBytes = (srcPitch < (UINT)rowSize[i]) ? srcPitch : (UINT)rowSize[i];
		for (UINT r = 0; r < rows; ++r)
			memcpy(dstMip + (size_t)r * dstPitch, srcMip + (size_t)r * srcPitch, copyBytes);
	}
	upload->Unmap(0, NULL);
	...
	for (int i = 0; i < mipCount; ++i)
	{
		...
		dstL.pResource = dst; dstL.Type = D3D12_TEXTURE_COPY_TYPE_SUBRESOURCE_INDEX; dstL.SubresourceIndex = (UINT)i;
		D3D12_TEXTURE_COPY_LOCATION srcL; ZeroMemory(&srcL, sizeof(srcL));
		srcL.pResource = upload; srcL.Type = D3D12_TEXTURE_COPY_TYPE_PLACED_FOOTPRINT; srcL.PlacedFootprint = fp[i];
		m_pList->CopyTextureRegion(&dstL, 0, 0, 0, &srcL, NULL);
	}
	D3D12_RESOURCE_BARRIER b; ZeroMemory(&b, sizeof(b));
	b.Type = D3D12_RESOURCE_BARRIER_TYPE_TRANSITION;
	b.Transition.pResource = dst; b.Transition.Subresource = D3D12_RESOURCE_BARRIER_ALL_SUBRESOURCES;
...
ID3D12Resource* D3D12TextureManager::AllocPlacedTexture(const void* resourceDesc, D3D12Texture& out)
{
	const D3D12_RESOURCE_DESC& td = *(const D3D12_RESOURCE_DESC*)resourceDesc;
	out.poolHeapIdx = -1; out.poolOffset = 0; out.poolSize = 0;
	D3D12_HEAP_PROPERTIES hpDef; ZeroMemory(&hpDef, sizeof(hpDef)); hpDef.Type = D3D12_HEAP_TYPE_DEFAULT;

	D3D12_RESOURCE_ALLOCATION_INFO ai = m_pDevice->GetResourceAllocationInfo(0, 1, &td);
	unsigned __int64 size  = ai.SizeInBytes;
	unsigned __int64 align = ai.Alignment ? ai.Alignment : (unsigned __int64)(64u << 10);
	if (size == 0 || size == (unsigned __int64)-1 || size > (unsigned __int64)TEX_HEAP_BYTES)
	{
		...
		if (FAILED(m_pDevice->CreateCommittedResource(&hpDef, D3D12_HEAP_FLAG_NONE, &td,
			D3D12_RESOURCE_STATE_COPY_DEST, NULL, IID_PPV_ARGS(&tex)))) return 0;
		return tex;
	}

	EnterCriticalSection(&m_cs);
	int heapIdx = -1; unsigned __int64 offset = 0;
	std::map<unsigned __int64, std::vector<FreeRegion> >::iterator it = m_texFreeBySize.find(size);
	if (it != m_texFreeBySize.end() && !it->second.empty())
	{ FreeRegion fr = it->second.back(); it->second.pop_back(); heapIdx = fr.heapIdx; offset = fr.offset; }
	else
	{
		for (size_t h = 0; h < m_texHeaps.size() && heapIdx < 0; ++h)   // bump within an existing heap
		{
			unsigned __int64 base = (m_texHeaps[h].used + (align - 1)) & ~(align - 1);
			if (base + size <= (unsigned __int64)TEX_HEAP_BYTES)
			{ heapIdx = (int)h; offset = base; m_texHeaps[h].used = base + size; }
		}
		if (heapIdx < 0)                                                 // grow: new heap
		{
			D3D12_HEAP_DESC hd; ZeroMemory(&hd, sizeof(hd));
			hd.SizeInBytes = (unsigned __int64)TEX_HEAP_BYTES;
			hd.Properties.Type = D3D12_HEAP_TYPE_DEFAULT;
			hd.Flags = D3D12_HEAP_FLAG_ALLOW_ONLY_NON_RT_DS_TEXTURES;
			ID3D12Heap* heap = 0;
			if (SUCCEEDED(m_pDevice->CreateHeap(&hd, IID_PPV_ARGS(&heap))) && heap)
			{ TexHeap th; th.heap = heap; th.used = size; m_texHeaps.push_back(th); heapIdx = (int)m_texHeaps.size() - 1; offset = 0; }
		}
	}
	...
	if (heap)
	{
		...
		if (SUCCEEDED(m_pDevice->CreatePlacedResource(heap, offset, &td,
			D3D12_RESOURCE_STATE_COPY_DEST, NULL, IID_PPV_ARGS(&tex))) && tex)
		{ out.poolHeapIdx = heapIdx; out.poolOffset = offset; out.poolSize = size; return tex; }
		EnterCriticalSection(&m_cs);   // placed create failed -> return the region, fall back to committed
		FreeRegion fr; fr.heapIdx = heapIdx; fr.offset = offset; m_texFreeBySize[size].push_back(fr);
		LeaveCriticalSection(&m_cs);
	}
...
void D3D12TextureManager::DeferTextureFree(ID3D12Resource* res, int heapIdx, unsigned __int64 offset, unsigned __int64 size)
{
	if (!res && heapIdx < 0) return;
	if (m_csInit) EnterCriticalSection(&m_cs);
	PendingFree pf; pf.res = res; pf.size = size; pf.offset = offset; pf.heapIdx = heapIdx; pf.freeEpoch = m_frameEpoch;
	m_texPendingFree.push_back(pf);
	if (m_csInit) LeaveCriticalSection(&m_cs);
}
...
{
	if (!m_csInit) { m_frameEpoch = renderEpoch; return; }
	EnterCriticalSection(&m_cs);
	...
	for (size_t i = 0; i < m_texPendingFree.size(); ++i)
	{
		...
		{
			if (pf.res) pf.res->Release();                     // GPU done -> safe to release the resource object
			if (pf.heapIdx >= 0)                               // pooled -> region is now reusable
			{ FreeRegion fr; fr.heapIdx = pf.heapIdx; fr.offset = pf.offset; m_texFreeBySize[pf.size].push_back(fr); }
		}
		else m_texPendingFree[w++] = pf;
	}
...
{ if (g_pD3D12TextureManager) g_pD3D12TextureManager->TickFrame(renderEpoch); }

//================================ creation ==================================
bool D3D12TextureManager::Create(D3D12Texture& out, int width, int height, int dxgiFormat,
                                 const TexMipData* mips, int mipCount)
{
	if (!m_pDevice || mipCount <= 0 || mipCount > 15 || width < 1 || height < 1) return false;
	...
	td.Width = (UINT64)width; td.Height = (UINT)height; td.DepthOrArraySize = 1; td.MipLevels = (UINT16)mipCount;
	td.Format = (DXGI_FORMAT)dxgiFormat; td.SampleDesc.Count = 1;
	td.Layout = D3D12_TEXTURE_LAYOUT_UNKNOWN;
	ID3D12Resource* tex = AllocPlacedTexture(&td, out);
	if (!tex)
	{ T12Log("[D3D12Tex] texture alloc failed %dx%d fmt=%d\n", width, height, dxgiFormat); return false; }

	// 2. copyable footprints.
	D3D12_PLACED_SUBRESOURCE_FOOTPRINT fp[16]; UINT numRows[16]; UINT64 rowSize[16]; UINT64 total = 0;
	m_pDevice->GetCopyableFootprints(&td, 0, (UINT)mipCount, 0, fp, numRows, rowSize, &total);
	...
	if (!UploadAndBarrier(tex, out.poolHeapIdx >= 0, mips, mipCount, fp, numRows, rowSize, total))
	{
		DeferTextureFree(tex, out.poolHeapIdx, out.poolOffset, out.poolSize);   // deferred (safe even w/o GPU work)
		out.poolHeapIdx = -1; out.poolOffset = 0; out.poolSize = 0;
		return false;
	}
	...
	if (srvSlot < 0) { T12Log("[D3D12Tex] staging heap full\n"); tex->Release(); return false; }
	D3D12_CPU_DESCRIPTOR_HANDLE h = m_pSrvStaging->GetCPUDescriptorHandleForHeapStart();
	...
	out.tex = tex; out.srvCpuPtr = (unsigned __int64)h.ptr;
	out.width = width; out.height = height; out.mips = mipCount; out.dxgiFormat = dxgiFormat;
	return true;
}

bool D3D12TextureManager::CreateBCn(D3D12Texture& out, int width, int height, int dxgiFormat,
                                    const void* blockData, int /*bytes*/)
{
	int blocksW  = (width + 3) / 4;
	int rowPitch = blocksW * BlockBytes(dxgiFormat);
	TexMipData mip; mip.data = blockData; mip.rowPitch = rowPitch;
	return Create(out, width, height, dxgiFormat, &mip, 1);
}

bool D3D12TextureManager::CreateRenderTarget(D3D12Texture& out, int width, int height)
{
	if (!m_pDevice || width < 1 || height < 1) return false;
	...
	D3D12_CLEAR_VALUE cv; ZeroMemory(&cv, sizeof(cv)); cv.Format = DXGI_FORMAT_R8G8B8A8_UNORM;   // black
	ID3D12Resource* tex = 0;
	if (FAILED(m_pDevice->CreateCommittedResource(&hp, D3D12_HEAP_FLAG_NONE, &td,
		D3D12_RESOURCE_STATE_PIXEL_SHADER_RESOURCE, &cv, IID_PPV_ARGS(&tex))))
	...
	if (srvSlot < 0 || rtvSlot < 0)
	{
		...
		if (m_csInit) EnterCriticalSection(&m_cs);
		if (srvSlot >= 0) m_srvFree.push_back((unsigned)srvSlot);
		if (rtvSlot >= 0) m_rtvFree.push_back((unsigned)rtvSlot);
		if (m_csInit) LeaveCriticalSection(&m_cs);
		tex->Release(); return false;
	}
	D3D12_CPU_DESCRIPTOR_HANDLE sh = m_pSrvStaging->GetCPUDescriptorHandleForHeapStart();
	...
	D3D12_CPU_DESCRIPTOR_HANDLE rh = m_pRtvHeap->GetCPUDescriptorHandleForHeapStart();
	rh.ptr += (SIZE_T)rtvSlot * m_rtvInc;
	m_pDevice->CreateRenderTargetView(tex, NULL, rh);

	out.tex = tex; out.srvCpuPtr = (unsigned __int64)sh.ptr; out.rtvCpuPtr = (unsigned __int64)rh.ptr;
	out.width = width; out.height = height; out.mips = 1; out.dxgiFormat = DXGI_FORMAT_R8G8B8A8_UNORM;
	out.isRT = true; out.rtState = (unsigned)D3D12_RESOURCE_STATE_PIXEL_SHADER_RESOURCE;
	return true;
}
...
int D3D12TextureManager::AllocSrvSlot()
{
	int slot = -1;
	if (m_csInit) EnterCriticalSection(&m_cs);
	if (!m_srvFree.empty())      { slot = (int)m_srvFree.back(); m_srvFree.pop_back(); }
	else if (m_srvHead < m_srvCount) { slot = (int)m_srvHead++; }
	if (m_csInit) LeaveCriticalSection(&m_cs);
	return slot;
}
int D3D12TextureManager::AllocRtvSlot()
{
	int slot = -1;
	if (m_csInit) EnterCriticalSection(&m_cs);
	if (!m_rtvFree.empty())      { slot = (int)m_rtvFree.back(); m_rtvFree.pop_back(); }
	else if (m_rtvHead < m_rtvCount) { slot = (int)m_rtvHead++; }
	if (m_csInit) LeaveCriticalSection(&m_cs);
	return slot;
}
void D3D12TextureManager::ReclaimSlots(const D3D12Texture& t)
{
	...
	{
		unsigned __int64 base = (unsigned __int64)m_pSrvStaging->GetCPUDescriptorHandleForHeapStart().ptr;
		unsigned slot = (unsigned)((t.srvCpuPtr - base) / m_srvInc);
		if (slot < m_srvCount) m_srvFree.push_back(slot);
	}
	if (t.isRT && m_pRtvHeap && t.rtvCpuPtr && m_rtvInc)
	{
		unsigned __int64 base = (unsigned __int64)m_pRtvHeap->GetCPUDescriptorHandleForHeapStart().ptr;
		unsigned slot = (unsigned)((t.rtvCpuPtr - base) / m_rtvInc);
		if (slot < m_rtvCount) m_rtvFree.push_back(slot);
	}
	if (m_csInit) LeaveCriticalSection(&m_cs);
}
...
void D3D12TextureManager::Destroy(D3D12Texture& t)
{
	...
	t.tex = 0;
	t.srvCpuPtr = 0; t.rtvCpuPtr = 0; t.isRT = false; t.width = t.height = t.mips = 0;
	t.poolHeapIdx = -1; t.poolOffset = 0; t.poolSize = 0;
}

ID3D12Resource* D3D12TextureManager::CreateVertexBufferGPU(const void* data, unsigned bytes)
{
	if (!m_pDevice || !data || bytes < 1) return 0;
	...
	HRESULT hrVb;
	{ hrVb = m_pDevice->CreateCommittedResource(&hpDef, D3D12_HEAP_FLAG_NONE, &bd,
		D3D12_RESOURCE_STATE_COMMON, NULL, IID_PPV_ARGS(&vb)); }
	if (FAILED(hrVb)) { T12Log("[D3D12Tex] VB create failed %u\n", bytes); return 0; }
	...
	if (!upload) { LeaveCriticalSection(&m_cs); vb->Release(); return 0; }
	void* mapped = 0; D3D12_RANGE noRead; noRead.Begin = 0; noRead.End = 0;
	if (FAILED(upload->Map(0, &noRead, &mapped)) || !mapped) { LeaveCriticalSection(&m_cs); vb->Release(); return 0; }
	...
	D3D12_RESOURCE_BARRIER b; ZeroMemory(&b, sizeof(b));
	b.Type = D3D12_RESOURCE_BARRIER_TYPE_TRANSITION;
	b.Transition.pResource = vb; b.Transition.Subresource = D3D12_RESOURCE_BARRIER_ALL_SUBRESOURCES;
	b.Transition.StateBefore = D3D12_RESOURCE_STATE_COPY_DEST;
...
D3D12Texture* D3D12TextureManager::Alloc() { return new D3D12Texture(); }
