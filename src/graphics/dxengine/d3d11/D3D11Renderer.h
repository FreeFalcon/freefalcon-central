...
#include "IRenderer.h"   // Artscout - 2026: #DX12 -- API-neutral renderer interface
...
class D3D11Renderer : public IRenderer   // Artscout - 2026: #DX12 -- implements the neutral renderer surface
{
public:
	D3D11Renderer();
	~D3D11Renderer();
	...
	// Upload the light set + ambient into cbLights (object-path lighting). #DX12: neutral (void*+stride) so it
	// overrides IRenderer -- 'lights' is a packed array of 64-byte GpuLightCPU (== GpuLight in the shader).
	void SetLights(const float ambient[4], int numLights, const void* lights, int lightStride);
	...
	// #DX12: neutral vbHandle (void* = ID3D11Buffer* here) so these override IRenderer.
	...
	// Artscout - 2026: #VFX Phase 1 -- GPU-instanced particles (D3D12 target). Stubbed on D3D11 for now.
...
private:
	...
	bool          m_irGrey;			// #DX12 A5: sensor grey pass active (sticky, FF_IRGREY bit 14)
