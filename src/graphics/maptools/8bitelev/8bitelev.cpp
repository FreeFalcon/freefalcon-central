...
int main(int argc, char **argv)
{
    ...
    if ((argc < 5) || (argc > 6))
    {
        printf("Usage:  16to8 <input file> <output file> <width> <height> "
               "[maxelevation]\n");
        printf("   This tool reads in a 16 bit RAW file and scales it to 8 "
               "bits.\n");
        printf("   Negative values are clamped to 0, and the largest positive "
               "value\n");
        printf("   encountered in the 16 bit data is scaled to 0xFF in the "
               "output file.\n");
        printf("   It is assumed that the input file is 16 bit interleaved RAW "
               "data.  If\n");
        printf("   the maxelevation parameter is supplied, the initial scan of "
               "the RAW file\n");
        printf("   is skipped and the provided value is used to compute the "
               "scale factor.\n");
    ...
    if (maxValue == 0)
    {
        ...
        for (row = height; row > 0; row--)
        {
            ...
            if (!ReadFile(inFile, buffer, width * 2, &bytes, NULL))
                bytes = 1;
    ...
    for (row = height; row > 0; row--)
    {
        ...
        // Process the row of data
        for (src = buffer, dst = (BYTE *)buffer; src < buffer + width;
             src++, dst++)
        {
    ...
    printf("The minimum representable change in the output is %0.4f units of "
           "the input.\n",
           1.0 / scale);
