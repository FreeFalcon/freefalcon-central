...
struct D3DXVECTOR3
{
	...
	D3DXVECTOR3 operator * (const D3DXVECTOR3& v) const { return D3DXVECTOR3(x * v.x, y * v.y, z * v.z); }	// component-wise (like the old _D3DVECTOR)
...
namespace _redviper_d3dx_detail
{
	...
	{
		return DirectX::XMLoadFloat4x4(reinterpret_cast<const DirectX::XMFLOAT4X4*>(p));
	}
...
inline D3DXMATRIX* D3DXMatrixPerspectiveFov(D3DXMATRIX* pOut, float fovy, float aspect, float zn, float zf)
{
	_redviper_d3dx_detail::StoreM(pOut, DirectX::XMMatrixPerspectiveFovLH(fovy, aspect, zn, zf));
	return pOut;
}
...
{
	using namespace DirectX;
	_redviper_d3dx_detail::StoreV3(pOut, XMVector3Normalize(_redviper_d3dx_detail::LoadV3(pV)));
	return pOut;
}
