...
#define THRESH_POINT_ON_PLANE                                                  \
    (0.10) /* Thickness of plane for front/back/inside test */
#define THRESH_VECTORS_ARE_PARALLEL                                            \
    (0.02) /* Vectors are parallel if dot product varies less than this */
...
public:
    ...
    {
        return fabs(x) < KINDA_SMALL_NUMBER && fabs(y) < KINDA_SMALL_NUMBER &&
    ...
    {
        return Vector(Clamp(x, -Radius, Radius), Clamp(y, -Radius, Radius),
    ...
    friend float PointPlaneDist(const Vector& Point, const Vector& PlaneBase,
                                const Vector& PlaneNormal);
    friend bool Parallel(const Vector& Normal1, const Vector& Normal2);
    friend bool Coplanar(const Vector& Base1, const Vector& Normal1,
                         const Vector& Base2, const Vector& Normal2);
...
public:
    ...
    Plane(const Plane& P) : Vector(P), w(P.w)
    {
    }

    Plane(const Vector& V) : Vector(V), w(0)
    {
    }
...
public:
    ...
    {
        RotateRad((float)(degsx * PI_FRAC), float(degsy * PI_FRAC),
                  float(degsz * PI_FRAC));
    }
    ...
    void SetViewMatrix(Vector& vFrom, Vector& vAt, Vector& vWorldUp)
    {
        ...
        if (1e-6f > (fLength = vUp.Size()))
        {
            ...
            if (1e-6f >
                (fLength =
                     vUp.Size())) // If we still have near-zero length, resort to a different axis.
            {
    ...
    void SetProjectionMatrix(float fFOV, float fAspect, float fNearPlane,
                             float fFarPlane)
    {
        fFOV *= (float)PI_FRAC;
...
public:
    ...
    LitVertex(Vector vLoc, Vector vNorm, float fTu = 0.0f, float fTv = 0.0f,
              DWORD colDiffuse = 0xFFFFFFFF)
        : Vertex(vLoc, vNorm)
    {
...
protected:
    virtual void UpdateMatrix()
    {
        ...
        case 7:
        {
            ...
            m_mx =
                mS * mR *
                mT; // Note the order is reversed compared to the right-to-left rule
            break;
        }
        }
...
public:
    ...
    void SetRotation(float x, float y,
                     float z) // Sets the new orientation (in degrees)
    {
        m_vRot.x = x;
        m_vRot.y = y;
        m_vRot.z = z;
...
inline float FSnap(FLOAT Location, FLOAT Grid)
{
    if (Grid == 0.0)
        ...
        return (float)appFloor((Location + 0.5 * Grid) / Grid) * Grid;
}
...
{
    return Point1 +
           (Point2 - Point1) * (((PlaneOrigin - Point1) | PlaneNormal) /
                                ((Point2 - Point1) | PlaneNormal));
}

inline Vector LinePlaneIntersection(const Vector& Point1, const Vector& Point2,
                                    const Plane& Plane)
{
    return Point1 + (Point2 - Point1) * ((Plane.w - (Point1 | Plane)) /
                                         ((Point2 - Point1) | Plane));
}

// Compute intersection point and direction of line joining two planes.
// Return 1 if valid, 0 if infinite.
inline int IntersectPlanes2(Vector& I, Vector& D, const Plane& P1,
                            const Plane& P2)
{
...
inline float PointPlaneDist(const Vector& Point, const Vector& PlaneBase,
                            const Vector& PlaneNormal)
{
...
inline bool Coplanar(const Vector& Base1, const Vector& Normal1,
                     const Vector& Base2, const Vector& Normal2)
{
    if (!Parallel(Normal1, Normal2))
        return false;
    else if (PointPlaneDist(Base2, Base1, Normal1) > THRESH_POINT_ON_PLANE)
        return false;
    else
        return true;
}
