...
#define WORLD_TO_GLOBAL_POST(distance)                                         \
    (FloatToInt32((float)floor(WORLD_TO_FLOAT_GLOBAL_POST(distance))))
#define WORLD_TO_FLOAT_LEVEL_POST(distance, LOD)                               \
    ((distance) / TheMap.Level(LOD)->FTperPOST())
#define WORLD_TO_LEVEL_POST(distance, LOD)                                     \
    (WORLD_TO_GLOBAL_POST(distance) >> (LOD))
#define WORLD_TO_LEVEL_BLOCK(distance, LOD)                                    \
    (WORLD_TO_GLOBAL_POST(distance) >> ((LOD) + POST_OFFSET_BITS))

#define GLOBAL_POST_TO_WORLD(distance) ((distance) * FeetPerPost)
#define LEVEL_POST_TO_WORLD(distance, LOD)                                     \
    (GLOBAL_POST_TO_WORLD((distance) << (LOD)))
#define LEVEL_BLOCK_TO_WORLD(distance, LOD)                                    \
    (GLOBAL_POST_TO_WORLD((distance) << ((LOD) + POST_OFFSET_BITS)))
