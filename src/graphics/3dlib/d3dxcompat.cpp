...
D3DX_SURFACEFORMAT D3DXMakeSurfaceFormat(struct _DDPIXELFORMAT* pddpfIn)
{
    ...
    {
        if (p->dwFourCC == MAKEFOURCC('D', 'X', 'T', '1'))
            return D3DX_SF_DXT1;
        if (p->dwFourCC == MAKEFOURCC('D', 'X', 'T', '3'))
            return D3DX_SF_DXT3;
        if (p->dwFourCC == MAKEFOURCC('D', 'X', 'T', '5'))
            return D3DX_SF_DXT5;
        return D3DX_SF_UNKNOWN;
    }
...
//-----------------------------------------------------------------------------
long D3DXInitialize(void)
{
    return S_OK;
}
long D3DXUninitialize(void)
{
    return S_OK;
}
...
//-----------------------------------------------------------------------------
long D3DXCreateTexture(struct IDirect3DDevice7* /*pDevice*/,
...
long D3DXLoadTextureFromMemory(struct IDirect3DDevice7* /*pDevice*/,
