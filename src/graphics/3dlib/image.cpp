...
GLint ReadTGA(CImageFileMemory *fi)
{
    ...
    if (tgaheader.imagetype not_eq 0x2)
        return BAD_FORMAT;
...
GLint ReadDDS(CImageFileMemory *fi)
{
    ...
    if (dwMagic not_eq MAKEFOURCC('D', 'D', 'S', ' '))
        return BAD_FORMAT;

#if defined(_M_IX86)
    if (not fi->glReadMem(&ddsd, sizeof(DDSURFACEDESC2)))
        return BAD_FORMAT;
#else
    ...
    {
        DDSDiskHeader _h;
        if (not fi->glReadMem(&_h, DDS_DISK_HEADER_SIZE))
            return BAD_FORMAT;
...
GLint ReadBMP(CImageFileMemory *fi)
{
    ...
    else
    {
        ...
        for (j = 0; j < i; j++)
        {
            ...
            *palptr++ = (bmprgb.rgbBlue << 16) bitor
                        (bmprgb.rgbGreen << 8) bitor bmprgb.rgbRed;
        }
    }
    ...
    while (j--)
    {
        ...
        if (padBytes)
            fi->glReadMem(&paddings, padBytes);
    }
...
GLint UnpackGIF(CImageFileMemory *fi)
{
    ...
    if (fi->glReadMem((GLubyte *)&gh, sizeof(gh)) not_eq sizeof(gh) or
        memcmp(gh.sig, "GIF", 3))
    {
        return (BAD_FILE);
    }
    ...
    if (gh.flags bitand 0x80)
    {
        ...
        {
            *palOut++ = 0xFF000000 bitor (palIn[2] << 16) bitor
                        (palIn[1] << 8) bitor palIn[0];
            palIn += 3;
        }
    }
    ...
    while ((c = fi->glReadCharMem()) == ',' or c == '!' or c == 0)
    {
        if (c == ',')
        {
            ...
            fi->image.image = (GLubyte *)glAllocateMemory(
                fi->image.width * fi->image.height, 0);
...
GLint GIF_UnpackImage(GLint bits, CImageFileMemory *fi, GLint currentFlag)
{
    ...
    GLuint wordmasktable[] = {0x0000, 0x0001, 0x0003, 0x0007, 0x000f, 0x001f,
    ...
    for (;;)
    {
        ...
        {
            if (++p >= q and
                (((blocksize = fi->glReadCharMem()) < 1) or
        ...
        else
        {
            ...
            else
            {
                if (++p >= q and (((blocksize = fi->glReadCharMem()) < 1) or
                                  (q = (p = b) + fi->glReadMem(b, blocksize)) <
                                      (b + blocksize)))
                    return (UNEXPECTED_EOF);
...
GLint UnpackLBM(CImageFileMemory *fi)
{
    ...
    if (memcmp((GLubyte *)&header[8], (GLubyte *)"PBM ", 4))
        doIFF = 0;
    else
        doIFF = 1;
    ...
    if (memcmp((GLubyte *)header, (GLubyte *)"BMHD", 4))
        return NULL;
...
GLulong *ReadLBMColorMap(CImageFileMemory *fi)
{
    ...
    do
    {
        ...
        {
            if (size bitand 1)
                size++; // All offsets on an even boundary
...
GLubyte *ReadLBMBody(CImageFileMemory *fi, LBM_BMHD *lpHeader, GLint doIFF)
{
    ...
    if (lpHeader->nPlanes <= 4)
        imageSize *= lpHeader->nPlanes;
    else if (lpHeader->nPlanes <= 8)
        imageSize <<= 3;
    ...
    for (i = 0; i < fi->image.height; i++)
    {
        ...
        {
            GLint masktable[8] = {0x80, 0x40, 0x20, 0x10,
                                  0x08, 0x04, 0x02, 0x01};
            GLint bittable[8] = {0x01, 0x02, 0x04, 0x08,
                                 0x10, 0x20, 0x40, 0x80};
            GLint k, l, m;
            GLbyte *line;
...
GLint UnpackPCX(CImageFileMemory *fi)
{
    ...
    if ((fi->glReadMem((GLubyte *)&pcx, sizeof(PCXHEAD)) not_eq
         sizeof(PCXHEAD)) or
        (pcx.manufacturer not_eq 10) or (pcx.version not_eq 5))
    {
    ...
    if ((fi->glReadCharMem() not_eq 0xc) or
        (not fi->glReadMem(pcxPalette, 768)))
    {
        glReleaseMemory((char *)fi->image.image);
        return (BAD_ALLOC);
    }
