...
\***************************************************************************/
DrawableShadowed::DrawableShadowed(int ID, const Tpoint *pos,
                                   const Trotation *rot, float s, int ShadowID)
...
void DrawableShadowed::Draw(class RenderOTW *renderer, int LOD)
{
    ...
    if (position.z >
        renderer->viewpoint->GetGroundLevel(position.x, position.y) - 2000.f)
    {
        float yaw;
        float pitch;
        float roll;
        ...
        TheTimeOfDay.GetLightDirection(
            &LightDir); // COBRA - RED - For Light direction
        ...
        if (pos.z < realWeather->VisibleLimit())
        {
            dZ = fabs(
                position.z -
                pos.z); // Absolute distance ( not oriented, but who cares...???)
            pos.x =
                position.x -
                LightDir.x * dZ *
                    (2 + LightDir.z); // COBRA - RED - Light Direction Casting
            pos.y =
                position.y -
                LightDir.y * dZ *
                    (2 + LightDir.z); // COBRA - RED - Light Direction Casting
            ...
            ds =
                max(0.005f,
                    1.0f - dZ / 1000.0f); // COBRA - RED - Approxximate distance
            ...
            ShadowBSPRendering =
                true; // COBRA - RED - We are rendering a Shadow affected by TOD Light
            ds = ds * ds; // The cube of Distance is used for Shadow Alpha
            ShadowAlphaLevel =
                TheTimeOfDay.GetAmbientValue() * 3.0f *
                ds; // calculate the Shadow Alpha based on TOD Light and Distance

            if (ShadowAlphaLevel > 1.0f)
                ShadowAlphaLevel = 1.0f; // Limit Check
            ...
            TheStateStack.DrawWarpedObject(&shadowInstance, &rot, &pos, sx, sy,
                                           1.0f, instance.Radius());
