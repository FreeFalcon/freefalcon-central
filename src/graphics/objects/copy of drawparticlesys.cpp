...
{
public:
    SubPartPoly(ParticleNode *owner): SubPart(owner)
    {
        next = 0;
    };
    virtual void Run(RenderOTW *renderer, ParticleNode *owner);
...
void SubEmitter::Run(RenderOTW *renderer, ParticleNode *owner)
{
    ...
    while (qty >= 1)
    {
        ...
        pos.y += owner->pos.y;
        pos.z += owner->pos.z;
        ParticleNode *n = new ParticleNode(pep->id, &pos, &owner->vel, &aim);
...
void ParticleNode::Draw(class RenderOTW *renderer, int LOD)
{
    ...
    age         = (curTime - birthTime) * .001;
    elapsedTime = (curTime - lastTime) * .001;
    life        = age / lifespan;
    ...
    if (elapsedTime)
    {
        ...
        if (ppn->simpleDrag)
        {
            ...
            vel.x -= vel.x * ppn->simpleDrag * elapsedTime;
            vel.y -= vel.y * ppn->simpleDrag * elapsedTime;
            vel.z -= vel.z * ppn->simpleDrag * elapsedTime;
            vel.x += DrawableParticleSys::winddx;
            vel.y += DrawableParticleSys::winddy;
        }
