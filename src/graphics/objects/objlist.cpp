...
static struct ObjListCSInit
{
    ObjListCSInit()
    {
        InitializeCriticalSection(&g_objListCS);
    }
} s_objListCSInit;
struct ObjListLock
{
    ObjListLock()
    {
        EnterCriticalSection(&g_objListCS);
    }
    ~ObjListLock()
    {
        LeaveCriticalSection(&g_objListCS);
    }
};
...
void ObjectDisplayList::RemoveObject(DrawableObject *object)
{
    ...
    if (object->prev)
    {
        if (not F4IsBadWritePtr(object->prev,
                                sizeof(DrawableObject))) // JB 010221 CTD
    ...
    if (object->next)
    {
        if (not F4IsBadWritePtr(object->next,
                                sizeof(DrawableObject))) // JB 010221 CTD
...
void ObjectDisplayList::UpdateMetrics(const Tpoint *pos)
{
    ...
    while (p and not F4IsBadReadPtr(p, sizeof(DrawableObject))) // JB 010318 CTD
    {
        ...
        if (p->parentList not_eq this)
            break;
        // Update the distance metric (not less than 0)
        p->distance =
            max((float)fabs(x - p->position.x), (float)fabs(y - p->position.y));
...
static void SafeCallUpdateCB(UpdateCallBack *nc, long listNo, const Tpoint *pos,
                             TransportStr *transList)
{
    __try
    {
        nc->fn(nc->self, listNo, pos, transList);
    }
    __except (EXCEPTION_EXECUTE_HANDLER)
    { /* dangling self -> skip this callback for the frame */
    }
}
...
static bool SafeDrawObject(DrawableObject *obj, class RenderOTW *renderer,
                           int lod)
{
    __try
    {
...
void ObjectDisplayList::UpdateMetrics(long listNo, const Tpoint *pos,
                                      TransportStr *transList)
{
    ObjListLock
        _lk; // #9 (recursive: a bridge-callback inside re-enters Remove/Insert/UpdateMetrics)
    register float x = pos->x;
    register float y = pos->y;
    // register float z = pos->z;
    ...
    while (q)
    {
        ...
        if (F4IsBadReadPtr(q, sizeof(DrawableObject)) or
            q->parentList not_eq this)
            break;
        ...
        if (++_iterGuard > 200000)
        {
            ...
            OutputDebugStringA("ObjectDisplayList::UpdateMetrics: CYCLE in "
                               "object list (>200k) -> break\n");
            break;
        }
        ...
        if (transList)
        {
            ...
            else if (p->position.z < transList->top[listNo] and
                     listNo < (_NUM_OBJECT_LISTS_ - 1))
            {
                i = listNo + 1;

                while (i < (_NUM_OBJECT_LISTS_ - 1) and
                       p->position.z < transList->top[i])
                    i++;
    ...
    long _cbGuard =
        0; // #41 HANG (caught in 22.png: SimLoopWrapper forever in PtrLooksBad): if
    ...
    for (UpdateCallBack *nextCall = updateCBlist; nextCall;
         nextCall = nextCall->next)
    {
        if (++_cbGuard > 100000)
            break; // #41 cyclic callback list -> break instead of hanging
        if (F4IsBadReadPtr(nextCall, sizeof(UpdateCallBack)))
            break; // bad node -> stop
        ...
        if (F4IsBadReadPtr(nextCall->self, sizeof(DrawableObject)))
            continue; // bad self -> skip
        if (((DrawableObject *)nextCall->self)->parentList not_eq this)
            continue; // orphaned/freed self
        SafeCallUpdateCB(nextCall, listNo, pos,
                         transList); // + SEH safety against the race
    }
}

void ObjectDisplayList::InsertionSortLink(DrawableObject **listhead,
                                          DrawableObject *listend)
{
    ...
    for (; walk not_eq listend; walk = save)
    {
        ...
        for (pnewlink = &newlist; *pnewlink not_eq listend and
                                  walk->distance <= (*pnewlink)->distance;
...
void ObjectDisplayList::QuickSortLink(DrawableObject **head,
                                      DrawableObject *end)
{
...
{
    ObjListLock
        _lk; // #41 CORRUPTION ROOT: this sort edits head/next/prev and walks sortCBlist
    ...
    // Now call anyone who has registered for a callback
    for (SortCallBack *nextCall = sortCBlist; nextCall;
         nextCall = nextCall->next)
    {
...
void ObjectDisplayList::PreLoad(class RenderOTW *renderer)
{
    ...
    while (nextToDraw)
    {
        ...
        if (F4IsBadReadPtr(nextToDraw, sizeof(DrawableObject)) or
            nextToDraw->parentList not_eq this)
        {
            nextToDraw = NULL;
            break;
        }
...
\*****************************************************************************/
void ObjectDisplayList::DrawBeyond(float ringDistance, int LOD,
                                   class RenderOTW *renderer)
{
    ...
    while (nextToDraw and
           not F4IsBadReadPtr(nextToDraw, sizeof(DrawableObject)) and
...
/*****************************************************************************\
 ...
 time.
\*****************************************************************************/
void ObjectDisplayList::InsertUpdateCallbacks(UpdateCallBack *up,
                                              SortCallBack *sort, void *self)
{
    ObjListLock
        _lk; // #41 UAF ROOT: edits to updateCBlist/sortCBlist ran WITHOUT the lock while
    ...
    // makes up->next point back -> the list becomes CYCLIC -> UpdateMetrics hangs. If present -> return.
    {
        ...
        for (UpdateCallBack *p = updateCBlist; p; p = p->next)
        {
            ...
            if (++g > 100000)
                break; // list is already cyclic -> don't make it worse
        }
    }
...
\*****************************************************************************/
void ObjectDisplayList::RemoveUpdateCallbacks(UpdateCallBack *up,
                                              SortCallBack *sort, void *self)
{
    ObjListLock
        _lk; // #41 UAF ROOT (see InsertUpdateCallbacks): unregistering a callback under the
