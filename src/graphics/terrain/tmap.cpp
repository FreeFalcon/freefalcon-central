...
int TMap::Setup(const char *mapPath)
{
    ...
    if ((!retval) or (bytesRead != sizeof(FeetPerPost)))
    {
        char string[80];
        char message[120];
        PutErrorString(string);
    ...
    if ((!retval) or (bytesRead != sizeof(FTtoMEAcell)))
    {
        char string[80];
        char message[120];
        PutErrorString(string);
    ...
    if ((!retval) or (bytesRead != sizeof(nLevels)))
    {
        char string[80];
        char message[120];
        PutErrorString(string);
    ...
    if ((!retval) or (bytesRead != sizeof(lastNearTexturedLOD)))
    {
        char string[80];
        char message[120];
        PutErrorString(string);
    ...
    if ((!retval) or (bytesRead != sizeof(lastFarTexturedLOD)))
    {
        char string[80];
        char message[120];
        PutErrorString(string);
    ...
    if (!retval  or bytesRead != sizeof(flags))
    {
        flags = 0;
        ResetLatLong();
    }
    ...
    if (!retval  or bytesRead != sizeof(maxtheateralt))
    {
...
void TMap::LoadColorTable(HANDLE inputFile)
{
    ...
    if ((!retval) or (bytesRead != sizeof(palette)))
    {
        char string[80];
        char message[120];
        PutErrorString(string);
...
void TMap::LoadMEAtable(const char *mapPath)
{
    ...
    for (row = MEAheight - 1; row >= 0; row--)
    {
        ...
        if ((!retval) or (bytesRead != MEAwidth * sizeof(*MEAarray)))
        {
            char string[80];
            char message[120];
            PutErrorString(string);
