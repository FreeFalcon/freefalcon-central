        ...
        if (LoadTarga16File(Token, &Image, &bmi))
        {
            if (ResX == -1 && ResY == -1)
            {
                ...
                for (i = 0; i < MyHeader.Height; i++)
                {
                    ...
                    WriteFile(ofp,
                              &Image[(MyHeader.Height - i - 1) *
                                     MyHeader.Width * MyHeader.BytesPerPixel],
                              MyHeader.Width * MyHeader.BytesPerPixel,
                              &bytesread, NULL);
                }
            }
            ...
            {
                if (ResX != bmi.bmiHeader.biWidth ||
                    ResY != bmi.bmiHeader.biHeight)
                {
                    printf(
                        "(%s) has a different resolution than the prev targa\n",
                        Token);
                ...
                else
                {
                    ...
                    for (i = 0; i < MyHeader.Height; i++)
                    {
                        ...
                        WriteFile(
                            ofp,
                            &Image[(MyHeader.Height - i - 1) * MyHeader.Width *
                                   MyHeader.BytesPerPixel],
                            MyHeader.Width * MyHeader.BytesPerPixel, &bytesread,
                            NULL);
                    }
