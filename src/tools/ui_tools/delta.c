...
long Compress16BitImage(WORD *Src, WORD *Dest, long StartX, long StartY,
                        long NewWidth, long NewHeight, long w, long h);
                   ...
                   LPSTR lpCmdLine, int nCmdShow)
{
    ...
    if (!ParseCommandLine(lpCmdLine))
    {
        ...
        printf("    <Input>          Input Animation file (Generated by "
               "MakeAnim.exe)\n");
        printf("    <Output>         Output file (Compressed Anim file\n");
        printf("  [OPTIONS]:\n");
        printf("    /x n             Specify new starting column (Default is "
               "0)\n");
        printf(
            "    /y n             Specify new starting Row (Default is 0)\n");
        printf("    /w n             Specify new width (Default is Image "
               "width)\n");
        printf("    /h n             Specify new height (Default is Image "
               "height)\n");
        printf("    /t n             Specify transparent color\n");
        printf("    /c n             Compression type (Default 1):\n");
        printf("                        1= Delta & RLE compress with previous "
               "frame\n");
        printf("                        2= Delta & RLE compress with previous "
               "and next frame\n");
        printf("                        3= RLE compress only NO Delta\n");
        printf("                        4= Delta & RLE compress with frame 0 "
               "(frame 0 not included in output\n");
        printf("    /f               Keep entire 1st frame (Default Don't "
               "Keep)\n");
        return (0);
    }
    ...
    OutputBuf =
        (char *)malloc(Size * 100 / 90); // Add 10 percent in size (for growing)
    ...
    for (i = 0; i < AnimData->Frames; i++)
    {
        ...
        case PREV_FRAME:
            ...
            memcpy(WorkImage,
                   &AnimData->Start[sizeof(long) + (sizeof(long) + Size) * i],
                   Size);

            if (!FirstFrame)
                DeltaImage(WorkImage,
                           &AnimData->Start[sizeof(long) +
                                            (sizeof(long) + Size) * PrevFrame],
                           AnimData->Width, AnimData->Height,
                           AnimData->BytesPerPixel);

            FirstFrame = 0;
            OutputSize = Compress16BitImage(
                (WORD *)WorkImage, (WORD *)OutputBuf, StartX, StartY, StartW,
                StartH, AnimData->Width, AnimData->Height);
        ...
        case PREV_NEXT_FRAME:
            ...
            DeltaImage(
                WorkImage,
                &AnimData
                     ->Start[sizeof(long) + (sizeof(long) + Size) * PrevFrame],
                AnimData->Width, AnimData->Height, AnimData->BytesPerPixel);
            // PrevFrame=i-1;
            // if(PrevFrame < 0)
            // PrevFrame=AnimData->Frames-1;
            // DeltaImage(WorkImage,
            ...
            DeltaImage(
                WorkImage,
                &AnimData
                     ->Start[sizeof(long) + (sizeof(long) + Size) * NextFrame],
                AnimData->Width, AnimData->Height, AnimData->BytesPerPixel);
            // NextFrame=i+2;
            // if(NextFrame >= AnimData->Frames)
            // NextFrame=0;
            ...
            for (k = 0; k < AnimData->Height; k++)
            {
                for (j = 0; j < AnimData->Width; j++)
                {
                    if ((AnimData->BytesPerPixel == 1) &&
                        (WorkImage[k * AnimData->Width *
                                       AnimData->BytesPerPixel +
                                   j * AnimData->BytesPerPixel] == TransColor))
                    {
                        WorkImage[k * AnimData->Width *
                                      AnimData->BytesPerPixel +
                                  j * AnimData->BytesPerPixel] = 0xff;
                    }
                    else if ((AnimData->BytesPerPixel == 2) &&
                             (WorkImage[k * AnimData->Width *
                                            AnimData->BytesPerPixel +
                                        j * AnimData->BytesPerPixel] ==
                              (TransColor % 256)) &&
                             (WorkImage[k * AnimData->Width *
                                            AnimData->BytesPerPixel +
                                        j * AnimData->BytesPerPixel + 1] ==
                              (TransColor / 256)))
                    {
                        WorkImage[k * AnimData->Width *
                                      AnimData->BytesPerPixel +
                        ...
                        WorkImage[k * AnimData->Width *
                                      AnimData->BytesPerPixel +
                                  j * AnimData->BytesPerPixel + 1] = 0xff;
                    }
                }
            }
        ...
        case SPECIAL_FRAME:
            if (i)
            {
                ...
                DeltaImage(
                    ...
                    AnimData->Width, AnimData->Height, AnimData->BytesPerPixel);
                OutputSize = Compress16BitImage(
                    (WORD *)WorkImage, (WORD *)OutputBuf, StartX, StartY,
                    StartW, StartH, AnimData->Width, AnimData->Height);
                WriteFile(ofp, &OutputSize, sizeof(long), &bytesread, NULL);
                WriteFile(ofp, OutputBuf, OutputSize, &bytesread, NULL);
            }
...
void DeltaImage(char *Image, char *Cmp1, long w, long h, long BytesPerPixel)
{
    ...
    for (i = 0; i < h; i++)
    {
        for (j = 0; j < w; j++)
        {
            ...
            if (BytesPerPixel == 1 &&
                Image[i * w * BytesPerPixel + j * BytesPerPixel] == TransColor)
            {
            }
            else if (BytesPerPixel == 2 &&
                     Image[i * w * BytesPerPixel + j * BytesPerPixel + 1] ==
                         TransColor % 256 &&
                     Image[i * w * BytesPerPixel + j * BytesPerPixel] ==
                         TransColor / 256)
            {
            }
            else if (Cmp1)
            {
                for (k = 0; k < BytesPerPixel; k++)
                    if (Image[i * w * BytesPerPixel + j * BytesPerPixel + k] !=
                        Cmp1[i * w * BytesPerPixel + j * BytesPerPixel + k])
                    {
    ...
    for (i = 0; i < height; i++)
    {
        ...
        if (!j)
        {
            ...
            while (start < NewWidth)
            {
                ...
                while (Src[(i + StartY) * w + start + j + StartX] ==
                           RLE_NO_DATA &&
                       (start + j) < NewWidth)
                    j++;
                ...
                if (start < NewWidth)
                {
                    ...
                    while (Src[(i + StartY) * w + start + StartX + width] !=
                               RLE_NO_DATA &&
                    ...
                    if (width)
                    {
                        ...
                        while (j < width)
                        {
                            if (Src[(i + StartY) * w + start + j + StartX] ==
                                cur)
                                runs++;
                            else
                            {
