...
static BOOL InitTOOLHELP32(void)
{
    ...
    if (NULL not_eq hKernel)
    {
        ...
        bRet = g_pModule32First and g_pModule32Next and g_pProcess32First and
               g_pProcess32Next and g_pThread32First and g_pThread32Next and
               g_pCreateToolhelp32Snapshot;
    }
    else
    {
...
----------------------------------------------------------------------*/
BOOL TLHELPGetLoadedModules(DWORD dwPID, UINT uiCount, HMODULE *paModArray,
                            LPUINT puiRealCount)
{
    ...
    if (TRUE == g_pModule32First(hModSnap, &stME32))
    {
        do
        {
            ...
            if (0 not_eq uiCount)
            {
                ...
                if ((TRUE == bBuffToSmall) or (*puiRealCount == uiCount))
                {
                    bBuffToSmall = TRUE;
                    break;
                }
