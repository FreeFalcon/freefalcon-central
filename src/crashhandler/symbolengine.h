...
#ifdef SYMENG_NEEDDECLS
BOOL __stdcall SymGetLineFromAddr(IN HANDLE hProcess, IN DWORD dwAddr,
                                  OUT PDWORD pdwDisplacement,
                                  OUT PIMAGEHLP_LINE Line);

BOOL __stdcall SymGetLineFromName(IN HANDLE hProcess, IN LPSTR ModuleName,
                                  IN LPSTR FileName, IN DWORD dwLineNumber,
                                  OUT PLONG plDisplacement,
                                  IN OUT PIMAGEHLP_LINE Line);

BOOL __stdcall SymGetLineNext(IN HANDLE hProcess, IN OUT PIMAGEHLP_LINE Line);

BOOL __stdcall SymGetLinePrev(IN HANDLE hProcess, IN OUT PIMAGEHLP_LINE Line);

BOOL __stdcall SymMatchFileName(IN LPSTR FileName, IN LPSTR Match,
...
typedef BOOL(__stdcall *PFNSYMGETLINEFROMADDR)(IN HANDLE hProcess,
                                               ...
                                               OUT PDWORD pdwDisplacement,
                                               OUT PIMAGEHLP_LINE Line);
typedef BOOL(__stdcall *PFNSYMGETLINEFROMNAME)(IN HANDLE hProcess,
                                               IN LPSTR ModuleName,
                                               IN LPSTR FileName,
                                               IN DWORD dwLineNumber,
                                               OUT PLONG plDisplacement,
                                               IN OUT PIMAGEHLP_LINE Line);
typedef BOOL(__stdcall *PFNSYMGETLINENEXT)(IN HANDLE hProcess,
                                           IN OUT PIMAGEHLP_LINE Line);
typedef BOOL(__stdcall *PFNSYMGETLINEPREV)(IN HANDLE hProcess,
                                           IN OUT PIMAGEHLP_LINE Line);
typedef BOOL(__stdcall *PFNSYMMATCHFILENAME)(IN LPSTR FileName, IN LPSTR Match,
...
public:
    ...
    CSymbolEngine(void)
    {
        ...
        m_pfnSymGetLineFromAddr = (PFNSYMGETLINEFROMADDR)GetProcAddress(
            hInstImageHlp, "SymGetLineFromAddr");
        ...
        {
            m_pfnSymGetLineFromName = (PFNSYMGETLINEFROMNAME)GetProcAddress(
                hInstImageHlp, "SymGetLineFromName");
            m_pfnSymGetLineNext = (PFNSYMGETLINENEXT)GetProcAddress(
                hInstImageHlp, "SymGetLineNext");
            m_pfnSymGetLinePrev = (PFNSYMGETLINEPREV)GetProcAddress(
                hInstImageHlp, "SymGetLinePrev");
            m_pfnSymMatchFileName = (PFNSYMMATCHFILENAME)GetProcAddress(
                hInstImageHlp, "SymMatchFileName");
        }
...
public:
    ...
    BOOL GetInMemoryFileVersion(LPCTSTR szFile, DWORD bitand dwMS,
                                DWORD bitand dwLS)
    {
...
public:
    ...
    {
        return (::SymEnumerateModules(m_hProcess, EnumModulesCallback,
                                      UserContext));
    }
    ...
    {
        return (::SymLoadModule(m_hProcess, hFile, ImageName, ModuleName,
                                BaseOfDll, SizeOfDll));
    }
...
public:
    BOOL
    SymEnumerateSymbols(IN DWORD BaseOfDll,
                        IN PSYM_ENUMERATESYMBOLS_CALLBACK EnumSymbolsCallback,
                        IN PVOID UserContext)
    {
        return (::SymEnumSymbols(m_hProcess, BaseOfDll, "*",
                           ...
                           OUT PIMAGEHLP_SYMBOL Symbol)
    {
        ...
        (void)dwAddr;
        (void)pdwDisplacement;
        (void)Symbol;
        return FALSE;
#else
        return (
            ::SymGetSymFromAddr(m_hProcess, dwAddr, pdwDisplacement, Symbol));
#endif
    }
...
public:
                            ...
                            OUT PIMAGEHLP_LINE Line)
    {
        ...
        while (FALSE ==
               pfnSGLFA(m_hProcess, dwAddr - dwTempDis, pdwDisplacement, Line))
        {
            dwTempDis += 1;
                            ...
                            IN OUT PIMAGEHLP_LINE Line)
    {
        ...
        return (m_pfnSymGetLineFromName(m_hProcess, ModuleName, FileName,
                                        dwLineNumber, plDisplacement, Line));
#else
        return (::SymGetLineFromName(m_hProcess, ModuleName, FileName,
                                     dwLineNumber, plDisplacement, Line));
#endif // SYMENG_EXTRAWORK
    }
                          ...
                          OUT LPSTR *FileNameStop, OUT LPSTR *MatchStop)
    {
        ...
        return (
            m_pfnSymMatchFileName(FileName, Match, FileNameStop, MatchStop));
#else
...
#else
        return (
            ::SymRegisterCallback(m_hProcess, CallbackFunction, UserContext));
#endif
    }
