...
DiagAssert(DWORD dwOverrideOpts, LPCTSTR szMsg, LPCSTR szFile, DWORD dwLine)
{
    ...
    for (loop = g_HMODVector.begin(); loop not_eq g_HMODVector.end(); loop++)
    {
        if (0 not_eq FormatMessage(FORMAT_MESSAGE_ALLOCATE_BUFFER |
                                       FORMAT_MESSAGE_IGNORE_INSERTS |
                                       FORMAT_MESSAGE_FROM_HMODULE,
                                   *loop, dwLastErr, 0, (LPTSTR)&szFmtMsg, 0,
                                   NULL))
        {
            break;
        }
    ...
    if (NULL == szFmtMsg)
    {
        FormatMessage(FORMAT_MESSAGE_ALLOCATE_BUFFER |
                      ...
                      NULL, dwLastErr, 0, (LPTSTR)&szFmtMsg, 0, NULL);
    }
    ...
    pCurrPos +=
        (wsprintf(szBuff,
                  _T("Debug Assertion Failed\n\n"
                     ...
                     "Expression : %s\n"
                     "Last Error (0x%08X) : %s\n\n"),
                  szModName, szFile, dwLine, szMsg, dwLastErr, szRealLastErr) *
         sizeof(TCHAR));
    ...
    if (DA_SHOWSTACKTRACE == (DA_SHOWSTACKTRACE bitand dwOpts))
    {
        DoStackTrace(pCurrPos,
                     sizeof(szBuff) - ((DWORD)pCurrPos - (DWORD)szBuff), 2);
    }
    ...
    if (DA_SHOWMSGBOX == (DA_SHOWMSGBOX bitand dwOpts))
    {
        ...
        iRet = MessageBox(hWndParent, szBuff, _T("ASSERTION FAILURE..."),
...
static DWORD __stdcall GetModBase(HANDLE hProcess, DWORD dwAddr)
{
    ...
    else
    {
        ...
        if (0 not_eq
            VirtualQueryEx(hProcess, (LPCVOID)dwAddr, &stMBI, sizeof(stMBI)))
        {
            // Try and load it.
            DWORD dwNameLen = 0;
            TCHAR szFile[MAX_PATH];

            dwNameLen = GetModuleFileName((HINSTANCE)stMBI.AllocationBase,
                                          szFile, MAX_PATH);
...
static DWORD ConvertAddress(DWORD dwAddr, LPTSTR szOutBuff)
{
    ...
    if (0 not_eq g_cSym.SymGetSymFromAddr(dwAddr, &dwDisp, pIHS))
    {
        ...
        if (0 not_eq g_cSym.SymGetLineFromAddr(dwAddr, &dwDisp, &stIHL))
        {
            // Put this on the next line and indented a bit.
            pCurrPos += wsprintf(pCurrPos, _T("\n\t\t%s, Line %d"),
                                 stIHL.FileName, stIHL.LineNumber);
...
#error ( "Unknown machine" )
        ...
        {
            if (FALSE == StackWalk(dwMachine, hProcess, hProcess, &stFrame,
                                   &stCtx, NULL, SymFunctionTableAccess,
                                   GetModBase, NULL))
            {
                break;
            }
        ...
        for (loop = vAddrs.begin(); loop not_eq vAddrs.end(); loop++)
        {
