...
extern char SoundTheaterFolder[_MAX_PATH];
...
int InitSoundManager(HWND hWnd, int, char *falconDataDir)
{
    ...
    ShiAssert(FALSE == IsBadStringPtr(SoundTheaterFolder, _MAX_PATH));
    ...
    sprintf(sfxtable, "%s\\%s", SoundTheaterFolder, FALCONSNDTABLETXT);
...
void LoadSFX(char *falconDataDir)
{
    ...
    //sprintf( fname, "%s\\%s", SoundTheaterFolder, "SoundError.log" );
    ...
    //fp=fopen("SoundError.log","w");
    {
        ...
        for (i = 0; i < NumSFX; i++) // first pass - most important buffers
        {
            ...
            sprintf(fname, "%s\\%s", SoundTheaterFolder, SFX_DEF[i].fileName);
