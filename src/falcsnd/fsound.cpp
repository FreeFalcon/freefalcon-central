...
extern char FalconSoundTheaterFolder[_MAX_PATH];
...
int InitSoundManager(HWND hWnd, int, char *falconDataDir)
{
    ...
    ShiAssert(FALSE == IsBadStringPtr(FalconSoundTheaterFolder, _MAX_PATH));
    ...
    sprintf(sfxtable, "%s\\%s", FalconSoundTheaterFolder, FALCONSNDTABLETXT);
...
void LoadSFX(char *falconDataDir)
{
    ...
    //sprintf( fname, "%s\\%s", FalconSoundTheaterFolder, "SoundError.log" );
    ...
    //fp=fopen("SoundError.log","w");
    {
        ...
        for (i = 0; i < NumSFX; i++) // first pass - most important buffers
        {
            ...
            sprintf(fname, "%s\\%s", FalconSoundTheaterFolder, SFX_DEF[i].fileName);
