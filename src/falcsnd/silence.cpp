...
#define IS_SILENCE(c)                                                          \
    ((c) > (0x7f - SILENCE_LIMIT) and (c) < (0x80 + SILENCE_LIMIT))
...
int expand[32] = {
    5,   15,  25,  35,  45,  54,  62,  70,  78,  86,  93,
    100, 106, 112, 118, 124, 131, 137, 143, 149, 155, 162,
    169, 177, 185, 193, 201, 210, 220, 230, 240, 250,
};
...
void CompressFile(FILE *input, BIT_FILE *output, int, char *[])
{
    ...
    for (k = steps; k > 0; k--)
    {
        value = (int)(128.0 * (pow(2.0, (double)k / steps) - 1.0) + 0.5);
        ...
        if (silence_run(look_ahead, index))
        {
            ...
            do
            {
                ...
                if (++run_length == 255)
                {
                    for (i = 0; i < 6; i++)
                    {
                        OutputBits(output, (unsigned long)silence_match[i],
                                   bits);
                    }
...
*/
long ExpandCompressionBuffer(long bytesToRead, compression_buf_t *compbuf,
                             unsigned char **buffer)
{
    unsigned char *ptr;
    int bits = COMPRESSION_BITS, c;
    long count;
    long bytesLeft;
    long byteMax;
    //Update so Silence and Compand do not conflict
...
*/
long ExpandCompSilenceBuff(long bytesRead, compression_buf_t *compBuf,
                           VOICE_STREAM_BUFFER *voice)
{
    unsigned char *ptr;
    int bits, c;
    long count;
