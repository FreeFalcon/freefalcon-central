...
typedef struct CodecInfo_tag
{
    ...
    *LPCODECINFO;   // Artscout - 2026 (Linux port): drop legacy near/far
...
} CODECINFOEX, *PCODECINFOEX,
    *LPCODECINFOEX;   // Artscout - 2026 (Linux port): drop legacy near/far
...
LH_PREFIX HANDLE LH_SUFFIX ST80_Open_Coder(DWORD dwFlags);

LH_PREFIX LH_ERRCODE LH_SUFFIX ST80_Encode(HANDLE hAccess,
                                           LPBYTE inputBufferPtr,
                                           LPWORD inputBufferLength,
                                           LPBYTE outputBufferPtr,
                                           LPWORD outputBufferLength);

LH_PREFIX LH_ERRCODE LH_SUFFIX ST80_Close_Coder(HANDLE hAccess);

LH_PREFIX HANDLE LH_SUFFIX ST80_Open_Decoder(DWORD dwFlags);

LH_PREFIX LH_ERRCODE LH_SUFFIX ST80_Decode(HANDLE hAccess,
                                           LPBYTE inputBufferPtr,
                                           LPWORD inputBufferLength,
                                           LPBYTE outputBufferPtr,
                                           LPWORD outputBufferLength);

LH_PREFIX LH_ERRCODE LH_SUFFIX ST80_Close_Decoder(HANDLE hAccess);

LH_PREFIX void LH_SUFFIX ST80_GetCodecInfo(LPCODECINFO lpCodecInfo);

LH_PREFIX void LH_SUFFIX ST80_GetCodecInfoEx(LPCODECINFOEX lpCodecInfoEx,
                                             DWORD cbSize);
