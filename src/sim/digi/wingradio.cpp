...
void AiMakeRadioCall(SimBaseClass* p_sender, int command, int extent,
                     VU_ID targetid)
{
    ...
    {
        ShiAssert(p_radioMsgs->dataBlock.message >= 0 and
                  p_radioMsgs->dataBlock.message < LastComm);
                       ...
                       FalconRadioChatterMessage** pp_radioMsgs)
{
    ...
    *pp_radioMsgs =
        new FalconRadioChatterMessage(p_sender->GetCampaignObject()->Id(),
                                      FalconLocalGame); // Create new message


    (*pp_radioMsgs)->dataBlock.voice_id =
        ((FlightClass*)p_sender->GetCampaignObject())
            ->GetPilotVoiceID(
                p_sender->GetCampaignObject()->GetComponentIndex(p_sender));
    ...
    {
        (*pp_radioMsgs)->dataBlock.to =
            MESSAGE_FOR_FLIGHT; // Digi's send message to the flight
    }
}
...
void AiFillCallsign(SimBaseClass* p_sender, int extent,
                    FalconRadioChatterMessage** pp_radioMsgs, BOOL fillCallName)
{
    ...
    if (flightIdx == AiFlightLead)
    {
        if (extent == AiWingman)
        {
            if (fillCallName)
            {
                (*pp_radioMsgs)->dataBlock.edata[0] =
                    ((FlightClass*)p_sender->GetCampaignObject())->callsign_id;
            }
            else
            {
            ...
            (*pp_radioMsgs)->dataBlock.edata[1] =
                4 * (((FlightClass*)p_sender->GetCampaignObject())
                         ->callsign_num -
                     1) +
                2;
        }
        ...
        else
        {
            (*pp_radioMsgs)->dataBlock.edata[0] =
                ((FlightClass*)p_sender->GetCampaignObject())->callsign_id;
            (*pp_radioMsgs)->dataBlock.edata[1] =
                ((FlightClass*)p_sender->GetCampaignObject())->callsign_num +
                CALLSIGN_NUM_OFFSET;
        }
    }
    else if (flightIdx == AiElementLead)
    {
        ...
        (*pp_radioMsgs)->dataBlock.edata[1] =
            4 * (((FlightClass*)p_sender->GetCampaignObject())->callsign_num -
                 1) +
            4;
    }
}
                         ...
                         int numRadioRecipients)
{
    ...
    if (numRadioRecipients)
    {
        ...
        for (i = 0; i < numRadioRecipients;
             i++) // Send the commmand to all recipients.
        {
...
void AiRespondShortCallSign(AircraftClass* p_aircraft)
{
    ...
    edata[0] = ((FlightClass*)p_aircraft->GetCampaignObject())
                   ->GetComponentIndex(p_aircraft);
    edata[1] = -1;
    edata[2] = -1;
    edata[3] = -1;
    edata[4] = -1;
                         ...
                         VU_ID targetid)
{
    ...
    case FalconWingmanMsg::WMGiveBra:
        ...
        (*pp_radioMsgs)->dataBlock.edata[2] =
            ((FlightClass*)p_sender->GetCampaignObject())->callsign_id;
        (*pp_radioMsgs)->dataBlock.edata[3] =
            (((FlightClass*)p_sender->GetCampaignObject())->callsign_num - 1) *
                4 +
            flightIdx + 1;
        break;
    ...
    case FalconWingmanMsg::WMSSOffset:
        ...
        AiFillCallsign(p_sender, extent, pp_radioMsgs, TRUE);
        (*pp_radioMsgs)->dataBlock.edata[2] =
            4; // was together with Pince, now correct eval setup
        break;
    ...
    case FalconWingmanMsg::WMRaygun:
        theRadar = (RadarClass*)FindSensor((SimMoverClass*)p_sender,
                                           SensorClass::Radar);
        ...
        if (theRadar->TargetUnderCursor() not_eq FalconNullId)
        {
            ...
            (*pp_radioMsgs)->dataBlock.edata[4] =
                -1 * FloatToInt32(theTarget->ZPos());
        }
        else
        {
            (*pp_radioMsgs)->dataBlock.edata[2] =
                SimToGrid(p_sender->YPos() + yPos * NM_TO_FT);
            (*pp_radioMsgs)->dataBlock.edata[3] =
                SimToGrid(p_sender->XPos() + xPos * NM_TO_FT);
            (*pp_radioMsgs)->dataBlock.edata[4] =
                -1 * FloatToInt32(p_sender->ZPos());
        }
    ...
    case FalconWingmanMsg::WMAssignGroup:
        ...
        if (pentity->GetDomain() == DOMAIN_LAND or
            pentity->GetDomain() == DOMAIN_SEA)
        {
        ...
        else if (pentity->IsFlight() or pentity->IsAirplane() or
                 pentity->IsHelicopter() or pentity->IsSquadron() or
                 pentity->IsPackage())
        {
            ...
            if (pentity->IsFlight() or pentity->IsSquadron() or
                pentity->IsPackage())
            {
                (*pp_radioMsgs)->dataBlock.edata[4] =
                    ((Unit)pentity)->GetUnitClassData()->VehicleType[0] * 2 + 1;
            ...
            {
                (*pp_radioMsgs)->dataBlock.edata[4] =
                    2 * (pentity->Type() - VU_LAST_ENTITY_TYPE);
                (*pp_radioMsgs)->dataBlock.edata[5] = -1;
            ...
            (*pp_radioMsgs)->dataBlock.edata[8] =
                -1 * FloatToInt32(pentity->ZPos());
        }
    ...
    case FalconWingmanMsg::WMJokerFuel:
        (*pp_radioMsgs)->dataBlock.edata[0] =
            ((AircraftClass*)p_sender)->vehicleInUnit;
    ...
    case FalconWingmanMsg::WMFumes:
        (*pp_radioMsgs)->dataBlock.edata[0] =
            ((AircraftClass*)p_sender)->vehicleInUnit;
        (*pp_radioMsgs)->dataBlock.edata[1] =
            (FloatToInt32(((AircraftClass*)p_sender)->af->Fuel() +
                          ((AircraftClass*)p_sender)->af->ExternalFuel()));
