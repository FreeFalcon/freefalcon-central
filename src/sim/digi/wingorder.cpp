...
char* gpAiExtentStr[AiTotalExtent] = {
    "AiWingman", "AiElement",      "AiFlight",  "AiPackage",
    "AiLeader",  "AiAllButSender", "AiNoExtent"};
...
void AiSendPlayerCommand(int command, int extent, VU_ID targetId)
{
    ...
    case FalconWingmanMsg::WMAssignGroup:
        ...
        case AiWingman:
            ...
            {
                myBrain->targetSpotWing =
                    new SpotEntity(F4FlyingEyeType + VU_LAST_ENTITY_TYPE);
                vuDatabase->/*Quick*/ Insert(myBrain->targetSpotWing);
                myBrain->targetSpotWing->SetDriver(
                    new SpotDriver(myBrain->targetSpotWing));
            }
            else
            {
            ...
            myBrain->targetSpotWingTarget =
                (FalconEntity*)vuDatabase->Find(targetId);
            myBrain->targetSpotWingTimer =
                SimLibElapsedTime + g_nTargetSpotTimeout;
        ...
        case AiElement:
            ...
            {
                myBrain->targetSpotElement =
                    new SpotEntity(F4FlyingEyeType + VU_LAST_ENTITY_TYPE);
                vuDatabase->/*Quick*/ Insert(myBrain->targetSpotElement);
                myBrain->targetSpotElement->SetDriver(
                    new SpotDriver(myBrain->targetSpotElement));
            }
            else
            {
                // Kill the current camera
            ...
            // Now init for our new target
            myBrain->targetSpotElementTarget =
                (FalconEntity*)vuDatabase->Find(targetId);
            myBrain->targetSpotElementTimer =
                SimLibElapsedTime + g_nTargetSpotTimeout;
        ...
        case AiFlight:
            ...
            {
                myBrain->targetSpotFlight =
                    new SpotEntity(F4FlyingEyeType + VU_LAST_ENTITY_TYPE);
                vuDatabase->/*Quick*/ Insert(myBrain->targetSpotFlight);
                myBrain->targetSpotFlight->SetDriver(
                    new SpotDriver(myBrain->targetSpotFlight));
            }
            else
            {
                // Kill the current camera
            ...
            // Now init for our new target
            myBrain->targetSpotFlightTarget =
                (FalconEntity*)vuDatabase->Find(targetId);
            myBrain->targetSpotFlightTimer =
                SimLibElapsedTime + g_nTargetSpotTimeout;
            ...
            if (myBrain->targetSpotWing)
            {
                ...
                if (myBrain
                        ->targetSpotWingTarget) // 2002-03-07 ADDED BY S.G. In case it's NULL. Shouldn't happen but happened
    ...
    case FalconWingmanMsg::WMClearSix:
        targetId =
            AiCheckForThreat(SimDriver.GetPlayerAircraft(), DOMAIN_AIR, 1);
        break;
    ...
    case FalconWingmanMsg::WMWeaponsFree:
        ...
        case AiWingman:
            ...
            if (myBrain->isWing == AiFlightLead)
                wingPlane = ((AircraftClass*)SimDriver.GetPlayerEntity()
                                 ->GetCampaignObject()
                                 ->GetComponentNumber(1));
            else
                wingPlane = ((AircraftClass*)SimDriver.GetPlayerEntity()
                                 ->GetCampaignObject()
                                 ->GetComponentNumber(3));
        ...
        case AiElement:
            ...
            if (wingTgt)
            {
                ...
                if (i == FalconLocalSession->CameraCount())
                {
                    FalconLocalSession->AttachCamera(
                        myBrain->targetSpotElement);
                }
            }
        ...
        case AiFlight:
            ...
            for (i = 0; not wingTgt and i < SimDriver.GetPlayerEntity()
                                                ->GetCampaignObject()
                                                ->NumberOfComponents();
                 i++)
            {
    ...
    case FalconWingmanMsg::WMRejoin:
        ...
        case AiWingman:
            if (myBrain->targetSpotWing)
                myBrain->targetSpotWingTimer =
                    SimLibElapsedTime + 15 * SEC_TO_MSEC;
    ...
    AiSendCommand((SimBaseClass*)SimDriver.GetPlayerEntity(), command, extent,
                  targetId);
}
                      ...
                      VU_ID targetid)
{
    ...
    if (g_bMPFix2)
        p_commandMsgs =
            new FalconWingmanMsg(p_sender->GetCampaignObject()->Id(),
                                 (VuTargetEntity*)vuDatabase->Find(
                                     vuLocalSessionEntity->Game()->OwnerId()));
    else
        p_commandMsgs = new FalconWingmanMsg(
            p_sender->GetCampaignObject()->Id(), FalconLocalGame);
                       ...
                       float* az)
{
    ...
    while (pobjectPtr)
    {
        ...
        {
            inSideATA =
                TRUE; // if something is behind us and we are looking there
        }
        else if (position == 0 and pobjectPtr->localData->ata <= 90.0F * DTR)
        {
            inSideATA =
                TRUE; // if something is ahead of us and we are looking ther
        }
        ...
        if ((vuDomain == DOMAIN_AIR and vuClass == CLASS_VEHICLE and
             (vuType == TYPE_AIRPLANE) and
             (vuSType == STYPE_AIR_FIGHTER_BOMBER or
              vuSType ==
                  STYPE_AIR_FIGHTER)) // Removed Helicopters as threat (or vuType == TYPE_HELICOPTER), added subtype check //JB 052701 (from MN)
            and inSideATA and pobjectPtr->localData->threatTime <= 60.0F)
        {
            if (pthreatPtr == NULL or pobjectPtr->localData->threatTime <
                                          pthreatPtr->localData->threatTime)
            {
                if (GetTTRelations(pobjectPtr->BaseData()->GetTeam(),
                                   paircraft->GetTeam()) >= Hostile)
                {
