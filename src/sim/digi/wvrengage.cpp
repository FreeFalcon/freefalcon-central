...
void DigitalBrain::WvrEngageCheck(void)
{
    ...
    /* return if no target */
    /*---------------------*/
    if (targetPtr == NULL or
        (mpActionFlags[AI_ENGAGE_TARGET] not_eq AI_AIR_TARGET and
    ...
    /*-------*/
        ...
        curMode not_eq WVREngageMode and targetData->range < engageRange and
        (targetPtr->BaseData()->IsAirplane() or
         targetPtr->BaseData()->IsFlight() or
         targetPtr->BaseData()
             ->IsHelicopter()) and // 2002-03-05 MODIFIED BY S.G. airplane, choppers and fligth are ok in here (choppers only makes it here if it passed the SensorFusion test first)
        SimLibElapsedTime > engagementTimer and
        CanEngage(
            self, self->CombatClass(), targetPtr,
            WVRManeuver)) // 2002-03-11 MODIFIED BY S.G. Added parameter WVRManeuver
    {
    ...
    if (SimLibElapsedTime > wvrTacticTimer or wvrCurrTactic == WVR_NONE)
    {
            ...
            targetPtr->BaseData()->IsHelicopter())
        {
            ...
            if (numChoices)
            {
                myChoice =
                    rand() %
                    numChoices; // changed back WvrMergeUnlimited;// me123 status test. lets do smart merge for now.
        ...
        case WVR_RANDP:
            ...
            wvrTacticTimer =
                SimLibElapsedTime + 1000; //me123 status test chenged from 10
            break;
        ...
        case WVR_GUNJINK:
            ...
            wvrTacticTimer =
                SimLibElapsedTime + 500; //me123 status test chenged from 6
            break;
    ...
    if (targetPtr)
    {
                ...
                ->IsHelicopter()) // 2002-03-05 MODIFIED BY S.G. airplane, choppers and fligth are ok in here (choppers only makes it here if it passed the SensorFusion test first)
        {
                ...
                maxAAWpnRange == 0 or aceAvoid)
            {
                ...
                if (targetData->range > 1.5F * NM_TO_FT or
                    targetData->ataFrom > 25.0F * DTR)
            ...
            else
            {
                ...
                if (targetData->range > 25.0F * NM_TO_FT)
                    ...
                    wvrCurrTactic =
                        WVR_BEAM; //me123 status test chenged from WVR_BEAM_RETURN
                else if (
                    targetData->range > 10.0F * NM_TO_FT and
                    wvrCurrTactic not_eq
                        WVR_BEAM_RETURN) //me123 status test chenged from >3
                {
        ...
        if (targetData->ata <= 90.0F * DTR and
        ...
        else if (targetData->ata > 90.0F * DTR)
        {
            if (self->GetKias() <
                cornerSpeed *
                    0.6f) //me123 status test. accelerate if we are below 210cas changed from 0.3
            {
...
void DigitalBrain::WvrRollOutOfPlane(void)
{
    ...
    if (wvrPrevTactic not_eq WVR_ROOP)
    {
        ...
        {
            newroll =
                self->Roll() - 45.0F * DTR; //me123 from 45. smaller repossition
        }
        else
        {
            newroll =
...
void DigitalBrain::SetThreat(FalconEntity* obj)
{
    ...
    if (obj and not obj->OnGround())
    {
        ...
        else if (randNum < 40)
        {
            ...
            if (threatPtr->localData and
                threatPtr->localData->range > 2.0F * NM_TO_FT)
            {
...
#if 1
int CanEngage(AircraftClass* self, int combatClass, SimObjectType* targetPtr,
              int type)
{
    int hisCombatClass;
        ...
        targetPtr->BaseData()->IsHelicopter())
    {
        ...
        if (type bitand DigitalBrain::WVRManeuver)
        {
            ...
            else if (theIntercept->numMerges == 1)
            {
                ...
                if (theIntercept->merge[0] ==
                        DigitalBrain::WvrMergeHitAndRun and
                    targetPtr->localData->ata > 45.0F * DTR)
                {
                    retWvr = FALSE;
                }
                // Can't be behind you for limited
                else if (theIntercept->merge[0] ==
                             DigitalBrain::WvrMergeLimited and
