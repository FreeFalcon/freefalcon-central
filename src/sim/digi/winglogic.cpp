    ...
    else
    {
        if (mDesignatedObject not_eq FalconNullId)
        {
            ...
            if (pnewTarget)
            {
                ...
                if (((FalconEntity *)pnewTarget)->IsCampaign() and ((CampBaseClass *)pnewTarget)->IsUnit() and !((CampBaseClass *)pnewTarget)->IsAggregate() and ((CampBaseClass *)pnewTarget)->NumberOfComponents() == 0)
                {
...
void DigitalBrain::AiSearchTargetList(VuEntity* pentity)
{
    ...
    if (((FalconEntity*)pentity)->IsSim())
    {
        ...
        if (!theTarget->OnGround() and (theTarget->IsDead() or theTarget->IsEject()))
            theTarget = NULL;
    ...
    if (theTarget)
    {
        ...
        if ((!targetPtr or targetPtr->BaseData() not_eq theTarget) and (!groundTargetPtr or groundTargetPtr->BaseData() not_eq theTarget))
        {
#ifdef DEBUG
...
SimBaseClass *DigitalBrain::FindSimGroundTarget(CampBaseClass *targetGroup, int targetNumComponents, int startPos)
{
    ...
    for (i = 0; i < usComponents; i++)
    {
        ...
        if (flightMember[i] and (flightMember[i]->IsDigital() or flightMember[i]->IsLocal()))
        {
            ...
            else
            {
                ...
                for (int j = 0; !haveHARMS and !(otherHaveHARMS and i > isWing) and flightMember[i]->Sms and j < flightMember[i]->Sms->NumHardpoints(); j++)
                {
    ...
    if (!haveHARMS and !hasAGMissile and targetNumComponents)
    {
        ...
        if (startPos == 0)
        {
            ...
            for (i = startPos; i < targetNumComponents and startPos > 0; i++)
                if (!targetGroup->GetComponentEntity(i)->IsVehicle() or !targetGroup->GetComponentEntity(i)->OnGround())
                    startPos = 0;
        }
    ...
    for (i = startPos; i < targetNumComponents; i = startPos ? 0 : i + 1, startPos = 0)
    {
        ...
        if (!simTarg or F4IsBadReadPtr(simTarg, sizeof(simTarg)))  //sanity check
            continue;
        ...
        if (simTarg->IsStatic()) // It's a feature
        {
            ...
            if (fc and !F4IsBadReadPtr(fc, sizeof(fc)) and fc->Priority > 2)  // higher priority number = lower priority
                continue;
        ...
        // FRB - So?!!
        //if (targetGroup->IsObjective() and !hasHARM) break;
            ...
            (simTarg->IsStatic() and ((SimStaticClass *)simTarg)->GetRadarType() not_eq RDR_NO_RADAR))  // It's a feature and it has a radar
        {
            ...
            if ((((FlightClass *)self->GetCampaignObject())->shotAt == targetGroup and ((FlightClass *)self->GetCampaignObject())->whoShot == self) or ((((FlightClass *)self->GetCampaignObject())->whoShot == NULL) and (haveHARMS or !otherHaveHARMS)))
            {
                gotRadar = TRUE;
    ...
    if (startPos < targetNumComponents)
    {
        ...
        if (!gotRadar and (unsigned)((unsigned)rand() % (unsigned)32) > (unsigned)((unsigned)SkillLevel() + (unsigned)28))
...
SimBaseClass *DigitalBrain::FindSimAirTarget(CampBaseClass *targetGroup, int targetNumComponents, int startPos)
{
    ...
    for (i = 0; i < usComponents; i++)
    {
        ...
        if (flightMember[i] and (!flightMember[i]->IsDigital() and !flightMember[i]->IsLocal()))
        {
            flightMember[i] = NULL;
        }
    }
...
void DigitalBrain::AiCheckEngage(void)
{
    ...
    // if(mpActionFlags[AI_ENGAGE_TARGET] /* REMOVED BY S.G. and !mpActionFlags[AI_EXECUTE_MANEUVER] */) {
...
void DigitalBrain::AiCheckLandTakeoff(void)
{
    ...
    else if (self->curWaypoint->GetWPAction() == WP_LAND and !self->OnGround() and distAirbase < 30.0F * NM_TO_FT
...
void DigitalBrain::AiCheckFormation(void)
{
    ...
    if (mpActionFlags[AI_ENGAGE_TARGET] and !mpActionFlags[AI_EXECUTE_MANEUVER] and (agDoctrine not_eq AGD_NONE or groundTargetPtr))
    {
        AddMode(WaypointMode);
    }
...
void DigitalBrain::AiCheckForUnauthLand(VU_ID lead)
{
    ...
    if (!self->OnGround() and leader and mpActionFlags[AI_FOLLOW_FORMATION] == TRUE and leader->DBrain()->ATCStatus() < tReqTaxi and leader->OnGround() and atcstatus == noATC)
    {
