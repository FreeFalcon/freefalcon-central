...
void DigitalBrain::ResetTaxiState(void)
{
    ...
    if ( not self->OnGround() or !Airbase)
    {
...
void DigitalBrain::Land(void)
{
    ...
    else if ( not abObj->IsObjective() or !abObj->brain)
    {
...
void DigitalBrain::TakeOff()
{
    ...
    if ( not Airbase or !Airbase->IsObjective() or !Airbase->brain)
    {
...
void DigitalBrain::DealWithBlocker(SimBaseClass *inTheWay, ObjectiveClass *Airbase)
{
    ...
    else if ((isWing and ( not inTheWay2 or inTheWay2->GetCampaignObject() not_eq self->GetCampaignObject() ||
...
void DigitalBrain::ChooseNextPoint(ObjectiveClass *Airbase)  // to Runway  Takeoff() & ResetTaxiState()
{
    ...
    switch (PtDataTable[GetPrevPtLoop(curTaxiPoint)].type)
    {
        ...
        case RunwayPt:
        {
            ...
            else if ( not isWing and !IsSetATC(PermitRunway))
            {
...
int DigitalBrain::ReadyToGo(void)
{
    ...
    if ( not isWing and !IsSetATC(PermitTakeoff))
        return FALSE;
...
void DigitalBrain::UpdateTaxipoint(void)  // Called by Airframe.cpp, AircraftInputs.cpp
{
    ...
    if (( not self->OnGround()) or !self->IsPlayer() and (atcstatus == tTakeRunway))
        return;
