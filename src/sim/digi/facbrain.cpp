...
FACBrain::FACBrain(AircraftClass* myPlatform, AirframeClass* myAf)
    : DigitalBrain(myPlatform, myAf)
{
    ...
    campaignTarget =
        ((UnitClass*)(self->GetCampaignObject()))->GetUnitMissionTarget();
}
    ...
    if (campaignTarget)
    {
        if (numHit > 0)
        {
            ...
            FalconFACMessage* facMsg =
                new FalconFACMessage(theFighter->Id(), FalconLocalGame);
...
void FACBrain::RequestTarget(SimVehicleClass*)
{
    ...
    FalconFACMessage* facMsg =
        new FalconFACMessage(controlledFighter->Id(), FalconLocalGame);
    facMsg->dataBlock.caller = self->Id();
    ...
    if (newTarget)
    {
        ...
        facMsg->dataBlock.data1 = facMsg->dataBlock.data2 =
            facMsg->dataBlock.data3 = 0;
