...
void DigitalBrain::SimpleTrack(SimpleTrackMode mode, float value)
{
        ...
        if (mode == SimpleTrackDist)
        {
            ...
            if (flightLead)
            {
                ...
                SimpleTrackDistance(
                    ...
                    (float)sqrt(
                        xft * xft +
                        yft * yft)); // Get Leader's speed, relative position
            }
    ...
    else
    {
        ...
        if (gameCompressionRatio)
            ...
            rStick = SimpleTrackAzimuth(rx, ry,
                                        self->GetVt()); ///gameCompressionRatio;
        else
            rStick = 0.0f;

        pStick = SimpleTrackElevation(zft, 5000.0F);
        throtl =
            SimpleGroundTrackSpeed(value); // value = desired speed (ft/sec)
    }
...
//
//--------------------------------------------------
void DigitalBrain::CalculateRelativePos(float* xft, float* yft, float* zft,
                                        float* rx, float* ry, float* rz)
{
    ...
    *ry = self->dmx[1][0] * *xft + self->dmx[1][1] * *yft +
          self->dmx[1][2] * *zft;
    *rz = self->dmx[2][0] * *xft + self->dmx[2][1] * *yft +
          self->dmx[2][2] * *zft;
}
...
float DigitalBrain::SimpleTrackDistance(float, float rx)
{
    ...
    desiredClosure =
        350.0F * rx / (1.0F * NM_TO_FT) -
        350.0F; // JB 011016 Increase closure rates to adjust for the vt/GetKias bug fix below

    // get actual closure
    actualClosure = -(rx - velocitySlope) / SimLibLastMajorFrameTime;
    //me123 machhold needs knots  chenged af->vt() to self->GetKias
    MachHold(self->GetKias() + desiredClosure - actualClosure, self->GetKias(),
             FALSE);
    velocitySlope = rx;
    return throtl;
}
...
float DigitalBrain::SimpleGroundTrackSpeed(float v)
{
    ...
    if (af->vt > 20.0F and v > 20.0F)
    {
        ...
        throtl = eProp * 0.005F + autoThrottle -
                 af->vtDot * SimLibMajorFrameTime * 0.005F;
    }
    else
    {
        autoThrottle = throtl =
            af->CalcThrotlPos(v); // v = desired speed (ft/sec)
    }
...
float DigitalBrain::SimpleTrackAzimuth(float rx, float ry, float)
{
    ...
    if (not self->OnGround())
    {
        ...
        if (rx < 0.0F and (fabs(rx) < 3.0F * NM_TO_FT))
        {
            ...
            else if ((azErr < 0.0F) and
                     (azErr < -90 * DTR)) // else to the left and behind
            {
        ...
        if (g_bPitchLimiterForAI and // We are asking to limit AI's turn agressiveness when flying to waypoints
            ...
            (curMode == WingyMode or curMode == WaypointMode or
             curMode == RTBMode) and // Following waypoint or lead
            agDoctrine ==
                AGD_NONE and // Not doing any A2G attack (since it's in FollowWaypoints during that time)
            (not flightLead or
             not flightLead->IsSetFlag(
                 MOTION_OWNSHIP))) // The lead isn't the player (we must follow him whatever he does)
        {

            azErr /= ((180.0f) * DTR);

            float maxRoll = self->af->GetRollLimitForAiInWP() * DTR;
            float curRoll = (float)fabs(self->Roll()) *
                            0.85f; // Current Roll with some leadway
            float scale;
            ...
            if (curRoll > maxRoll)
                ...
                scale =
                    (float)sqrt((maxRoll - curRoll) /
                                maxRoll); // Give more weights being toward zero
...
float DigitalBrain::SimpleTrackElevation(float zft, float scale)
{
    ...
    if (g_bPitchLimiterForAI and not groundAvoidNeeded and
        ...
        (not flightLead or not flightLead->IsSetFlag(MOTION_OWNSHIP) or
         ((AircraftClass*)flightLead)->autopilotType ==
             AircraftClass::
                 CombatAP) and // The lead isn't the player (we must follow him whatever he does)
        altErr > 0.0f and
        self->Pitch() > 0.0f)
    {
        float maxPitch =
            min(MAX_AF_PITCH, aeroDataset[self->af->VehicleIndex()]
                                  .inputData[AeroDataSet::ThetaMax]);
        float curPitch =
            self->Pitch() * 0.85f; // Current pitch with some leadway
        float scale;
        ...
        if (curPitch > maxPitch)
            ...
            scale =
                (float)sqrt((maxPitch - curPitch) /
                            maxPitch); // Give more weights being toward zero

        altErr *= scale;

        // scale the pitch based on on the difference of speed
        float minVcas = aeroDataset[self->af->VehicleIndex()]
        ...
        else
        {
            ...
            // If we're above or best climb speed, adjust the pitch
                ...
                scale = (float)sqrt(
                    (curKias - minVcas) /
                    (minVcas * 0.5f)); // Give more weights being toward zero
            else
