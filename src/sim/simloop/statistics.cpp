...
static int frame_count = 0, frame_time[100], last_time, num_frames = 0,
           cur_time, avg_time, min_time, max_time, total_time;
...
void WriteStatistics(void)
{
    if (log_frame_rate)
    {
        ...
        sprintf(buffer, "%f, %f, %f, %f, %d, %d, %d\n",
                1000.0 / frame_time[frame_count], 1000.0 / min_time,
                1000.0 / max_time, 1000.0 / avg_time, gTotSfx, numObjsProcessed,
                numObjsInDrawList);
...
void PrintMemUsage(void)
{
    ...
    MonoPrint("T Block    %5d:%5d    TBlock List %5d:%5d\n",
              ...
              MemPoolSize(TBlockList::pool) / 1024);
    MonoPrint(
        "Bomb       %5d:%5d    Chaff %5d:%5d\n", MemPoolCount(BombClass::pool),
        MemPoolSize(BombClass::pool) / 1024, MemPoolCount(ChaffClass::pool),
        MemPoolSize(ChaffClass::pool) / 1024);
    MonoPrint(
        "Flare      %5d:%5d    Debris %5d:%5d\n",
    ...
    MonoPrint("                :         Sim Feature %5d:%5d\n",
              ...
              MemPoolSize(SimFeatureClass::pool) / 1024);
    MonoPrint(
        "Sim Grnd   %5d:%5d    Grnd AI     %5d:%5d\n",
    ...
    MonoPrint("Sim Gun    %5d:%5d    Sim Missile %5d:%5d\n",
              ...
              MemPoolSize(MissileClass::pool) / 1024);
    ...
    MonoPrint("Default    %5d:%5d    ConvList   %5d:%5d\n",
              ...
              MemPoolSize(VM_CONVLIST::pool) / 1024);
    MonoPrint("Conversations    %5d:%5d   VoiceBuffers    %5d:%5d \n",
              MemPoolCount(CONVERSATION::pool),
              MemPoolSize(CONVERSATION::pool) / 1024,
              MemPoolCount(VM_BUFFLIST::pool),
              MemPoolSize(VM_BUFFLIST::pool) / 1024);
    MonoPrint("Persistant %5d:%5d\n", MemPoolCount(SimPersistantClass::pool),
              MemPoolSize(SimPersistantClass::pool) / 1024);

    simTotal =
        MemPoolSize(BombClass::pool) + MemPoolSize(ChaffClass::pool) +
        MemPoolSize(FlareClass::pool) + MemPoolSize(DebrisClass::pool) +
        MemPoolSize(SimFeatureClass::pool) + MemPoolSize(GroundClass::pool) +
        MemPoolSize(GNDAIClass::pool) + MemPoolSize(GunClass::pool) +
        MemPoolSize(MissileClass::pool) + MemPoolSize(AircraftClass::pool) +
        MemPoolSize(AirframeClass::pool) + MemPoolSize(SimObjectType::pool) +
        MemPoolSize(SimObjectLocalData::pool) + MemPoolSize(AirframeDataPool);
    simTotal /= 1024;

    campTotal =
        MemPoolSize(ObjectiveClass::pool) + MemPoolSize(SquadronClass::pool) +
        MemPoolSize(BattalionClass::pool) + MemPoolSize(FlightClass::pool) +
        MemPoolSize(BrigadeClass::pool) + MemPoolSize(TaskForceClass::pool) +
        MemPoolSize(PackageClass::pool) + MemPoolSize(SimPersistantClass::pool);
    campTotal /= 1024;
    ...
    graphTotal =
        ...
        MemPoolSize(DrawableRoadbed::pool) + MemPoolSize(DrawableBSP::pool) +
        MemPoolSize(DrawableTrail::pool) + MemPoolSize(DrawableOvercast::pool) +
        MemPoolSize(DrawablePuff::pool) + MemPoolSize(DrawableShadowed::pool) +
        MemPoolSize(Drawable2D::pool) + MemPoolSize(TrailElement::pool) +
        MemPoolSize(TListEntry::pool) + MemPoolSize(TBlock::pool) +
        MemPoolSize(TBlockList::pool) + MemPoolSize(SfxClass::pool);
    graphTotal /= 1024;
...
void WriteMemUsage(void)
{
    ...
    fprintf(fp, "Bomb       %5d:%5d    Sim Feature %5d:%5d\n",
            ...
            MemPoolSize(SimFeatureClass::pool) / 1024);
    fprintf(
        fp, "Sim Grnd   %5d:%5d    Grnd AI     %5d:%5d\n",
        MemPoolCount(GroundClass::pool), MemPoolSize(GroundClass::pool) / 1024,
        MemPoolCount(GNDAIClass::pool), MemPoolSize(GNDAIClass::pool) / 1024);
    ...
    simTotal =
        MemPoolSize(BombClass::pool) + MemPoolSize(SimFeatureClass::pool) +
        MemPoolSize(GroundClass::pool) + MemPoolSize(GNDAIClass::pool) +
        MemPoolSize(GunClass::pool) + MemPoolSize(MissileClass::pool) +
        MemPoolSize(AircraftClass::pool) + MemPoolSize(AirframeClass::pool) +
    ...
    fprintf(fp,
            "Sim Pools = %7d Campaign Pools = %7d Graphics Pools = %7d\n "
            "SoundPools = %7d",
            simTotal, campTotal, graphTotal, soundTotal);
    fclose(fp);
