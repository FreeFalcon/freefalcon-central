...
void MissileClass::Yaw(void)
{
    ...
    if (not(flags bitand EndGame))
        nycmd = min(max(nycmd, -auxData->maxGNormal / 1.41f),
                    auxData->maxGNormal / 1.41f);
    else
        nycmd = min(max(nycmd, -auxData->maxGTerminal / 1.41f),
    ...
    beta = Math.F7Tust(betcmd, ifd->ty02, ifd->ty03, ifd->ty04,
                       SimLibMinorFrameTime, ifd->oldy03, &ifd->oldbet);
    betdt1 = Math.F8Tust(betcmd, ifd->ty02, ifd->ty03, ifd->ty04,
                         SimLibMinorFrameTime, ifd->oldy04, &ifd->oldbetdt);
