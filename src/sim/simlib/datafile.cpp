...
bool AssignField(const InputDataDesc *field, void *dataPtr, const char *value)
{
    ...
    case InputDataDesc::ID_VECTOR: // X, Y, Z
    {
        ...
        if (sscanf(value, "%g %g %g", &tp->x, &tp->y, &tp->z) not_eq 3)
            if (sscanf(value, "%g, %g, %g", &tp->x, &tp->y, &tp->z) not_eq
                3) // MLR 12/4/2003 - Make the vector reading a little more flexible
            {
                return false;
            }
    }
...
void FileReader::Initialise(void *dataPtr)
{
...
bool FileReader::AssignField(const InputDataDesc *desc, void *dataPtr,
                             const char *value)
{
...
bool ParseSimlibFile(void *dataPtr, const InputDataDesc *desc,
