...
public:
    ...
    enum DigiRadarMode
    {
        ...
        DigiRWS,
        DigiOFF
    };  // 2002-02-09 ADDED BY S.G. The different radar mode.
    enum RadarFlag
    {
        FirstSweep = 0x00000001
    }; // 2002-03-10 S.G.
protected:
...
protected: // Pointer into the class table data for this type of radar
    ...
    virtual void SetSensorTarget(
        SimObjectType* newTarget); // Used for things in the target list
...
public:
    ...
    virtual void
    SendTrackMsg(SimObjectType* tgtptr, unsigned int trackType,
    ...
    virtual void ClearSensorTarget(
        void); // 2002-02-10 ADDED BY S.G. Need to deal with digiRadarMode before calling SensorClass::ClearSensorTarget

    virtual void DefaultAAMode(void) {
    }; // If not in AA go to default AA mode, otherwise nothing
    ...
    virtual void StepAAelvation(int) {
    }; // Adjust the lookup/lookdown angle of the radar (cmd=0 means center it)

    virtual void DefaultAGMode(void) {
    }; // If not in AG go to default AG mode, otherwise nothing
    ...
    virtual void SetGroundPoint(float, float, float) {
    }; //Set center of ground map radar
...
public:
    static const UInt32
        TrackUpdateTime; // How long between lock message retransmits?
    DigiRadarMode
        digiRadarMode; // 2002-02-09 ADDED BY S.G. The current radar mode of the AI (STT, TWS, RWS or OFF)
