...
public:
    ...
    int WarnReset(void)
    {
        return NeedsWarnReset;
    }; //MI
    int DidManWarn(void)
    {
        return DidManWarnReset;
    }; //MI
    void SetManWarnReset(void)
    {
        DidManWarnReset = TRUE;
    }; //MI
    void ClearManWarnReset(void)
    {
        DidManWarnReset = FALSE;
    }; //MI
    void ClearWarnReset(void)
    {
        NeedsWarnReset = FALSE;
    };
    void SetWarnReset(void)
    {
        NeedsWarnReset = TRUE;
    };
    int GetFFaultCount(void)
    {
        return mFaults.GetFaultCount();
    }
    //MI
    ...
    void AddLanding(VU_TIME thetime)
    {
        mFaults.AddMflList(thetime, FaultClass::landing, 0);
    };
    void SetStartTime(VU_TIME thetime)
    {
    ...
    int GetMflListCount()
    {
        return mFaults.GetMflListCount();
    };
    void ClearMfl()
    {
        mFaults.ClearMfl();
