...
#define SFX_PARTICLE_KLUDGE                                                    \
    119 // <--- never use this in a constructor - it's a kludge
...
protected:
    ...
    static void CalculateRestingObjectMatrix(float pitch, float roll,
                                             float mat[3][3]);
    static void CalculateGroundMatrix(Tpoint *normal, float yaw,
                                      float mat[3][3]);
    static void MultiplyMatrix(float result[3][3], float mat[3][3],
                               float mat1[3][3]);
    static void TransformPoint(Tpoint *result, Tpoint *point, float mat[3][3]);
    static void CopyMatrix(float result[3][3], float mat[3][3]);
    static void GetOrientation(float mat[3][3], float *yaw, float *pitch,
                               float *roll);
    static float AdjustAngle180(float angle);
    static int RestPiece(float *angle, float rest, float multiplier = 0.125f,
                         float min = 0.02f);

public:
    int TryParticleEffect(void);
    SfxClass(int typeSfx, int flagsSfx, SimBaseClass *baseobjSfx,
             float timeToLiveSfx, float scaleSfx, Tpoint *slot, float restpitch,
             float restroll);
    /////////////
...
public:
    ...
    */
    ...
    // Message timer
    SfxClass(FalconMissileEndMessage *endM, FalconDamageMessage *damM);

    // non-moving effect
    SfxClass(int type, Tpoint *pos, float timeToLive, float scale);

    // Secondary effect driver effect
    SfxClass(int type, Tpoint *pos, int count, float interval);

    // Secondary effect driver effect with vector
    SfxClass(int type, Tpoint *pos, Tpoint *vec, int count, float interval);

    // moving effect
    SfxClass(int type, int flags, Tpoint *pos, Tpoint *vec, float timeToLive,
             float scale);

    // moving effect with rotation
    SfxClass(int type, int flags, Tpoint *pos, Trotation *rot, Tpoint *vec,
             float timeToLive, float scale);

    // BSP effect may or may not move based on deltas and ypr deltas
    SfxClass(int type, int flags, SimBaseClass *baseobj, float timeToLive,
             float scale);

    // this just times the life of a trail then deletes it
    SfxClass(float timeToLive, DrawableTrail *trail);

    // moving BSP
    SfxClass(int type, Tpoint *pos, Tpoint *vec, DrawableBSP *theObject,
             float timeToLive, float scale);
