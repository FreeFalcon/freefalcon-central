...
public:
    ...
    {
        NumEEGSSegments = 24
    }; // (Starts one segment away from ownship)
    enum
    {
        EEGSTimePerSegment = 200
    }; // ms.  200 means each segment is 1/5 second long.
    enum
    {
        EEGSTimeLength = (NumEEGSSegments)*EEGSTimePerSegment
    }; // milliseconds
    enum
    {
        EEGSUpdateTime = 33
    }; // Limit how often we can grab new data (ms)
    enum
    {
        NumEEGSFrames = EEGSTimeLength / EEGSUpdateTime + 1
    };
    //TJL 01/25/04
...
private:
    ...
    FireControlComputer*
        FCC; // Points to ownship->FCC.  Repeated here to save one dereference.
    ...
    void DrawDesignateMarker(DesignateShape shape, float az, float el,
                             float dRoll);
    ...
    void DrawDLZSymbol(float percentRange, char* tmpStr, float rMin, float rMax,
                       float rNeMin, float rNeMax, BOOL aaMode,
                       char* tmpStrpole);
    void DrawAim9Reticle(float radius, int showRange,
                         int showAspect); //Wombat778 10-16-2003
    ...
    void DrawEEGS(void);
    void DrawSSLC(
        void); // ASSOCIATOR 03/12/03: Added the combined SnapShot LCOS Gunmode SSLC
    void DrawFunnel(void);
    void DrawMRGS(void);
    void DrawLCOS(void);
    void DrawLCOSForSSLC(
        void); // ASSOCIATOR 03/12/03: Helper function for DrawSSLC method
    ...
    {
        return eegsFrameArray[after].x +
               t * (eegsFrameArray[before].x - eegsFrameArray[after].x);
    };
    float HudClass::EEGSvalueY(float t, int before, int after)
    {
        return eegsFrameArray[after].y +
               t * (eegsFrameArray[before].y - eegsFrameArray[after].y);
    };
    float HudClass::EEGSvalueZ(float t, int before, int after)
    {
        return eegsFrameArray[after].z +
               t * (eegsFrameArray[before].z - eegsFrameArray[after].z);
    };
    float HudClass::EEGSvalueVX(float t, int before, int after)
    {
        return eegsFrameArray[after].vx +
               t * (eegsFrameArray[before].vx - eegsFrameArray[after].vx);
    };
    float HudClass::EEGSvalueVY(float t, int before, int after)
    {
        return eegsFrameArray[after].vy +
               t * (eegsFrameArray[before].vy - eegsFrameArray[after].vy);
    };
    float HudClass::EEGSvalueVZ(float t, int before, int after)
    {
        return eegsFrameArray[after].vz +
               t * (eegsFrameArray[before].vz - eegsFrameArray[after].vz);
    };
    ...
    DWORD curHudColor;
    float HudBrightness, HudContrast,
        AutoHudCx; // COBRA - RED - Hud Brightness bitand Contrast
...
public:
    ...
    VuEntity* CanSeeTarget(
        int wid, VuEntity* entity,
        FalconEntity* platform); // Note: This is specific to the RECON pod

    void PushButton(int whichButton, int whichMFD = 0);
    void SetEEGSData(float x, float y, float z, float gamma, float sigma,
