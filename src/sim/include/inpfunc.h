...
{
    InputFunctionType
        func[MAX_POV_DIR]; //this is the pointer to the appropriate function
    int cpButtonID
        [MAX_POV_DIR]; //this is the ID of the associated cockpit button
} POVfunc;
...
#define SIMLIB_MAX_POV 4
    ...
    enum
    {
        NumHashEntries = 256 /*DIK_APPS +1*/,
        NumButtons = SIMLIB_MAX_DIGITAL * SIM_NUMDEVICES,
        NumPOVs = SIMLIB_MAX_POV
    };
...
public:
    ...
    ~InputFunctionHashTable(void);
    void AddFunction(int key, int flags, int buttonId, int mouseSide,
                     InputFunctionType funcPtr);
    void RemoveFunction(int key, int flags);
    void ClearTable(void);
    InputFunctionType GetFunction(int key, int flags, int* pbuttonId,
                                  int* pmouseSide);
    ...
    BOOL SetButtonFunction(int buttonId, InputFunctionType theFunc,
                           int cpButtonID);
    InputFunctionType GetButtonFunction(int buttonID, int* cpButtonID);
    BOOL SetPOVFunction(int POV, int dir, InputFunctionType theFunc,
                        int cpButtonID);
