...
public:
    ...
    virtual int ObjectDetected(
        ...
        int radarMode =
            0);  // 2002-02-09 MODIFIED BY S.G. Added the radarMode var

    int LowAltPriority(void)
    {
        return lowAltPriority;
    };
    int AutoDrop(void)
    {
        return dropPattern;
    ...
    FalconEntity* CurSpike(
        FalconEntity* byHim = NULL,
        int* data =
            NULL); // 2002-02-10 MODIFIED BY S.G. Added 'byHim' and 'data' which defaults to NULL if not passed
...
protected:
    enum
    {
        ...
        NewDetection,
        Hat
    }; // JB 010727 added NewDetection MI added 010812 Hat
    enum
    {
        MaxRWRTracks = 16,
        PriorityContacts = 5
...
public:
    typedef struct DetectListElement
    {
        ...
        VU_TIME lastHit;
        VU_TIME
        lastPlayed; // When was audio last played? (for player search tones)
        unsigned long isLocked : 1, // Is locked on us
            ...
            newDetection
            : 1, // Have we played audio for this new hit yet? (only player cares)
            newDetectionDisplay : 1, // JB 010727
            selected
            : 1, // Used to mark the "selected" emitter for playing audio
            previouslyLocked
            : 1, // Used to decide about making budy spike calls
            // JB 010727 RP5 RWR
            // 2001-02-15 ADDED BY S.G. SO WHEN CAN'T PAINT, IT DOESN'T PLAY THE SOUND...
            cantPlay
            : 1, // Used to decide if the radar can trigger a 'paint' signal on the RWR
...
protected:
    ...
    char
        dropPattern; // 0 = manual chaff/flare drop pattern, 1 = auto chaff/flare drop pattern

    // Helper functions
    /* S.G. I NEED IT */
public:
    DetectListElement* IsTracked(FalconEntity* object); /* S.G. */
protected:
