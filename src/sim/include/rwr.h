...
public:
    RwrClass(
        int type,
        SimMoverClass*); // Pass in the type index and the platform this RWR is mounted upon
    virtual ~RwrClass(void);

    virtual int ObjectDetected(
        FalconEntity*, int trackType,
        int dummy =
            0) = 0; // 2002-02-09 MODIFIED BY S.G. Added the unused dummy var
    virtual void DisplayInit(ImageBuffer*);
    virtual void GetAGCenter(float* x, float* y); // Center of rwr ground search
    static void DrawSymbol(VirtualDisplay* display, int symbolID,
                           int boxed = 0); // Draw the specified symbol
    //MI
    static void DrawHat(VirtualDisplay* display);

public:
    static const int
        RADIATE_CYCLE; // How long (in ms) will we "coast" a radiating emitter
    static const int
        TRACK_CYCLE; // How long (in ms) will we "coast" a locked on emitter
    static const int
        GUIDANCE_CYCLE; // How long (in ms) will we "coast" a guiding emitter

    int CanDetectObject(
        SimObjectType*); // Are emmisions strong enough to warrant annunciation?
    RwrDataType* GetTypeData(void)
    {
        return typeData; // 2002-02-11 ADDED BY S.G. Like the other SensorClasses
    }
...
protected:
    ...
    int CanDetectObject(
        FalconEntity*); // Are emmisions strong enough to warrant annunciation?
    void DrawEmitterSymbol(int symbolID,
                           int boxed = 0); // Draw the specified symbol
