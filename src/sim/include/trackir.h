...
public:
    ...
    void Allow_2D_Panning()
    {
        panningAllowed = true;
    }
    bool Get_Panning_Allowed()
    {
        return panningAllowed;
    }

    void GetTrackIR_ViewValues(
        float*,
        float*); // for 3d cockpit panning, gets yaw/pitch values in radians
    int
    TrackIR_2D_Map(); // for 2d cockpit panning, returns POV_N, POV_S, POV_E, POV_W values, -1 for neutral
...
private:
    ...
    bool
        isActive; // flag indicating if the TIR receives updates.. eg when the user hits F9 (stop TIR) then this one gets false after
