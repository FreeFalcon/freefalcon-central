...
void TargetAzEl(FalconEntity* af1, float x, float y, float z, float& az,
                float& el);  // MLR 6/28/2004 -
void GetXYZ(SimBaseClass* platform, float az, float el, float range, float* x,
            float* y, float* z);
void CalcRelAzEl(SimBaseClass*, float, float, float, float*,
                 float*); // added by VWF 7/1/98
void CalcRelValues(SimBaseClass*, FalconEntity*, float*, float*, float*, float*,
                   float*);  // added by VWF 7/6/98
void CalcRelAzElRangeAta(SimBaseClass* ownObject,
                         SimObjectType* targetPtr);  // KCK 8/6

void CalcRelGeom(SimBaseClass* obj, SimObjectType* objList,
                 TransformMatrix vmat, float elapsedTime);
struct vector;
int FindCollisionPoint(SimBaseClass* obj, SimBaseClass* ownShip,
                       vector* collPoint, float speedBoost = 0.0F);
