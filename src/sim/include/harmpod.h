...
public:
    ...
    Submode GetSubMode(void)
    {
        return submode;
    }
    Submode GetLastSubmode(void)
    {
        return lastSubmode;
    }
    void SetSubMode(Submode HarmMode)
    {
        submode = HarmMode;
    }
    void SetLastSubMode(Submode HarmMode)
    {
        lastSubmode = HarmMode;
    }
    int GetRange(void)
    {
        return trueDisplayRange;
    };
    void SetRange(int newRange)
    {
        displayRange = trueDisplayRange = newRange;
    };
    ...
    HASFilterMode GetFilterMode(void)
    {
        return filterMode;
    }
    void SetFilterMode(HASFilterMode mode)
    {
        filterMode = mode;
    }
    bool GetHandedoff(void)
    {
        return handedoff;
    }
    void SetHandedoff(bool value)
    {
        handedoff = value;
    }
    void ResetHASTimer(void)
    {
        HASTimer = SimLibElapsedTime;
        HASNumTargets = 0;
    }
    ...
    void ResetCursor()
    {
        cursorX = 0.0f, cursorY = -HTS_Y_OFFSET;
    }
