            ...
            if (theMissile and theMissile->IsMissile())
            {
                ...
                if (theDisplay)
                {
                    if (not theDisplay->GetDisplay())
                    {
                        if (privateDisplay)
                        {
                            theDisplay->DisplayInit(
                                ((Render2D*)(privateDisplay))
                                    ->GetImageBuffer());
                        }
                        ...
                        Sms->hardPoint[Sms->curHardpoint]->GetSubPosition(
                            Sms->curWpnNum, &rx, &ry, &rz);
                        rx += 5.0F;
                        pos.x = Sms->ownship->XPos() +
                                Sms->ownship->dmx[0][0] * rx +
                                Sms->ownship->dmx[1][0] * ry +
                                Sms->ownship->dmx[2][0] * rz;
                        pos.y = Sms->ownship->YPos() +
                                Sms->ownship->dmx[0][1] * rx +
                                Sms->ownship->dmx[1][1] * ry +
                                Sms->ownship->dmx[2][1] * rz;
                        pos.z = Sms->ownship->ZPos() +
                                Sms->ownship->dmx[0][2] * rx +
                                Sms->ownship->dmx[1][2] * ry +
...
void SmsDrawable::UpdateGroundSpot(void)
{
    ...
    MissileClass* theMissile =
        static_cast<MissileClass*>(Sms->GetCurrentWeapon());
    ...
    if (Sms->curHardpoint > 0)
    {
            ...
            if (Sms->curWeaponType == wtAgm65)
            {
                if (theMissile and theMissile->IsMissile())
                {
                    ...
                    if (rx < 0.0F or ry < 0.0F)
                        SetGroundSpotPos(Sms->ownship->XPos(),
                                         Sms->ownship->YPos(),
                                         Sms->ownship->ZPos());
                    else
        ...
        case wcHARMWpn:
            if (theMissile and theMissile->IsMissile())
            {
                ...
                if (rx < 0.0F)
                    SetGroundSpotPos(Sms->ownship->XPos(), Sms->ownship->YPos(),
                                     Sms->ownship->ZPos());
                else
                    SetGroundSpotPos(rx, ry, rz);
...
void SmsDrawable::DisplayInit(ImageBuffer* image)
{
    ...
    privateDisplay->SetColor(
        0xffffffff); // #2: the SMS page (right MFD) was green -> white (realism, per request 2026-06-17)
    isDisplayed = TRUE;
}
...
{
    isDisplayed =
        TRUE; // MLR 3/22/2004 - Because sometimes DisplayInit is NOT called when the SMS is displayed.
    ...
    BombClass* bc =
        Sms->CurHardpoint() == -1 ?
            NULL :
            static_cast<BombClass*>(
                Sms->hardPoint[Sms->CurHardpoint()]->weaponPointer.get());

    if ((Sms->GetCurrentWeapon() not_eq NULL) and
        (Sms->GetCurrentHardpoint() > 0) and bc and
        bc->IsSetBombFlag(BombClass::IsJSOW))
    {
        isJSOW = TRUE;
    }
    else
    ...
    if ((Sms->GetCurrentWeapon() not_eq NULL) and
        (Sms->GetCurrentHardpoint() > 0) and bc and
        bc->IsSetBombFlag(BombClass::IsGPS))
    {
        isJDAM = TRUE;
    }
    else
    ...
    case Wpn:
        ...
        else
        {
            ...
            else
            {
                ...
                switch (Sms->hardPoint[Sms->GetCurrentHardpoint()]
                            ->GetWeaponClass())
                {
                case wcAimWpn:
                case wcAgmWpn:
...
void SmsDrawable::WpnPushButton(int whichButton, int whichMFD)
{
    ...
    case 1:
        if (g_bRealisticAvionics)
        {
            ...
            else if (pFCC->GetMasterMode() == FireControlComputer::Dogfight or
                     pFCC->GetMasterMode() ==
...
void SmsDrawable::WpnAGPushButton(int whichButton, int whichMFD)
{
    ...
    case 2:
        if (Sms->curWeaponType == wtAgm65 and Sms->curWeapon)
        {
            ...
            MaverickDisplayClass* mavDisplay =
                (MaverickDisplayClass*)((MissileClass*)Sms->GetCurrentWeapon())
                    ->display;
        ...
        else if (Sms->curWeaponClass == wcGbuWpn)
        {
            LaserPodClass* laserPod =
                (LaserPodClass*)FindLaserPod(Sms->ownship);
    ...
    case 4:
        if (g_bRealisticAvionics)
        {
            if ((pFCC->GetMasterMode() ==
                 FireControlComputer::
                     AirGroundBomb) or //me123 status test. addet four lines
                (pFCC->GetMasterMode() ==
                 FireControlComputer::AirGroundRocket) or // MLR 4/3/2004 -
    ...
    case 7:
        if (g_bRealisticAvionics and Sms->CurHardpoint() >= 0) //MI
        {
            if ((pFCC->GetMasterMode() ==
                 FireControlComputer::
                     AirGroundBomb) or //me123 status test. addet four lines
                (pFCC->GetMasterMode() ==
                 FireControlComputer::AirGroundLaser) or
        ...
        else
        {
            if (pFCC->GetMasterMode() ==
                FireControlComputer::
                    AirGroundBomb) //me123 status test. addet four lines
            {
                Sms->SetAGBPair(not Sms->GetAGBPair());
                /*
                    if (Sms->pair)
            ...
            if (pFCC->GetMasterMode() == FireControlComputer::AirGroundMissile)
                ...
                (pFCC->GetMasterMode() ==
                 FireControlComputer::
                     AirGroundBomb) or //me123 status test. addet four lines
                (pFCC->GetMasterMode() ==
                 FireControlComputer::AirGroundLaser)) //MI LGB's
            {
                if (g_bMLU)
                    ChangeToInput(whichButton);
                else
    ...
    case 18:
        ...
        else if (g_bRealisticAvionics and
                 pFCC->GetMasterMode() ==
                     FireControlComputer::AirGroundMissile and
                 Sms->curWeaponType == wtAgm65)
            pFCC->WeaponStep();
...
void SmsDrawable::WpnAGMenu(int whichButton, int whichMFD)
{
    ...
    case 19:
        pFCC->SetSubMode(
            FireControlComputer::
                CCIP); // MLR 4/4/2004 - SetSubMode calls Sms->SetAGBSubMode()
        break;
...
void SmsDrawable::WpnOtherPushButton(int whichButton, int whichMFD)
{
    ...
    case 1:
        if (pFCC->GetMasterMode() == FireControlComputer::Dogfight and
...
void SmsDrawable::StepSelectiveJettisonMode(int hp)
{
    ...
    //if(Sms->hardPoint[hp]->GetRackDataFlags() bitand RDF_BMSDEFINITION)
    {
        ...
        case JettisonNone:
            ...
            if ((Sms->hardPoint[hp]->GetRackDataFlags() bitand
                 RDF_SELECTIVE_JETT_WEAPON))
                break;
...
void SmsDrawable::DogfightDisplay(void)
{
    ...
    if (g_bRealisticAvionics)
    {
        ...
        if (Sms->curWeaponType == wtAim9 and
            ((CampBaseClass*)wpn)->GetSPType() not_eq SPTYPE_AIM9P)
        {
            ...
            if (((MissileClass*)Sms->GetCurrentWeapon())->isTD and
                ((CampBaseClass*)wpn)->GetSPType() not_eq SPTYPE_AIM9P)
            {
                LabelButton(17, "TD");
            }
        ...
        else if (Sms->curWeaponType == wtAim120)
        {
            ...
            LabelButton(17, "TM",
                        "OFF"); // JPG 12 Jan 03 - ON is for telemetry missile

            if (Sms and Sms->GetCurrentWeapon() and
...
void SmsDrawable::MissileOverrideDisplay(void)
{
    ...
    if (g_bRealisticAvionics)
    {
        if (Sms->curWeaponType == wtAim120)
        {
            ...
            LabelButton(16, "ID", idnum);
            LabelButton(17, "TM",
                        "OFF"); // 20 Jan 04 ON is for telemetry missile

            if (Sms->curWeapon)
            {
                ShiAssert(Sms->curWeapon->IsMissile());

                if (((MissileClass*)Sms->GetCurrentWeapon())
                        ->isSlave) // and Sms->curWeaponType == wtAim9)
                {
                    LabelButton(18, "SLAV");
                }
        ...
        else if (Sms->curWeaponType == wtAim9 and wpn and
                 ((CampBaseClass*)wpn)->GetSPType() not_eq SPTYPE_AIM9P)
        {
            ShiAssert(Sms->curWeapon and Sms->curWeapon->IsMissile());
            ...
            if (Sms and Sms->curWeapon and
                ((MissileClass*)Sms->GetCurrentWeapon())->isTD and wpn and
                ((CampBaseClass*)wpn)->GetSPType() not_eq SPTYPE_AIM9P)
            {
                LabelButton(17, "TD");
            }
            ...
            // Marco Edit - SPOT/SCAN Mode
            if (Sms and Sms->curWeapon and
                ((MissileClass*)Sms->GetCurrentWeapon())->isSpot and wpn and
                ((CampBaseClass*)wpn)->GetSPType() not_eq SPTYPE_AIM9P)
            {
                LabelButton(2, "SPOT");
            }
...
void SmsDrawable::ShowMissiles(int buttonNum)
{
    ...
    sprintf(tmpStr, "%d%s", Sms->NumCurrentWpn(),
            Sms->hardPoint[Sms->curHardpoint]->GetWeaponData()->mnemonic);
    ...
    //MI
    if (g_bRealisticAvionics and Sms->curWeaponType == wtAgm65 and
        Sms->curWeapon)
    {
    ...
    for (i = 0; i < Sms->NumHardpoints(); i++)
    {
        ...
        if (i < 6)
        {
            ...
            display->TextLeft(leftEdge, y, tmpStr,
        ...
        else
        {
            ...
            display->TextRight(leftEdge, y, tmpStr,
                               (Sms->curHardpoint == i ? 2 : 0));
        }
    }
}
...
void SmsDrawable::MaverickDisplay(void)
{
    ...
    if ((MissileClass*)Sms->GetCurrentWeapon() and
    ...
    if (mavDisplay and mavDisplay->CurFOV() >
                           12.0f / (ZoomMax - (ZoomMax - ZoomMin) / 2.0f) * DTR)
        LabelButton(2, "FOV");
    else
...
void SmsDrawable::AAMDisplay(void)
{
    ...
    if (g_bRealisticAvionics and Sms->curWeapon)
    {
        ...
        if (((MissileClass*)Sms->GetCurrentWeapon())->isSlave and
            ((CampBaseClass*)wpn)->GetSPType() not_eq
                SPTYPE_AIM9P) // and Sms->curWeaponType == wtAim9)
        {
            LabelButton(18, "SLAV");
        }
        else if (Sms->curWeaponType not_eq wtAim9 or
                 ((CampBaseClass*)wpn)->GetSPType() not_eq SPTYPE_AIM9P)
        {
            LabelButton(18, "BORE");
        }
...
void SmsDrawable::WpnAAMissileButton(int whichButton, int whichMfd)
{
    ...
    case 7:
        ...
        {
            if (Sms->GetCoolState() == SMSClass::WARM or
                Sms->GetCoolState() == SMSClass::WARMING)
            {
...
void SmsDrawable::AGMenuDisplay(void)
{
    ...
    LabelButton(19, "CCIP", NULL,
                pFCC->GetSubMode() == FireControlComputer::CCIP);
    LabelButton(18, "CCRP", NULL,
                pFCC->GetSubMode() == FireControlComputer::CCRP);
    LabelButton(17, "DTOS", NULL,
                pFCC->GetSubMode() == FireControlComputer::DTOSS);
    LabelButton(16, "LADD", NULL,
                pFCC->GetSubMode() == FireControlComputer::LADD);
    LabelButton(15, "MAN", NULL,
                pFCC->GetSubMode() == FireControlComputer::MAN);
}
///////////////////////////////////////////////////////////////////////////////
///////////////////////////////////////////////////////////////////////////////
///////////////////////////////////////////////////////////////////////////////
        ...
        if (FCC->IsAGMasterMode())
        {
            ...
            if (g_bRealisticAvionics)
            {
                if (FCC->GetMasterMode() ==
                        FireControlComputer::AirGroundLaser or
                    Sms and Sms->curWeaponDomain not_eq wdGround)
                    LabelButton(1, "");
                else
                    LabelButton(1, FCC->subModeString);
            }
    ...
    for (curStation = Sms->numHardpoints - 1; curStation > 0; curStation--)
    {
        ...
        if (not(((AircraftClass*)Sms->ownship)->IsF16() and
                (curStation == 1 or curStation == 9 or
                 Sms->hardPoint[curStation]->GetWeaponClass() == wcECM or
                 Sms->hardPoint[curStation]->GetWeaponClass() == wcAimWpn)) and
            (Sms->hardPoint[curStation]->GetRack() or
             curStation == 5 and
                 Sms->hardPoint[curStation]->GetWeaponClass() ==
...
void SmsDrawable::SetWeapParams(void)
{
    if (Sms->curHardpoint >= 0)
    {
        if (Sms->hardPoint[Sms->curHardpoint]->GetWeaponData()->flags bitand
            SMSClass::HasBurstHeight)
        {
...
void SmsDrawable::JDAMDisplay(void)
{
    ...
    else
    {
        ...
        if (g_bRealisticAvionics and not g_bMLU)
        {
            if ((Sms->JDAMtargeting == SMSBaseClass::PB and
                 self->JDAMtargetRange > 0) or
                (CAPplayer))
            {
                char
                    tempstr[80]; //JAM 27Sep03 - Changed from 10, stack over run
                //sprintf(tempstr, "AD %.2fSEC", Sms->armingdelay / 100);
            ...
            else
            {
                ...
                float latitude = (FALCON_ORIGIN_LAT * FT_PER_DEGREE + xCurr) /
                                 EARTH_RADIUS_FT;
                ...
                sprintf(tempstr, "RNG: %.2f",
                        SqrtF(dX * dX + dY * dY) * FT_TO_NM);
                display->TextLeft(-0.6F, 0.0F, tempstr);
            }
        }
        ...
        if (Sms->JDAMtargeting == SMSBaseClass::PB)
        {
            ...
            if (self->JDAMAllowAutoStep and Sms->JDAMPowered and
                Sms->JDAMInitTimer <= 4.0f and pFCC->designateCmd == FALSE)
            {
