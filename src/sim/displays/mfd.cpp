...
extern bool
    g_bEnableMfdColors; // a.s. begin: makes Mfds transparent and colored
...
char* MFDClass::ModeNames[MAXMODES] = {
    "  ", // blank
    "", // menu
    "TFR", "FLIR", "TEST", "DTE", "FLCS", "WPN",
    "TGP", "HSD",  "FCR",  "SMS", "RWR",  /*"HUD,*/ "HAD",
};
...
MFDClass::MFDClass(int count, Render3D* r3d)
{
    ...
    // COBRA - RED - MFD
    privateImage->Setup(&FalconDisplay.theDisplayDevice, MfdSize, MfdSize,
                        /*VideoMem*/ SystemMem, None);
    privateImage->SetChromaKey(0x0);
    image = OTWDriver.OTWImage;
    color =
        0x00ffffff; // #2: MFD shell base color green->white. The shell calls SetColor(Color())
...
void MFDClass::CreateDrawables(void)
{
    ...
    if (mfdDraw[MfdOff] == NULL)
        mfdDraw[MfdOff] = new BlankMfdDrawable;

    if (mfdDraw[MfdMenu] == NULL)
        mfdDraw[MfdMenu] = new MfdMenuDrawable;

    if (mfdDraw[TFRMode] == NULL)
        mfdDraw[TFRMode] = new LantirnDrawable;

    if (mfdDraw[FLIRMode] == NULL)
        mfdDraw[FLIRMode] = new FlirMfdDrawable;

    if (mfdDraw[TestMode] == NULL)
        mfdDraw[TestMode] = new TestMfdDrawable;

    if (mfdDraw[DTEMode] == NULL)
        mfdDraw[DTEMode] = new DteMfdDrawable;

    if (mfdDraw[FLCSMode] == NULL)
        mfdDraw[FLCSMode] = new FlcsMfdDrawable;

    if (mfdDraw[WPNMode] == NULL)
        mfdDraw[WPNMode] = new WpnMfdDrawable;

    if (mfdDraw[TGPMode] == NULL)
        mfdDraw[TGPMode] = new TgpMfdDrawable;
    ...
    if (HadDrawable == NULL)
        HadDrawable = new HadMfdDrawable;
...
void MFDClass::SetOwnship(AircraftClass* newOwnship)
{
    ...
    if (playerAC and playerAC->IsSetFlag(MOTION_OWNSHIP) and
        ownship == playerAC and newOwnship not_eq playerAC)
    {
...
{
    if (not drawable or not drawable->GetDisplay())
        return;
...
void MFDClass::Exec(int clearFrame, int virtualCockpit)
{
    ...
    if (virtualCockpit)
    {
        if (drawable)
        {
            ...
            {
                drawable->GetDisplay()->SetRttCanvas(&cUL, &cUR, &cLL, cBlend,
                                                     cAlpha);
                drawable->GetDisplay()->SetRttRect(tLeft, tTop, tRight,
                                                   tBottom);
            }
        }
    }
    ...
    if (ownship and ownship->mFaults and
        ((ownship->mFaults->GetFault(FaultClass::flcs_fault) bitand
         ...
         ((ownship->mFaults->GetFault(FaultClass::mfds_fault) bitand
           FaultClass::lfwd) and
          (id % 2 == 0)) or
         ((ownship->mFaults->GetFault(FaultClass::mfds_fault) bitand
           FaultClass::rfwd) and
    ...
    if (virtualCockpit)
    {
        if (drawable)
        {
            ...
            if (drawable->GetDisplay())
            {
                if (((Render2D*)(drawable->GetDisplay()))
                        ->GetImageBuffer() not_eq image)
                    ((Render2D*)(drawable->GetDisplay()))
                        ->SetImageBuffer(image);

                ((Render2D*)(drawable->GetDisplay()))
                    ->GetViewport(&vpLeft, &vpTop, &vpRight, &vpBottom);
                ((Render2D*)(drawable->GetDisplay()))
                    ->SetViewport(vTop, vLeft, vBottom, vRight);
                //OTWDriver.SetPopup(id, TRUE);
                ...
                if (g_b3DRTTCockpitDebug)
                {
                    ...
                    drawable->GetDisplay()->Line(-0.995F, -0.995F, 0.995F,
                                                 -0.995F);
                    drawable->GetDisplay()->Line(-0.995F, 0.995F, 0.995F,
                                                 0.995F);
                    drawable->GetDisplay()->Line(-0.995F, 0.995F, -0.995F,
                ...
                ((Render2D*)(drawable->GetDisplay()))
                    ->SetViewport(vpLeft, vpTop, vpRight, vpBottom);
            }
            else
            {
        ...
        if (drawable and not(mode == MfdOff and clearFrame))
        {
            ...
            if (drawable->GetDisplay())
            {
                ...
                if (clearFrame)
                {
                    ...
                    {
                        mycolor =
                            (int)((g_fMfdRed / 100.0F) * 255.0F) +
                            (int)((g_fMfdGreen / 100.0F) * 255.0F) * 0x100 +
                            (int)((g_fMfdBlue / 100.0F) * 255.0F) * 0x10000 +
                            (int)((g_fMfdTransparency / 100.0F) * 255.0F) *
                                0x1000000;
                        ...
                        if (mode == FCCMode)
                        {
                            ...
                            OTWDriver.renderer->context.RestoreState(
                                STATE_ALPHA_SOLID);
                            drawable->GetDisplay()->Tri(-1.0F, -1.0F, -1.0F,
                                                        1.0F, 1.0F, 1.0F);
                            drawable->GetDisplay()->Tri(-1.0F, -1.0F, 1.0F,
                                                        -1.0F, 1.0F, 1.0F);
                            ...
                            drawable->Display(
                                drawable
                                    ->GetDisplay()); // second Display is necessary for FCC
                            drawable->GetDisplay()->EndDraw();
                            ((Render2D*)(drawable->GetDisplay()))
                                ->SetViewport(vpLeft, vpTop, vpRight,
                                              vpBottom); // EFOV bugfix
                    ...
                    drawable->GetDisplay()->Tri(-1.0F, -1.0F, -1.0F, 1.0F, 1.0F,
                                                1.0F);
                    drawable->GetDisplay()->Tri(-1.0F, -1.0F, 1.0F, -1.0F, 1.0F,
                                                1.0F);
                }
                              ...
                              float bottom, float right)
{
    ...
    if (top == bottom)
    {
        vLeft = 1.0F - VirtualMFD[id].top / (DisplayOptions.DispHeight * 0.5F);
        vRight =
            1.0F - VirtualMFD[id].bottom / (DisplayOptions.DispHeight * 0.5F);
        vTop = -1.0F + VirtualMFD[id].left / (DisplayOptions.DispWidth * 0.5F);
        vBottom =
            -1.0F + VirtualMFD[id].right / (DisplayOptions.DispWidth * 0.5F);
    }
    else
    {
        vTop = top;
...
int MFDClass::Color()
{
    ...
    int g = 0xff00 bitand
            MFDMasks[intensity]; // brightness byte in the G position (8..15)
...
void MFDSwapDisplays()
{
    ...
    for (int mm = 0; mm < MFDClass::MAXMM; mm++)
    {
        ...
        for (int i = 0; i < 3; i++)
        {
            ...
            MfdDisplay[0]->primarySecondary[mm][i] =
                MfdDisplay[1]->primarySecondary[mm][i];
...
void MfdDrawable::DisplayInit(ImageBuffer* image)
{
    ...
    privateDisplay->SetColor(
        0xffffffff); // #7: MFD default WHITE (was green) -- F-16 realism
}
...
void MfdDrawable::PushButton(int whichButton, int whichMFD)
{
    ...
    case 13:
        ...
        if (mode == MFDClass::MfdOff or otherMfd < 0 or
            MfdDisplay[otherMfd]->mode not_eq mode)
...
void MfdDrawable::DefaultLabel(int button)
{
    ...
    case 11:
        LabelButton(button, MFDClass::ModeName(mfd->GetSP(0)), NULL,
                    mfd->cursel[mfd->curmm] == 0);
        break;

    case 12:
        LabelButton(button, MFDClass::ModeName(mfd->GetSP(1)), NULL,
                    mfd->cursel[mfd->curmm] == 1);
        break;

    case 13:
        LabelButton(button, MFDClass::ModeName(mfd->GetSP(2)), NULL,
                    mfd->cursel[mfd->curmm] == 2);
        break;
...
void MfdDrawable::DrawReference(AircraftClass* self)
{
    ...
    case FireControlComputer::AAGun:
        //case (FireControlComputer::Gun and FCC->GetSubMode() not_eq FireControlComputer::STRAF):
        {
            ...
            if (theRadar and theRadar->CurrentTarget() and
                ...
                not FCC->IsAGMasterMode())
            {
                ...
                if (FindCollisionPoint(
                        (SimBaseClass*)theRadar->CurrentTarget()->BaseData(),
                        self, &collPoint))
                {
                ...
                if (fabs(xPos) < (60.0F * DTR))
                {
                    ...
                    offset = xPos;
                    offset -= TargetAz(
                        self, theRadar->CurrentTarget()->BaseData()->XPos(),
...
void MfdDrawable::TGPAttitudeWarning(VirtualDisplay* display)
{
    ...
    if (tmpWarnflash)
    {
        ...
        for (float warningBottom = -0.18f; warningBottom < 0.25f;
             warningBottom += 0.0025f)
        {
            display->Line(-warningSide, warningBottom, warningSide,
                          warningBottom);
        }
