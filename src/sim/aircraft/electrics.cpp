...
const unsigned long AircraftClass::systemStates[PowerMaxState] = {
    ...
    systemStates[2] | HUDPower bitor InteriorLightPower, // PowerEmergencyBus,
    systemStates[3] | RaltPower bitor MFDPower bitor UFCPower bitor
        APPower bitor IFFPower, // PowerEssentialBus,
    systemStates[4] | SMSPower | FCCPower bitor GPSPower bitor FCRPower bitor
        EWSRWRPower bitor MAPPower bitor DLPower bitor TISLPower |
        LeftHptPower bitor RightHptPower bitor RwrPower |
        InstrumentLightPower bitor InteriorLightPower bitor SpotLightPower |
        EWSJammerPower bitor EWSChaffPower bitor EWSFlarePower bitor
        ChaffFlareCount bitor PFDPower, // PowerNonEssentialBus,
};

BOOL AircraftClass::HasPower(AvionicsPowerFlags fl)
{
    ShiAssert(currentPower >= 0 and currentPower < PowerMaxState);
    return (powerFlags bitand systemStates[currentPower] bitand fl) ==
                   (unsigned int)fl ?
               1 :
               0;
#if 0
