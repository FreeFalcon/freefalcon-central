...
float AircraftClass::GlocPrediction(void)
{
    ...
    if (SimDriver.MotionOn())
    {
        if (gLoadSeconds >= 0.0F)
        {
            ...
            {
                gLoadSeconds += (BLACKOUT_RATE * (gs - (G0Plus + skillFactor)) *
                                 SimLibMajorFrameTime);
            }
            else if (gs > G0Neg)
            {
                gLoadSeconds *=
                    (1.0F - AWAKEN_RATE * skillFactor * SimLibMajorFrameTime);
            }
            else
            {
                gLoadSeconds *=
                    (1.0F - AWAKEN_RATE * skillFactor * SimLibMajorFrameTime);
                gLoadSeconds -=
                    (REDOUT_RATE * (G0Neg - gs) * SimLibMajorFrameTime);
            }

            gFact = (gLoadSeconds - GREYOUT_ONSET * skillFactor) /
                    ((BLACKOUT_ONSET - GREYOUT_ONSET) * skillFactor);
        ...
        else
        {
            ...
            {
                gLoadSeconds +=
                    (REDOUT_RATE * (gs - G0Neg) * SimLibMajorFrameTime);
            }
            else if (gs < G0Plus + skillFactor)
            {
                gLoadSeconds *=
                    (1.0F - UNREDDEN_RATE * skillFactor * SimLibMajorFrameTime);
            }
            else
            {
                gLoadSeconds *=
            ...
            gFact = (gLoadSeconds - PINKOUT_ONSET * skillFactor) /
                    ((PINKOUT_ONSET - REDOUT_ONSET) * skillFactor);
