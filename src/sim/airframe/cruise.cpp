...
float AirframeClass::GetOptimumCruise(void)
{
    ...
    Cl = Math.TwodInterp(0.7F,
                         OPTIMUM_ALPHA + (dragIndex / 60 -
                                          1.4f), // me123 take drag into acount
                         aeroData->mach, aeroData->alpha, aeroData->clift,
                         aeroData->numMach, aeroData->numAlpha, &curMachBreak,
                         &curAlphaBreak) *
         aeroData->clFactor;
    //me13 RHOASL from rho
    OptimumAirspeed =
        (float)sqrt(2.0F * weight / (RHOASL * area * Cl)) * FTPSEC_TO_KNOTS;
...
float AirframeClass::GetOptimumAltitude(void)
{
    ...
    optimum_altitude =
        OPTIMUM_ALT_M1 * dragIndex * 3 + // me123 take drag into acount
        OPTIMUM_ALT_M2 * weight + OPTIMUM_ALT_B;
...
float AirframeClass::GetOptKias(int mode)
{
    ...
    if (mode == 0)
        OptimumAirspeed += 100.0f;

    if (mode == 2)
        OptimumAirspeed += 30.0f;
