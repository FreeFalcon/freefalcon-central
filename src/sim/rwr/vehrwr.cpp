...
SimObjectType* VehRwrClass::Exec(SimObjectType* targetList)
{
    ...
    while (curObj and platform not_eq SimDriver.GetPlayerAircraft() and
           not F4IsBadReadPtr(curObj, sizeof(SimObjectType))) // JB 010318 CTD
    {
            ...
            (curObj->BaseData()->GetRadarType())) // JB 010318 CTD
        {
            ...
            {
                if (CanSeeObject(curObj) and BeingPainted(curObj) and
    ...
    for (int i = numContacts - 1; i >= 0; i--)
    {
        ...
        if (detectionList[i].entity->IsSim())
        {
            ...
            if (SimLibElapsedTime > detectionList[i].lastHit +
                                        ((SimBaseClass*)detectionList[i].entity)
                                                ->RdrCycleTime() *
                                            SEC_TO_MSEC +
                                        1000 and
                SimLibElapsedTime >
                    detectionList[i].lastHit +
                        (platform == SimDriver.GetPlayerAircraft() ?
                             3 * SEC_TO_MSEC :
                             30 * SEC_TO_MSEC))
            {
                if (not detectionList[i].isLocked or
                    (SimBaseClass*) detectionList[i].entity->IsDead())
                    DropTrack(i);
            }
        }
        else
        {
            if (SimLibElapsedTime > detectionList[i].lastHit + RADIATE_CYCLE)
            {
                ...
                if (not detectionList[i].isLocked or
                    detectionList[i].entity->IsDead() or
                    not((CampBaseClass*)detectionList[i].entity)->IsAggregate())
                    DropTrack(i);
            }
        }
    }
...
int VehRwrClass::ObjectDetected(
    FalconEntity* theObject, int trackType,
    int radarMode) // 2002-02-09 MODIFIED BY S.G. Added the radarMode var
{
    int retval = FALSE;
    float lethality;
    DetectListElement* listElement;
    ...
    */
    ...
    if (listElement)
    {
        ...
        if (trackType == Track_Lock or trackType == Track_Ping)
        {
            ...
            listElement->lastHit = SimLibElapsedTime;
            listElement->bearing =
                (float)atan2(theObject->YPos() - platform->YPos(),
                             theObject->XPos() - platform->XPos());
        ...
        case Track_Launch:
                ...
                listElement->missileActivity == 0)
            {
                ...
                if (not listElement->entity->IsMissile() or
                    ((MissileClass*)listElement->entity)->GetSeekerType() not_eq
                        SensorClass::Radar)
            ...
            if (listElement->entity->IsMissile())
            {
                if (platform == SimDriver.GetPlayerAircraft() and
                    ...
                    // 2000-09-03 S.G. SO ARH DON'T GET A LAUNCH WARNING
                    if (((MissileClass*)listElement->entity)
        ...
        case Track_Lock:
            ...
            if (radarMode ==
                RadarClass::
                    DigiSTT) // 2002-02-10 ADDED BY S.G. Only in STT will the target be flagged as having a lock.
                listElement->isLocked = 1;
            else
                listElement->isLocked = 0;

            if (whoPingedMe->OnGround() and
                SimLibElapsedTime > spikeTimer) //Cobra
            {
                ...
                FalconRadioChatterMessage* radioMessage =
                    new FalconRadioChatterMessage(theObject->Id(),
                                                  FalconLocalGame);
                radioMessage->dataBlock.from = spiked->Id();
                radioMessage->dataBlock.message = rcSPIKE;
                radioMessage->dataBlock.to = MESSAGE_FOR_TEAM;
                radioMessage->dataBlock.voice_id = static_cast<uchar>(
                    ((Flight)spiked->GetCampaignObject())
                        ->GetPilotVoiceID(spiked->vehicleInUnit));
                radioMessage->dataBlock.edata[0] =
                    static_cast<short>(spiked->GetCallsignIdx());
                radioMessage->dataBlock.edata[1] = static_cast<short>(
                    ((Flight)spiked->GetCampaignObject())
                        ->GetPilotCallNumber(spiked->vehicleInUnit));
                ...
                spikeTimer =
                    SimLibElapsedTime +
                    g_nChatterInterval *
                        CampaignSeconds; // FRB - decrease number of reports per minute
        ...
        if (trackType == Track_Lock and listElement->previouslyLocked == 0 and
            radarMode ==
                RadarClass::
                    DigiSTT) // 2002-02-20 MODIFIED BY S.G. Added the radarMode test so only STT will generate a buddy spike.
        {
            if (theObject->IsPlayer() and
                TeamInfo[platform->GetTeam()]->TStance(theObject->GetTeam()) <
                    Neutral)
            {
...
void VehRwrClass::ReportBuddySpike(FalconEntity* theObject)
{
    ...
    {
        FalconRadioChatterMessage* radioMessage =
            new FalconRadioChatterMessage(theObject->Id(), FalconLocalGame);
        radioMessage->dataBlock.from = spiked->Id();
        radioMessage->dataBlock.message = rcBUDDYSPIKE;
        radioMessage->dataBlock.to = MESSAGE_FOR_TEAM;
        radioMessage->dataBlock.voice_id =
            static_cast<uchar>(((Flight)spiked->GetCampaignObject())
                                   ->GetPilotVoiceID(spiked->vehicleInUnit));
        radioMessage->dataBlock.edata[0] =
            static_cast<short>(spiked->GetCallsignIdx());
        radioMessage->dataBlock.edata[1] =
            static_cast<short>(((Flight)spiked->GetCampaignObject())
                                   ->GetPilotCallNumber(spiked->vehicleInUnit));
        FalconSendMessage(radioMessage, FALSE);
    }
}
...
float VehRwrClass::GetLethality(FalconEntity* theObject)
{
    ...
    if (GetRoE(theObject->GetTeam(), platform->GetTeam(),
               ROE_AIR_ENGAGE)) // JB 010730
        lethality += .5;
...
FalconEntity* VehRwrClass::CurSpike(
    FalconEntity* byHim,
    int*
        data) // 2002-02-10 MODIFIED BY S.G. Added 'byHim' and 'data' which defaults to NULL if not passed
{
    ...
    for (i = 0; i < numContacts; i++)
    {
        ...
        {
            if (detectionList[i].isLocked and
                detectionList[i].radarMode ==
                    RadarClass::
                        DigiSTT) // 2002-02-09 MODIFIED BY S.G. Added the radarMode check
            {
                if (not byHim or
                    detectionList[i].entity ==
                        byHim) // 2002-02-10 ADDED BY S.G. If we are looking at a specific target, limit your search to it
                {
                    curSpike = detectionList[i].entity;

                    // Only react to enemy activity
                    if (not curSpike->IsMissile() and
                        TeamInfo[platform->GetTeam()]->TStance(
                            curSpike->GetTeam()) >
                            Neutral) // 2002-02-09 MODIFIED BY S.G. Added the not IsMissile since AI do not attack missiles anyway...
                    {
                        return curSpike;
                    }
                }
        ...
        {
            if (detectionList[i]
                    .isLocked) // If not even locked, don't bother...
            {
                if (((*data bitand RWR_GET_STT) and
                     detectionList[i].radarMode ==
                         RadarClass::
                             DigiSTT) or // The lock is of the right type
                    ((*data bitand RWR_GET_SAM) and
                     detectionList[i].radarMode == RadarClass::DigiSAM) or
                    ((*data bitand RWR_GET_TWS) and
                     detectionList[i].radarMode == RadarClass::DigiTWS) or
                    ((*data bitand RWR_GET_RWS) and
                     detectionList[i].radarMode == RadarClass::DigiRWS))
                {
                    if (not byHim or
                        detectionList[i].entity ==
                            byHim) // 2002-02-10 ADDED BY S.G. If we are looking at a specific target, limit your search to it
                    {
                                ...
                                curSpike->GetTeam()) > Neutral)
                        {
                            ...
                            {
                                if (((*data bitand RWR_PRIORITIZE_STT) and
                                     detectionList[i].radarMode ==
                                         RadarClass::
                                             DigiSTT) or // The lock is of the right priority, return it
                                    ((*data bitand RWR_PRIORITIZE_SAM) and
                                     detectionList[i].radarMode ==
                                         RadarClass::DigiSAM) or
                                    ((*data bitand RWR_PRIORITIZE_TWS) and
                                     detectionList[i].radarMode ==
                                         RadarClass::DigiTWS) or
                                    ((*data bitand RWR_PRIORITIZE_RWS) and
                                     detectionList[i].radarMode ==
                                         RadarClass::DigiRWS))
                                {
                                    *data =
                                        detectionList[i]
                                            .radarMode; // Pass back the radar mode to the calling function
                                    return curSpike;
                                }
                                else if (
                                    curIndex ==
                                    -1) // Keep only the first one (highest one)
                                    curIndex =
                                        i; // Although not a priority, it is still a radar mode we are looking for
                            }
                            else
                            {
                                *data =
...
FalconEntity* SpikeCheck(
    AircraftClass* self, FalconEntity* byHim,
    int*
        data) // 2002-02-10 MODIFIED BY S.G. Added 'byHim' and 'data' which defaults to NULL if not passed
{
