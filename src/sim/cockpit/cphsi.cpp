...
CPHsi::CPHsi()
{
    ...
    if (SimDriver.RunningTactical() and current_tactical_mission and
        ...
        not strcmpi(current_tactical_mission->get_title(),
...
void CPHsi::ExecNav(void)
{
    ...
    if (mpHsiFlags[HSI_FLAG_INIT] == FALSE)
    {
        ...
        mpHsiValues[HSI_VAL_DEV_LIMIT] = 10.0F;
        mpHsiValues[HSI_VAL_HALF_DEV_LIMIT] =
            mpHsiValues[HSI_VAL_DEV_LIMIT] * 0.5F;
    ...
    if (F4IsBadReadPtr(pcurrentWaypoint,
                       sizeof *pcurrentWaypoint)) // JPO CTD check
    {
        mpHsiFlags[HSI_FLAG_CRS_WARN] = TRUE;
        ExecBeaconProximity(SimDriver.GetPlayerEntity()->XPos(),
                            playerAC->YPos(), 0.0F, 0.0F);
    ...
    if (mpHsiValues[HSI_VAL_CRS_DEVIATION] < 270 and
        mpHsiValues[HSI_VAL_CRS_DEVIATION] > 90)
    {
...
void CPHsi::ExecTacan(void)
{
    ...
    if (SimDriver.GetPlayerEntity())
    {
        ...
        mpHsiFlags[HSI_FLAG_CRS_WARN] =
            not gNavigationSys->GetTCNPosition(&tacanX, &tacanY, &tacanZ);
        gNavigationSys->GetTCNAttribute(NavigationSystem::RANGE, &tacanRange);

        if (gNavigationSys->IsTCNTanker() or gNavigationSys->IsTCNAirbase() or
            gNavigationSys->IsTCNCarrier())   // now Carrier support
        {
            mpHsiFlags[HSI_FLAG_ILS_WARN] = FALSE;
        }
        else
...
void CPHsi::ExecILSNav(void)
{
    ...
    else
    {
        ...
        ExecBeaconProximity(playerAC->XPos(), playerAC->YPos(), waypointX,
                            waypointY);
        mpHsiFlags[HSI_FLAG_ILS_WARN] = not gNavigationSys->GetILSAttribute(
            NavigationSystem::GP_DEV, &gpDew);
        CalcILSCrsDev(gpDew);
    }
...
void CPHsi::ExecBeaconProximity(float x1, float y1, float x2, float y2)
{
    ...
    mpHsiValues[HSI_VAL_BEARING_TO_BEACON] =
        ConvertRadtoNav((float)bearingToBeacon);
    mpHsiValues[HSI_VAL_DISTANCE_TO_BEACON] =
        (float)sqrt((float)(xdiff * xdiff + ydiff * ydiff)) *
        0.0001666F; // in nautical miles (1 / 6000)

    if (mpHsiValues[HSI_VAL_DISTANCE_TO_BEACON] > 999.0F)
    {
        mpHsiValues[HSI_VAL_DISTANCE_TO_BEACON] = 999.0F;
    }
    else if ((mpHsiValues[HSI_VAL_DISTANCE_TO_BEACON] -
              (float)floor(mpHsiValues[HSI_VAL_DISTANCE_TO_BEACON])) > 0.5F)
    {
        mpHsiValues[HSI_VAL_DISTANCE_TO_BEACON] =
            (float)ceil(mpHsiValues[HSI_VAL_DISTANCE_TO_BEACON]);
    }
    ...
    if (g_bRealisticAvionics and g_bINS and
        not playerAC->INSState(AircraftClass::INS_HSI_OFF_IN))
    {
...
void CPHsi::CalcTCNCrsDev(float course)
{
    ...
    mpHsiValues[HSI_VAL_CRS_DEVIATION] =
        course - mpHsiValues[HSI_VAL_BEARING_TO_BEACON]; // in degrees
...
BOOL CPHsi::BeaconInRange(float rangeToBeacon, float nominalBeaconrange)
{
    ...
    float rndNum =
        PRANDFloatPos(); // number picked from 4x4 array to add intermitant reception

    alt = SimDriver.GetPlayerEntity()
              ->GetAltitude(); // Get current altitude (AGL)

    if (alt <= 0)
        alt = 1; // Prevent taking the sqrt of 0 (may not be needed)

    radioHorizon =
        1.06f *
        sqrt(
            (float)
                alt); // Simplified equation for radio line-of-site  //JPG 18 Apr 04 - Replaced sqrt(2*alt) w/ better equation

    // Ensure there is no divide by 0
    if (nominalBeaconrange == 0)
        nominalBeaconrange = 1;

    if (rangeToBeacon == 0)
        rangeToBeacon = 1;

    // used to add random reception at edge of max range and lower altitudes
    float detectionChanceRange =
        1.0f - (rangeToBeacon - nominalBeaconrange) * 2.0f / nominalBeaconrange;
    float detectionChanceAlt =
        (float)(0.5f - (rangeToBeacon - radioHorizon) * 2.0f / radioHorizon);
    ...
    if ((rangeToBeacon < radioHorizon) and (detectionChanceRange > 1) and
        (detectionChanceAlt > 1)) // definite receive
        lastResult = TRUE;
    // outside of radio horizon or detection range or altitude detection range to low for reception - no reception
    else if ((rangeToBeacon > radioHorizon) or (detectionChanceRange <= 0) or
             (detectionChanceAlt <= 0.5))
        lastResult = FALSE;
...
void CPHsi::Exec(void)
{
    ...
    if (playerAC->mFaults and
        playerAC->mFaults->GetFault(FaultClass::tcn_fault))
    {
        return;
    }
...
// HSIVIEW //
/////////////
CPHsiView::CPHsiView(ObjectInitStr *pobjectInitStr, HsiInitStr *phsiInitStr)
    : CPObject(pobjectInitStr)
{
...
#ifdef USE_SH_POOLS
    mpCompassCircle =
        (int *)MemAllocPtr(gCockMemPool, sizeof(int) * arraySize, FALSE);
#else
    ...
    else if (g_bCockpitAutoScale and
             ((mVScale not_eq 1.0f) or (mHScale not_eq 1.0f)))
    {

        CompassBuffer = new ImageBuffer;
        CompassBuffer->Setup(
            &FalconDisplay.theDisplayDevice, mCompassWidth, mCompassHeight,
            SystemMem, None,
            FALSE); //Wombat778 10-06 2003 Setup a new imagebuffer the size of the Compass
...
void CPHsiView::DisplayBlit()
{
    ...
    if (g_bCockpitAutoScale and
        ((mVScale not_eq 1.0f) or
         (mHScale not_eq
          1.0f))) //dont run this code if the var is set but no scaling is occuring
    {
        ...
        CompassBuffer->Clear(
            0xFFFF0000); //clear the temp buffer with chromakey blue;
        CompassBuffer->ComposeRoundRot(
            mpTemplate, &mCompassSrc, &temprect, ConvertNavtoRad(angle),
            mpCompassCircle); //Rotate the image from template to temp buffer
        mpOTWImage->ComposeTransparent(CompassBuffer, &temprect, &mCompassDest);
    }
    else
    {
        mpOTWImage->ComposeRoundRot(mpTemplate, &mCompassSrc, &mCompassDest,
                                    ConvertNavtoRad(angle), mpCompassCircle);
    }
...
void RenderHSIPoly(tagRECT *srcrect, tagRECT *destrect, GLint alpha,
                   TextureHandle *pTex, float angle)
{
    int entry;
    ...
    float hw_rate =
        OTWDriver.pCockpitManager->mHScale / OTWDriver.pCockpitManager->mVScale;
    ...
    for (entry = 0; entry <= CircleSegments - 2; entry++)
    {
        ...
        pVtx[entry].x = x + ((angcos * (pVtx[entry].x - x)) +
                             (-angsin * (pVtx[entry].y - y))) *
                                hw_rate;
        pVtx[entry].y =
            y + (angsin * (tempx - x)) + (angcos * (pVtx[entry].y - y));
        x1 = (x + Radius * CircleX[entry] * hw_rate);
        pVtx[entry].u = fStartU + (((float)(x1 - destrect->left) /
                                    (float)(destrect->right - destrect->left)) *
                                   (fMaxU - fStartU));
        pVtx[entry].v = fStartV + (((float)(y1 - destrect->top) /
...
void CPHsiView::DisplayBlit3D()
{
    ...
    if (g_bFilter2DPit)
    {
        ...
        RenderHSIPoly(&mCompassSrc, &DestRect, STATE_TEXTURE, m_arrTex[0],
                      ConvertNavtoRad(angle));
    }
    else
    {
        RenderHSIPoly(&mCompassSrc, &DestRect, STATE_TEXTURE_NOFILTER,
                      m_arrTex[0], ConvertNavtoRad(angle));
    }
...
void CPHsiView::DrawToFrom(void)
{
    ...
    DWORD colorArrows = OTWDriver.pCockpitManager->ApplyLighting(
        mColor[0][HSI_COLOR_ARROWS], true);
    DWORD colorArrowGhost = OTWDriver.pCockpitManager->ApplyLighting(
        mColor[0][HSI_COLOR_ARROWGHOST], true);

    if (crsToTrueFlag == TRUE)
    {
        // Draw To-From Arrows
        OTWDriver.renderer->SetColor(colorArrows);
        OTWDriver.renderer->Tri(toArrow[0][0], toArrow[0][1], toArrow[1][0],
                                toArrow[1][1], toArrow[2][0], toArrow[2][1]);
        // draw ghost arrow
        OTWDriver.renderer->SetColor(colorArrowGhost);
        OTWDriver.renderer->Line(-toArrow[0][0], toArrow[0][1], -toArrow[1][0],
                                 toArrow[1][1]);
        OTWDriver.renderer->Line(-toArrow[1][0], toArrow[1][1], -toArrow[2][0],
                                 toArrow[2][1]);
        OTWDriver.renderer->Line(-toArrow[2][0], toArrow[2][1], -toArrow[0][0],
                                 toArrow[0][1]);
    }
...
void CPHsiView::DrawCourseWarning(void)
{
    ...
    OTWDriver.renderer->SetViewport(-1.0F, 1.0F, 1.0F, -1.0F);
    DWORD color = OTWDriver.pCockpitManager->ApplyLighting(
        mColor[0][HSI_COLOR_COURSEWARN], true);
    OTWDriver.renderer->SetColor(color);
    OTWDriver.renderer->Render2DTri(
        (float)mWarnFlag.left, (float)mWarnFlag.top, (float)mWarnFlag.right,
        (float)mWarnFlag.top, (float)mWarnFlag.right, (float)mWarnFlag.bottom);
    OTWDriver.renderer->Render2DTri(
        (float)mWarnFlag.left, (float)mWarnFlag.top, (float)mWarnFlag.left,
        (float)mWarnFlag.bottom, (float)mWarnFlag.right,
        (float)mWarnFlag.bottom);
}
...
void CPHsiView::DrawHeadingMarker(float desiredHeading)
{
    ...
    OTWDriver.renderer->AdjustRotationAboutOrigin(
        -ConvertNavtoRad(360.0F - currentHeading + desiredHeading));
    DWORD color = OTWDriver.pCockpitManager->ApplyLighting(
        mColor[0][HSI_COLOR_HEADINGMARK], true);
    OTWDriver.renderer->SetColor(color);

    // draw tail
    OTWDriver.renderer->Tri(headingMarker[0][0], headingMarker[0][1],
...
void CPHsiView::DrawStationBearing(float bearing) // in nav units
{
    ...
    OTWDriver.renderer->AdjustRotationAboutOrigin(
        -ConvertNavtoRad(360.0F - currentHeading + bearing));

    DWORD color = OTWDriver.pCockpitManager->ApplyLighting(
        mColor[0][HSI_COLOR_STATIONBEARING], true);
    OTWDriver.renderer->SetColor(color);

    // draw arrow
    OTWDriver.renderer->Tri(bearingArrow[0][0], bearingArrow[0][1],
                            bearingArrow[1][0], bearingArrow[1][1],
    ...
    // {
    OTWDriver.renderer->Tri(bearingTail[0][0], bearingTail[0][1],
...
void CPHsiView::DrawCourse(float desiredCourse, float deviaiton)
{
    ...
    DWORD colorCourse = OTWDriver.pCockpitManager->ApplyLighting(
        mColor[0][HSI_COLOR_COURSE], true);
    OTWDriver.renderer->SetColor(colorCourse);

    OTWDriver.renderer->Tri(courseArrow[1][0], courseArrow[1][1],
                            courseArrow[1][0], -courseArrow[1][1],
                            courseArrow[0][0], courseArrow[0][1]);
    OTWDriver.renderer->Tri(courseArrow[3][0], courseArrow[3][1],
                            courseArrow[2][0], -courseArrow[2][1],
                            courseArrow[2][0], courseArrow[2][1]);
    OTWDriver.renderer->Tri(courseArrow[3][0], courseArrow[3][1],
                            courseArrow[3][0], -courseArrow[3][1],
                            courseArrow[2][0], -courseArrow[2][1]);

    // Draw Tail
    OTWDriver.renderer->Tri(courseTail[0][0], courseTail[0][1],
    ...
    DWORD colorCircle = OTWDriver.pCockpitManager->ApplyLighting(
        mColor[0][HSI_COLOR_CIRCLES], true);
    ...
    for (i = 0; i < 2; i++)
    {
        OTWDriver.renderer->Circle(courseDevScale[0], -startPoint,
                                   courseDevScale[2]);
        OTWDriver.renderer->Circle(courseDevScale[0], startPoint,
                                   courseDevScale[2]);
    ...
    OTWDriver.renderer->AdjustOriginInViewport(devBarCenter[0],
                                               devBarCenter[1]);
    OTWDriver.renderer->AdjustRotationAboutOrigin(-desiredCourse);

    // Draw the course dev bar
    DWORD colorDevBar = OTWDriver.pCockpitManager->ApplyLighting(
        mColor[0][HSI_COLOR_DEVBAR], true);
    OTWDriver.renderer->SetColor(colorDevBar);
    OTWDriver.renderer->Tri(courseDevBar[0][0], courseDevBar[0][1],
    ...
    if (ilsWarnFlag) // and (gNavigationSys->GetInstrumentMode() == NavigationSystem::ILS_TACAN or gNavigationSys->GetInstrumentMode() == NavigationSystem::ILS_NAV)) {
    {
        ...
        DWORD colorWarning = OTWDriver.pCockpitManager->ApplyLighting(
            mColor[0][HSI_COLOR_ILSDEVWARN], true);
        OTWDriver.renderer->SetColor(colorWarning);
        OTWDriver.renderer->Tri(courseDevWarn[0], courseDevWarn[1],
...
void CPHsiView::MoveToCompassCenter(void)
{
    ...
    OTWDriver.renderer->AdjustOriginInViewport(COMPASS_X_CENTER,
                                               COMPASS_Y_CENTER);
}
...
void CPHsiView::DrawAircraftSymbol(void)
{
    ...
    static const float aircraftSymbol[10][2] = {
        {0.017F, 0.133F},  {0.133F, 0.017F}, {0.067F, -0.133F},
        {0.067F, -0.158F}, {0.017F, -0.2F},
    };
    ...
    DWORD color = OTWDriver.pCockpitManager->ApplyLighting(
        mColor[0][HSI_COLOR_AIRCRAFT], true);
    OTWDriver.renderer->SetColor(color);

    // draw the aircraft symbol
    OTWDriver.renderer->Tri(-aircraftSymbol[0][0], aircraftSymbol[0][1],
                            aircraftSymbol[0][0], aircraftSymbol[0][1],
                            aircraftSymbol[4][0], aircraftSymbol[4][1]);
    OTWDriver.renderer->Tri(-aircraftSymbol[0][0], aircraftSymbol[0][1],
                            aircraftSymbol[4][0], aircraftSymbol[4][1],
                            -aircraftSymbol[4][0], aircraftSymbol[4][1]);
    OTWDriver.renderer->Tri(-aircraftSymbol[1][0], aircraftSymbol[1][1],
                            aircraftSymbol[1][0], aircraftSymbol[1][1],
                            aircraftSymbol[1][0], -aircraftSymbol[1][1]);
    OTWDriver.renderer->Tri(-aircraftSymbol[1][0], aircraftSymbol[1][1],
                            aircraftSymbol[1][0], -aircraftSymbol[1][1],
                            -aircraftSymbol[1][0], -aircraftSymbol[1][1]);
    OTWDriver.renderer->Tri(-aircraftSymbol[2][0], aircraftSymbol[2][1],
                            aircraftSymbol[2][0], aircraftSymbol[2][1],
...
void CPHsiView::CreateLit(void)
{
    if (DisplayOptions.bRender2DCockpit)
    {
        try
        {
            ...
            if ((dwMaxTextureWidth >= (unsigned int)mCompassWidth) and
                (dwMaxTextureHeight >= (unsigned int)mCompassHeight))
            {
                ...
                if (not pTex->Create("CPHsi",
                                     MPR_TI_PALETTE bitor MPR_TI_CHROMAKEY, 8,
                                     mCompassWidth, mCompassHeight))
                {
