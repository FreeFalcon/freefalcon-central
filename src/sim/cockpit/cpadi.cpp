...
// CPAdi::CPAdi
//====================================================//
CPAdi::CPAdi(ObjectInitStr *pobjectInitStr, ADIInitStr *padiInitStr)
    : CPObject(pobjectInitStr)
{
    ...
    mColor[0][4] =
        padiInitStr->color4; //0xFF6CF3F3; A/C reference symbol, light yellow
    ...
    {
        mpAircraftBar[i][0] =
            (unsigned)(mpAircraftBarData[i][0] * (float)mWidth * mHScale) +
            mDestRect.left;
        mpAircraftBar[i][1] =
            (unsigned)(mpAircraftBarData[i][1] * (float)mHeight * mVScale) +
            mDestRect.top;
    }
    ...
    mHorizCenter =
        (unsigned)(abs(int(mRightLimit - mLeftLimit)) * 0.5 + mLeftLimit);
    mVertCenter =
        (unsigned)(abs(int(mBottomLimit - mTopLimit)) * 0.5 + mTopLimit);
    ...
    //this is unnecessary in using rendered pit
             ...
             ((mHScale not_eq 1.0f) or (mVScale not_eq 1.0f)))
    {
        ADIBuffer = new ImageBuffer;
        ADIBuffer->Setup(
            &FalconDisplay.theDisplayDevice,
            (int)((mDestRect.right - mDestRect.left) / mHScale),
            (int)((mDestRect.bottom - mDestRect.top) / mVScale), SystemMem,
            None,
            FALSE); //Wombat778 10-06 2003 Setup a new imagebuffer the size of the
...
{
    const DWORD dwMaxTextureWidth = mpOTWImage->GetDisplayDevice()
                                        ->GetDefaultRC()
    ...
    if (DisplayOptions
            .bRender2DCockpit) //Wombat778 3-24-04 we are gonna do this my way...dont know about all that crap below
    {
        try
        {
            ...
            if ((LONG)dwMaxTextureWidth >= (mSrcRect.right - mSrcRect.left) and
                (LONG) dwMaxTextureHeight >= (mSrcRect.bottom - mSrcRect.top))
            {
                ...
                if (not pTex->Create("CPAdi",
                                     MPR_TI_PALETTE bitor MPR_TI_CHROMAKEY, 8,
                                     (UInt16)(mSrcRect.right - mSrcRect.left),
                                     (UInt16)(mSrcRect.bottom - mSrcRect.top)))
        ...
        if (not DisplayOptions.bRender2DCockpit or
            (DisplayOptions.bRender2DCockpit and
             (mDoBackRect and ((LONG)dwMaxTextureWidth < mBackSrc.right or
                               (LONG) dwMaxTextureHeight < mBackSrc.bottom))))
        {
            if (mDoBackRect)
            {
                ...
                MPRSurfaceType front =
                    (FalconDisplay.theDisplayDevice.IsHardware() and
                     DisplayOptions.bRender2DCockpit) ?
                        LocalVideoMem :
                        SystemMem;

                if (not mpSurfaceBuffer->Setup(&FalconDisplay.theDisplayDevice,
                                               mBackSrc.right, mBackSrc.bottom,
                                               front, None) and
                    front == LocalVideoMem)
                {
...
#ifdef _DEBUG
                    OutputDebugString(
                        "CPAdi::CreateLit - Probably out of video memory. "
                        "Retrying with system memory)\n");
#endif

                    BOOL bResult = mpSurfaceBuffer->Setup(
                        &FalconDisplay.theDisplayDevice, mBackSrc.right,
                        mBackSrc.bottom, SystemMem, None);
        ...
        else
        {
            try
            {
                ...
                {
                    m_pPalette = new PaletteHandle(
                        mpOTWImage->GetDisplayDevice()->GetDefaultRC()->m_pDD,
                        32, 256);
                    ...
                    if (not pTex->Create(
                            "CPAdi", MPR_TI_PALETTE bitor MPR_TI_CHROMAKEY, 8,
                            (UInt16)mBackSrc.right, (UInt16)mBackSrc.bottom))
                        throw _com_error(E_FAIL);

                    if (not pTex->Load(
                            0, 0xFFFF0000, (BYTE *)mpSourceBuffer, true,
                            true)) // soon to be re-loaded by CPObject::Translate3D
                        throw _com_error(E_FAIL);
...
void CPAdi::Exec(SimBaseClass *pSimBaseClass)
{
    ...
    if (g_bRealisticAvionics and g_bINS)
    {
        ...
        if (playerAC and
            Persistant ==
                1) //backup ADI, continue to function until out of energy
        {
        ...
        else if (playerAC and
                 not playerAC->INSState(AircraftClass::INS_ADI_OFF_IN))
        {
    ...
    PercentHalfXscale =
        mPitch / (25.0F * DTR); //tan(mPitch) / mTanVisibleBallHalfAngle;
...
void CPAdi::ExecILS()
{
    ...
    if (mpCPManager->mHiddenFlag and
        (gNavigationSys->GetInstrumentMode() == NavigationSystem::TACAN or
         gNavigationSys->GetInstrumentMode() == NavigationSystem::NAV))
        return;
        ...
        gNavigationSys->GetILSAttribute(NavigationSystem::GP_DEV, &gpDeviation))
    {
        ...
        gpDeviation = min(max(gpDeviation, -HORIZONTAL_SCALE * 0.5F),
                          HORIZONTAL_SCALE * 0.5F);
        gsDeviation = min(max(gsDeviation, -VERTICAL_SCALE * 0.5F),
                          VERTICAL_SCALE * 0.5F);

        mpCPManager->ADIGpDevReading =
            mpCPManager->ADIGpDevReading +
            0.1F * (gpDeviation - mpCPManager->ADIGpDevReading);
        mpCPManager->ADIGsDevReading =
            mpCPManager->ADIGsDevReading +
            0.1F * (gsDeviation - mpCPManager->ADIGsDevReading);

        mpCPManager->ADIGpDevReading =
            min(max(mpCPManager->ADIGpDevReading, -HORIZONTAL_SCALE * 0.5F),
                HORIZONTAL_SCALE * 0.5F);
        mpCPManager->ADIGsDevReading =
            min(max(mpCPManager->ADIGsDevReading, -VERTICAL_SCALE * 0.5F),
                VERTICAL_SCALE * 0.5F);

        mVertBarPos = (unsigned)abs(
            FloatToInt32(mHorizScale * mpCPManager->ADIGpDevReading +
                         mHorizCenter)); // Calc location of deviation bars
        mHorizBarPos = (unsigned)abs(FloatToInt32(
            mVertScale * -mpCPManager->ADIGsDevReading + mVertCenter));
    ...
    else
    {
        ...
        else if (mVertBarPos <= mDestRect.left and
        ...
        else
        {
            mpCPManager->ADIGpDevReading =
                mpCPManager->ADIGpDevReading +
                0.1F *
                    (-HORIZONTAL_SCALE * 1.5F - mpCPManager->ADIGpDevReading);
            mpCPManager->ADIGsDevReading =
                mpCPManager->ADIGsDevReading +
                0.1F * (-VERTICAL_SCALE * 1.5F - mpCPManager->ADIGsDevReading);
            ...
            mVertBarPos =
                max(mVertBarPos, mDestRect.left); // Bound the positions
...
void RenderADIPoly(
    tagRECT *srcrect, tagRECT *srcloc, tagRECT *destrect, GLint alpha,
    TextureHandle *pTex,
    float
        angle) //Wombat778 3-22-04 helper function to keep the displayblit3d tidy.
{
    int entry;
...
void CPAdi::DisplayBlit3D()
{
    ...
    else if (DisplayOptions.bRender2DCockpit)
    {
        ...
        srcRect.top =
            (LONG)(mSrcRect.top + mSrcHalfHeight - mRadius - (int)mSlide);
        srcRect.left = (LONG)(mSrcRect.left);
        srcRect.bottom =
            (LONG)(srcRect.top + (mDestRect.bottom - mDestRect.top) / mVScale);
        srcRect.right =
            (LONG)(srcRect.left + (mDestRect.right - mDestRect.left) / mHScale);

        if (g_bFilter2DPit) //Wombat778 3-30-04 Add option to filter
            RenderADIPoly(&mSrcRect, &srcRect, &DestRect, STATE_CHROMA_TEXTURE,
                          m_arrTex[0], -mRoll);
        else
            RenderADIPoly(&mSrcRect, &srcRect, &DestRect,
                          STATE_ALPHA_TEXTURE_NOFILTER, m_arrTex[0], -mRoll);
    }
}
...
void CPAdi::DisplayBlit(void)
{
          ...
          1.0f))) //dont run this code if the var is set but no scaling is occuring
    {
        ...
        ADIBuffer->ComposeRoundRot(
            mpTemplate, &srcRect, &temprect, -mRoll,
            mpADICircle); //Rotate the image from template to temp buffer
        mpOTWImage->ComposeTransparent(ADIBuffer, &temprect, &mDestRect);
    }

    else
    {
        mpOTWImage->ComposeRoundRot(mpTemplate, &srcRect, &mDestRect, -mRoll,
                                    mpADICircle);
    }
...
void CPAdi::DisplayDraw(void)
{
    ...
    for (i = 0; i < NUM_AC_BAR_POINTS - 1; i++)
    {
        OTWDriver.renderer->Render2DLine(
            (float)mpAircraftBar[i][0], (float)mpAircraftBar[i][1],
            (float)mpAircraftBar[i + 1][0], (float)mpAircraftBar[i + 1][1]);
    }
    ...
    if (not mpCPManager->mHiddenFlag)
    {
        ...
        OTWDriver.renderer->Render2DTri(
            (float)mVertBarPos, (float)mTopLimit - 3.0F * width,
            (float)mVertBarPos + width, (float)mTopLimit - 3.0F * width,
            (float)mVertBarPos + width, (float)mBottomLimit + 3.0F * width);
        OTWDriver.renderer->Render2DTri(
            (float)mVertBarPos, (float)mTopLimit - 3.0F * width,
            (float)mVertBarPos, (float)mBottomLimit + 3.0F * width,
            (float)mVertBarPos + width, (float)mBottomLimit + 3.0F * width);
        ...
        //OTWDriver.renderer->SetColor(mColor[OTWDriver.renderer->GetGreenMode() not_eq 0][1]);
        OTWDriver.renderer->Render2DTri(
            (float)mLeftLimit - 3.0F * width, (float)mHorizBarPos,
            (float)mLeftLimit - 3.0F * width, (float)mHorizBarPos + width,
            (float)mRightLimit + 3.0F * width, (float)mHorizBarPos + width);
        OTWDriver.renderer->Render2DTri(
            (float)mLeftLimit - 3.0F * width, (float)mHorizBarPos,
            (float)mRightLimit + 3.0F * width, (float)mHorizBarPos,
            (float)mRightLimit + 3.0F * width, (float)mHorizBarPos + width);
        //OTWDriver.renderer->SetColor(mColor[OTWDriver.renderer->GetGreenMode() not_eq 0][2]);
        OTWDriver.renderer->Render2DTri(
            (float)mLeftLimit - 3.0F * width, (float)mHorizBarPos,
            (float)mLeftLimit - 3.0F * width, (float)mHorizBarPos - 2.0F,
            (float)mRightLimit + 3.0F * width,
            (float)mHorizBarPos - width); //MI (float)mHorizBarPos - 2.0F);
        OTWDriver.renderer->Render2DTri(
            (float)mLeftLimit - 3.0F * width, (float)mHorizBarPos,
            (float)mRightLimit + 3.0F * width, (float)mHorizBarPos,
            (float)mRightLimit + 3.0F * width,
