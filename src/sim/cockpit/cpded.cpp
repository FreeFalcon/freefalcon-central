...
CPDed::CPDed(ObjectInitStr *pobjectInitStr, DedInitStr *pdedInitStr)
    : CPObject(pobjectInitStr)
{
    ...
    if (not g_bRealisticAvionics)
        ...
        yOffset = (mDestRect.bottom - mDestRect.top) /
                  6; // 5 lines of DED text but spaced tighter
...
void CPDed::DisplayDraw(void)
{
    ...
    if (playerAC and playerAC->mFaults and
    ...
    if (mpCPManager->mpIcp)
    {
        if (not g_bRealisticAvionics)
        {
            if (mDedType == DEDT_DED)
            {
                ...
                OTWDriver.renderer->SetViewport(ViewPort.left, ViewPort.top,
                                                ViewPort.right,
                                                ViewPort.bottom);
                ...
                VirtualDisplay::SetFont(
                    mpCPManager
                        ->DEDFont()); // dedfont from 16_ckpit.dat (test: =2 -> Ded_Font_d)
...
#else
                OTWDriver.renderer->ScreenText((float)mpLinePos[0].left + 1.0F,
                                               (float)mpLinePos[0].top, line1);
                OTWDriver.renderer->ScreenText((float)mpLinePos[1].left + 1.0F,
                                               (float)mpLinePos[2].top, line2);
                OTWDriver.renderer->ScreenText((float)mpLinePos[2].left + 1.0F,
                                               (float)mpLinePos[4].top, line3);
#endif
        ...
        else
        {
            ...
            if (not OTWDriver.renderer->GetGreenMode())
                OTWDriver.renderer->SetColor(
                    mColor[0]); //FF009FFF 00? 00B 00G 00R
            else
                OTWDriver.renderer->SetColor(mColor[1]);
            ...
            if (mDedType == DEDT_DED)
            {
                ...
                if ((g_bDEDSpacingFix) or
                    (DisplayOptions.DispWidth >
                     1024)) //Wombat778 12-12-2003 Changed to allow automatic selection of the fix at high resolutions
                    stepx =
                        VirtualDisplay::pFontSet
                            ->fontData[VirtualDisplay::pFontSet->fontNum][32]
                            .pixelWidth;
                else
                    stepx = 5.0F;
                ...
                for (int j = 0; j < 5; j++)
                {
                    ...
                    for (int i = 0; i < 26; i++)
                    {
                        ...
                        if (buf[0] not_eq ' ' and
                            mpCPManager->mpIcp->Invert[j][i] == 0)
                            OTWDriver.renderer->ScreenText(
                                x, y, buf, mpCPManager->mpIcp->Invert[j][i]);
            ...
            else
            {
                ...
                for (int j = 0; j < 5; j++)
                {
                    ...
                    for (int i = 0; i < 26; i++)
                    {
                        ...
                        if (buf[0] not_eq ' ' and
                            mpCPManager->mpIcp->PFLInvert[j][i] == 0)
                            OTWDriver.renderer->ScreenText(
                                x, y, buf, mpCPManager->mpIcp->PFLInvert[j][i]);
