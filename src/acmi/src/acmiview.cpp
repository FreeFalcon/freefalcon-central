...
extern LRESULT CALLBACK WndProc(HWND hwnd, UINT iMsg, WPARAM wParam,
                                LPARAM lParam);
LRESULT CALLBACK ACMIWndProc(HWND hwnd, UINT message, WPARAM wParam,
                             LPARAM lParam);
...
void ACMIView::SetObjectName(SimBaseClass* theObject, char* tmpStr)
{
    ...
    if (classPtr->dataType == DTYPE_VEHICLE)
    {
        ...
        sprintf(((VehicleClassDataType*)(classPtr->dataPtr))->Name, "%s",
                tmpStr);
    }
    else if (classPtr->dataType == DTYPE_WEAPON)
    {
        //sprintf(tmpStr, "%s",((WeaponClassDataType*)(classPtr->dataPtr))->Name);
        sprintf(((WeaponClassDataType*)(classPtr->dataPtr))->Name, "%s",
                tmpStr);
    }
}
...
void ACMIView::SetupEntityUIMappings()
{
    ...
    for (i = 0; i < numEntities; i++)
    {
        ...
        ((DrawableBSP*)(ep->objBase->drawPointer))
            ->SetLabel(
                _entityUIMappings[i].name,
                ((DrawableBSP*)(ep->objBase->drawPointer))->LabelColor());
    }
}
...
BOOL ACMIView::LoadTape(char* fname, BOOL reload)
{
    ...
    if (not _tape->IsLoaded())
    {
        ...
        MonoPrint("InitACMIFile() --> Could not load ACMI Tape: %s.\n",
                  _fileName);
...
void ACMIView::GetObjectName(SimBaseClass* theObject, char* tmpStr)
{
    ...
    if (classPtr->dataType == DTYPE_VEHICLE or
        classPtr->dataType == DTYPE_WEAPON)
    {
...
void ACMIView::InitGraphics(C_Window* win)
{
    ...
    if (_doWireFrame == FALSE)
    {
        ...
        _renderer->SetObjectTextureState(
            TRUE); //PlayerOptions.ObjectTexturesOn());
    }
    else
    {
        _viewPoint->Setup(20.0f * FEET_PER_KM, 0, 2, 0.0f);
        _renderer->Setup(gMainHandler->GetFront(), _viewPoint);
