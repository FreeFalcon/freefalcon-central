...
public:
    ...
    void SetFlightBg(short x, short y, short w, short h)
    {
        FlightBg_.left = x;
        FlightBg_.top = y;
        FlightBg_.right = w;
    ...
    void SetTask(short x, short y, LISTBOX *tasklist)
    {
        TaskX_ = x;
        TaskY_ = y;
    ...
    void SetIcon(short x, short y, IMAGE_RSC *Image)
    {
        if (Icon_)
        {
            Icon_->SetXY(x, y);
    ...
    void SetCallsign(short x, short y, _TCHAR *str)
    {
        if (Callsign_)
        {
            Callsign_->SetXY(x, y);
            Callsign_->SetText(str);
            Callsign_->SetInfo();
        }
    }
    void SetAirbase(short x, short y, _TCHAR *str)
    {
        if (Airbase_)
        {
            Airbase_->SetXY(x, y);
            Airbase_->SetText(str);
            Airbase_->SetInfo();
        }
    }
    void SetPlanes(short x, short y, _TCHAR *str)
    {
        if (Planes_)
        {
            Planes_->SetXY(x, y);
    ...
    void SetStatus(short x, short y, _TCHAR *str)
    {
        if (Status_)
        {
            Status_->SetXY(x, y);
    ...
    void SetFlightBgColor(COLORREF Off, COLORREF On)
    {
        FlightBgColor_[0] = Off;
        FlightBgColor_[1] = On;
    }
