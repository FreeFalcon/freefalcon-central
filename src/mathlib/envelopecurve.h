#ifndef _ENVELOPE_CURVE_H
#define _ENVELOPE_CURVE_H
//------------------------------------------------------------------------------
//  CLASS
...
{   
    ...
    {
        value += _sin(pos * _PI * 2 * this->frequency) * this->amplitude;
    }
    else
    {
        value += _cos(pos * _PI * 2 * this->frequency) * this->amplitude;
    }
...
float nEnvelopeCurve::GetMaxPossibleValue() const
{
    ...
    {
        maxVal = _max(maxVal, this->keyFrameValues[keyFrame]);
    }
