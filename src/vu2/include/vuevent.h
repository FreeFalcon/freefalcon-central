...
public:
    ...
    {
        return MemAllocPtr(gVuMsgMemPool, size, FALSE);
    };
...
public:
    ...
    VU_ID Destination() const
    {
        return tgtid_;
    }
    VU_BOOL IsLocal() const;
    VU_ID EntityId() const
    {
        return entityId_;
    }
    VU_BYTE Flags() const
    ...
    VuTargetEntity *Target() const
    {
        return target_.get();
    }

    void SetPostTime(VU_TIME posttime)
    {
        postTime_ = posttime;
    }
...
public:
    ...
    VU_BOOL WasCreated()
    {
        return (VU_BOOL)(Entity() == expandedData_ ? TRUE : FALSE);
    }
    void MarkAsKeepalive()
    {
        flags_ or_eq VU_KEEPALIVE_MSG_FLAG;
