...
static VuEntityType vuTypeTable[] = {
    {
        ...
        0,
        0.0f,                    // collision type, radius
        {0, 0, 0, 0, 0, 0, 0, 0},   // class info
        0,
        0,                       // update rate, tolerance
        0.0f,
        0.0f,
        0.0f,           // fine update range/force/multiplier
        0,
        0,                       // damage seed, hitpoints
        0,
        0,                       // major/minor rev numbers
    ...
    },
    {
        ...
        0.0f,
        0.0f,
        1.0f,             // fine update range/force/multiplier
        0,
        0,                         // damage seed, hitpoints
        0,
        0,                         // major/minor rev numbers
...
void VuEntity::SetEntityType(ushort entityType)
{
    ...
    if (share_.flags_.breakdown_.persistent_ and
        share_.flags_.breakdown_.transfer_)
    {
...
{
    if ((newdriver not_eq NULL) and
        (F4IsBadReadPtr(newdriver, sizeof(*newdriver))))
    {
...
#define VUENTITY_LOCALSIZE                                                     \
    (sizeof(share_.entityType_) + sizeof(share_.flags_) +                      \
     sizeof(share_.id_.creator_) + sizeof(share_.id_.num_) +                   \
     sizeof(share_.ownerId_.creator_) + sizeof(share_.ownerId_.num_) +         \
     sizeof(share_.assoc_.creator_) + sizeof(share_.assoc_.num_) +             \
     sizeof(pos_) + sizeof(orient_))
    ...
    if (stream)
    {
        if (*stream)
        {
            ...
            *stream += sizeof(share_.ownerId_.creator_);
            memcpy(*stream, &share_.ownerId_.num_,
                   sizeof(share_.ownerId_.num_));
            *stream += sizeof(share_.ownerId_.num_);
            memcpy(*stream, &share_.assoc_.creator_,
        ...
        if (sender->IsTarget())
        {
            ...
            else if ((1 << Domain()) bitand vuLocalSessionEntity->DomainMask())
            {
                    ...
                    assoc->OwnerId() not_eq vuLocalSession)
                {
                    ...
                    VuMessage* resp =
                        new VuErrorMessage(VU_TRANSFER_ASSOCIATION_ERROR,
    ...
    if (sender)
    {
        if (sender->IsTarget())
        {
            ...
            else
            {
                ...
                resp = new VuErrorMessage(VU_DONT_MANAGE_ENTITY_ERROR,
                                          msg->Sender(), Id(), sender);
            }
...
VU_BOOL
VuEntity::CollisionCheck(VuEntity* other, SM_SCALAR dt)
{
    SM_SCALAR size =
        EntityType()->collisionRadius_ + other->EntityType()->collisionRadius_;
    ...
    if (Id() not_eq other->Id())
    {
        ...
        if (vuabs(ZPos() - other->ZPos()) >
            size + vuabs(dt * (ZDelta() - other->ZDelta())))
            return FALSE;

        if (vuabs(YPos() - other->YPos()) >
            size + vuabs(dt * (YDelta() - other->YDelta())))
            return FALSE;

        if (vuabs(XPos() - other->XPos()) >
            size + vuabs(dt * (XDelta() - other->XDelta())))
            return FALSE;
...
VU_BOOL
VuEntity::CustomCollisionCheck(VuEntity* other, SM_SCALAR dt)
{
...
static inline VU_BOOL collisionLineEntity1D(BIG_SCALAR d1, BIG_SCALAR d2,
                                            BIG_SCALAR objd, BIG_SCALAR radius)
{
...
VU_BOOL
VuEntity::LineCollisionCheck(BIG_SCALAR x1, BIG_SCALAR y1, BIG_SCALAR z1,
                             BIG_SCALAR x2, BIG_SCALAR y2, BIG_SCALAR z2,
                             SM_SCALAR dt, SM_SCALAR sizeFactor)
{
    BIG_SCALAR mult = sizeFactor * dt;
    SM_SCALAR size = EntityType()->collisionRadius_;

    if (collisionLineEntity1D(z1, z2, ZPos(),
                              (size + vuabs(mult * ZDelta()))) and
        collisionLineEntity1D(y1, y2, YPos(),
                              (size + vuabs(mult * YDelta()))) and
