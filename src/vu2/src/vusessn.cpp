...
{
    VuPacketHeader(VU_ID tid) : targetId_(tid)
    {
    }
    VuPacketHeader()
    {
...
//-----------------------------------------------------------------------------
static int MessageReceive(VU_ID targetId,
                          VU_ADDRESS senderAddress, // sfr added address
                          VU_ID senderid, VU_MSG_TYPE type, VU_BYTE** data,
                          int size, VU_TIME timestamp)
{
    VuMessage* event = 0;
    ...
    if (vuLocalSessionEntity->InTarget(targetId))
    {
        ...
        case VU_BROADCAST_GLOBAL_EVENT:
            event =
                new VuBroadcastGlobalEvent(senderAddress, senderid, targetId);
            break;
    ...
    while (TRUE)
    {
        if ((length = ComAPIGet(ch)) > 0)
        {
            ...
            while (data < end)
            {
                ...
                count +=
                    MessageReceive(phdr.targetId_, senderAddress, senderid,
                                   mhdr.type_, &data, mhdr.length_, timestamp);
            }
        ...
        else if (length == -2)
        {
            ...
            {
                VuFullUpdateEvent* msg = new VuFullUpdateEvent(
                    vuLocalSessionEntity.get(), vuGlobalGroup);
...
VuSessionsIterator::VuSessionsIterator(VuGroupEntity* group)
    : VuListIterator(group ?
                         group->sessionCollection_ :
                         (vuLocalSessionEntity->Game() ?
                              vuLocalSessionEntity->Game()->sessionCollection_ :
                              0))
{
}
...
VuSessionFilter::VuSessionFilter(VuSessionFilter* other)
    : VuFilter(other), groupId_(other->groupId_)
{
    // empty
}
...
{
    return static_cast<VU_BOOL>(
        (ent->IsSession() and ((VuSessionEntity*)ent)->GameId() == groupId_) ?
            TRUE :
            FALSE);
}
...
// VuTargetEntity
//-----------------------------------------------------------------------------
VuTargetEntity::VuTargetEntity(ushort typeindex, VU_ID_NUMBER eid)
    : VuEntity(typeindex, eid)
{
...
VuTargetEntity::VuTargetEntity(VU_BYTE** stream, long* rem)
    : VuEntity((ushort)VU_UNKNOWN_ENTITY_TYPE, 0)
{
    Init(&bestEffortComms_);
    Init(&reliableComms_);
    reliableComms_.reliable_ = TRUE;
...
{
    return sizeof(share_.entityType_) + sizeof(share_.flags_) +
...
int VuTargetEntity::Save(FILE* file)
{
    ...
    {
        retval +=
            fwrite(&share_.entityType_, sizeof(share_.entityType_), 1, file);
        retval += fwrite(&share_.flags_, sizeof(share_.flags_), 1, file);
        retval +=
            fwrite(&share_.id_.creator_, sizeof(share_.id_.creator_), 1, file);
...
int VuTargetEntity::BytesPending()
{
     ...
     */
    if (GetReliableCommsStatus() not_eq VU_CONN_INACTIVE and
        GetReliableCommsHandle())
    {
...
void VuTargetEntity::SetCommsHandle(com_API_handle ch, int bufSize,
                                    int packSize)
{
    ...
    if (ch and bufSize > 0)
    {
        ...
        bestEffortComms_.normalSendPacket_ =
            new VU_BYTE[bufSize + MIN_HDR_SIZE];
...
void VuTargetEntity::SetReliableCommsHandle(com_API_handle ch, int bufSize,
                                            int packSize)
{
    unsigned long chbufsize = 0;
    ...
    if (reliableComms_.handle_ and reliableComms_.status_ == VU_CONN_ACTIVE)
    {
        ...
        else if (ping > reliableComms_.ping)
        {
            ...
            VuSessionEntity* session =
                (VuSessionEntity*)vuDatabase->Find(this->OwnerId());

            if (vuLocalGame->OwnerId() not_eq vuLocalSessionEntity->Id() or
                session == vuLocalSessionEntity)
            {
                session = NULL; //we are not host or its us self
            }

            if (session and ((FalconSessionEntity*)vuDatabase->Find(this->Id()))
                                    ->GetFlyState() not_eq FLYSTATE_FLYING)
            {
                session = NULL; //session is not in cockpit
            }

            if (session and ((FalconSessionEntity*)vuDatabase->Find(
...
int VuTargetEntity::SendOutOfBand(VuCommsContext* ctxt, VuMessage* msg)
{
    ...
    if (size > 0)
    {
        retval = ComAPISendOOB(ctxt->handle_, buffer - start_buffer,
                               mhdr.CAPIType());
    }
...
int VuTargetEntity::SendNormalPriority(VuCommsContext* ctxt, VuMessage* msg)
{
    ...
    if (totsize > ctxt->maxMsgSize_)
    {
        ...
        (
            ...
            totsize + ctxt->normalSendPacketPtr_ - ctxt->normalSendPacket_ >
                ctxt->maxPackSize_ or
            (
                // different origins
                ctxt->normalSendPacketPtr_ not_eq ctxt->normalSendPacket_ and
                (msg->Sender().creator_ not_eq
                     ctxt->normalPendingSenderId_.creator_ or
    ...
    else
    {
        ...
        if (ctxt->normalSendPacketPtr_ == ctxt->normalSendPacket_)
        {
            // begin a new packet
            VuPacketHeader phdr(msg->Destination());
            ctxt->normalSendPacketPtr_ +=
                WritePacketHeader(ctxt->normalSendPacketPtr_, &phdr);
        }

        VuMessageHeader mhdr(msg->Type(), static_cast<short>(size));
        ctxt->normalSendPacketPtr_ +=
            WriteMessageHeader(ctxt->normalSendPacketPtr_, &mhdr);
...
int VuTargetEntity::SendLowPriority(VuCommsContext* ctxt, VuMessage* msg)
{
    ...
    else if (ctxt->lowSendPacketPtr_ not_eq ctxt->lowSendPacket_ and
             (
                 // data is waiting in queue
                 totsize + ctxt->lowSendPacketPtr_ - ctxt->lowSendPacket_ >
                     ctxt->maxPackSize_ or
                 (
                     // write would exceed packet size
                     ...
                     (
                         // different origins
                         msg->Sender().creator_ not_eq
                             ctxt->lowPendingSenderId_.creator_ or
                         msg->Destination() not_eq
                             ctxt->lowPendingSendTargetId_))))
    {
        if (FlushLow(ctxt) > 0 and
            ctxt->lowSendPacketPtr_ == ctxt->lowSendPacket_)
        {
            // try resending...
    ...
    {
        // write fits within buffer -- add to/begin packet
        if (ctxt->lowSendPacketPtr_ ==
            ctxt->lowSendPacket_) // begin a new packet
        {
            VuPacketHeader phdr(msg->Destination());
            ctxt->lowSendPacketPtr_ +=
                WritePacketHeader(ctxt->lowSendPacketPtr_, &phdr);
        }

        VuMessageHeader mhdr(msg->Type(), static_cast<short>(size));

        ctxt->lowSendPacketPtr_ +=
            WriteMessageHeader(ctxt->lowSendPacketPtr_, &mhdr);
...
int VuTargetEntity::SendMessage(VuMessage* msg)
{
    ...
    if ((msg->Flags() bitand VU_RELIABLE_MSG_FLAG) and
...
VuSessionEntity::VuSessionEntity(ushort typeindex, ulong domainMask,
                                 char* callsign)
    : VuTargetEntity(typeindex, VU_SESSION_ENTITY_ID),
      sessionId_(0), // assigned on session open
      address_(VU_ADDRESS(0, com_API_get_my_receive_port(),
                          com_API_get_my_reliable_receive_port())), //sfr
      domainMask_(domainMask), callsign_(0), loadMetric_(1),
      gameId_(VU_SESSION_NULL_GROUP), // assigned on session open
      groupCount_(0), groupHead_(0),
// bandwidth_(33600),
#ifdef VU_SIMPLE_LATENCY
      timeDelta_(0), latency_(0),
#endif //VU_SIMPLE_LATENCY
#ifdef VU_TRACK_LATENCY
      timeSyncState_(VU_NO_SYNC), latency_(0), masterTime_(0),
      masterTimePostTime_(0), responseTime_(0), masterTimeOwner_(0),
      lagTotal_(0), lagPackets_(0), lagUpdate_(LAG_COUNT_START),
#endif //VU_TRACK_LATENCY
      lastMsgRecvd_(0), game_(0), action_(VU_NO_GAME_ACTION)
{
...
VuSessionEntity::VuSessionEntity(VU_BYTE** stream, long* rem)
    : VuTargetEntity(stream, rem), sessionId_(0),
      address_(0), //sfr: converts added address
      gameId_(0, 0), groupCount_(0), groupHead_(0),
#ifdef VU_SIMPLE_LATENCY
      timeDelta_(0), latency_(0),
#endif //VU_SIMPLE_LATENCY
#ifdef VU_TRACK_LATENCY
      latency_(0), masterTime_(0), masterTimePostTime_(0), responseTime_(0),
      masterTimeOwner_(0), lagTotal_(0), lagPackets_(0),
      ...
      lastMsgRecvd_(0), game_(0), action_(VU_NO_GAME_ACTION)
{
    ...
    if (session)
    {
        ...
        VuTimingMessage* msg =
            new VuTimingMessage(vuLocalSession, (VuTargetEntity*)session);
...
VuSessionEntity::VuSessionEntity(FILE* file)
    : VuTargetEntity(file), sessionId_(0),
      address_(0), //sfr: converts added address
      gameId_(0, 0), groupCount_(0), groupHead_(0),
#ifdef VU_TRACK_LATENCY
      latency_(0), masterTime_(0), masterTimePostTime_(0), responseTime_(0),
...
{
    return sizeof(lastCollisionCheckTime_) + sizeof(sessionId_) +
           address_
               .Size() // #104: serialized address width (8), not sizeof(struct) which includes LP64 padding
           +
           DISK_LONG // #104: on-wire 32-bit domainMask_ (native ulong is 8 on LP64)
           + sizeof(loadMetric_)

           + sizeof(gameId_)
           //+ sizeof(VU_ID)

           + sizeof(groupCount_) + (groupCount_ * sizeof(VU_ID))
#ifdef VU_SIMPLE_LATENCY
           + sizeof(VU_TIME) + sizeof(VU_TIME)
#endif
#ifdef VU_TRACK_LATENCY
           + sizeof(timeSyncState_) + sizeof(latency_) + sizeof(masterTime_) +
           sizeof(masterTimePostTime_) + sizeof(responseTime_) +
           sizeof(masterTimeOwner_)
#endif
           + strlen(callsign_) +
           1
           // camera
           + sizeof(unsigned char) + (cameras_.size() * sizeof(VU_ID));
}
...
int VuSessionEntity::Save(FILE* file)
{
    ...
    if (file)
    {
        ...
        {
            unsigned int _t32 = (unsigned int)domainMask_;
            retval += fwrite(&_t32, sizeof(int), 1, file);
        } // #104: on-disk 32-bit ulong
        ...
        retval +=
            fwrite(&masterTimePostTime_, sizeof(masterTimePostTime_), 1, file);
...
int VuSessionEntity::RemoveCamera(VuEntity* e, bool silent)
{
    ...
    for (VuEntityVectorIterator it = cameras_.begin(); it not_eq cameras_.end();
         ++it)
    {
        VuEntity* ce = it->get();
...
void VuSessionEntity::SetCallsign(const char* callsign)
{
    ...
    if (oldsign)
    {
        ...
        {
            VuSessionEvent* event = new VuSessionEvent(
                this, VU_SESSION_CHANGE_CALLSIGN, vuGlobalGroup);
            VuMessageQueue::PostVuMessage(event);
        }
    }
}
...
VU_ERRCODE VuSessionEntity::InsertionCallback()
{
    if (this not_eq vuLocalSessionEntity)
    {
        ...
        {
            VuGroupEntity* group =
                (VuGroupEntity*)vuDatabase->Find(gnode->gid_);
...
VU_SESSION_ID VuSessionEntity::OpenSession()
{
    ...
    if (sessionId_ not_eq vuLocalSession.creator_)
    {
        ...
        {
            if ((ent->OwnerId() == vuLocalSession) and
                (ent not_eq vuPlayerPoolGroup) and (ent not_eq vuGlobalGroup))
            {
...
void VuSessionEntity::CloseSession()
{
    ...
    if (IsLocal())
    {
        ...
        VuSessionEvent* event =
            new VuSessionEvent(this, VU_SESSION_CLOSE, vuGlobalGroup);
        VuMessageQueue::PostVuMessage(event);
        VuMessageQueue::FlushAllQueues();
    ...
    else
    {
        ...
        for (VuEntity *ent = iter.GetFirst(), *nextEnt; ent not_eq NULL;
             ent = nextEnt)
        {
            nextEnt = iter.GetNext();

            // checks if entity belongs to session
            if ((ent not_eq this) and (ent->OwnerId() == Id()) and
                (ent->IsGlobal()))
            {
                if ((sess == NULL) or (ent->IsTransferrable() == 0) or
                    ((ent->IsGame() and
                      ((VuGameEntity*)ent)->SessionCount() == 0)))
                {
        ...
        if (IsLocal())
        {
                ...
                newgroup not_eq vuGlobalGroup)
            {
                ...
                VuTargetEntity* target =
                    (VuTargetEntity*)vuDatabase->Find(newgroup->OwnerId());
...
VU_ERRCODE VuSessionEntity::LeaveGroup(VuGroupEntity* group)
{
    ...
    if (group)
    {
        ...
        if (retval == VU_SUCCESS and group->RemoveSession(this) >= 0 and
            IsLocal())
        {
            VuSessionEvent* event =
                new VuSessionEvent(this, VU_SESSION_LEAVE_GROUP, vuGlobalGroup);
            VuMessageQueue::PostVuMessage(event);
            retval = VU_SUCCESS;
        }
    }
    ...
    if (game)
    {
        if (game->RemoveSession(this) >= 0)
        {
            ...
            if (IsLocal())
            {
                ...
                if (newgame)
                {
                    event = new VuSessionEvent(this, VU_SESSION_CHANGE_GAME,
                                               vuGlobalGroup);
                    event->group_ = newgame->Id();
                }
                else
                {
                    event = new VuSessionEvent(this, VU_SESSION_CLOSE,
                                               vuGlobalGroup);
                    gameId_ = VU_SESSION_NULL_GROUP;
                    game_.reset();
                }
...
#define VU_JOIN_DONT_UPDATE 0
    ...
    if (newgame)
    {
        ...
        if (IsLocal())
        {
            ...
            if (not game)
            {
                event = new VuSessionEvent(this, VU_SESSION_JOIN_GAME,
                                           vuGlobalGroup);
...
void VuSessionEntity::SetTimeSync(VU_BYTE newstate)
{
    ...
    if (timeSyncState_ not_eq oldstate and timeSyncState_ not_eq VU_NO_SYNC and
        this == vuLocalSessionEntity)
    {
        VuMessage* msg = 0;

        if (timeSyncState_ == VU_MASTER_SYNC and Game() and
            Game()->OwnerId() not_eq vuLocalSession)
        {
...
VU_ERRCODE VuSessionEntity::Handle(VuFullUpdateEvent* event)
{
    ...
    if (event->EventData())
    {
        ...
        if (this not_eq vuLocalSessionEntity)
        {
            VuSessionEntity* sData =
                static_cast<VuSessionEntity*>(event->EventData());
            ...
            if (vuLocalSessionEntity->TimeSyncState() == VU_MASTER_SYNC)
            {
                    ...
                    latency_ == sData->Latency())
                {
                    ...
                    int lag =
                        ((event->PostTime() - sData->masterTime_) -
                    ...
                    if (lagPackets_ >= lagUpdate_)
                    {
                        ...
                        {
                            VuSessionEvent* event = new VuSessionEvent(
                                this, VU_SESSION_LATENCY_NOTICE, Game());
...
void VuSessionEntity::EnqueuePositionUpdate(SM_SCALAR distance,
                                            VuEntity* entity)
{
...
void VuSessionEntity::SendBestEnqueuedPositionUpdatesAndClear(unsigned int qty,
                                                              VU_TIME timestamp)
{
    // send OOB
    for (VuOobPositionUpdateQ::iterator it = oobPositionUpdateQ_.begin();
         it not_eq oobPositionUpdateQ_.end(); ++it)
    {
        VuEntity* e = *it;
        ...
        if (not e->EnqueuedForPositionUpdate())
        {
            ...
            static_cast<VuMaster*>(e->EntityDriver())
                ->GeneratePositionUpdate(false, true, timestamp, this);
    ...
    // respect BW
    for (VuPositionUpdateQ::iterator it = positionUpdateQ_.begin();
         it not_eq positionUpdateQ_.end() and (qty-- > 0); ++it)
    {
        ...
        if (not e->EnqueuedForPositionUpdate())
        {
            ...
            static_cast<VuMaster*>(e->EntityDriver())
                ->GeneratePositionUpdate(false, false, timestamp, this);
        }
    }
...
VuGroupEntity::VuGroupEntity(char* groupname)
    : VuTargetEntity(VU_GROUP_ENTITY_TYPE, VuxGetId()), groupName_(0),
...
VuGroupEntity::VuGroupEntity(int type, char* gamename, VuFilter* filter)
    : VuTargetEntity(type, VuxGetId()), groupName_(0),
      sessionMax_(VU_DEFAULT_GROUP_SIZE)
{
...
int VuGroupEntity::LocalSize()
{
    ...
    return sizeof(share_.ownerId_.creator_) + sizeof(share_.ownerId_.num_) +
           sizeof(share_.assoc_.creator_) + sizeof(share_.assoc_.num_) +
           strlen(groupName_) + 1 + sizeof(sessionMax_) + sizeof(count) +
...
int VuGroupEntity::Save(FILE* file)
{
    ...
    if (file)
    {
        ...
        // VuEntity part
        retval += fwrite(&share_.ownerId_.creator_,
                         sizeof(share_.ownerId_.creator_), 1, file);
        retval += fwrite(&share_.ownerId_.num_, sizeof(share_.ownerId_.num_), 1,
                         file);
        retval += fwrite(&share_.assoc_.creator_,
                         sizeof(share_.assoc_.creator_), 1, file);
        retval +=
            fwrite(&share_.assoc_.num_, sizeof(share_.assoc_.num_), 1, file);
...
VuGameEntity::VuGameEntity(ulong domainMask, char* gamename)
    : VuGroupEntity(VU_GAME_ENTITY_TYPE, VU_GAME_GROUP_NAME),
      domainMask_(domainMask), gameName_(0)
{
...
VuGameEntity::VuGameEntity(int type, ulong domainMask, char* gamename,
                           char* groupname)
    : VuGroupEntity(type, groupname), domainMask_(domainMask), gameName_(0)
{
    // every local game is sent imediately
    SetSendCreate(VuEntity::VU_SC_SEND_OOB);
    gameName_ = new char[strlen(gamename) + 1];
...
VuGameEntity::VuGameEntity(FILE* file) : VuGroupEntity(file)
{
    VU_BYTE len = 0;
...
int VuGameEntity::LocalSize()
{
#ifdef VU_TRACK_LATENCY
    short count = sessionCollection_->Count();
#endif

    return DISK_LONG // #104: on-wire 32-bit domainMask_ (native ulong is 8 on LP64)
           + strlen(gameName_) + 1
#ifdef VU_TRACK_LATENCY
           + sizeof(count) + (sizeof(VU_TIME) * count) // session latency
#endif //VU_TRACK_LATENCY
        ;
}

int VuGameEntity::SaveSize()
{
    return VuGroupEntity::SaveSize() + VuGameEntity::LocalSize();
}

int VuGameEntity::Save(VU_BYTE** stream)
{
    VU_BYTE len;
...
int VuGameEntity::Save(FILE* file)
{
    int retval = 0;
    VU_BYTE len;
...
VU_ERRCODE VuGameEntity::RemovalCallback()
{
    ...
    for (VuSessionEntity *sess = iter.GetFirst(), *nextSess;
         sess and (sess->Game() == this); sess = nextSess)
    {
    ...
    if (not sess or (Id() == sess->GameId()))
    {
        ...
        if (not cs)
        {
            ...
            else
            {
                ...
                while (ent)
                {
                    ...
                    if ((ent->OwnerId().creator_ == sess->SessionId()) and
                        (sess not_eq ent) and
        ...
        else if (Id() == vuLocalSessionEntity->GameId())
        {
            ...
            while (ent)
            {
                ...
                if (not ent->IsTransferrable() and sess and
                    sess->VuState() not_eq VU_MEM_ACTIVE and
                    ent->OwnerId().creator_ == sess->SessionId())
                {
                    vuDatabase->Remove(ent);
                         ...
                         ((1 << ent->Domain()) bitand
                          vuLocalSessionEntity->DomainMask()))
                {
...
VU_ERRCODE VuPlayerPoolGame::Distribute(VuSessionEntity* sess)
{
    ...
    if (sess and (Id() == sess->GameId()))
    {
        ...
        for (VuEntity *ent = dbiter.GetFirst(), *next; ent not_eq NULL;
             ent = next)
        {
            next = dbiter.GetNext();
