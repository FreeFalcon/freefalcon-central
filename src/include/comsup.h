...
namespace ComSup
{
template <class T> class SingleAutoLock
{
    T *m_pT;
    bool m_bAquired;
...
inline HRESULT CoInitializeEx(void *pvReserved, DWORD dwCoInit)
{
    HRESULT(__stdcall * pprocCoInitializeEx)(void *pvReserved, DWORD dwCoInit) =
        (HRESULT(__stdcall *)(void *pvReserved, DWORD dwCoInit))GetProcAddress(
            GetModuleHandle("ole32.dll"), "CoInitializeEx");

    if (pprocCoInitializeEx)
        return pprocCoInitializeEx(pvReserved, dwCoInit);
    else
        return E_NOTIMPL;
}
...
inline HRESULT CoInitializeSecurity(PSECURITY_DESCRIPTOR pVoid, DWORD cAuthSvc,
                                    ...
                                    DWORD dwCapabilities, void *pvReserved2)
{
    HRESULT(__stdcall * pprocCoInitializeSecurity)(
        PSECURITY_DESCRIPTOR, DWORD, SOLE_AUTHENTICATION_SERVICE *, void *,
        DWORD, DWORD, RPC_AUTH_IDENTITY_HANDLE, DWORD, void *) =
        (HRESULT(__stdcall *)(
            PSECURITY_DESCRIPTOR, DWORD, SOLE_AUTHENTICATION_SERVICE *, void *,
            DWORD, DWORD, RPC_AUTH_IDENTITY_HANDLE, DWORD,
            void *))GetProcAddress(GetModuleHandle("ole32.dll"),
                                   "CoInitializeSecurity");

    if (pprocCoInitializeSecurity)
        return pprocCoInitializeSecurity(pVoid, cAuthSvc, asAuthSvc, pReserved1,
                                         dwAuthnLevel, dwImpLevel, pAuthInfo,
                                         dwCapabilities, pvReserved2);
    else
        return E_NOTIMPL;
}
...
inline HRESULT CoCreateInstanceEx(REFCLSID rclsid, IUnknown *punkOuter,
                                  ...
                                  ULONG cmq, MULTI_QI *rgmqResults)
{
    HRESULT(__stdcall * pprocCoCreateInstanceEx)(
        REFCLSID, IUnknown *, DWORD, COSERVERINFO *, ULONG, MULTI_QI *) =
        (HRESULT(__stdcall *)(REFCLSID, IUnknown *, DWORD, COSERVERINFO *,
                              ULONG, MULTI_QI *))
            GetProcAddress(GetModuleHandle("ole32.dll"), "CoCreateInstanceEx");

    if (pprocCoCreateInstanceEx)
        return pprocCoCreateInstanceEx(rclsid, punkOuter, dwClsCtx, pServerInfo,
                                       cmq, rgmqResults);
    else
        return E_NOTIMPL;
}
...
inline HRESULT RegisterServer(HINSTANCE hInst)
{
    ...
    HRESULT(__stdcall * pprocDLLRegisterServer)() =
        (HRESULT(__stdcall *)())GetProcAddress(hInst, "DllRegisterServer");

    if (not pprocDLLRegisterServer)
        return -2;
...
inline HRESULT UnregisterServer(HINSTANCE hInst)
{
    ...
    HRESULT(__stdcall * pprocDLLUnregisterServer)() =
        (HRESULT(__stdcall *)())GetProcAddress(hInst, "DllUnregisterServer");

    if (not pprocDLLUnregisterServer)
        return -2;
...
inline void ReportException(_com_error &e)
{
    ...
    if ((LPSTR)bstrDescription)
        strMsg += (LPSTR)bstrDescription;
    else if (e.ErrorMessage())
        strMsg += e.ErrorMessage();
    else
        strMsg += "Unspecified Error.";

    ::MessageBox(NULL, strMsg.c_str(), "Information",
                 MB_OK bitor MB_SETFOREGROUND bitor MB_ICONINFORMATION);
}
