...
#define FIND_FINDUNSPOTTED                                                     \
    0x80 // Returns somewhat lesser score for unspotted stuff
#define FIND_THISOBJONLY                                                       \
    0x100 // Find children within this secondary objective only
...
extern CampaignHeading DirectionTo(GridIndex ox, GridIndex oy, GridIndex tx,
                                   GridIndex ty);
...
extern CampaignTime TimeBetween(GridIndex x, GridIndex y, GridIndex tx,
                                GridIndex ty, int speed);
...
extern Unit FindNearestRealUnit(GridIndex X, GridIndex Y, float *last,
                                GridIndex max);

extern Unit FindNearestUnit(VuFilteredList *l, GridIndex X, GridIndex Y,
                            float *last);

extern Unit FindNearestUnit(GridIndex X, GridIndex Y, float *last);

extern Unit FindUnitByXY(VuFilteredList *l, GridIndex X, GridIndex Y,
                         int domain);
...
extern Objective FindNearestObjective(VuFilteredList *l, GridIndex X,
                                      GridIndex Y, float *last);
...
extern Objective FindNearestFriendlyObjective(VuFilteredList *l, Team who,
                                              GridIndex *x, GridIndex *y,
                                              int flags);

extern Objective FindNearestFriendlyObjective(Team who, GridIndex *x,
                                              GridIndex *y, int flags);

extern Objective FindNearestFriendlyPowerStation(VuFilteredList *l, Team who,
                                                 GridIndex x, GridIndex y);

extern Objective GetObjectiveByXY(GridIndex X, GridIndex Y);

extern int AnalyseThreats(GridIndex X, GridIndex Y, MoveType mt, int alt,
                          int roe_check, Team who, int flags);

extern int CollectThreats(GridIndex X, GridIndex Y, int Z, Team who, int flags,
                          F4PFList foundlist);

extern int CollectThreatsFast(GridIndex X, GridIndex Y, int altlevel, Team who,
...
extern CampaignTime TimeTo(GridIndex x, GridIndex y, GridIndex tx, GridIndex ty,
                           int speed);
