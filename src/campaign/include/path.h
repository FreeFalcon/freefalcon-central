...
#define PATH_ENEMYOK                                                           \
    0x04 // Ok to move through enemy territory (private implimentation)
#define PATH_ENEMYCOST                                                         \
    0x08 // Additional cost for moving through enemy territory
...
#define PATH_ENGINEER                                                          \
    0x80 // Allows engineers to move into tiles with broken bridges
...
extern int GetGridPath(Path p, GridIndex x, GridIndex y, GridIndex xx,
                       GridIndex yy, int type, int who, int flags);

extern costtype GetPathCost(GridIndex x, GridIndex y, Path path, MoveType mt,
                            int flags);

extern costtype GetPathCost(Objective o, Path path, MoveType mt, int flags);

extern int GetObjectivePath(Path p, Objective o, Objective t, int type, int who,
                            int flags);

extern int GetObjectivePath(Path p, GridIndex x, GridIndex y, GridIndex xx,
                            GridIndex yy, int type, int who, int flags);

extern int FindLinkPath(Path p, Objective O1, Objective O2, MoveType mt);

extern float GetMovementCost(GridIndex x, GridIndex y, MoveType move, int flags,
                             CampaignHeading h);

extern float GetObjectiveMovementCost(Objective o, int neighbor, MoveType type,
                                      Team team, int flags);
