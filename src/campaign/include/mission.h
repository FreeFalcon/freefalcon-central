...
enum MissionRollEnum
{
    ...
    ARO_GA =
        4, // AMIS_SAD bitor AMIS_BAI bitor AMIS_ONCALLCAS bitor AMIS_PRPLANCAS   // Ground attack
    ...
    ARO_TRANS =
        11, // AMIS_AIRBORNE bitor AMIS_AIRLIFT           // Drop off cargo
...
enum MissionDataFlagEnum
{
    ...
    AMIS_ADDOCASTRIKE =
        0x100, // Trigger friendly OCA Strike vs enemy assets found
    ...
    AMIS_ADDINTERCEPT =
        0x1000, // Add an enemy intercept mission to intercept this mission
    ...
    AMIS_DONT_COORD =
        0x100000, // Don't match ingress/egress times with the main flight
    AMIS_AIR_LAUNCH_OK =
        0x200000, // This mission can launch in mid-air (ie: from the edge of the map)
    ...
    AMIS_FUDGE_RANGE =
        0x1000000, // This mission is always in range - we'll just load more fuel
    AMIS_EXPECT_DIVERT =
        0x2000000, // We want to sit around until we're told to divert
    AMIS_ASSIGNED_TAR =
        0x4000000, // Used for diverts with a specific target in mind
    AMIS_NO_DIST_BONUS =
        0x8000000, // Don't adjust mission priority by distance to front (add a constant)
    AMIS_NO_TARGETABORT =
        0x10000000, // Don't abort after reaching our target - just fly home
...
{
    REQF_USERESERVES =
        0x01, // Use reserve aircraft for this request, if nessesary (mostly for support)
    REQF_CHECKED = 0x02, // This request has already been checked at least once
    REQF_NEEDRESPONSE =
        0x04, // This needs to be dealt with immediately, and returns a response
    ...
    REQF_PART_OF_ACTION =
        0x400, // This target is part of a larger action by this team
    REQF_ALLOW_ERRORS =
        0x800, // Allow missions to be planned with errors (for tactical engagement)
    REQF_TE_MISSION =
        0x1000, // This is a TE mission, correct mission length calculation mission.cpp
};
...
public:
    ...
    CampaignTime tot;
    GridIndex tx, ty;
    // #104 (Linux LP64): this record is bulk fread/fwrite'd and VU-streamed as a whole
    // struct (sizeof(MissionRequestClass)). 'ulong' is 4 bytes in the on-disk/reference
    // (Win LLP64) ABI but 8 bytes on Linux LP64, which both grows sizeof AND inserts 4
    // bytes of 8-byte alignment padding before the field -- desyncing every mission-request
    // read. That shifts the FILE position after each ATM, so teams saved after the first
    // one carrying an ATO (e.g. South Korea after US in a Korea TE) decode garbage and never
    // register into TeamInfo[]. Pin to 32 bits to match the on-disk layout (flags fit in 32).
    unsigned int flags;
    short caps;
    short target_num;
    short speed;
...
struct MissionDataType
{
    ...
    uchar
        min_time; // Minimum # of minutes in advance to consider a planned mission
    uchar
        max_time; // Maximum # of minutes in advance to consider a planned mission
...
extern int BuildPathToTarget(Flight u, MissionRequestClass *mis,
                             VU_ID airbaseID);
...
extern long SetWPTimesTanker(Flight u, MissionRequestClass *mis, bool type,
                             CampaignTime time);
...
extern int TargetThreats(Team team, int priority, F4PFList list, MoveType mt,
