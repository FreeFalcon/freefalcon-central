...
{
    if (xPos >= but->left and xPos <= but->right and yPos >= but->top and
        yPos <= but->bottom)
        return 1;
    else
        return 0;
...
void UpdateNames(HWND hDlg, Objective O)
{
    ...
    if (i < 1)
    {
        ...
        SetDlgItemText(hDlg, IDC_OBJ_NAMEVAL,
                       ReadNameString(O->GetObjectiveNameID(), buffer, 79));
    ...
    else
    {
        ...
        i = SendMessage(
            GetDlgItem(hDlg, IDC_OBJ_NAMECOMBO), CB_FINDSTRINGEXACT, 0,
            (LPARAM)ReadNameString(O->GetObjectiveNameID(), buffer, 79));
...
void UpdateSTypeCombo(HWND hDlg, Objective O)
{
    ...
    SendMessage(GetDlgItem(hDlg, IDC_OBJ_STYPEVAL), CB_ADDSTRING, 0,
                (LPARAM) "Vanilla");
    i = 1;
    j = -1;
    ...
    SendMessage(GetDlgItem(hDlg, IDC_OBJ_STYPEVAL), CB_SETCURSEL, O->GetSType(),
                0);
}
    ...
    case WM_INITDIALOG: /* message: initialize dialog box */
        ...
        for (i = 1; i <= NumObjectiveTypes; i++)
        {
            ...
            if (j)
            {
                ...
                if (oc)
                    SendMessage(GetDlgItem(hDlg, IDC_OBJ_TYPEVAL), CB_ADDSTRING,
                                0, (LPARAM)ObjectiveStr[i]);
            }
            else
                SendMessage(GetDlgItem(hDlg, IDC_OBJ_TYPEVAL), CB_ADDSTRING, 0,
                            (LPARAM) "<Davism>");
        }
        ...
        {
            SendMessage(GetDlgItem(hDlg, IDC_OBJ_TYPEVAL), CB_GETLBTEXT, i,
                        (LPARAM)buffer);
            ...
            if (ObjTypeConverter[i] == O->GetType())
                SendMessage(GetDlgItem(hDlg, IDC_OBJ_TYPEVAL), CB_SETCURSEL, i,
                            0);
        }
        ...
        {
            SendMessage(GetDlgItem(hDlg, IDC_OBJ_OWNVAL), CB_ADDSTRING, 0,
                        (LPARAM)Side[i]);
            SendMessage(GetDlgItem(hDlg, IDC_OBJ_OCCVAL), CB_ADDSTRING, 0,
                        (LPARAM)Side[i]);
        }

        SendMessage(GetDlgItem(hDlg, IDC_OBJ_OWNVAL), CB_SETCURSEL,
                    O->GetObjectiveOldown(), 0);
        SendMessage(GetDlgItem(hDlg, IDC_OBJ_OCCVAL), CB_SETCURSEL,
                    O->GetOwner(), 0);
        unique = 0;
        ...
        // Fill Name Combo box
        SendMessage(GetDlgItem(hDlg, IDC_OBJ_NAMECOMBO), CB_ADDSTRING, 0,
                    (LPARAM) "<None>");
        ...
        for (i = 2; i < NameEntries; i++)
        {
            ...
            if (sptr[0] and sptr[0] not_eq '<')
                SendMessage(GetDlgItem(hDlg, IDC_OBJ_NAMECOMBO), CB_ADDSTRING,
                            0, (LPARAM)ReadNameString(i, buffer, 79));
        }
        ...
        if (i > 0)
        {
            i = SendMessage(GetDlgItem(hDlg, IDC_OBJ_NAMECOMBO),
                            CB_FINDSTRINGEXACT, 0,
                            (LPARAM)ReadNameString(i, buffer, 79));
            SendMessage(GetDlgItem(hDlg, IDC_OBJ_NAMECOMBO), CB_SETCURSEL, i,
                        0);
        }
        else
            SendMessage(GetDlgItem(hDlg, IDC_OBJ_NAMECOMBO), CB_SETCURSEL, 0,
                        0);
        ...
        if (GlobList)
        {
            ...
            if (o)
            {
                GlobObj = o;
                GlobList->Remove(o);
                DialogBox(hInst, MAKEINTRESOURCE(IDD_OBJECTIVEDIALOG), hDlg,
                          (DLGPROC)EditObjective);
                GlobObj = O;
            }
        }
    ...
    case WM_COMMAND: /* message: received command */
        ...
        case IDCANCEL:
            ...
            if (renaming)
                SendMessage(hDlg, WM_COMMAND,
                            IDC_OBJ_NAMECOMBO bitor (CBN_KILLFOCUS << 16), 0);
        ...
        case IDC_OBJ_NAMECOMBO:
            if (HIWORD(wParam) == CBN_SELENDOK)
            {
                ...
                if (i > 1)
                {
                    SendMessage(GetDlgItem(hDlg, IDC_OBJ_NAMECOMBO),
                                CB_GETLBTEXT, i, (LPARAM)buffer);
            ...
            if (HIWORD(wParam) == CBN_KILLFOCUS and renaming)
            {
                ...
                SendMessage(GetDlgItem(hDlg, IDC_OBJ_NAMECOMBO), CB_ADDSTRING,
                            0, (LPARAM)buffer);
                j = SendMessage(GetDlgItem(hDlg, IDC_OBJ_NAMECOMBO),
                                CB_GETCURSEL, 0, 0);
                i = O->GetObjectiveNameID();
                ...
                if (i > 1)
                {
                    ...
                    j = SendMessage(GetDlgItem(hDlg, IDC_OBJ_NAMECOMBO),
                                    CB_FINDSTRINGEXACT, 0,
                                    (LPARAM)ReadNameString(i, last, 79));
                    SendMessage(GetDlgItem(hDlg, IDC_OBJ_NAMECOMBO),
                                CB_DELETESTRING, j, 0);
                    SetName(i, buffer);
                }
        ...
        case IDC_OBJ_TYPEVAL:
            if (HIWORD(wParam) == CBN_SELENDOK)
            {
                i = SendMessage(GetDlgItem(hDlg, IDC_OBJ_TYPEVAL), CB_GETCURSEL,
                                0, 0);
        ...
        case IDC_OBJ_STYPEVAL:
            if (HIWORD(wParam) == CBN_SELENDOK)
            {
                i = SendMessage(GetDlgItem(hDlg, IDC_OBJ_STYPEVAL),
                                CB_GETCURSEL, 0, 0);
        ...
        case IDC_OBJ_OWNVAL:
            if (HIWORD(wParam) == CBN_SELENDOK)
            {
                i = SendMessage(GetDlgItem(hDlg, IDC_OBJ_OWNVAL), CB_GETCURSEL,
                                0, 0);
        ...
        case IDC_OBJ_OCCVAL:
            if (HIWORD(wParam) == CBN_SELENDOK)
            {
                i = SendMessage(GetDlgItem(hDlg, IDC_OBJ_OCCVAL), CB_GETCURSEL,
                                0, 0);
        ...
        case IDC_OBJ_REMOVE:
            i = SendMessage(GetDlgItem(hDlg, IDC_OBJ_NAMECOMBO), CB_GETCURSEL,
                            0, 0);

            if (i > 0)
            {
                SendMessage(GetDlgItem(hDlg, IDC_OBJ_NAMECOMBO), CB_GETLBTEXT,
                            i, (LPARAM)buffer);
                SendMessage(GetDlgItem(hDlg, IDC_OBJ_NAMECOMBO),
                            CB_DELETESTRING, i, 0);
                SendMessage(GetDlgItem(hDlg, IDC_OBJ_NAMECOMBO), CB_SETCURSEL,
                            0, 0);
                RemoveName(buffer);
            }
...
BOOL WINAPI MissionTriggerProc(HWND hDlg, UINT message, WPARAM wParam,
                               LPARAM lParam)
{
    RECT rect;
    ...
    case WM_INITDIALOG: /* message: initialize dialog box */
        ...
        for (i = 0; i < AMIS_OTHER; i++)
            SendMessage(GetDlgItem(hDlg, IDC_MT_MISSIONCOMBO), CB_ADDSTRING, 0,
                        (LPARAM)MissStr[i]);
        ...
        SendMessage(GetDlgItem(hDlg, IDC_MT_TARGETTYPECOMBO), WM_SETREDRAW, 0,
                    0);

        for (i = 0; i < 3; i++)
            SendMessage(GetDlgItem(hDlg, IDC_MT_TARGETTYPECOMBO), CB_ADDSTRING,
                        0, (LPARAM)TargetTypeStr[i]);

        SendMessage(GetDlgItem(hDlg, IDC_MT_TARGETTYPECOMBO), CB_SETCURSEL, 0,
                    0);
        SendMessage(GetDlgItem(hDlg, IDC_MT_TARGETTYPECOMBO), WM_SETREDRAW, 1,
                    0);

        for (i = 0; i < 7; i++)
            SendMessage(GetDlgItem(hDlg, IDC_MT_TYPECOMBO), CB_ADDSTRING, 0,
                        (LPARAM)TOTStr[i]);
        ...
        for (i = 0; i < NUM_TEAMS; i++)
        {
            ...
            SendMessage(GetDlgItem(hDlg, IDC_MT_TEAMCOMBO), CB_ADDSTRING, 0,
                        (LPARAM)buffer);
        }
    ...
    case WM_COMMAND: /* message: received a command */
        ...
        case IDC_MT_MISSIONCOMBO:
            if (HIWORD(wParam) == CBN_SELENDOK)
                mission = SendMessage(GetDlgItem(hDlg, IDC_MT_MISSIONCOMBO),
                                      CB_GETCURSEL, 0, 0);
        ...
        case IDC_MT_TYPECOMBO:
            if (HIWORD(wParam) == CBN_SELENDOK)
                tottype = SendMessage(GetDlgItem(hDlg, IDC_MT_TYPECOMBO),
                                      CB_GETCURSEL, 0, 0);
        ...
        case IDC_MT_TARGETTYPECOMBO:
            ...
            {
                targettype =
                    SendMessage(GetDlgItem(hDlg, IDC_MT_TARGETTYPECOMBO),
                                CB_GETCURSEL, 0, 0);
        ...
        case IDC_MT_TEAMCOMBO:
            if (HIWORD(wParam) == CBN_SELENDOK)
            {
                toteam = SendMessage(GetDlgItem(hDlg, IDC_MT_TEAMCOMBO),
                                     CB_GETCURSEL, 0, 0);
...
BOOL WINAPI WeatherEditProc(HWND hDlg, UINT message, WPARAM wParam,
                            LPARAM lParam)
{
    RECT rect;
    static int head;
    ...
    case WM_INITDIALOG: /* message: initialize dialog box */
        ...
        sprintf(buffer, "%d",
                ((WeatherClass*)realWeather)->GetCloudCover(CurX, CurY));
        SetWindowText(GetDlgItem(hDlg, IDC_WD_COVEREDIT), buffer);
        sprintf(buffer, "%d",
                ((WeatherClass*)realWeather)->GetCloudLevel(CurX, CurY));
    ...
    case WM_COMMAND: /* message: received a command */
        ...
        case IDC_WD_WHVAL:
            ...
            {
                head = i;
                ((WeatherClass*)realWeather)->windHeading =
                    (float)(i * PI / 180.0F);
            }
    ...
    case WM_INITDIALOG: /* message: initialize dialog box */
    {
        ...
        for (i = 1; i < NUM_TEAMS; i++)
        {
            ...
            if (TeamInfo[i]->HasSatelites())
                PostMessage(GetDlgItem(hDlg, (IDC_TEAM_SAT1 - 1 + i)),
                            BM_SETCHECK, 1, 0);

            if (TeamInfo[i]->flags bitand TEAM_ACTIVE)
                PostMessage(GetDlgItem(hDlg, (IDC_TEAM_ACT1 - 1 + i)),
                            BM_SETCHECK, 1, 0);
        }
...
BOOL WINAPI EditTeams(HWND hDlg, UINT message, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_INITDIALOG: /* message: initialize dialog box */
    {
        ...
        // Team combo box
        for (i = 0; i < NUM_TEAMS; i++)
            SendMessage(GetDlgItem(hDlg, IDC_TEAM_COMBO), CB_ADDSTRING, 0,
                        (LPARAM)TeamInfo[i]->GetName());
        ...
        // Set up the copy
        tempTeam = new TeamClass(GetClassID(DOMAIN_ABSTRACT, CLASS_MANAGER,
                                            TYPE_TEAM, 0, 0, 0, 0, 0) +
                                     VU_LAST_ENTITY_TYPE,
                                 0);
        ...
        if (GetUpdateRect(hDlg, &rect, FALSE))
        {
            ...
            for (i = 0; i < NUM_COUNS; i++)
            {
                if (tempTeam->member[i])
                    PostMessage(GetDlgItem(hDlg, (IDC_COUNTRY_MEM_0 + i)),
                                BM_SETCHECK, 1, 0);
                else
                    PostMessage(GetDlgItem(hDlg, (IDC_COUNTRY_MEM_0 + i)),
            ...
            SendMessage(GetDlgItem(hDlg, IDC_OBJPRICOMBO), CB_RESETCONTENT, 0,
                        0);

            for (i = 0; i < MAX_TGTTYPE; i++)
            {
                sprintf(buffer, "%s:   %d", ObjectiveStr[i],
                        tempTeam->GetObjTypePriority(i));
                SendMessage(GetDlgItem(hDlg, IDC_OBJPRICOMBO), CB_ADDSTRING, 0,
                            (LPARAM)buffer);
            }
            ...
            // Obj priority combo box
            SendMessage(GetDlgItem(hDlg, IDC_UNITPRICOMBO), CB_RESETCONTENT, 0,
                        0);

            for (i = 0; i < MAX_UNITTYPE; i++)
            {
                sprintf(buffer, "%s:   %d", GroundSTypesStr[i],
                        tempTeam->GetUnitTypePriority(i));
                SendMessage(GetDlgItem(hDlg, IDC_UNITPRICOMBO), CB_ADDSTRING, 0,
                            (LPARAM)buffer);
            }

            SendMessage(GetDlgItem(hDlg, IDC_UNITPRICOMBO), CB_SETCURSEL, su,
                        0);
            ...
            // Obj priority combo box
            SendMessage(GetDlgItem(hDlg, IDC_MISSPRICOMBO), CB_RESETCONTENT, 0,
                        0);

            for (i = 0; i < AMIS_OTHER; i++)
            {
                sprintf(buffer, "%s:   %d", MissStr[i],
                        tempTeam->GetMissionPriority(i));
                SendMessage(GetDlgItem(hDlg, IDC_MISSPRICOMBO), CB_ADDSTRING, 0,
                            (LPARAM)buffer);
            }

            SendMessage(GetDlgItem(hDlg, IDC_MISSPRICOMBO), CB_SETCURSEL, sm,
                        0);
    ...
    case WM_COMMAND: /* message: received a command */
        ...
        case IDC_TEAM_COMBO:
            if (HIWORD(wParam) == CBN_SELENDOK)
            {
                ...
                {
                    memcpy(TeamInfo[tempTeam->who], tempTeam,
                           sizeof(TeamClass));
...
BOOL WINAPI MapDialogProc(HWND hDlg, UINT message, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_PAINT:
    {
        ...
        case IDC_MAP_SAMS3:
            ...
            if (not map_data)
            {
                TheCampaign.SamMapData =
                    MakeCampMap(MAP_SAMCOVERAGE, TheCampaign.SamMapData, 0);
                map_data = TheCampaign.SamMapData;
            }
        ...
        case IDC_MAP_RADAR3:
            ...
            if (not map_data)
            {
                TheCampaign.RadarMapData =
                    MakeCampMap(MAP_RADARCOVERAGE, TheCampaign.RadarMapData, 0);
                map_data = TheCampaign.RadarMapData;
            }
    ...
    case WM_COMMAND: /* message: received a command */
        ...
        case IDC_MAP_RADAR3:
            ...
            RedrawWindow(hDlg, NULL, NULL,
                         RDW_INTERNALPAINT bitor RDW_INVALIDATE);
            break;
...
BOOL WINAPI AdjustForceRatioProc(HWND hDlg, UINT message, WPARAM wParam,
                                 LPARAM lParam)
{
    switch (message)
    {
    case WM_INITDIALOG: /* message: initialize dialog box */
        SendMessage(GetDlgItem(hDlg, IDC_SLIDER1), TBM_SETRANGE, 0,
                    (LPARAM)MAKELONG(0, 4));
        SendMessage(GetDlgItem(hDlg, IDC_SLIDER2), TBM_SETRANGE, 0,
                    (LPARAM)MAKELONG(0, 4));
        SendMessage(GetDlgItem(hDlg, IDC_SLIDER3), TBM_SETRANGE, 0,
                    (LPARAM)MAKELONG(0, 4));
        SendMessage(GetDlgItem(hDlg, IDC_SLIDER4), TBM_SETRANGE, 0,
                    (LPARAM)MAKELONG(0, 4));
        SendMessage(GetDlgItem(hDlg, IDC_SLIDER1), TBM_SETPOS, (WPARAM)TRUE,
                    (LPARAM)2);
        SendMessage(GetDlgItem(hDlg, IDC_SLIDER2), TBM_SETPOS, (WPARAM)TRUE,
                    (LPARAM)2);
        SendMessage(GetDlgItem(hDlg, IDC_SLIDER3), TBM_SETPOS, (WPARAM)TRUE,
                    (LPARAM)2);
        SendMessage(GetDlgItem(hDlg, IDC_SLIDER4), TBM_SETPOS, (WPARAM)TRUE,
                    (LPARAM)2);
        return (TRUE);
        break;
    ...
    case WM_COMMAND: /* message: received a command */
        ...
        {
        case IDOK:
            TheCampaign.GroundRatio = static_cast<short>(
                SendMessage(GetDlgItem(hDlg, IDC_SLIDER1), TBM_GETPOS, 0, 0));
            TheCampaign.AirRatio = static_cast<short>(
                SendMessage(GetDlgItem(hDlg, IDC_SLIDER2), TBM_GETPOS, 0, 0));
            TheCampaign.NavalRatio = static_cast<short>(
                SendMessage(GetDlgItem(hDlg, IDC_SLIDER3), TBM_GETPOS, 0, 0));
            TheCampaign.AirDefenseRatio = static_cast<short>(
                SendMessage(GetDlgItem(hDlg, IDC_SLIDER4), TBM_GETPOS, 0, 0));
            EndDialog(hDlg, TRUE); /* Exits the dialog box        */
            return (TRUE);
...
BOOL WINAPI CampClipperProc(HWND hDlg, UINT message, WPARAM wParam,
                            LPARAM lParam)
{
...

            {
                ...
                while (ent)
                {
                    ...
                    if (x >= left and x <= right and y >= bottom and y <= top)
                    {
                        ...
                        {
                            for (i = 0;
                                 i < ((ObjectiveClass*)ent)->GetTotalFeatures();
                                 i++)
                            {
                                for (j = 0; j < 7; j++)
                                {
                                    k = Falcon4ClassTable[((ObjectiveClass*)ent)
                                                              ->GetFeatureID(i)]
                                            .visType[j];
...
BOOL WINAPI FistOfGod(HWND hDlg, UINT message, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_INITDIALOG: /* message: initialize dialog box */
        ReadIndexedString(
            300 + TheCampaign.MissionEvaluator->player_element->mission,
            tbuffer1, 512);
        ReadIndexedString(300 + TheCampaign.MissionEvaluator->package_mission,
                          tbuffer2, 512);
        sprintf(buffer, "%s (%s)", tbuffer1, tbuffer2);
        SetWindowText(GetDlgItem(hDlg, IDC_FIST_MISSION), buffer);

        SendMessage(GetDlgItem(hDlg, IDC_FIST_SUCCESS), CB_ADDSTRING, 0,
                    (LPARAM) "Horrible");
        SendMessage(GetDlgItem(hDlg, IDC_FIST_SUCCESS), CB_ADDSTRING, 0,
                    (LPARAM) "Poor");
        SendMessage(GetDlgItem(hDlg, IDC_FIST_SUCCESS), CB_ADDSTRING, 0,
        ...
        SendMessage(GetDlgItem(hDlg, IDC_FIST_SUCCESS2), CB_ADDSTRING, 0,
                    (LPARAM) "Failed");
        SendMessage(GetDlgItem(hDlg, IDC_FIST_SUCCESS2), CB_ADDSTRING, 0,
                    (LPARAM) "Partially Failed");
        SendMessage(GetDlgItem(hDlg, IDC_FIST_SUCCESS2), CB_ADDSTRING, 0,
                    (LPARAM) "Partially Accomplished");
        SendMessage(GetDlgItem(hDlg, IDC_FIST_SUCCESS2), CB_ADDSTRING, 0,
                    (LPARAM) "Accomplished");
    ...
    case WM_PAINT:
        target =
            FindEntity(TheCampaign.MissionEvaluator->player_element->target_id);
        ...
        if (target and target->GetClass() == CLASS_OBJECTIVE)
        {
            ...
            if (i < ((Objective)target)->GetTotalFeatures())
            {
                ...
                SetWindowText(GetDlgItem(hDlg, IDC_FIST_TARGET_FEATURE),
                              buffer);
            }

            sprintf(buffer, "%s (%d%%)", target->GetName(buffer, 40, FALSE),
                    ((Objective)target)->GetObjectiveStatus());
        }
        else if (target)
        {
            sprintf(buffer, "%s (%d vehs)",
                    target->GetFullName(buffer, 40, FALSE),
                    ((Unit)target)->GetTotalVehicles());
            target->GetFullName(buffer, 40, FALSE);
        }
        else
            sprintf(buffer, "None");
    ...
    case WM_COMMAND: /* message: received a command */
        ...
        case IDOK:
        {
            ...
            Objective o;
            Flight flight;
            rating = 10 * (SendMessage(GetDlgItem(hDlg, IDC_FIST_SUCCESS),
                                       CB_GETCURSEL, 0, 0) -
                           2);
            // KCK: Technically, we should probably use the parent PO if there is one, but whatever...
            o = FindNearestObjective(POList, TheCampaign.MissionEvaluator->tx,
                                     TheCampaign.MissionEvaluator->ty, NULL);

            if (o)
                ApplyPlayerInput(FalconLocalSession->GetTeam(), o->Id(),
                                 rating);
        ...
        {
            target = FindEntity(
                TheCampaign.MissionEvaluator->player_element->target_id);

            if (target)
            {
                if (target->IsObjective())
                {
                    uchar target_b = TheCampaign.MissionEvaluator
                                         ->player_element->target_building;

                    if (target_b >= ((Objective)target)->GetTotalFeatures() or
                        ((Objective)target)->GetFeatureStatus(target_b) ==
                            VIS_DESTROYED)
                    {
                        int count = 0;
                        target_b =
                            rand() % ((Objective)target)->GetTotalFeatures();

                        while (
                            count < 20 and
                            (((Objective)target)->GetFeatureStatus(target_b) ==
                                 VIS_DESTROYED or
                             not((Objective)target)->GetFeatureValue(target_b)))
                        {
                            count++;
                            target_b = rand() %
                                       ((Objective)target)->GetTotalFeatures();
                        }
                    }

                    ((Objective)target)
                        ->SetFeatureStatus(target_b, VIS_DESTROYED);
                ...
                else
                {
                    for (i = 0; i < 5; i++)
                    {
                        ...
                        while (count < 20 and
                               ((Unit)target)->GetNumVehicles(veh) == 0)
                        {
                            count++;
                            veh = rand() % VEHICLE_GROUPS_PER_UNIT;
                        }
                ...
                SetWindowText(GetDlgItem(hDlg, IDC_FIST_DESTROYED),
                              "Click here to smash target again");
            }
            else
            {
                // No target found
                SetWindowText(GetDlgItem(hDlg, IDC_FIST_DESTROYED),
                              "No target found.  No smashing done.");
            }
...
int DoACheatFlight(void)
{
...

    {
        ...
        {
            if (u->IsFlight() and ((Flight)u)->GetUnitSquadronID() ==
                                      FalconLocalSession->GetPlayerSquadronID())
            {
                ...
                if (gCheatPriority == 0 and
                    flight->GetUnitPriority() > best->GetUnitPriority())
                    best = flight;
                else if (gCheatPriority == 1 and
                         flight->GetUnitPriority() < best->GetUnitPriority())
                    best = flight;
...
BOOL WINAPI CheatTool(HWND hDlg, UINT message, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_INITDIALOG: /* message: initialize dialog box */
        ...
        SendMessage(GetDlgItem(hDlg, IDC_PRIORITY_COMBO), CB_ADDSTRING, 0,
                    (LPARAM) "Highest");
        SendMessage(GetDlgItem(hDlg, IDC_PRIORITY_COMBO), CB_ADDSTRING, 0,
                    (LPARAM) "Lowest");
        SendMessage(GetDlgItem(hDlg, IDC_PRIORITY_COMBO), CB_ADDSTRING, 0,
        ...
        SendMessage(GetDlgItem(hDlg, IDC_RESULT_COMBO), CB_ADDSTRING, 0,
                    (LPARAM) "Horrible");
        SendMessage(GetDlgItem(hDlg, IDC_RESULT_COMBO), CB_ADDSTRING, 0,
                    (LPARAM) "Poor");
        SendMessage(GetDlgItem(hDlg, IDC_RESULT_COMBO), CB_ADDSTRING, 0,
    ...
    case WM_COMMAND: /* message: received a command */
        ...
        case IDOK:
        {
            ...
            gCheatRating = 10 * (SendMessage(GetDlgItem(hDlg, IDC_RESULT_COMBO),
                                             CB_GETCURSEL, 0, 0) -
                                 2);
            gCheatPriority = SendMessage(GetDlgItem(hDlg, IDC_PRIORITY_COMBO),
                                         CB_GETCURSEL, 0, 0);
        ...
        case IDC_CHEATONOFF:
            ...
            {
                gCheatingOn = 0;
                PostMessage(GetDlgItem(hDlg, IDC_CHEATONOFF), BM_SETCHECK, 0,
                            0);
            ...
            {
                gCheatingOn = 1;
                PostMessage(GetDlgItem(hDlg, IDC_CHEATONOFF), BM_SETCHECK, 1,
                            0);
...
BOOL CALLBACK
FileOpenHookProc(HWND hDlg, /* window handle of the dialog box */
...
BOOL OpenCampFile(HWND hWnd)
{
    ...
    CampFileName.Flags = OFN_SHOWHELP bitor OFN_PATHMUSTEXIST bitor
                         OFN_FILEMUSTEXIST | OFN_NOCHANGEDIR bitor
                         OFN_HIDEREADONLY /* bitor OFN_ENABLEHOOK */ bitor
                         OFN_LONGNAMES;
...
BOOL OpenTheaterFile(HWND hWnd)
{
    ...
    TheaterFileName.Flags = OFN_SHOWHELP bitor OFN_PATHMUSTEXIST bitor
                            OFN_FILEMUSTEXIST bitor OFN_HIDEREADONLY bitor
                            OFN_ENABLEHOOK;
    TheaterFileName.lpfnHook =
        (LPOFNHOOKPROC)MakeProcInstance(FileOpenHookProc, NULL);
...
BOOL CALLBACK
FileSaveHookProc(HWND hDlg, /* window handle of the dialog box */
                 UINT message, /* type of message                 */
                 WPARAM wParam, /* message-specific information    */
                 LPARAM lParam)
{
...
BOOL SaveAsCampFile(HWND hWnd, int mode)
{
    ...
    CampFileName.lpstrFileTitle = cmpFileTitle;
    CampFileName.nMaxFileTitle = sizeof(cmpFileTitle);
    CampFileName.lpstrInitialDir =
        F4FindFile("savegame.dir", buf, 80, &offset, &len);
    CampFileName.nFileOffset = 0;
    CampFileName.nFileExtension = 0;
    CampFileName.lCustData = 0;
    CampFileName.Flags = OFN_ENABLEHOOK bitor OFN_HIDEREADONLY bitor
                         OFN_OVERWRITEPROMPT bitor OFN_NOCHANGEDIR;
    CampFileName.lpfnHook =
        (LPOFNHOOKPROC)MakeProcInstance(FileSaveHookProc, NULL);
...
BOOL SaveAsTheaterFile(HWND hWnd)
{
    ...
    TheaterFileName.lpstrFileTitle = thrFileTitle;
    TheaterFileName.nMaxFileTitle = sizeof(thrFileTitle);
    TheaterFileName.lpstrInitialDir =
        F4FindFile("theater.dir", buf, 80, &offset, &len);
    TheaterFileName.nFileOffset = 0;
    TheaterFileName.nFileExtension = 0;
    TheaterFileName.lCustData = 0;
    TheaterFileName.Flags = OFN_ENABLEHOOK bitor OFN_HIDEREADONLY bitor
                            OFN_OVERWRITEPROMPT bitor OFN_NOCHANGEDIR;
    TheaterFileName.lpfnHook =
        (LPOFNHOOKPROC)MakeProcInstance(FileSaveHookProc, NULL);
...
BOOL SaveAsScriptedUnitFile(HWND hWnd)
{
    ...
    this_file.lpstrInitialDir =
        F4FindFile("theater.dir", buf, 80, &offset, &len);
    this_file.nFileOffset = 0;
    this_file.nFileExtension = 0;
    this_file.lCustData = 0;
    this_file.Flags = OFN_ENABLEHOOK bitor OFN_HIDEREADONLY bitor
                      OFN_OVERWRITEPROMPT bitor OFN_NOCHANGEDIR;
    this_file.lpfnHook =
        (LPOFNHOOKPROC)MakeProcInstance(FileSaveHookProc, NULL);
