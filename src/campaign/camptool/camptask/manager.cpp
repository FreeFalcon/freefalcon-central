...
CampManagerClass::CampManagerClass(VU_BYTE **stream) : FalconEntity(VU_LAST_ENTITY_TYPE)
{
    ...
    *stream += sizeof(VU_ID);
    memcpy(&share_.ownerId_, *stream, sizeof(VU_ID));
    *stream += sizeof(VU_ID);
    memcpy(&share_.entityType_, *stream, sizeof(ushort));
    *stream += sizeof(ushort);
    SetEntityType(share_.entityType_);

    memcpy(&managerFlags, *stream, sizeof(short));
    *stream += sizeof(short);
    memcpy(&owner, *stream, sizeof(Team));
    *stream += sizeof(Team);
