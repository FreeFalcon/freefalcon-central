...
int DrawWeather = 0;
int ShowReal = 1;
GridIndex CurX, CurY, CenX, CenY, Movx, Movy, PBx, PBy, Sel1X, Sel1Y, Sel2X,
    Sel2Y;
costtype cost;
CampaignState StateToEdit = NULL;
ObjectiveType TempType;
Objective OneObjective, FromObjective, ToObjective, Neighbor, GlobObj,
    DragObjective = NULL;
...
LRESULT CALLBACK CampaignWndProc(HWND hwnd, UINT message, WPARAM wParam,
                                 LPARAM lParam);
...
void ReBltWnd(MapData md, HDC DC)
{
    ...
    BitBlt(md->hMapDC, r.left, r.top, r.right, r.bottom, DC, r.left, r.top,
           SRCCOPY);
...
void ShowWP(MapData md, GridIndex X, GridIndex Y, int color)
{
    ...
    DC = BeginPaint(md->hMapWnd, &ps);
    _setcolor(DC, color);
    _rectangle(DC, _GBORDER, POSX(X), POSY(Y), POSX(X) + md->CellSize * 4,
               POSY(Y) + md->CellSize * 4);
    EndPaint(md->hMapWnd, &ps);
}

void ShowWPLeg(MapData md, GridIndex x, GridIndex y, GridIndex X, GridIndex Y,
               int color)
{
    RECT r;
    PAINTSTRUCT ps;
...
void ShowPath(MapData md, GridIndex X, GridIndex Y, Path p, int color)
{
    ...
    for (i = 0; i < p->GetLength(); i++)
    {
        ...
        if (d >= 0)
        {
            x = X + dx[d] * step;
            y = Y + dy[d] * step;
            _lineto(DC, POSX(x) + (md->CellSize >> 1),
                    POSY(y) + (md->CellSize >> 1));
            X = x;
            Y = y;
        }
    }
        ...
        while (d)
        {
            ...
            while (u)
            {
                if (u->GetSType() == d->type)
                {
                    DisplayUnit(
                        DC, u,
                        (short)(POSX(d->x - 2) + (md->CellSize >> 3) * 5),
                        (short)(POSY(d->y + 2) + (md->CellSize >> 2) * 5),
                        (short)(md->CellSize >> 1) * 5);
                    u = NULL;
                }
                else
...
void RedrawUnit(Unit u)
{
    ...
    InvalidateRect(md->hMapWnd, &r, FALSE);
    DC = BeginPaint(md->hMapWnd, &ps);
    DisplayUnit(DC, u, (short)(POSX(x) + (md->CellSize >> 3)),
                (short)(POSY(y) + (md->CellSize >> 2)),
                (short)(md->CellSize >> 1));
    EndPaint(md->hMapWnd, &ps);
}
...
void RedrawCell(MapData md, GridIndex X, GridIndex Y)
{
    ...
    InvalidateRect(md->hMapWnd, &r, FALSE);
    DC = BeginPaint(md->hMapWnd, &ps);
    DisplayCellData(DC, X, Y, POSX(X), POSY(Y), md->CellSize, Mode, RoadsOn,
                    RailsOn);
    ...
    if (u)
        DisplayUnit(DC, u, (short)(POSX(X) + (md->CellSize >> 3) * scale),
                    (short)(POSY(Y) + (md->CellSize >> 2) * scale),
                    (short)((md->CellSize >> 1) * scale));
...
void ShowEmitters(MapData md, HDC DC)
{
    ...
    while (e)
    {
        ...
        if (range > 0)
        {
            e->GetLocation(&x, &y);
            DisplaySideRange(
                DC, e->GetOwner(), (short)(POSX(x) + (md->CellSize >> 1)),
                (short)(POSY(y) + (md->CellSize >> 1)), range * md->CellSize);
        }
...
void ShowSAMs(MapData md, HDC DC)
{
    ...
    while (e)
    {
        ...
        if (range > 0 and
            (e->IsObjective() or (e->GetDomain() not_eq DOMAIN_AIR and
                                  e->IsUnit() and not((Unit)e)->Moving())))
        {
            e->GetLocation(&x, &y);
            DisplaySideRange(
                DC, e->GetOwner(), (short)(POSX(x) + (md->CellSize >> 1)),
...
F4PFList GetSquadsFlightList(VU_ID id)
{
    ...
    while (u)
    {
        if (u->GetUnitParentID() == id and u->GetDomain() == DOMAIN_AIR and
            ...
            else if (u->IsSquadron())
            {
                ...
                for (i = 0; i < PILOTS_PER_SQUADRON; i++)
                {
                    ...
                    ((Squadron)u)
                        ->GetPilotData(i)
                        ->ResetStats(
                            TeamInfo[((Squadron)u)->GetOwner()]->airExperience);
...
void MatchObjectiveTypes(void)
{
    ...
    while (FromObjective not_eq NULL)
    {
        ...
        while (j)
        {
            j = GetClassID(DOMAIN_LAND, CLASS_OBJECTIVE, (uchar)type, i, 0, 0,
                           0, 0);
        ...
        if (not matches)
        {
            ...
            index = GetClassID(DOMAIN_LAND, CLASS_OBJECTIVE, (uchar)type, 1, 0,
                               0, 0, 0);
    ...
    for (i = 1; i < MAX_OBJ_TYPES; i++)
    {
        ...
        while (index)
        {
            index = GetClassID(DOMAIN_LAND, CLASS_OBJECTIVE, (uchar)i, (uchar)j,
                               0, 0, 0, 0);
            ...
            for (k = 0; k < MaxTextureType + 1 and index; k++)
            {
                ...
                if (counts[k * NumEntities + index] > 0)
                    sprintf(buffer,
                            "%s - %s on texture %s: %d - x: %d  y: %d\n",
                            ObjectiveStr[i], oc->Name, file,
                            counts[k * NumEntities + index],
                            locs[(k * NumEntities + index) * 2],
                            locs[(k * NumEntities + index) * 2 + 1]);
                else if (counts[k * NumEntities + index] < 0 and index)
                    sprintf(buffer,
                            "%s - %s on texture %s: %d - NEEDED x: %d  y: %d\n",
                            ObjectiveStr[i], oc->Name, file,
                            -counts[k * NumEntities + index],
                            locs[(k * NumEntities + index) * 2],
                            locs[(k * NumEntities + index) * 2 + 1]); //DSP
...
void AssignBattToBrigDiv(void)
{
    ...
    while (unit not_eq NULL)
    {
        if (unit->GetType() == TYPE_BRIGADE)
        {
            ...
            while (battalion)
            {
                battalion->SetUnitDivision(
                    ((GroundUnitClass *)unit)->GetUnitDivision());
                battalion = unit->GetNextUnitElement();
            }
        }
...
void StartUnitEdit(void)
{
    ...
    if (GlobUnit)
        DialogBox(hInst, MAKEINTRESOURCE(IDD_UNITDIALOG1), hMainWnd,
                  (DLGPROC)EditUnit);
...
void StartObjectiveEdit(void)
{
    ...
    CampLeaveCriticalSection();
    DialogBox(hInst, MAKEINTRESOURCE(IDD_OBJECTIVEDIALOG), hMainWnd,
...
void RefreshMap(MapData md, HDC DC, RECT *rect)
{
    ...
    for (y = NLY - 1; y >= NFY; y--)
        for (x = NFX; x < NLX; x++)
            DisplayCellData(DC, x, y, POSX(x), POSY(y), md->CellSize, Mode,
                            RoadsOn, RailsOn);
    ...
    if (md->ShowObjectives)
    {
        ...
        VuGridIterator oit(ObjProxList, (BIG_SCALAR)GridToSim(md->CenX),
                           (BIG_SCALAR)GridToSim(md->CenY),
                           (BIG_SCALAR)GridToSim((short)(Distance(
                               md->LX, md->LY, md->CenX, md->CenY))));
#else
        VuGridIterator oit(ObjProxList, (BIG_SCALAR)GridToSim(md->CenY),
                           (BIG_SCALAR)GridToSim(md->CenX),
                           (BIG_SCALAR)GridToSim((short)(Distance(
                               md->LX, md->LY, md->CenX, md->CenY))));
#endif
        int i;
                ...
                if (md->ShowLinks)
                {
                    ...
                    for (i = 0; i < OneObjective->NumLinks(); i++)
                    {
                        ...
                        if (Neighbor)
                        {
                            ...
                            _moveto(DC, POSX(x) + (md->CellSize >> 1),
                                    POSY(y) + (md->CellSize >> 1));
                            _lineto(DC, POSX(xx) + (md->CellSize >> 1),
                                    POSY(yy) + (md->CellSize >> 1));
                        }
                    }
                }

                DisplayObjective(DC, OneObjective, POSX(x), POSY(y),
                                 md->CellSize);
            }
    ...
    if (md->ShowUnits)
    {
        ...
        if (uit)
        {
            ...
            while (OneUnit not_eq NULL)
            {
                ...
                DisplayUnit(DC, OneUnit,
                            (short)(POSX(x + xd) + (md->CellSize >> 3) * scale),
                            (short)(POSY(y + yd) + (md->CellSize >> 2) * scale),
                            (short)((md->CellSize >> 1) * scale));
                ...
                if (ShowPaths and OneUnit->IsBattalion())
                {
                    if (OneUnit->GetType() == TYPE_BATTALION)
                    {
                        ...
                        ShowPath(md, x, y, ((Battalion)OneUnit)->path, White);
                        ShowPathHistory(md, x, y, ((Battalion)OneUnit)->path,
                                        Red);
                    }
                }
                ...
                if (ShowFlanks and OneUnit->GetDomain() == DOMAIN_LAND)
                {
                    ...
                    _moveto(DC, POSX(fx) + (md->CellSize >> 1),
                            POSY(fy) + (md->CellSize >> 1));
                    _lineto(DC, POSX(x) + (md->CellSize >> 1),
                            POSY(y) + (md->CellSize >> 1));
                    OneUnit->GetRightFlank(&fx, &fy);
                    _lineto(DC, POSX(fx) + (md->CellSize >> 1),
                            POSY(fy) + (md->CellSize >> 1));
                }
...
LRESULT CALLBACK ToolWndProc(HWND hwnd, UINT message, WPARAM wParam,
                             LPARAM lParam)
{
    LRESULT retval = 0;
    ...
    case WM_PAINT:
    {
        ...
        if (GetUpdateRect(hToolWnd, &r, FALSE))
        {
            ...
            if (OneObjective)
                sprintf(time, "%s               ",
                        OneObjective->GetName(buffer, 60, FALSE));
            else
                sprintf(time,
                        "                                                  ");
            ...
            if (OneUnit)
                if (OneUnit->GetType() == TYPE_BATTALION and
                    OneUnit->GetDomain() == DOMAIN_LAND)
                {
                    if (((GroundUnitClass *)OneUnit)->GetDivision())
                    {
                        ...
                        if (unit)
                            sprintf(time, "%d-%d-%s               ",
                                    ((GroundUnitClass *)OneUnit)->GetDivision(),
                                    unit->GetNameId(),
                                    OneUnit->GetName(buffer, 60, FALSE));
                        else
                            sprintf(time, "%d-%s               ",
                                    ((GroundUnitClass *)OneUnit)->GetDivision(),
                                    OneUnit->GetName(buffer, 60, FALSE));
                ...
                else if (OneUnit->GetType() == TYPE_BRIGADE and
                         OneUnit->GetDomain() == DOMAIN_LAND)
                {
                    if (((GroundUnitClass *)OneUnit)->GetDivision())
                    {
...
BOOL MainWndCommandProc(HWND hWndFrame, WPARAM wParam, LONG lParam)
{
    ...
    case ID_VIEW_BOTH:
        Mode = EditMode = 0;
        CheckMenuItem(GetMenu(hMainWnd), ID_VIEW_BOTH,
                      MF_BYCOMMAND bitor MF_CHECKED);
        CheckMenuItem(GetMenu(hMainWnd), ID_VIEW_COVER,
                      MF_BYCOMMAND bitor MF_UNCHECKED);
        CheckMenuItem(GetMenu(hMainWnd), ID_VIEW_RELIEF,
                      MF_BYCOMMAND bitor MF_UNCHECKED);
        CheckMenuItem(GetMenu(hMainWnd), ID_VIEW_CLOUDS,
                      MF_BYCOMMAND bitor MF_UNCHECKED);
        CheckMenuItem(GetMenu(hMainWnd), ID_VIEW_LEVELS,
                      MF_BYCOMMAND bitor MF_UNCHECKED);
        SetRefresh(MainMapData);
        break;
    ...
    case ID_VIEW_ROADS:
        ...
        if (RoadsOn)
            CheckMenuItem(GetMenu(hMainWnd), ID_VIEW_ROADS,
                          MF_BYCOMMAND bitor MF_CHECKED);
        else
            CheckMenuItem(GetMenu(hMainWnd), ID_VIEW_ROADS,
                          MF_BYCOMMAND bitor MF_UNCHECKED);
    ...
    case ID_VIEW_RAILS:
        ...
        if (RailsOn)
            CheckMenuItem(GetMenu(hMainWnd), ID_VIEW_RAILS,
                          MF_BYCOMMAND bitor MF_CHECKED);
        else
            CheckMenuItem(GetMenu(hMainWnd), ID_VIEW_RAILS,
                          MF_BYCOMMAND bitor MF_UNCHECKED);
    ...
    case ID_VIEW_EMITTERS:
        ...
        if (MainMapData->Emitters)
            CheckMenuItem(GetMenu(hMainWnd), ID_VIEW_EMITTERS,
                          MF_BYCOMMAND bitor MF_CHECKED);
        else
            CheckMenuItem(GetMenu(hMainWnd), ID_VIEW_EMITTERS,
                          MF_BYCOMMAND bitor MF_UNCHECKED);
    ...
    case ID_VIEW_SAMS:
        ...
        if (MainMapData->SAMs)
            CheckMenuItem(GetMenu(hMainWnd), ID_VIEW_SAMS,
                          MF_BYCOMMAND bitor MF_CHECKED);
        else
            CheckMenuItem(GetMenu(hMainWnd), ID_VIEW_SAMS,
                          MF_BYCOMMAND bitor MF_UNCHECKED);
    ...
    case ID_VIEW_PLAYERBUBBLE:
        ...
        if (PBubble)
            CheckMenuItem(GetMenu(hMainWnd), ID_VIEW_PLAYERBUBBLE,
                          MF_BYCOMMAND bitor MF_CHECKED);
        else
            CheckMenuItem(GetMenu(hMainWnd), ID_VIEW_PLAYERBUBBLE,
                          MF_BYCOMMAND bitor MF_UNCHECKED);
    ...
    case ID_VIEW_OBJPRIORITY:
        ObjMode = 1;
        CheckMenuItem(GetMenu(hMainWnd), ID_VIEW_OBJOWNER,
                      MF_BYCOMMAND bitor MF_UNCHECKED);
        CheckMenuItem(GetMenu(hMainWnd), ID_VIEW_OBJPRIORITY,
                      MF_BYCOMMAND bitor MF_CHECKED);
        CheckMenuItem(GetMenu(hMainWnd), ID_VIEW_OBJTYPE,
                      MF_BYCOMMAND bitor MF_UNCHECKED);
        SetRefresh(MainMapData);
        break;
    ...
    case ID_VIEW_REALUNITS:
        ShowReal = 1;
        CheckMenuItem(GetMenu(hMainWnd), ID_VIEW_REALUNITS,
                      MF_BYCOMMAND bitor MF_CHECKED);
        CheckMenuItem(GetMenu(hMainWnd), ID_VIEW_PARENTUNITS,
                      MF_BYCOMMAND bitor MF_UNCHECKED);
        CheckMenuItem(GetMenu(hMainWnd), ID_VIEW_REINFORCEMENTS,
                      MF_BYCOMMAND bitor MF_UNCHECKED);
        CheckMenuItem(GetMenu(hMainWnd), ID_VIEW_DIVISIONS,
                      MF_BYCOMMAND bitor MF_UNCHECKED);
        SetRefresh(MainMapData);
        break;
    ...
    case ID_VIEW_TEXTURECODES:
        if (ShowCodes)
        {
            ShowCodes = 0;
            CleanupConverter();
            CheckMenuItem(GetMenu(hMainWnd), ID_VIEW_TEXTURECODES,
                          MF_BYCOMMAND bitor MF_UNCHECKED);
        }
        else
        {
            ...
            while (FromObjective not_eq NULL)
            {
                if (FromObjective->GetType() == TYPE_PORT)
                {
                    ...
                    for (i = 1; i < 8 and not done; i += 2)
                    {
                        ...
                        {
                            SetRoadCell(GetCell(ox + dx[i], oy + dy[i]), 1);
                            SetRoadCell(GetCell(ox + dx[i - 1], oy + dy[i - 1]),
                                        1);
                            done = 1;
                        }
                    }
    ...
    case ID_TOOLS_OUTPUTREINFORCEMENTS:
    {
        ...
        Unit brigade;
        Objective obj;
        char buffer[60], domain[8], type[12], div[5], brig[5], vehicle[20],
            objective[40];
        ...
        if (fp)
        {
            ...
            while (unit not_eq NULL)
            {
                ...
                else if (unit->GetDomain() == DOMAIN_LAND)
                {
                    ...
                    {
                        sprintf(div, "%d",
                                ((GroundUnitClass *)unit)->GetDivision());
                        strcpy(brig, "n/a");
                        strcpy(type, "BRIGADE");
                        strcpy(vehicle, "n/a");
                    }
                ...
                fprintf(fp,
                        "%d : %-4s : %-8s : %-5s : %-4s : %-4s : %-15s : %-20s "
                        ": x : %d : y : %d : %s \n",
                        unit->GetReinforcement(), Side[unit->GetOwner()],
                        domain, type, div, brig,
                        unit->GetName(buffer, 60, FALSE), vehicle, (int)x,
                        (int)y, objective);
                unit = (Unit)myit.GetNext();
            }
    ...
    case ID_TOOLS_OUTPUTUNITLOCATIONS:
    {
        ...
        if (fp)
        {
            ...
            while (unit not_eq NULL)
            {
                ...
                fprintf(fp,
                        "%-4s : %-8s : %-5s : %-4s : %-4s : %-15s : %-20s : x "
                        ": %d : y : %d : %s : %d\n",
                        Side[unit->GetOwner()], domain, type, div, brig,
                        unit->GetName(buffer, 60, FALSE), vehicle, (int)x,
                        (int)y, objective, unit->Type());
                unit = (Unit)myit.GetNext();
            }
        ...
        while (type <= NumObjectiveTypes)
        {
            ...
            while (FromObjective not_eq NULL)
            {
                if (FromObjective->GetType() == type)
                {
                    ...
                    oc = (ObjClassDataType *)
                             Falcon4ClassTable[FromObjective->Type() -
                                               VU_LAST_ENTITY_TYPE]
                                 .dataPtr;

                    if (oc)
                        fprintf(fp,
                                "%-4s : %-15s : %-15s : %-20s : on : %-14s : "
                                "at x: %4d : y: %4d : %d : %d\n",
                                Side[FromObjective->GetOwner()],
                                ObjectiveStr[type], oc->Name,
                                FromObjective->GetName(buffer, 60, FALSE), file,
                                y, x, FromObjective->GetObjectivePriority(),
                                FromObjective->GetCampId());
                }
    ...
    case ID_TOOLS_APPLYFORCERATIOS:
        DialogBox(hInst, MAKEINTRESOURCE(IDD_FORCERARIODIALOG), hMainWnd,
                  (DLGPROC)AdjustForceRatioProc);
        AdjustForceRatios();
        break;

    case ID_TOOLS_CLIPCAMPAIGN:
        DialogBox(hInst, MAKEINTRESOURCE(IDD_CAMP_CLIPPER), hMainWnd,
                  (DLGPROC)CampClipperProc);
        break;
    ...
    case ID_TOOLS_AUTOSETSAMARTSITES:
    {
        ...
        CampEnterCriticalSection();
        {
            ...
            while (FromObjective)
            {
                ...
                FromObjective->GetLocation(&x, &y);
                rel = ((float)(GetRelief(x, y) + GetRelief(x + 1, y) +
                               GetRelief(x - 1, y) + GetRelief(x, y + 1) +
                               GetRelief(x, y - 1))) /
                      5.0F;
    ...
    case ID_TOOLS_CALCULATERADARARCS:
    {
        ...
        CampEnterCriticalSection();
        {
            ...
            while (o)
            {
                if (o->SamSite() or o->RadarSite() or
                    o->GetElectronicDetectionRange(LowAir) > 0)
                {
    ...
    case ID_HELP_ABOUTCAMPTOOL:
    {
        ...
        DialogBox(hInst, MAKEINTRESOURCE(IDD_ABOUTDIALOG), hMainWnd,
                  (DLGPROC)lpProcAbout);
    ...
    case ID_PB_CENTERONPLAYERBUBBLE:
        ...
        {
            TheCampaign.GetPlayerLocation(&MainMapData->CenX,
                                          &MainMapData->CenY);
            SetRefresh(MainMapData);
        }
                                 ...
                                 LPARAM lParam)
{
    ...
    case WM_CREATE:
        ...
        EnableMenuItem(GetMenu(hMainWnd), ID_FILE_MYSAVE,
                       MF_BYCOMMAND bitor MF_DISABLED);
    ...
    case WM_MOUSEMOVE:
    {
        ...
        pt.y += coffy;
        xPos =
            (WORD)(pt.x - MainMapData->WULX -
                   MainMapData->CULX); // Get position relative to client area
        yPos = (WORD)(pt.y - MainMapData->WULY - MainMapData->CULY);
        CurX = (WORD)(xPos + MainMapData->PFX) /
               MainMapData->CellSize; // Get grid location in Campaign terms
    ...
    case WM_SIZE:
    {
        ...
        if (resize)
        {
            ...
            SetWindowPos(hMainWnd, HWND_TOP, 0, 0, r.right - r.left,
                         r.bottom - r.top, SWP_NOMOVE bitor SWP_NOZORDER);
    ...
    case WM_PAINT:
    {
        ...
        if (GetUpdateRect(hMainWnd, &r, FALSE))
        {
            ...
            FindBorders(MainMapData);
            SetScrollPos(hMainWnd, SB_HORZ, CenPer(MainMapData, CenX, XSIDE),
                         TRUE);
            SetScrollPos(hMainWnd, SB_VERT, CenPer(MainMapData, CenY, YSIDE),
                         TRUE);

            if (r.right > Map_Max_X * CellSize)
            {
                _setcolor(DC, Blue);
                _rectangle(DC, _GFILLINTERIOR, Map_Max_X * CellSize, 0, r.right,
                           r.bottom);
            }
            ...
            {
                _setcolor(DC, Blue);
                _rectangle(DC, _GFILLINTERIOR, 0, Map_Max_Y * CellSize, r.right,
                           r.bottom);
            }
...
void ProcessCommand(int Key)
{
    ...
    case 'a':
        ...
        CampLeaveCriticalSection();
        DialogBox(hInst, MAKEINTRESOURCE(IDD_UNITDIALOG), hMainWnd,
                  (DLGPROC)EditUnit);
    ...
    case 'c':
        DialogBox(hInst, MAKEINTRESOURCE(IDD_WEATHERDIALOG), hMainWnd,
                  (DLGPROC)WeatherEditProc);
        break;
    ...
    case 'e':
        DialogBox(hInst, MAKEINTRESOURCE(IDD_TEAMEDIT_DIALOG), hMainWnd,
                  (DLGPROC)EditTeams);
        break;
    ...
    case 'l':
        if (OneObjective)
        {
            ...
            else
            {
                ...
                {
                    PathClass path;
                    i = LinkCampaignObjectives(&path, FromObjective,
                                               ToObjective);

                    if (i < 1)
                        MessageBox(NULL, "No valid path found", "Error",
                                   MB_OK bitor MB_ICONSTOP bitor
                                       MB_SETFOREGROUND);
                    ...
                    if (MainMapData->ShowLinks and i > 0)
                    {
                        ...
                        _moveto(DC, POSX(x) + (CellSize >> 1),
                                POSY(y) + (CellSize >> 1));
                        FromObjective->GetLocation(&x, &y);
                        _lineto(DC, POSX(x) + (CellSize >> 1),
                                POSY(y) + (CellSize >> 1));
                        EndPaint(hMainWnd, &ps);
                    }
                    else
    ...
    case 'm':
        DialogBox(hInst, MAKEINTRESOURCE(IDD_MISSTRIGDIALOG), hMainWnd,
                  (DLGPROC)MissionTriggerProc);
        break;

    case 'M':
        DialogBox(hInst, MAKEINTRESOURCE(IDD_MAPDIALOG), hMainWnd,
                  (DLGPROC)MapDialogProc);
    ...
    case 'p':
        ...
        else
        {
            ...
            GetGridPath(&path, Movx, Movy, X, Y, gMoveType, gMoveWho,
                        gMoveFlags);
    ...
    case 'P':
        ...
        else
        {
            ...
            {
                CampEnterCriticalSection();
                GetObjectivePath(&path, FromObjective, OneObjective, gMoveType,
                                 gMoveWho, gMoveFlags);
                CampLeaveCriticalSection();
    ...
    case 'r':
        DialogBox(hInst, MAKEINTRESOURCE(IDD_TEAMDIALOG), hMainWnd,
                  (DLGPROC)EditRelations);
        break;
    ...
    case 's':
        DialogBox(hInst, MAKEINTRESOURCE(IDD_SQUADRONDIALOG), hMainWnd,
    ...
    case 'w':
        if (WPUnit)
        {
            ...
            while (w and not gotone)
            {
                ...
                {
                    GlobWP = w;
                    DialogBox(hInst, MAKEINTRESOURCE(IDD_WPDIALOG), hMainWnd,
            ...
            if (not gotone)
            {
                ...
                if (not w)
                {
                    ...
                    w = WPUnit->GetFirstUnitWP();
                    SetWPTimes(w, w->GetWPArrivalTime(),
                               WPUnit->GetCruiseSpeed(), 0);
                }
                else
                {
                    while (w->GetNextWP())
...
void CampMain(HINSTANCE hInstance, int nCmdShow)
{
    ...
    mainwc.lpfnWndProc =
        (WNDPROC)CampaignWndProc; // The client window procedure.
    ...
    hMainWnd =
        CreateWindow("CampTool", "Campaign Tool",
                     WS_OVERLAPPEDWINDOW bitor WS_HSCROLL bitor
                         WS_VSCROLL, // bitor WS_MAXIMIZE,
                     CW_USEDEFAULT, //  WS_CLIPCHILDREN |
                     CW_USEDEFAULT, rect.right - rect.left, /* init. x size */
                     rect.bottom - rect.top, /* init. y size */
                     NULL, NULL, hInstance, NULL);

    // set up data associated with this window
    SetWindowPos(hMainWnd, HWND_TOP, 600, 400, 650, 600,
                 SWP_NOSIZE bitor SWP_NOZORDER);
...
void CampaignWindow(HINSTANCE hInstance, int nCmdShow)
{
    ...
    hToolWnd =
        CreateWindow("Toolbar", "Toolbar", WS_OVERLAPPEDWINDOW, CW_USEDEFAULT,
                     CW_USEDEFAULT, 300, 100, NULL, NULL, hInstance, NULL);

    // set up data associated with this window
    SetWindowPos(hToolWnd, HWND_TOP, 10, 10, 10, 10,
                 SWP_NOSIZE bitor SWP_NOZORDER);
