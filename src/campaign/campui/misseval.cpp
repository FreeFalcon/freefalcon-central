...
void MissionEvaluationClass::PreEvalFlight(Flight element, Flight flight)
{
    ...
    if (element)
    {
        ...
        while (tw and !(tw->GetWPFlags() & WPF_TARGET))
            tw = tw->GetNextWP();

        if ( not tw or !tw->GetWPTarget())
...
int MissionEvaluationClass::PostMissionEval(void)
{
    ...
    while (flight_ptr)
    {
        if (flight_ptr->camp_id)
        {
            ...
            while (pilot_data)
            {
                ...
                if (pilot_data->pilot_slot >= PILOTS_PER_FLIGHT and !(logbook_data.Flags & LANDED_AIRCRAFT) and flight_ptr->mission_success == Success and ( not gCommsMgr or !gCommsMgr->Online()))
    ...
    if (player_pilot)
    {
        ...
        for (i = 0; i < player_pilot->weapon_types; i++)
        {
            ...
            {
                if ( not (WeaponDataTable[player_pilot->weapon_data[i].weapon_id].Flags & WEAP_ONETENTH))
...
int MissionEvaluationClass::MissionSuccess(FlightDataClass *flight_ptr)
{
    ...
    if (flight_ptr->status_flags & MISEVAL_FLIGHT_ABORT_BY_AWACS)
        if ( not (flight_ptr->status_flags & MISEVAL_FLIGHT_TARGET_HIT))
        {
    ...
    if ( not (flight_ptr->status_flags & MISEVAL_FLIGHT_GOT_TO_TARGET)  and 
    ...
    switch (flight_ptr->mission)
    {
        ...
        case AMIS_AMBUSHCAP:
            ...
            if ( not (flight_ptr->status_flags & MISEVAL_FLIGHT_OFF_STATION))
            {
        ...
        case AMIS_SEADESCORT:
            ...
            {
                if (package_element->status_flags & MISEVAL_FLIGHT_HIT_BY_GROUND and losses)
                {
                    retval = PartSuccess;
                ...
                flight_ptr->status_flags & MISEVAL_FLIGHT_GOT_NKILL)
            {
                ...
                while (pilot_data)
                {
                    for (i = 0; i < pilot_data->weapon_types; i++)
                    {
                        ...
                        if ( not (WeaponDataTable[pilot_data->weapon_data[i].weapon_id].GuidanceFlags & WEAP_GUIDED_MASK))
                            hit += pilot_data->weapon_data[i].hit; // double credit for unguided hits

                        if ( not (WeaponDataTable[pilot_data->weapon_data[i].weapon_id].Flags & WEAP_ONETENTH))
                ...
                if (kills > 0 and ((fired > 0 and ((hit * 100) / fired) > 49) or (hit > 0 and !fired)))
                {
                    retval = Success;
    ...
    if (retval < Failed and !(flight_ptr->status_flags & MISEVAL_FLIGHT_GOT_HOME))
        retval -= 1;

    // If we didn't fly it from the start, cap success at Partial.
    if (retval == Success and (flight_ptr->status_flags & MISEVAL_FLIGHT_STARTED_LATE))
        retval = PartSuccess;
...
void MissionEvaluationClass::CollectThreats(GridIndex X, GridIndex Y, int Z, int flags, int *dists)
{
    ...
    while (e and !threat_ids[MAX_COLLECTED_THREATS - 1])
    {
        if (tteam[e->GetTeam()])
        {
            if ( not (flags & FIND_NOMOVERS and e->Moving())  and 
...
void MissionEvaluationClass::RegisterShotAtPlayer(FalconWeaponsFire *wfm, unsigned short CampID, uchar fPilotID)
{
    ...
    //find the target
    while (target_flight and !target_data)
    {
    ...
    // find the shooter
    while (shooter_flight and !shooter_data)
    {
            ...
            if (wfm->dataBlock.fPilotID == pilot_data->pilot_slot)
            {
                ...
                for (wn = 0; wn < pilot_data->weapon_types; wn++)
                {
                    ...
                    if (wfm->dataBlock.fWeaponID == windex) // and wfm->dataBlock.fireOnOff == 1)
                    {
                        ...
                        if ( not (WeaponDataTable[pilot_data->weapon_data[wn].weapon_id].Flags & WEAP_ONETENTH) and wfm->dataBlock.weaponType not_eq FalconWeaponsFire::Recon)
...
void MissionEvaluationClass::RegisterHit(FalconDamageMessage *dmm)
{
    ...
    if (Falcon4ClassTable[dmm->dataBlock.dIndex - VU_LAST_ENTITY_TYPE].dataType == DTYPE_FEATURE)
    {
        ...
        if (fc and (fc->Flags & FEAT_NO_HITEVAL))
            return;
    }
    ...
    while (flight_ptr)
    {
        ...
        if (dtm->dataBlock.dCampID and dtm->dataBlock.dCampID == flight_ptr->camp_id) // hit plane in this flight
        {
            ...
            if (pilot_data and pilot_data->pilot_status == PILOT_IN_USE)
            {
                ...
                if (dtm->dataBlock.fPilotID > PILOTS_PER_FLIGHT and !logbook_data.KilledBySelf)
                    ...
                    (dtm->dataBlock.fPilotID not_eq dtm->dataBlock.dPilotID or dtm->dataBlock.fCampID not_eq dtm->dataBlock.dCampID))
                {
                    ...
                    else if (Falcon4ClassTable[dtm->dataBlock.dIndex - VU_LAST_ENTITY_TYPE].vuClassData.classInfo_[VU_CLASS] == CLASS_FEATURE)
                    {
                        ...
                        else
                        {
                            ...
                            for (f = 0; f < o->static_data.class_data->Features and !value; f++, fid++)
                            {
...
void MissionEvaluationClass::RegisterKill(FalconEntity *shooter, FalconEntity *target, int targetEl)
{
    ...
    if ( not (flags & MISEVAL_MISSION_IN_PROGRESS))
        return;
...
void MissionEvaluationClass::RegisterMove(Flight flight)
{
    ...
    if ((flags & MISEVAL_MISSION_IN_PROGRESS) and flight->InPackage())
    {
    ...
    if (GetOwner(TheCampaign.CampMapData, fx, fy) == flight->GetTeam())
    {
        ...
        if (meflags & MISEVAL_FLIGHT_GOT_TO_TARGET and (meflags & MISEVAL_FLIGHT_STATION_OVER or meflags & MISEVAL_FLIGHT_RELIEVED))
    ...
    while (w)
    {
        ...
        if (w->GetWPFlags() & WPF_TARGET)
        {
            ...
            if (nw and nw->GetWPFlags() & WPF_TARGET)
            ...
            {
                if (flight_ptr and flight_ptr == player_element and !(meflags & MISEVAL_FLIGHT_GOT_TO_TARGET))
...
void MissionEvaluationClass::RebuildEvaluationData(void)
{
    ...
    else
    {
            ...
            while (uelement)
            {
                if (uelement->IsFlight())
                {
                    ...
                    if (flight_ptr)
                    {
                        ...
                        for (i = 0; i < PILOTS_PER_FLIGHT; i++)
                        {
                            if (flight->plane_stats[i] == AIRCRAFT_AVAILABLE and !FindPilotDataFromAC(flight_ptr, i))
                            {
                                // Add this pilot
        ...
        // evalCount++;
        // if (evalCount > 500 and !(SimDogfight.flags & DF_GAME_OVER))
        // {
