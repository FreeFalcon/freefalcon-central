...
{
    memcpy(*stream, &max_length, sizeof(uchar));
    *stream += sizeof(uchar);
    memcpy(*stream, &length, sizeof(uchar));
    *stream += sizeof(uchar);
    memcpy(*stream, &current_location, sizeof(uchar));
    *stream += sizeof(uchar);
    memcpy(*stream, path, sizeof(uchar) * (length + 1) / PATH_DIV);
    *stream += sizeof(uchar) * (length + 1) / PATH_DIV;
    return SaveSize();
}
