...
static struct ia_type
{
    ...
    char sptype;
                      ...
                      0,
                      0},
  ia_grnd_objects[] = {"chinese_t80",
                       ia_chinese_t80,
                       STYPE_UNIT_ARMOR,
                       SPTYPE_CHINESE_TYPE80,
                       "chinese_t85",
                       ia_chinese_t85,
                       STYPE_UNIT_ARMOR,
    ...
    if (wave_created)
    {
        ...
        if (FalconLocalSession->GetPlayerEntity())
        {
            player_flight->GetLocation(&px, &py);
            {
                ...
                while (unit)
                {
                    if ((unit->GetDomain() == DOMAIN_AIR) or
                        (unit->GetDomain() == DOMAIN_LAND))
                    {
            ...
            else
            {
                ...
                if (((count_flights == 0) or
                     ((wave_time) and
                      (TheCampaign.CurrentTime > wave_time))) and
                    (aircraft_left < 4))
                {
                    current_wave++;
...
void instant_action::set_start_wave(int wave)
{
    ShiAssert(
        wave >= 0 and
        wave <= 4); // Since its used as sim skill it had better be this way...
...
void instant_action::create_wave(void)
{
    ...
    int loop = 0, valid_data = 0, value = 0;

    char *ptr = NULL, *str = NULL, buffer[MAX_PATH] = {0};
    ...
    sprintf(buffer, "%s/%c%d.ia", FalconCampUserSaveDirectory, current_mode,
            current_wave);
...
void instant_action::create_player_flight(void)
{
    ...
    int loop = 0, valid_data = 0, value = 0, tid, i, type, subtype, specific,
        num_vehicles;
    ...
    if (fp)
    {
        ...
        fclose(fp);
        tid = GetClassID(DOMAIN_AIR, CLASS_UNIT, type, subtype, specific, 0, 0,
                         0);
    }
    else
    {
    ...
    for (i = 0; i < PILOTS_PER_FLIGHT; i++)
    {
        if (i < num_vehicles)
        {
            ...
            player_flight->MakeFlightDirty(DIRTY_PLANE_STATS,
                                           SEND_RELIABLEANDOOB);
        }
        else
        {
    ...
    GetCallsignID(&player_flight->callsign_id, &player_flight->callsign_num,
                  vc->CallsignSlots);
    ...
    TheCampaign.MissionEvaluator->PreMissionEval(
        player_flight, FalconLocalSession->GetPilotSlot());
...
void instant_action::move_player_flight(void)
{
    ...
    WayPointClass *waypoint, *last_waypoint;
    ...
    for (i = 0; i < numWaypoints; i++)
    {
        ...
        time +=
            FloatToInt32((Distance(lx, ly, nx, ny)) / FLIGHT_PLAN_SPEED) * 1000;
...
void instant_action::create_flight(ia_data &data)
{
    ...
    GridIndex px, py, lx, ly, nx, ny, x, y;

    ia_type *ia;

    FlightClass *flight, *new_flight;
    ...
    if (not ia)
    {
        MonoPrint("Cannot find Aircraft Type in Instant Action Object Table "
                  "(ia_air_objects)");
        return;
    }
    ...
    rad = (flight->GetLastDirection() * 45 + data.aspect) *
          DTR; // WARNING - Dodgy code from Kevin...

    MonoPrint("Create Flight %d,%d %d ", px, py,
              flight->GetLastDirection() * 45);

    x = (short)(px + FloatToInt32((float)sin(rad) * data.distance));
    y = (short)(py + FloatToInt32((float)cos(rad) * data.distance));

    tid = GetClassID(DOMAIN_AIR, CLASS_UNIT, TYPE_FLIGHT, ia->stype, ia->sptype,
                     0, 0, 0);
    ...
    GetCallsignID(&new_flight->callsign_id, &new_flight->callsign_num,
                  vc->CallsignSlots);
    ...
    waypoint = new WayPointClass(x, y, FloatToInt32(data.altitude), 0, time, 0,
                                 WP_CA, 0);
    ...
    if (data.num_vector)
    {
        ...
        {
            rad = (flight->GetLastDirection() * 45 + data.vector[loop]) *
                  DTR; // WARNING - Dodgy code from Kevin...

            nx += (short)FloatToInt32((float)sin(rad) * data.v_dist[loop]);
            ny += (short)FloatToInt32((float)cos(rad) * data.v_dist[loop]);

            waypoint = new WayPointClass(nx, ny, FloatToInt32(data.v_alt[loop]),
                                         0, time, 0, WP_CA, 0);
    ...
    else
    {
        ...
        waypoint = new WayPointClass(nx, ny, FloatToInt32(data.altitude), 0,
                                     time, 0, WP_CA, 0);
    ...
    waypoint =
        new WayPointClass(nx, ny, FloatToInt32(data.altitude), 0, time, 0,
                          WP_CA, WPF_REPEAT_CONTINUOUS bitor WPF_TARGET);
    ...
    SetWPTimes(new_flight->GetFirstUnitWP(), TheCampaign.CurrentTime,
               new_flight->GetCombatSpeed(), 0);
    ...
    {
        new_flight->LoadWeapons(NULL, DefaultDamageMods, Air, 2, 0,
                                WEAP_HEATSEEKER);
    }
    ...
    if ((data.type == ia_kc10) and (data.side == 1)) // allied kc10
    {
        ...
        {
            new_package =
                (Package)NewUnit(DOMAIN_AIR, TYPE_PACKAGE, 0, 0, NULL);
...
#else
            new_package = (PackageClass *)vuDatabase->Find(
                player_flight->GetUnitPackage());
#endif
...
void instant_action::create_battalion(ia_data &data)
{
    ...
    if (not ia)
    {
        MonoPrint("Cannot find Battalion Type in Instant Action Object Table "
                  "(ia_grnd_objects)");
        return;
    }
    ...
    tid = GetClassID(DOMAIN_LAND, CLASS_UNIT, TYPE_BATTALION, ia->stype,
                     ia->sptype, 0, 0, 0);
    ...
    for (i = 0; i < HARDPOINT_MAX; i++)
    {
        ...
        {
            classPtr =
                &(Falcon4ClassTable[GetWeaponDescriptionIndex(vc->Weapon[i])]);

            if (not InstantActionSettings.SamSites and
                classPtr->vuClassData.classInfo_[VU_TYPE] == TYPE_MISSILE)
                new_battalion->SetUnitSupply(0);

            if (not InstantActionSettings.AAASites and
    ...
    {
        new_battalion->SetSearchMode(
            (unsigned char)(FEC_RADAR_SEARCH_1)); //me123 + rand()%3));
...
static const struct
{
...
#if 1
    {
        ...
        {ia_dprk_sp122, 10.0f},   {ia_dprk_hq, 10.0f},
        {ia_dprk_t55, 15.0f},

        {ia_dprk_sp152, 10.0f}, // Moving Mud Level 0 "Recruit"
        {ia_dprk_scud, 15.0f},    {ia_dprk_tow_art, 10.0f},
        {ia_dprk_aaa, 10.0f},

        {ia_dprk_bmp2, 10.0f}, // Moving Mud Level 1  "Cadet"
        {ia_dprk_frog, 10.0f},    {ia_drpk_sa2, 20.0f},
        {ia_dprk_t62, 15.0f},

        {ia_dprk_inf, 7.0f}, // Moving Mud Level 2 "Rookie"
        {ia_dprk_bm21, 10.0f},    {ia_dprk_sa5, 40.0f},
        {ia_soviet_t72, 15.0f},

        {ia_dprk_sa3, 15.0f}, // Moving Mud Level 3 "Veteran"
        {ia_soviet_bm24, 10.0f},  {ia_soviet_sa8, 12.0f},
        {ia_soviet_mech, 10.0f},

        {ia_chinese_zu23, 10.0f}, // Moving Mud Level 4 "Ace"
        {ia_chinese_sa6, 30.0f},  {ia_soviet_t80, 10.0f},
        {ia_soviet_sa15, 10.0f},
};
#else
    {
        {ia_dprk_aaa, 10.0f}, {ia_dprk_sa5, 40.0f},    {ia_drpk_sa2, 20.0f},
        {ia_dprk_inf, 20.0f}, {ia_dprk_inf, 20.0f},    {ia_dprk_inf, 20.0f},
        {ia_dprk_inf, 20.0f}, {ia_soviet_sa15, 20.0f},
};
#endif
...
void instant_action::create_more_stuff(void)
{
    ...
    else
    {
        level = (current_wave + 1) *
                4; // In fighter sweep, we bias down one level for ground things
    ...
    data.distance =
        groundStuff[thing].range * (0.75f + 0.5f * rand() / (float)RAND_MAX);
