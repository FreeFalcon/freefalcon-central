...
ComAPIHandle ComIPMulticastOpen(int buffersize, char *gamename, int mc_scope)
{
...
#if 1
    c->recv_sock =
        WSASocket(AF_INET, SOCK_DGRAM, IPPROTO_UDP, 0, 0,
#else
    c->recv_sock =
    ...
    if (err = bind(c->recv_sock, (struct sockaddr *)&comRecvAddr,
                   sizeof(comRecvAddr)))
    {
        err = WSAGetLastError();
#ifdef DEBUG_COMMS
    ...
    WSAIoctl(c->recv_sock, SIO_MULTIPOINT_LOOPBACK, &falseValue,
             sizeof(falseValue), 0, 0, 0, 0, 0);
    WSAIoctl(c->recv_sock, SIO_MULTICAST_SCOPE, &mc_scope, sizeof(mc_scope), 0,
             0, 0, 0, 0);
    ...
    c->recv_sock = WSAJoinLeaf(c->recv_sock, (struct sockaddr *)&comRecvAddr,
                               sizeof(comRecvAddr), 0, 0, 0, 0, JL_BOTH);
    ...
    WSAIoctl(c->recv_sock, FIONBIO, &trueValue, sizeof(trueValue), 0, 0, 0, 0,
             0);
...
int ComIPMulticastSend(ComAPIHandle c, int msgsize, int oob)
{
    if (c)
    {
        ...
        if (senderror = WSASendTo(
                cudp->send_sock, &cudp->send_buffer, 1, &bytesSent, 0,
                (struct sockaddr *)&comSendAddr, sizeof(comSendAddr), 0, 0))
        {
...
int ComIPMulticastGet(ComAPIHandle c)
{
    if (c)
    {
        ...
        if (SOCKET_ERROR == WSARecv(cudp->recv_sock, &cudp->recv_buffer, 1,
                                    &bytesRecvd, &flags, 0, 0))
#else
        ...
        if (bytesRecvd > 0)
        {
            ...
            {
#ifdef DEBUG_COMMS
                ((ComAPIHeader *)cudp->recv_buffer.buf)
                    ->gamename[GAME_NAME_LENGTH] = 0;
                printf(
                    "ComAPIReceive -- got a message from another game '%s'\n",
