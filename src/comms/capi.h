// CONFIGURATION notes. within: capiopt.h #define LOAD_DLLS for expicit 
// LoadLibrary() calls to be used for WS2_32.DLL and DPLAYX.DLL and OLE32.DLL
// If not #defined application must link with WS2_32.LIB and DPLAYX.LIB and
// OLE32.LIB otherwise these LIBs are not needed
...
// Protocol values for com_API_enumerate_protocols
...
#define CAPI_JOIN_BW_TYPE 4
    ...
    // Init comms optional - done automatically by any open,
    // but needed if calling utility functions before opening a handle
    int com_API_init_comms();
    ...
    // ip is in host order
    int com_API_private_IP(unsigned long IP_address);

    // Enumerate available protocols
    int com_API_enumerate_protocols(int* protocols, int max_protocols);
    ...
    com_API_handle ComTCPOpenListen(int buffersize, char *gamename, int tcpPort , void (*AcceptCallback)(com_API_handle c, int ret));

    /* begin a TCP connection to a targeted listening TCP listener */
    com_API_handle ComTCPOpenConnect(int buffersize, char *gamename, int tcpPort, unsigned long IPaddress, void (*ConnectCallback)(com_API_handle c, int ret), int timeoutsecs);
    ...
    com_API_handle ComTCPOpenAccept(unsigned long IPaddress, int tcpPort , int timeoutsecs);
    ...
    com_API_handle ComTCPGetGroupHandle(int buffersize);

    //com_API_handle ComDPLAYOpen(int protocol,int mode, char *address, int buffersize, void *guid, void (*ConnectCallback)(com_API_handle c, int ret), int timeoutsecs);
    ...
    ***/
    ...
    /* sets the receive thread priority for UDP .. receive thread .. if exists*/
    void ComAPISetReceiveThreadPriority(com_API_handle c, int priority);


    /** translates address to ip. Answer in machine order */
