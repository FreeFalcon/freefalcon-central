...
/* Any modern ANSI/ISO C or C++ compiler (clang/gcc, or a C++ build) is MODERN. */
...
#define SSTAT stat
#else
// #104: archive_create() stat()s the .zip BEFORE opening it. The engine passes '\'-separated, mixed-case paths
// ("<base>\zips\Simdata.zip") which a raw stat misses on ext4, so the archive silently fails to attach and every
// file inside the zip goes unfound. Resolve like _open does: try verbatim, then normalise '\'->'/' + case-fold.
#include <sys/stat.h>
#ifdef __cplusplus
extern "C"
#endif
    int FF_CIResolvePath(const char *want, char *out, unsigned long cap);
static inline int ff_ci_stat(const char *p, struct stat *st)
{
    if (stat(p, st) == 0)
        return 0;
    {
        char ci[4096];
        if (p && FF_CIResolvePath(p, ci, sizeof ci))
            return stat(ci, st);
    }
    return -1;
}
#define SSTAT ff_ci_stat
#endif
...
int find_end_central_dir __((long searchlen, ecdir_rec *,
                             ARCHIVE *arc)); /* find_ecrec */
...
#define SKIP_(length)                                                          \
    if (length && ((error = do_string(length, SKIP, NULL)) != 0))              \
    {                                                                          \
...
#else
#define FLUSH(w) /* if (mem_mode) outcnt=(w); else */                          \
    flush(cmp->slide, (ulg)w, 0, cmp)
#define NEXTBYTE                                                               \
    (cmp->csize-- <= 0 ?                                                       \
         EOF :                                                                 \
         (--cmp->in_count >= 0 ?                                               \
              /* (*((int*)cmp -> in_ptr))++ */ *cmp->in_ptr++ :                \
              readbyte(cmp)))
#endif
