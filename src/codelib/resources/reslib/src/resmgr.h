...
#define STRING_SAFETY_SIZE                                                     \
    0.85 /* safety buffer ratio for string pools         */
...
#define ARCHIVE_TABLE_SIZE                                                     \
    53 /* size of hash table to start with when 
...
#define ARCHIVE_TABLE_SIZE                                                     \
    29 /* if you are using hierarchies, and
...
typedef struct RES_DIR
{
    ...
    char **
        filenames; /* dynamically allocated array of filenames                     */
    char *
        string_pool; /* to glob allocations                                          */
    char *
        string_ptr; /* current write ptr into string_pool                           */
    char *
        name; /* directory name                                               */
...
typedef struct HASH_ENTRY
{
    ...
    size_t
        offset; /* offset within an archive                                     */
    size_t
        size; /* size of file                                                 */
    size_t
        csize; /* compressed size of file                                      */
    short
        method; /* compress method (deflate or store)                           */
    long
        file_position; /* = -1 for loose file                                          */
    ...
    void
        *dir; /* if this entry is a directory                                 */

    struct HASH_ENTRY *
        next; /* for imperfect hashes    -or- a subdirectory                  */
...
typedef struct HASH_TABLE
{
    ...
    char *
        name; /* name the table (usually the directory name)                  */
    char *
        ptr_in; /* ptr to insert strings                                        */
    char *
        ptr_end; /* end of string space for this allocation                      */
    LIST *
        str_list; /* list of string space allocations                             */


    struct HASH_ENTRY *
        table; /* array -table- of hash entries                                */
...
{
    UCH *
        slide; /* sliding window                               was: slide      */
    UCH *
        in_buffer; /* input buffer                                 was: inbuf      */
    UCH *
        in_ptr; /* current write-to ptr within in_buffer        was: inptr      */
    int in_count; /*                                              was: incnt      */
    int in_size; /* size of input buffer                         was: inbufsiz   */
    char *
        out_buffer; /* output buffer                                was: outbuf     */
    ULG out_count; /* number of bytes written to output buffer     was: outcnt     */

    unsigned
        wp; /* Inflate state variable: current position in slide            */
    unsigned long
        bb; /* Inflate state variable: bit buffer                           */
    unsigned
        bk; /* Inflate state variable: bits in bit buffer                   */
    long
        csize; /* Inflate state variable:                                      */

    struct ARCHIVE *
        archive; /* archive from which this file is extracted                    */
...
{
    char name
        [_MAX_FNAME]; /* filename of the compressed archive                           */
    ...
    char
        volume; /* used to purge during ResDismount()                           */
    char
        directory; /* used to purge during ResDismount()                           */
    ...
    UCH *
        tmp_hold; /* was: fold?                                                   */
    UCH *
        tmp_slide; /* sliding window                               was: slide      */
    UCH *
        tmp_in_buffer; /* input buffer                                 was: inbuf      */
    UCH *
        tmp_in_ptr; /* current write-to ptr within in_buffer        was: inptr      */
    int tmp_in_count; /*                                              was: incnt      */
    int tmp_in_size; /* size of input buffer                         was: inbufsiz   */
    UCH *
        tmp_out_buffer; /* output buffer                                was: outbuf     */
    ...
    LIST *
        open_list; /* list of COMPRESSED_FILE structs pointing back here           */

    struct HASH_TABLE *
        table; /* hash table for this archive                                  */

} ARCHIVE;


typedef struct
    FILE_ENTRY /* called "filedes" within AFU                                  */
{
    ...
    struct COMPRESSED_FILE
        *zip; /* struct for maintaining unzip state data                      */
...
typedef struct DEVICE_ENTRY
{
    ...
    char
        type; /* type of device                                               */
    char name
        [32]; /* volume name of device                                        */
    char id; /* which cd number                                              */

    unsigned long
        serial; /* serial number of volume                                      */
...
typedef struct PATH_ENTRY
{
    ...
    char
        device; /* index pointing to specific device                            */
    char
        state; /* open, mounted, error, etc.                                   */
    char
        open_count; /* number of open files within directory                        */
...
RES_EXPORT int ResAttach(const char *attach_point, const char *filename,
                         int replace_flag);
...
RES_EXPORT FILE *__cdecl RES_FOPEN(const char *name, const char *mode);
RES_EXPORT int __cdecl RES_FCLOSE(FILE *file);
RES_EXPORT long __cdecl
RES_FTELL(FILE *stream); /* default is to use ftell !!! */
...
#define _O_APPEND                                                              \
    0x0008 /* writes done at eof                                           */
#define _O_CREAT                                                               \
    0x0100 /* create and open file                                         */
#define _O_TRUNC                                                               \
    0x0200 /* open and truncate                                            */
#define _O_EXCL                                                                \
    0x0400 /* open only if file doesn't already exist                      */
#define _O_TEXT                                                                \
    0x4000 /* file mode is text (translated)                               */
#define _O_BINARY                                                              \
    0x8000 /* file mode is binary (untranslated)                           */
#define _O_RAW                                                                 \
    _O_BINARY /* msvc 2.0 compatability                                       */
#endif /* _O_APPEND */

#ifndef O_RDONLY /* Yes, there was fcntl.h before Microsoft (even if not ansi) */
#define O_RDONLY                                                               \
    _O_RDONLY /* and... #define O_RDONLY 0x0000 will complain because MS.SUX */
...
// #104: reslib canonicalises every path to ONE separator internally. On Windows that is '\'; on Linux it must be
// '/' so the keys it builds (and the physical paths it opens) match the real ext4 tree and the '/'-separated names
// inside .zip archives. ASCII_DIRSEP is the native separator, ASCII_DIRSEP_ALT the foreign one that coercion folds
// into it, RES_DIRSEP_S the same as a string literal for sprintf path joins.
// #104: reslib's INTERNAL canonical separator is '/' on BOTH platforms. Forward slashes are accepted by the Win32
// file API and the CRT on Windows too, and by the OS natively on Linux -- so one form works everywhere and needs no
// per-platform #ifdef. Every incoming path (engine lookups spelled with '\', zip entries, attach points) is folded
// to '/' before it is hashed/compared. ASCII_DIRSEP_ALT is the foreign '\' that coercion folds into ASCII_DIRSEP.
#define ASCII_DIRSEP ASCII_FORESLASH
#define ASCII_DIRSEP_ALT ASCII_BACKSLASH
#define RES_DIRSEP_S "/"
