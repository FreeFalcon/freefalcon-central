	 ...
	 LPCSTR szBitmap, int wid, int ht, ST_RECT *bmRect)
{
    ...
    DDCopyBitmap(pdds, hbm, bmRect->x, bmRect->y, bmRect->wid, bmRect->ht);
	...
	BITMAPINFO *bmi, int x, int y, int dx, int dy, int src_y )
{
    ...
    dx = dx == 0 ? bmi->bmiHeader.biWidth  : dx;    // use the passed size, unless zero
    dy = dy == 0 ? bmi->bmiHeader.biHeight : dy;
	...
	BITMAPINFO *bmi, int x, int y, int w, int h, int dest_w, int dest_h, int src_y )
{
    ...
    w = w == 0 ? bmi->bmiHeader.biWidth  : w;    // use the passed size, unless zero
    h = h == 0 ? bmi->bmiHeader.biHeight : h;
