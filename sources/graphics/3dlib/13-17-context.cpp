...
{ 
	...
	m_colFG = m_colBG = m_spec = 0xFFFFFFFF;
	gZBias = 0.f;
	bZBuffering = TRUE;
	ZFAR = 280000.f;
	ZNEAR = 1.f;
...
BOOL ContextMPR::Setup(ImageBuffer *pIB,DXContext *c)
{
	...
	try
	{
		...
		vbdesc.dwFVF = D3DFVF_XYZRHW | D3DFVF_DIFFUSE | D3DFVF_SPECULAR | D3DFVF_TEX2;
		...
		// Setup stage 0
		m_pD3DD->SetTextureStageState(0,D3DTSS_COLORARG1,D3DTA_CURRENT);
		...
		mIdx = 0;
		translucentPolys = terrainPolys = NULL;
		translucentPolyVCnt = terrainPolyVCnt = 0;
...
void ContextMPR::StartFrame(void)
{
	...
	SetState(MPR_STA_ENABLES,MPR_SE_Z_COMPARE);
...
void ContextMPR::Wipe(DWORD Color)
{
	m_pD3DD->Clear(0,NULL,D3DCLEAR_TARGET,Color,0,0);
}

void ContextMPR::SetState(WORD State, DWORD Value, short stage)
{
	...
	switch(State)
	{
		case MPR_STA_ENABLES:
		{
			if(Value & MPR_SE_MODULATION)
			{
				...
				m_pD3DD->SetTextureStageState(stage,D3DTSS_COLOROP,D3DTOP_MODULATE);
			}
			...
			if(Value & MPR_SE_TEXTURING)
			{
				...
				if(stage != 0)
				{
					m_pD3DD->SetTextureStageState(stage,D3DTSS_COLORARG1,D3DTA_CURRENT);
					m_pD3DD->SetTextureStageState(stage,D3DTSS_COLORARG2,D3DTA_TEXTURE);
				}
				else
			...
			if(Value & MPR_SE_ALPHA)
			{
				...
				m_pD3DD->SetTextureStageState(stage,D3DTSS_ALPHAOP,D3DTOP_MODULATE);
			}
			...
			if(Value & MPR_SE_CHROMA)
			{
				...
				m_pD3DD->SetRenderState(D3DRENDERSTATE_SRCBLEND,D3DBLEND_SRCALPHA);
				m_pD3DD->SetRenderState(D3DRENDERSTATE_DESTBLEND,D3DBLEND_INVSRCALPHA);
				m_pD3DD->SetTextureStageState(stage,D3DTSS_ALPHAOP,D3DTOP_SELECTARG1);
			}
		...
		case MPR_STA_DISABLES:
		{
			if(Value & MPR_SE_MODULATION)
			{
				...
				m_pD3DD->SetTextureStageState(stage,D3DTSS_COLOROP,D3DTOP_SELECTARG1);
			}
			...
			if(Value & MPR_SE_FILTERING)
			{
				...
				m_pD3DD->SetTextureStageState(stage,D3DTSS_MAGFILTER,D3DTFG_POINT);
				m_pD3DD->SetTextureStageState(stage,D3DTSS_MINFILTER,D3DTFN_POINT);
			}
		...
		case MPR_STA_COLOR_OP_FUNCTION:
		{
			...
			m_pD3DD->SetTextureStageState(stage,D3DTSS_COLOROP,Value);

			break;
		}

		case MPR_STA_COLOR_ARG1_FUNCTION:
		{
			FlushVB();

			m_pD3DD->SetTextureStageState(stage,D3DTSS_COLORARG1,Value);

			break;
		}

		case MPR_STA_COLOR_ARG2_FUNCTION:
		{
			FlushVB();

			m_pD3DD->SetTextureStageState(stage,D3DTSS_COLORARG2,Value);
		...
		case MPR_STA_ALPHA_OP_FUNCTION:
		{
			...
			m_pD3DD->SetTextureStageState(stage,D3DTSS_ALPHAOP,Value);
		...
		case MPR_STA_TEXTURE_FACTOR:
		{
 		...
 		case MPR_STA_FOG_COLOR:
 		{
 			...
 			m_pD3DD->SetRenderState(D3DRENDERSTATE_FOGCOLOR,Value);
 			break;
 		}
		...
		case MPR_STA_TEX_FILTER:
		{
			...
			switch(Value)
			{
				...
				case MPR_TX_BILINEAR_NOCLAMP:
				{
					...
					{
						m_pD3DD->SetTextureStageState(stage,D3DTSS_MAGFILTER,D3DTFG_ANISOTROPIC);
						m_pD3DD->SetTextureStageState(stage,D3DTSS_MINFILTER,D3DTFN_ANISOTROPIC);
						m_pD3DD->SetTextureStageState(stage,D3DTSS_MAXANISOTROPY,m_pCtxDX->m_pD3DHWDeviceDesc->dwMaxAnisotropy);
					}

					else
					{
						m_pD3DD->SetTextureStageState(stage,D3DTSS_MAGFILTER,D3DTFG_LINEAR);
						m_pD3DD->SetTextureStageState(stage,D3DTSS_MINFILTER,D3DTFN_LINEAR);
					}

					if(Value == MPR_TX_BILINEAR)
					{
						m_pD3DD->SetTextureStageState(stage,D3DTSS_ADDRESS,D3DTADDRESS_CLAMP);
					}
				...
				{
					m_pD3DD->SetTextureStageState(stage,D3DTSS_MIPFILTER,D3DTFP_NONE);
					break;
				}
				...
				case MPR_TX_MIPMAP_LINEAR:
				{
					if(DisplayOptions.bLinearMipFiltering)
						m_pD3DD->SetTextureStageState(stage,D3DTSS_MIPFILTER,D3DTFP_LINEAR);
...
void ContextMPR::SetCurrentState(GLint state, GLint flag)
{
	...
	switch(state)
	{
		...
		case STATE_TEXTURE_TERRAIN_NIGHT:
			...
			m_pD3DD->SetTextureStageState(1,D3DTSS_TEXCOORDINDEX,0);
			m_pD3DD->SetTextureStageState(1,D3DTSS_ALPHAOP,D3DTOP_DISABLE);
			...
			SetState(MPR_STA_TEX_FILTER,MPR_TX_MIPMAP_NEAREST,1);
			if(PlayerOptions.FilteringOn())
				SetState(MPR_STA_TEX_FILTER,MPR_TX_BILINEAR,1);
...
{
#ifdef _CONTEXT_TRACE_ALL
	MonoPrint("ContextMPR::SelectTexture1(0x%X)\n",texID);
#endif
...
{
#ifdef _CONTEXT_TRACE_ALL
	MonoPrint("ContextMPR::SelectTexture2(0x%X)\n",texID);
#endif
...
void ContextMPR::SetFog(DWORD color)
{
	if(color != m_spec)
		m_spec = color;
}
...
void ContextMPR::ToggleZBuffering()
{
	if(!bZBuffering)
	{
		bZBuffering = TRUE;
...
inline TLVERTEX* BPolygon::CopyToVertexBuffer(TLVERTEX *bufferPos)
{
	BVertex *curVert = pVertexList;
...
inline void BPolygon::CalcPolyZ()
{
	float Z = 0.f;
	BVertex *curVert = pVertexList;
	...
	Z /= float(numVertices);
	zBuffer = FloatToInt32(Z * 16777215.f);
}

inline void ContextMPR::AllocatePolygon(BPolygon *&curPoly, const DWORD numVertices)
{
	curPoly = (BPolygon *)Alloc(sizeof(BPolygon)+numVertices*sizeof(BVertex));
	curPoly->numVertices = numVertices;
	curPoly->pVertexList = (BVertex *)(DWORD(curPoly)+sizeof(BPolygon));

	BVertex *curVert = curPoly->pVertexList;
...
inline void ContextMPR::AddPolygon(BPolygon *&polyList, BPolygon *&curPoly)
{
	curPoly->pNext = polyList;
	polyList = curPoly;
}

void ContextMPR::RenderPolyList(BPolygon *&pHead)
{
	TLVERTEX *pIns;
	BPolygon *pStart,*pEnd,*pCur;
	DWORD offset,vertcnt = 0,verttot = 0;

	if(pHead->renderState >= STATE_TEXTURE_TERRAIN)
	{
		offset = DWORD(&pHead->zBuffer)-DWORD(pHead);
		pHead = (BPolygon *)RadixSortAscending((radix_sort_t *)pHead,offset);
	}
	else
	{
		offset = DWORD(&pHead->zBuffer)-DWORD(pHead);
		pHead = (BPolygon *)RadixSortDescending((radix_sort_t *)pHead,offset);
	}
	...
	while(pEnd != NULL)
	{
		...
		if(pEnd == NULL || vertcnt + pEnd->numVertices > 1024)
		{
			...
			while(pEnd != pCur)
			{
				...
				if(pCur->renderState > STATE_ALPHA_GOURAUD)
					SetTexture1(pCur->textureID0);
				else
					SetTexture1(-1);

				if(pCur->renderState == STATE_TEXTURE_TERRAIN_NIGHT)
...
void ContextMPR::DrawPoly(DWORD opFlag, Poly *poly, int *xyzIdxPtr, int *rgbaIdxPtr, int *IIdxPtr, Ptexcoord *uv, bool bUseFGColor)
{
	...
	if(bZBuffering && currentState >= STATE_ALPHA_SOLID)
	{
		AllocatePolygon(BPolygon,poly->nVerts);
		BPolygon->renderState = currentState;
		BPolygon->textureID0 = currentTexture1;
		BPolygon->pNext = NULL;
		BVertex = BPolygon->pVertexList;
	}
	else
	{
		// Lock VB
	...
	if(bZBuffering && currentState >= STATE_ALPHA_SOLID)
	{
		for(int i = 0; i < poly->nVerts; i++)
		{
			...
			zNear = ZNEAR + gZBias + .005f;

			// NOTE: HACK!!
			if(xyz->z > 5)
				BVertex->sz = (ZFAR/(ZFAR-zNear))+(ZFAR*zNear/(zNear-ZFAR))/xyz->z;
			else
				BVertex->sz = 0.f;

			BVertex->rhw = 1.f/xyz->z;
			BVertex->specular = m_spec;
			...
			if(opFlag&PRIM_COLOP_COLOR)
			{
				...
				if(rgba)
				{
					if(opFlag & PRIM_COLOP_INTENSITY)
					{
						...
						BVertex->color = D3DRGBA(rgba->r * *I,rgba->g * *I,rgba->b * *I,rgba->a);
					}

					else
					{
						BVertex->color = D3DRGBA(rgba->r,rgba->g,rgba->b,rgba->a);
					}
				}
			}
	...
	else
	{
		for(int i = 0; i < poly->nVerts; i++)
		{
			...
			// NOTE: HACK!!
			if(xyz->z > 5)
				pVtx->sz = (ZFAR/(ZFAR-zNear))+(ZFAR*zNear/(zNear-ZFAR))/xyz->z;
			else
				pVtx->sz = 0.f;

			pVtx->rhw = 1.f/xyz->z;
			pVtx->specular = m_spec;
	...
	if(bZBuffering && currentState >= STATE_ALPHA_SOLID)
	{
		...
		AddPolygon(translucentPolys,BPolygon);
	}
	else
	{
...
void ContextMPR::Draw2DPoint(Tpoint *v0)
{
	...
	if(v0->z)
		pVtx->sz = (ZFAR/(ZFAR-zNear))+(ZFAR*zNear/(zNear-ZFAR))/v0->z;
	else
		pVtx->sz = 0.f;
...
void ContextMPR::DrawPrimitive(int nPrimType, WORD VtxInfo, WORD nVerts, MPRVtxTexClr_t **pData, bool terrain)
{
	...
	if(bZBuffering && currentState >= STATE_ALPHA_SOLID)
	{
		AllocatePolygon(BPolygon,nVerts);
		BPolygon->renderState = currentState;
		BPolygon->textureID0 = currentTexture1;
		BPolygon->textureID1 = currentTexture2;
		BPolygon->pNext = NULL;
		BVertex = BPolygon->pVertexList;
	}
	else
	...
	if(bZBuffering && currentState >= STATE_ALPHA_SOLID)
	{
		for(int i = 0; i < nVerts; i++)
		{
			...
			BVertex->sx = pData[i]->x;
			BVertex->sy = pData[i]->y;

			// NOTE: HACK!!
			if(pData[i]->q)
				BVertex->sz = (ZFAR/(ZFAR-zNear))+(ZFAR*zNear/(zNear-ZFAR))/(pData[i]->q/Q_SCALE);
			else
				BVertex->sz = 0.f;

			BVertex->rhw = pData[i]->q > 0.0f ? 1.0f / (pData[i]->q / Q_SCALE) : 1.0f;

			BVertex->color = D3DRGBA(pData[i]->r,pData[i]->g,pData[i]->b,pData[i]->a);
			BVertex->specular = 0xFFFFFFFF;
	
			if(VtxInfo & MPR_VI_TEXTURE)
			{
				BVertex->tu0 = pData[i]->u;
				BVertex->tv0 = pData[i]->v;
			}

		#ifdef _CONTEXT_ENABLE_RENDERSTATE_HIGHLIGHT
			BVertex->color = currentState != -1 ? RGBA_MAKE((currentState << 1) + 50,(currentState << 1) + 50,(currentState << 1) + 50,(currentState << 1) + 50) : D3DRGBA(1.0f,1.0f,1.0f,1.0f);
		#endif
	...
	else
	{
		for(int i = 0; i < nVerts; i++)
		{
			...
			// NOTE: HACK!!
			if(pData[i]->q)
				pVtx->sz = (ZFAR/(ZFAR-zNear))+(ZFAR*zNear/(zNear-ZFAR))/(pData[i]->q/Q_SCALE);
			else
				pVtx->sz = 0.f;
	...
	if(bZBuffering && currentState >= STATE_ALPHA_SOLID)
	{
		...
		if(currentState >= STATE_TEXTURE_TERRAIN)
		{
			terrainPolyVCnt += nVerts;
			AddPolygon(terrainPolys,BPolygon);
		}
		else
		{
