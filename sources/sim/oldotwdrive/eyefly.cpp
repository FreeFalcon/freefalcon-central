...
float tmpX, tmpY, tmpZ;
   ...
   {
	  p = Math.DeadBand(IO.ReadAnalog(AXIS_PITCH), -0.05F, 0.05F) * (1.0F / 0.95F); 	// Retro 31Dec2003
      p = max ( min (p, 1.0F), -1.0F) * dT * 1.0f;

	  q = Math.DeadBand(IO.ReadAnalog(AXIS_ROLL), -0.05F, 0.05F) * (1.0F / 0.95F); 		// Retro 31Dec2003
